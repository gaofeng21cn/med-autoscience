...
| --- | --- | --- | --- | --- |
| workspace / profile 预检 | `doctor --profile <profile>` | profile、runtime contract、launcher contract、overlay readiness、blocking verdict 是否结构化输出 | `docs/external_runtime_dependency_gate.md`、`src/med_autoscience/doctor.py` | 通过 `doctor` 不等于 external runtime gate 已解除 |
| external runtime dependency gate 预检 | `med-deepscientist-upgrade-check --profile <profile> --refresh` | controlled fork、`MEDICAL_FORK_MANIFEST.json`、`behavior_equivalence_gate.yaml`、workspace contract 检查是否 fail-closed | `docs/external_runtime_dependency_gate.md`、`src/med_autoscience/controllers/med_deepscientist_upgrade_check.py` | external fork truth、workspace truth、human interaction 仍在仓外 |
...
| study delivery projection | `sync-study-delivery --paper-root <study_root>/paper --stage submission_minimal` | 只做 controller-authorized `paper -> manuscript` 投影，不把 external workspace 的论文真相缺口伪装成 repo-side 已解决 | `controllers/README.md`；study delivery artifacts | 不负责自动清除 human-required metadata / paper truth gap |
