...
The turn prompt authorization patch fixed worker instructions, but DM002 then exposed the matching read-model defect: `paper-mission-owner-surface` still evaluated the older source-provenance terminal blocker before the newer provenance-limited owner result, so the next queued action returned to `methodology_reframe_route_decision` instead of continuing the already-authorized unit-harmonized rerun.
...
- `scripts/run-pytest-clean.sh tests/paper_mission_owner_surface_cases/test_methodology_reframe_route_priority.py::test_scan_prefers_current_provenance_limited_rebuild_handoff_over_stale_audit_decision -q`: 1 passed.
- `scripts/run-pytest-clean.sh tests/paper_mission_owner_surface_cases/test_methodology_reframe_route_priority.py tests/paper_mission_owner_surface_cases/test_methodology_reframe_currentness.py tests/paper_mission_owner_surface_cases/test_analysis_harmonization_owner_result_consumption.py tests/paper_mission_owner_surface_cases/test_hard_methodology_currentness.py -q`: 18 passed.
