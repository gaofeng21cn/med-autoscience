...
Life Science Research 类外部 source helper 模式只能作为 `life_science_source_discovery_pack` 和 MAS source adapter output 被消费。实体规范化、多 lane 路由、公共数据库 discovery、cohort / tissue / ancestry caveat、cross-source conflict 和 evidence gap synthesis 可以提升 source refs 与 reviewer input 的结构质量；它们不能成为 source readiness verdict、evidence authority、quality verdict、publication readiness、submission readiness 或 MAS truth write。完整 intake 见 [Life Science Research Learning Intake](../../references/mainline/life_science_research_learning_intake.md)。
