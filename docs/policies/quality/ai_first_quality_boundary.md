...
- Life Science Research 类 source helper 模式同样只能作为 clean-room source discovery / evidence helper pattern。外部 router、数据库 skill、公共 API wrapper、cohort result、association score、structure/pathway context 或 pharmacology summary 可以作为 AI reviewer input refs、source caveat refs 或 typed blocker 输入；不能替代独立 AI reviewer / auditor record，不能授权 source readiness、医学质量、publication readiness、submission readiness 或 artifact mutation。当前 closeout 见 [Life Science Research Learning Intake](../../references/mainline/life_science_research_learning_intake.md)。
...
后续涉及论文质量、Stage Review、publication 或 finalize authority 的改动，至少检查：
