...
- 变更 route 合同时，只修改 `agent/stages/stage_route_contract.yaml` 这一份结构化 source，再同步 `docs/runtime/contracts/stage_route_contract.md` 与 Codex/OpenClaw prompt 输出；Python package 不保留 YAML mirror。
- 变更 route 合同属于 contract surface 变更，至少补跑 `tests/test_stage_manifest_contract_extensions.py`、`tests/test_opl_standard_pack_cases/test_stage_contract_cases.py` 与 `scripts/verify.sh full`。
