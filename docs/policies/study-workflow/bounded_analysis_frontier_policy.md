...
Stage knowledge plane 中的 `research_frontier_board` 承接这个候选板的机器可读 refs / summary 投影。它只保存 Stage-local working board、research memory delta 和 rollback target advisory，不创建 Stage 外 Frontier Plane，也不替代 controller、owner receipt、typed blocker、human gate、runtime queue 或 publication eval。materialization 时可独立写出 `<study_root>/artifacts/stage_outputs/<stage>/research_frontier_board.json`；该文件只是可引用 artifact，不是 study truth 或 publication/package authority。
...
- next route：decisive Attempt 选择的 canonical Stage，或继续同一 bounded analysis；public id 以 action catalog 为准
