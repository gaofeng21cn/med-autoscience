...
2026-05-11 status calibration，经 2026-05-23 文档生命周期审计校准：Stage-Led Autonomy 的核心 operating surface 已经落地；机械分流层已经降级为 audit/router/materializer。仍在源码或文档中可见的 historical reader、fixture、旧命名或迁移桥只按 provenance、parity oracle、diagnostic 或 deletion-gate 输入读取。开发纪律是先用 live caller、contracts、tests 和 tombstone refs 证明默认路径不再调用，确认 direct skill 与 OPL handoff 都回到同一 MAS-owned stage/route surface，再删除旧 vocabulary 或迁入 history；不能为了“清干净”破坏 restore/provenance/parity 证据。
...
2026-05-12 OPL platform calibration 后续已由 2026-05-18 fresh read model 更新：OPL family index 显示 MAS/MAG/RCA 三个 domain agent 均 descriptor-aligned，family stage plane 和 domain-memory descriptor 可解析；MAS 的 `mas_publication_route_memory` descriptor 是 MAS-owned locator。OPL family-runtime production required provider 是 `temporal`，默认 provider 已选到 Temporal，managed service / worker residency proof 可用；`local_sqlite` 只在显式选择时作为 dev/CI/offline diagnostic baseline。这个状态把 MAS 的 OPL-hosted 条件推进到 provider-backed descriptor / memory / sidecar 可投影阶段，但真实 paper-line live apply soak、human gate/resume 和更多 owner receipt scaleout 仍是目标态证据缺口。
...
2026-07-08 落地口径：`research_frontier_board` 是 Stage 内滚动候选路线板和 research memory delta，不是 `Frontier Plane`、不是新的 control layer，也不是 Stage 外 controller。它只能从 stage packet / closeout packet 风格的 Mapping 中提取 active、testing、rejected、deferred、stop-loss 候选、evidence refs、failed-path / negative-result refs、route-back refs、writeback refs 和 next hypothesis suggestion，并投影 refs / summary 给 OPL。

它的 rollback target policy 只做 advisory：failure scope、route impact、missing evidence、stop-loss、claim downgrade 等信号可以映射到 8-stage pack 建议 target：`01-study_intake`、`02-protocol_and_analysis_plan`、`03-data_asset_and_cohort_build`、`04-analysis_execution`、`05-evidence_synthesis`、`06-manuscript_authoring`、`07-quality_review_and_revision`、`08-publication_package_handoff`。这些 target 只帮助 operator 判断回看哪一段 pack，不控制推进、替代 `StageOutcome` / `NextAction`、写 domain truth、授权质量裁决或关闭 stage。Progress First 不变：真正的推进 authority 仍是 `StageOutcome`、`NextAction`、owner receipt、typed blocker、route-back 或合法 human gate。

物理落点：stage packet / closeout materialization 会顺手写出 `<study_root>/artifacts/stage_outputs/<stage>/research_frontier_board.json`，作为独立可引用的 board artifact。该 artifact 是 stage-local working board / refs projection，不是第二 truth source；study truth、publication/package authority、owner receipt、typed blocker、human gate、runtime queue 和 provider attempt 仍归各自 owner surface。
