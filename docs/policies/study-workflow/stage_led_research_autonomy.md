...
它的 rollback target policy 只做 advisory：failure scope、route impact、missing evidence、stop-loss、claim downgrade 等信号可以映射到建议 target stage，但不能控制推进、替代 `StageOutcome` / `NextAction`、写 domain truth、授权质量裁决或关闭 stage。Progress First 不变：真正的推进 authority 仍是 `StageOutcome`、`NextAction`、owner receipt、typed blocker、route-back 或合法 human gate。
