...
- `architecture_fitness_functions`：架构约束应进入自动化检查；在本项目中体现为 `tests/test_standard_agent_boundary.py`、source closure、tracked-path hygiene、Sentrux / `opl quality details` 与 no-resurrection scan。
...
- `scripts/verify.sh structure` 直接运行 Sentrux，失败时由 `opl quality details` 提供诊断；MAS 不保留 strict alias 或独立 line-budget。
