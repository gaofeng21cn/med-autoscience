...
- MAS-owned packaging surface：`artifacts/runtime/progress_portal/latest.json`、`artifacts/runtime/progress_portal/hosted_package.json`、`ops/mas/progress/index.html`
...
- `scripts/verify.sh structure` 继续覆盖 advisory line budget 与结构质量；历史 strict 入口只保留兼容，line budget 由定时结构治理任务统一消化，不作为本地 hard gate。
