...
| managed study stage 受控推进 | `launch-study --profile <profile> --study-id <study_id>`，必要时由 OPL `current_control_state` hydrate stage attempt | 只通过 controller action 与 OPL stage attempt 进入 create / resume / pause；live provider attempt 时必须切 supervisor-only；不得旁路写 runtime-owned surface | `progress_projection`；OPL `current_control_state`；`studies/<study_id>/artifacts/runtime/last_launch_report.json` | external runtime 未健康、human confirmation、startup boundary 未过时不得冒进 |
...
| quest / runtime health diagnostic 与监管外环 | `runtime domain-diagnostic-report --quest-root <quest_root> --apply` 或 `runtime domain-diagnostic-report --runtime-root <runtime_root> --apply --request-opl-stage-attempts --profile <profile>` | 只有 `runtime_liveness_audit.status=live` 且 `worker_running=true` 且 `active_run_id!=null` 才能视为 live；否则必须进入 recovering / degraded / escalated | `domain_diagnostic_report`；quest 侧 legacy namespace `artifacts/reports/domain_diagnostic_report/`；`runtime_escalation_record.json` | 监管面可以升级但不能伪装成 cutover 已完成 |
...
4. `runtime_escalation_record.json` 与 `domain_diagnostic_report` 是 quest-owned runtime artifact / diagnostic projection，不得与 study-owned artifact 混写；`artifacts/reports/domain_diagnostic_report/` 只作为 legacy path namespace。
