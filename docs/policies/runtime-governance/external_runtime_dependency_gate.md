# Optional External Runtime Dependency Gate

这份文档原本服务 MAS 吸收 MDS 过程中的 external runtime blocker。2026-05-11 之后，MAS monolith closeout 已经把默认运行、默认诊断、默认进度面和默认质量入口收回到 `med-autoscience`；因此本文降级为 **optional provider / historical backend / explicit archive import / parity audit gate**，不再是 MAS 默认可用性的停车结论。
...
1. 什么时候仍然不能声称 optional hosted/provider path、historical backend audit 或旧 cutover readiness 已完成
...
- 把 optional provider / historical backend blocker 重新解释成 MAS 默认 runtime、默认 diagnostic 或质量依赖
...
- repo 内已经把 `MedAutoScience domain owner -> OPL/provider sidecar bridge -> optional provider / historical backend audit` 的边界冻结完成
- MAS 默认运行与默认诊断已经不要求外部 MDS repo / daemon / runtime root / WebUI
- 继续前进到 OPL provider long-run soak、optional Hermes evidence、historical backend audit 或 archive-import parity，还需要外部证据
...
### 3.1 optional `Hermes` / provider runtime 真证据

repo-side 能检查的是：

- optional provider / Hermes legacy path 是否有结构化 readiness proof
...
- optional `Hermes` / provider runtime 与当前 repo-side contract 的真实端到端一致性证明
...
- optional provider、Hermes legacy path、historical MDS backend audit、explicit archive import 和 parity oracle 仍可被检查，但它们不是 MAS 默认可用性前置条件。
- repo 内不再需要伪造新的 same-repo tranche 来解释 optional provider 或 historical backend 阻塞。
- 当前 Hermes continuation / activation / deconstruction map 的完成，不替代 optional provider 或 historical backend blocker 本身。
- 真正继续前进仍然依赖 OPL provider soak、optional runtime evidence、external workspace truth 或 human-required interaction。
