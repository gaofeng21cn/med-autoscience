...
| study delivery projection | `study delivery-sync --paper-root <study_root>/paper --stage submission_minimal` | 只做 controller-authorized `paper -> manuscript` 投影，不把 external workspace 的论文真相缺口伪装成 repo-side 已解决 | `docs/runtime/controllers.md`；study delivery artifacts | 不负责自动清除 human-required metadata / paper truth gap |
