# Policies

这里存放 `MedAutoScience` 已经稳定、适合长期仓库跟踪的规则文档。

它们默认服务于内部技术同事与 Agent 的稳定操作面，不默认进入首页双语公开面。
如果未来某份规则要提升到对外公开面，必须同步补齐英文 `.md` 与中文 `.zh-CN.md` 镜像。

与 `docs/` 其他说明文档的区别：

- `docs/*.md` 解释“怎么理解、怎么使用”
- `docs/policies/*.md` 规定“什么是正式边界、什么不能退回去”

当前核心 policy：

- [运行模型 Policy](platform_operating_model.md)
- [数据资产策略](data_asset_management.md)
- [Publication Gate Policy](publication_gate_policy.md)
- [AI-first Quality Boundary Policy](ai_first_quality_boundary.md)
- [External Runtime Dependency Gate](external_runtime_dependency_gate.md)
- [Mainline Integration And Cleanup](mainline_integration_and_cleanup.md)
- [Manual Runtime Stabilization Checklist](manual_runtime_stabilization_checklist.md)
- [MAS/MDS Owner Boundary Contract](mas_mds_owner_boundary_contract.md)
- [Merge And Cutover Gates](merge_and_cutover_gates.md)
- [Repository CI Preflight](repository_ci_preflight.md)
- [Study Route Contract](study_route_contract.md)
- [Evidence Review Contract](evidence_review_contract.md)
- [默认研究场景](study_archetypes.md)
- [研究路线偏置](research_route_bias_policy.md)
- [Workspace 规则](workspace_autoscience_rules.md)
