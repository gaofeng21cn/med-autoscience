...
机器入口：`medautosci data-lifecycle inspect --format json` 输出 body/index/study/runtime/export/retention plane summary；`medautosci data-lifecycle index-assets --apply --format json` 写 `memory/portfolio/data_assets/index.sqlite` 的 release registry 与 file inventory refs，不存临床正文；`medautosci data-lifecycle compact-runtime --apply --format json` 只写 `runtime/index.sqlite`；`medautosci data-lifecycle compact-study --apply --format json` 只写 `studies/index.sqlite`；`medautosci data-lifecycle closeout-completed-project --apply --format json` 只写 semantic reproducible capsule；`medautosci data-lifecycle finalize-governance --apply --format json` 只写 TTL/pin、owner-gated deletion、OMOP-like mapping、Parquet/DuckDB sidecar registry 和 RO-Crate-like metadata refs。以上 apply 均保留源文件，不执行物理删除，不转换临床数据，不写 owner receipt、typed blocker 或 runtime queue。
