...
- 决策：`ai_reviewer_re_eval` 在 domain transition 中优先于 generic `publication_gate_blocker`；`publication_eval` 若是 `mechanical_projection` 或 `medical_journal_prose_quality` 未 ready，必须回到 AI reviewer workflow。`medical_prose_review_status=ready` 也必须从 medical publication surface 透传到 publication eval action；缺失或 underdefined 时，clear gate 不能继续 write-stage / bundle-stage advancement。只有 AI reviewer-backed、`bundle_only_remaining` 且只剩 submission milestone 的 live runtime，才允许 autopark，而不是再唤醒写作。
...
## 2026-05-16：MAS functional privatization 分类进入机器边界

- 决策：MAS repo 内非知识层功能按 A/B/C 分类写入 `functional_consumer_boundary` 和 `mas-functional-consumer-followthrough` lane，并通过 `functional_module_inventory` 记录代码路径级清单。A 类由 OPL 持有、MAS 只消费或暴露 refs-only adapter/receipt，包括 runtime lifecycle SQLite reference adapter、paper work-unit outbox index、runtime storage maintenance、workspace/source intake shell、publication-route memory locator/writeback transport、artifact lifecycle/storage audit shell、workbench/portal generic shell、terminal attach transport、runtime supervisor scan/consume/dispatch/reconcile shell、generic CLI/MCP/product wrappers、generic daemon/scheduler lifecycle、queue/attempt/retry/dead-letter 和 generic transition runner。B 类由 MAS 保留，包括 study truth、`study_runtime_status`、`runtime_watch` domain health、AI reviewer、publication gate、artifact authority、owner receipt、domain transition table、publication-route memory body、memory writeback decision、typed blocker 和 safe action refs。C 类在 replacement proof 或 no-active-caller 证明后退役，包括 local LaunchAgent scheduler install path、workspace-local watch wrappers、MAS generic workbench shell、legacy scheduler default aliases、daemon-ish terminal attach owner claim 和 scheduler residue。
...
- 影响：`runtime_lifecycle.sqlite` 降格为 MAS domain sidecar index / reference adapter；它可以索引 MAS owner receipt、surface ref、restore/retention proof 和 locator，但不能被写成 MAS generic persistence engine。`paper_work_unit_outbox` 这类仍在 MAS 内的索引面只允许保留 paper work-unit identity、publication gate context、artifact delta obligations 和 owner receipt refs；queue/outbox/retry/attempt 的通用语义应继续上收到 OPL。新增 CLI/MCP/product-entry/sidecar surface 若触及 A 类能力，必须先引用 OPL replacement expectation/audit ref；若属于 C 类，按无 active caller、无 fixture/provenance 必需和 replacement proof 直接退役，不新增兼容别名。
...
- 2026-05-15 追加：fresh prompt 只是第一层 postcondition；第二层 postcondition 是 prompt 内的 callable action 也必须消费当前 authorization。MAS managed Codex CLI worker 调用 `runtime-supervisor-execute-dispatch` 时使用受限 `--managed-runtime-worker` 通道：只有环境标记、quest root、quest id、run id、`worker_running=true`、`active_run_id` 和 `last_controller_decision_authorization.controller_actions` 全部匹配，executor 才能用当前 controller authorization 重建 action owner route。外层 developer heartbeat / 人工 CLI 仍必须走 `developer_apply_safe` GitHub/user-config gate；旧 default executor dispatch 不能在 managed worker 内覆盖当前 controller transition authority。
