...
- 决策：同一 stable blocker 规则适用于 AI reviewer record-production currentness blocker，包括 `ai_reviewer_record_stale_after_current_manuscript`、`ai_reviewer_record_stale_after_unit_harmonized_rerun` 和 `ai_reviewer_record_manuscript_story_provenance_leakage`。这些 blocker 已经代表 MAS AI reviewer owner 对当前 record 不可消费的 typed 判断，sidecar dispatch 必须签出 accepted receipt 并投影 `stable_typed_blocker`；`ai_reviewer_request_missing`、`owner_callable_surface_missing` 等真正缺 owner request 或 callable 的 blocker 仍保持 rejected dispatch，不得被稳定化。
...
- 理由：DM002 恢复后又暴露出 AI reviewer recheck callable 已返回 `ai_reviewer_record_stale_after_current_manuscript` owner receipt 和 record-production request，但 sidecar 顶层仍因 `accepted=false` 让 OPL dead-letter。该 case 的正确后续是由 AI reviewer record-only owner 生产当前 record，再经 MAS materializer/dispatcher 消费，不是 provider retry、手工 queue update 或直接改 `publication_eval/latest.json`。
