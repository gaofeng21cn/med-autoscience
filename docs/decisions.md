...
- 影响：该 suite 必须同时输出 `mas_agent_lab_mechanism_evolution_inputs`，把 research wiki / failed-route memory refs、AI reviewer direct-evidence refs 和 analysis queue manifest refs 接到 OPL Agent Lab 的 mechanism evolution surface。OPL 只能消费这些 refs 来生成机制 candidate、promotion decision、canary 和 rollback refs；不得读取或写入 MAS memory body、publication eval、controller decisions、canonical manuscript、current package 或 submission readiness。
