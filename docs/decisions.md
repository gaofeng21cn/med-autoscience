...
- 理由：DM002 暴露出 prompt 已能看到 5/15 的 AI reviewer redrive，但 runtime state 仍停在更旧的 `analysis_claim_evidence_repair` 授权。旧私有 worker bypass 曾从 runtime state 校验同 run 授权；该通道已被 OPL current_control_state + MAS owner receipt / typed blocker 边界取代。若该层未同步，AI reviewer workflow 会被阻断或空跑，旧 `publication_eval ready` 也容易被误读为新质量闭环。
