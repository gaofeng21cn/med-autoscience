...
- 决策：MAS repo 内非知识层功能按 A/B/C 分类写入 `functional_consumer_boundary` 和 `mas-functional-consumer-followthrough` lane。A 类由 OPL 持有、MAS 只消费或暴露 refs-only adapter/receipt，包括 runtime lifecycle SQLite reference adapter、runtime storage maintenance、artifact lifecycle/storage audit shell、workbench/portal generic shell、terminal attach transport、generic daemon/scheduler lifecycle、queue/attempt/retry/dead-letter 和 generic transition runner。B 类由 MAS 保留，包括 study truth、`study_runtime_status`、`runtime_watch` domain health、AI reviewer、publication gate、artifact authority、owner receipt、domain transition table、publication-route memory body、memory writeback decision、typed blocker 和 safe action refs。C 类在 replacement proof 或 no-active-caller 证明后退役，包括 local LaunchAgent scheduler install path、workspace-local watch wrappers、MAS generic workbench shell、legacy scheduler default aliases、daemon-ish terminal attach owner claim 和 scheduler residue。
