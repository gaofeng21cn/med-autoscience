...
- 决策：`methodology_reframe_route_decision` 物化后不得把 `methodology_reframe_route_decision` 自身作为 runtime 下一工作单元继续派发。controller decision 必须保留 `decision::methodology_reframe_route_decision` 作为决策 fingerprint，同时把 `next_work_unit` 指向可执行的 `analysis-campaign` 工作单元，例如 `medical_prose_quality_analysis_source_documentation_repair`，并携带 `analysis_harmonization_owner` / `unit_harmonized_external_validation_rerun` 的 hard-methodology 约束，避免 decision 自循环。
- 决策：controller authorization 的 work-unit compact/read-model 不能丢弃 hard-methodology 约束字段。`hard_methodology`、`required_owner`、`required_next_work_unit`、`typed_blocker` 和 `route_options` 是 runtime prompt 的机器合同字段，不是普通说明文本；若丢失，MAS worker 会把 DM002 误当成普通 prose/source-documentation repair 并再次产生假进度。
...
- 决策：`product-entry-manifest` 与 sidecar export 必须暴露 `runtime_transport_handoff_projection`。该投影逐项约束 `mas_runtime_core`、turn runner、worker lease、runtime supervisor scan/consume/dispatch/reconcile 和 `runtime_lifecycle_store.py`：它们只能作为 domain owner receipt adapter、refs-only sidecar index、guarded apply / typed blocker、safe action projection 或 standalone diagnostic，不得声明 generic scheduler、queue、attempt ledger、retry/dead-letter、worker residency、transition runner、persistence/lifecycle engine 或 workbench owner。
- 理由：MAS 历史 runtime transport 与 SQLite lifecycle 仍有 tracked physical code 和 focused tests。直接按文件存在与否判断会误导审计：这些文件既不能继续被理解为 MAS 私有 runtime 基座，也不能在 domain direct path、receipt parity 和 OPL replacement parity 未证明时盲删。
