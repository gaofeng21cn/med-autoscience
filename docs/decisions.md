...
- 理由：DM002 恢复后又暴露出 AI reviewer recheck callable 已返回 `ai_reviewer_record_stale_after_current_manuscript` owner receipt 和 record-production request，但旧 sidecar 顶层仍因 `accepted=false` 让 OPL dead-letter，后续又被稳定化为 observe-only blocker。该 case 的正确后续是由 AI reviewer record-only owner 生产当前 record，再经 MAS materializer/dispatcher 消费，不是 provider retry、手工 queue update 或直接改 `publication_eval/latest.json`。
...
- 决策：该 task 的 dispatch owner / queue owner 是 `one-person-lab`，domain truth owner 仍是 `med-autoscience`。MAS 不用 sidecar export 直接启动 Codex owner executor，不写 OPL queue 或 stage attempt；OPL 只能据此建立 Codex-default stage attempt，再由后续 MAS owner receipt、AI reviewer-backed publication eval 和 publication gate 判定论文进展。
...
- 决策：已进入 history/provenance 的旧 MDS / Hermes / local scheduler residue 不再通过 current product-entry manifest、sidecar export、status 或 read-model 暴露独立 audit surface。当前 active/default 面统一为 `standard_agent_purity`：`Declarative Medical Research Pack + OPL generated/hosted surfaces + minimal authority functions + refs-only domain projections`，并只暴露 domain refs、owner receipts、typed blockers 和可审计 blocker/ref。
- 理由：旧 audit surface 已无 active caller，只是在 current manifest 上重复呈现历史结论，继续保留会让 product/sidecar 面看起来仍有 legacy cleanup entrypoint。清理后，测试应断言旧 audit 字段不存在；历史解释和删除依据只从 history/provenance 或明确 legacy tombstone contract 读取。
...
- 理由：DM003 进一步暴露出 handoff 文件已生成，但当前 route reason `quest_waiting_opl_runtime_owner_route` 与 writer blocker `manuscript_story_surface_delta_missing` 不同，旧构造器改走 fallback route 并丢失 `runtime_health_epoch`，导致 owner-route attempt protocol fail closed，OPL sidecar 看不到 DM003 default executor writer task。桥接规则只继承当前 controller route 的 currentness，不授权 stale route 或缺 currentness 的 handoff。
...
- 影响：`runtime/archives`、storage audit、turn closeout、migration history 和旧 artifact 中的旧 token 继续作为 history/provenance 保存；会被当前 CLI/MCP/product-entry/sidecar/read-model 读取或抑制当前动作的 active path 只能通过当前 owner surface、tombstone refs 或重新物化处理。迁移不是兼容层，不新增 alias、fallback、regex 医学判断或旧 scheduler/runner 入口。
...
- 影响：后续修改 gap plan、status、current development lines、product-entry、sidecar、supervision projection 或 focused tests 时，必须拆开写 `功能/结构差距` 与 `测试/证据差距`。若发现新 surface 重新持有 generic owner，应作为 regression 修复；若 OPL replacement proof、standard Agent purity guard 和 no-forbidden-write proof 已成立，旧面直接退役，不新增 compat alias、facade、shim、wrapper 或兼容聚合测试。

## 2026-05-17：MAS package 外壳收敛为 OPL pack compiler 输入与 generated surface consumer

- 决策：MAS 不再把 CLI、MCP、Skill、product-entry、sidecar、status、workbench、projection shell 或 test-lane harness 视为长期手写 owner surface。它们在 `functional_consumer_boundary.generated_surface_handoff` 中统一声明为 OPL generated/hosted target 或 MAS domain handler / refs projection source。
