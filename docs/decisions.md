...
- 决策：`sync_transportability_reporting_surface` 的 currentness 判断必须以当前 resolved reporting contract 和当前 display stubs 为准。缺旧 `multicenter_generalizability_inputs.json` 不能返回 `already current`；只能表示没有 legacy F5 display payload 可迁移。`time_to_event_grouped_inputs.json` 的 stale 判定必须看当前 risk-group binding 下是否仍是 `groups` 结构且缺 `risk_group_summaries`，不能依赖旧 template id 是否命中。
...
- 影响：DM-CVD 002 这类跨队列 transportability 论文由新 MAS 的 reporting contract 决定 F5 owner surface；旧 multicenter F5 只在旧 contract 仍显式要求时生成。两种 F5 都缺失时继续 fail closed 为 missing display binding；未知旧 display shape 继续交给 owner 重建，不新增兼容读取层，不把 legacy artifact 读取、token 映射或 template-name normalizer 作为迁移路径。
