...
- 文件生命周期管理进入 SQLite sidecar 方向：runtime watch/report state 与 workspace storage audit 先写入 `artifacts/runtime/runtime_lifecycle.sqlite` 索引层，现有 `latest.json`、timestamped JSON/Markdown、publication/controller truth 与投稿产物继续保留文件 authority。该层用于降低 `.ds` / runtime history 小文件枚举成本，不替代 Git、paper authority 或 restore metadata。当前真实 workspace 仍处于 index/ledger/audit-only 为主的迁移状态，尚未完成百万级 `.ds` 深层文件压降。
...
16. Runtime lifecycle storage 的当前落地面是 SQLite sidecar + 文件 authority 双轨：MAS 已把 runtime report store、runtime event index、workspace storage audit、compatibility read/export 与 migration ledger 登记进 `runtime_lifecycle.sqlite` 相关 contract；MDS 侧 runtime maintenance 继续持有 cold/archive/restore 判定。真实 workspace 当前状态是：NF-PitNET、DM-CVD、AS biologics 和 HeRR 均已有 ledger/readiness 证据，但多数 `.ds` 热点仍因为 live、paused、stop-loss、dirty workspace、pinned/recent worktree 或缺 restore proof 保持 audit-only。NF-PitNET 是无功能垂体瘤的 canonical workspace；`/Users/gaofeng/workspace/Yang/无功能垂体瘤` 只是本机轻量 stale alias/scaffold，不作为独立项目重复迁移统计，也不得作为 repo-tracked `nfpitnet` profile 或 fixture 的真实 workspace root。后续迁移按 [Runtime Lifecycle SQLite Migration Program](./program/runtime_lifecycle_sqlite_migration_program.md) 推进 stopped-cold restore-proof lane；event/run/bash/codex history metadata、cursor pagination 与 retention ledger 优先进入 SQLite，原始 terminal/stdout/session payload、tar/gzip 冷归档、expanded worktree、paper/manuscript/package/dataset restore contract 继续保持文件或归档形态。
