...
MAS repo 现在把该 blocker 接成可执行 owner surface：`complete_medical_paper_readiness_surface` 是 `MedAutoScience` owner callable、default-executor allowed action、owner-route reason 和 domain-owner dispatch action。它消费 `artifacts/medical_paper/readiness.json` 的 `next_action.surface_key`、dispatch / request 中的 provider-backed operator payload，调用 guarded medical-paper operator action 写入对应 `artifacts/medical_paper/<surface>.json`，随后重建 readiness；缺 payload 或 surface key 时只写 stable owner blocker 到 `artifacts/controller_decisions/latest.json`。2026-06-06 follow-through 已补齐 `literature_provider_runtime` payload authoring：可从 ready `literature_intelligence_os.json` 生成 operator payload；apply 模式下可从 study contract、verified literature materialization、PubMed、Crossref 和 Semantic Scholar adapter 生成 provider-backed payload 与 provider response ledger；provider 缺失、非 ready evidence 或 adapter 异常时 fail closed 为 owner blocker。该 callable 不写 `publication_eval/latest.json`、paper、submission package、`current_package` 或 publication quality verdict。当前 live readiness next action 对两篇均是 `surface_key=literature_provider_runtime`；DM002 已有 `ready_count=1/13` 且 `literature_provider_runtime=missing_canonical_artifact`，DM003 为 `ready_count=0/13` 且同样先缺 `literature_provider_runtime`。该功能面 landing 不等于 DM002 / DM003 live owner closeout；后者仍要由对应 owner run 产出匹配 StageRun/source/idempotency 的 owner receipt、quality gate receipt、typed blocker、human gate 或 route-back evidence。
...
| DM002/DM003 current owner handoff | Stage-native read-model 当前 owner 已是 `MedAutoScience / complete_medical_paper_readiness_surface`，原因是 `medical_paper_readiness_not_ready`。MAS repo callable / policy / route / dispatch tests 已落地，可从 dispatch / owner request 中取得 provider-backed operator payload，按 `readiness.next_action.surface_key` 物化 `artifacts/medical_paper/<surface>.json` 或写 stable owner blocker；`domain-owner-action-dispatch` 顶层返回 `owner_delta_result`，直接暴露 `owner_receipt` / `quality_gate_receipt` / `stable_typed_blocker` shape，不再要求 OPL 默认面继续做 read-model/currentness 解释。2026-06-06 follow-through 已把 `literature_provider_runtime` operator payload authoring 接入 owner surface：ready literature intelligence 可直接复用，apply 模式可调用 PubMed / Crossref / Semantic Scholar adapter 写 provider response ledger，失败时 fail closed。下一步由 live owner run 补 `literature_provider_runtime` 的真实 StageRun closeout binding，再按 readiness surface 继续推进剩余医学论文准备能力；不得直接 tick、手写论文 truth，或把 stage-folder typed blocker 写成 publication-ready。 |
