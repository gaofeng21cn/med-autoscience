...
- 2026-05-21 本轮已把 `status_and_decision.py` 的 status finalization / refs-only projection attachment 拆入 `study_runtime_decision_parts/status_projection_shell.py`。这只把 runtime summary、supervision refresh、runtime event、family companion、worker/parked projection、truth/control-plane snapshot 和 progress projection 从主决策文件中分离出来；MAS decision authority、publication gate、manual finish dominance、controller authorization、runtime health semantics、artifact/source/owner boundary 均未迁出，也不能写成 OPL status/control plane 已接管。
