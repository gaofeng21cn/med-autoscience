...
- Portable supervisor 现在已经把 request ownership 与 submission milestone parking 收口进运行合同：`publication_gate_specificity_required` 只生成 `publication_gate` owner 的 request packet，`return_to_ai_reviewer_workflow` 只生成 `ai_reviewer` owner 的 request packet，`runtime-supervisor-consume` 只消费 queue 并写 handoff task；它不能直接写 `publication_eval/latest.json`、不能修改 paper/current_package，也不能替代 AI reviewer 或 publication gate authority。
...
16. Runtime lifecycle storage 的当前落地面是 SQLite sidecar + 文件 authority 双轨：MAS 已把 runtime report store 与 workspace storage audit 登记进 `runtime_lifecycle.sqlite`，MDS 侧 runtime maintenance 继续持有 cold/archive/restore 判定。后续迁移按 [Runtime Lifecycle SQLite Migration Program](./program/runtime_lifecycle_sqlite_migration_program.md) 直接推进到全 workspace baseline、dry-run、eligible stopped/cold apply、compatibility export 与 restore proof；event/run/bash/codex history metadata、cursor pagination 与 retention ledger 优先进入 SQLite，原始 terminal/stdout/session payload、tar/gzip 冷归档、expanded worktree、paper/manuscript/package/dataset restore contract 继续保持文件或归档形态。
