...
0. `agent_lab_medical_manuscript_self_evolution`
   ...
   随后的 source provenance follow-up 把 Agent Lab work order 扩展到 `source_provenance_owner_recovery`：`source_provenance_owner` 会在受控 study/runtime root 内搜索模型、RESULT 和 provenance 候选，并只接受完整 `canonical_transport_model_provenance_bundle`。只有指标摘要、旧稿方法描述或重新拟合替代模型时，它会保留 `transport_model_provenance_recovery_required` typed blocker，并把 `provenance_search` 写入 owner result 供下一步决策审计。
