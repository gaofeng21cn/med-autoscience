...
均有显式 per-Stage policy，其中四个产出 Stage 绑定独立 reviewer
Attempt/session，`review_and_quality_gate` 固定为 cross-Stage Meta Review，final
handoff 固定为 reviewed bytes 的单次机械封装 Attempt；六阶段到八个
paper-study physical Stage 的映射归一，三轮语义修订预算与 provider/dispatch
retry 分账。OPL Framework 仍负责
...
| Stage prompt model | 六个精简目标型 prompt；同 thread 自检仅为 `in_thread_refinement`，需 formal Review 的 Stage 使用 fresh Attempt/session；专业/证据/authority 顺序由 Stage policy、domain Skill 与 quality gate 托底 |
| Quality cycle | `contracts/stage_quality_cycle_policy.json`；六个 policy 均满足严格 Framework schema，四个开放判断/产物 Stage 默认三轮 repair + re-review；预算耗尽且有可消费 artifact 时带质量债终局，Meta Review 不递归 Review，final handoff 只机械封装 exact reviewed bytes |
