...
| DM002/DM003 current owner handoff | Stage-native read-model 当前 owner 为 `publication_gate_owner / publication_handoff_owner_gate`，需要 `publication_handoff_owner_receipt_or_typed_blocker`；pause closeout 的 OPL queue hold / human gate 是 provider control truth，仍需从 OPL current-control fresh 读取。下一步必须由 MAS publication owner 产出 owner receipt、typed blocker、human gate 或 route-back evidence；不得直接 tick、手写论文 truth，或把 stage-folder closure 写成 publication-ready。 |
