...
| Package transition | repo manifest 当前声明 source version `0.2.25`、required presence edge、配置的 Codex Plugin carrier，以及仍待迁移的 ScholarSkills version range/ABI、activation materialization/receipt 与 validator Release Set ref；这些是当前机器事实，不是目标 composition 规则，也不能证明 owner GHCR `latest-stable` 或完整 carrier installed/callable currentness |
...
## 0.2.25 validator Release Set 边界

`mas-validator-0.2.25` 只支持 exact-byte domain validation：generation manifest、
