...
- 文件生命周期管理已进入 SQLite sidecar + restore-proof archive 双轨：runtime watch/report state、workspace storage audit、archive refs 与 migration ledger 写入 `artifacts/runtime/runtime_lifecycle.sqlite` 索引层，现有 `latest.json`、timestamped JSON/Markdown、publication/controller truth 与投稿产物继续保留文件 authority。2026-05-05 current eligible workspace 迁移已把 NF-PitNET、DM-CVD / DPCC 和 AS biologics 的 11 个 migrated quest `.ds` 从约 1,024,244 个文件压到 772 个文件；DM002/DM003 盘面仍标记 `active`，但 `active_run_id=null`，本轮按用户确认的 parked active 窗口和 operator-confirmed gate 迁移，没有 relaunch 或 resume。
...
16. Runtime lifecycle storage 的当前落地面是 SQLite sidecar + 文件 authority + restore-proof archive：MAS 已把 runtime report store、runtime event index、workspace storage audit、compatibility read/export 与 migration ledger 登记进 `runtime_lifecycle.sqlite` 相关 contract；MDS 侧 runtime maintenance 持有 cold/archive/restore 判定。2026-05-05 本轮真实 workspace 迁移已完成 NF-PitNET、DM-CVD / DPCC 和 AS biologics 的 eligible quest restore-proof compaction：DM002 从约 496,470 个 `.ds` 文件降到 129，DM003 从约 201,744 降到 178，DM001 legacy 从 5,815 降到 14，NF legacy 002 从 17,318 降到 14，AS001 从约 62,383 降到 56；11 个 migrated quest 合计从约 1,024,244 个文件降到 772 个文件，restore proof 均为 verified、errors 为 0。NF-PitNET 是无功能垂体瘤的 canonical workspace；`/Users/gaofeng/workspace/Yang/无功能垂体瘤` 只是本机轻量 stale alias/scaffold，不作为独立项目重复迁移统计，也不得作为 repo-tracked `nfpitnet` profile 或 fixture 的真实 workspace root。后续维护按 [Runtime Lifecycle SQLite Migration Program](./program/runtime_lifecycle_sqlite_migration_program.md) 继续执行：active quest 默认 audit-only，只有 `active_run_id=null` 且用户明确确认 parked active 时才允许 operator-confirmed compaction；任何 DM001 live writer 或自动恢复意图仍应先归类为 MAS/MDS 状态机问题。event/run/bash/codex history metadata、cursor pagination 与 retention ledger 优先进入 SQLite，原始 terminal/stdout/session payload、tar/gzip 冷归档、expanded worktree、paper/manuscript/package/dataset restore contract 继续保持文件或归档形态。
