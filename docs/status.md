...
- 当前默认 runtime machine id 已收敛为 `runtime_substrate/runtime_ref=opl_hosted_stage_runtime` 与 `runtime_engine_id=opl-hosted-stage-runtime`；`opl_runtime_default_operation_contract`、product-entry manifest 与 sidecar export 声明默认 generic runtime owner 为 `one-person-lab`，domain adapter 为 `mas_domain_intent_adapter` / `mas_domain_owner_receipt_adapter`。`default_autonomous_runtime`、`provider_topology.default_autonomous_runtime`、`managed_temporal_state_consistency.default_autonomous_runtime` 与 `opl_unique_control_plane_handoff.default_caller_policy` 共同声明：hosted autonomous runtime 默认启用，provider 为 `temporal`，wakeup/retry/resume owner 为 OPL，`codex_app_outer_driver_required=false`，`mas_daemon_scheduler_attempt_loop_allowed=false`。
...
- Progress Portal 顶层源码已收薄为 MAS-owned payload / refs projection API；workspace 静态包写入、本地只读 HTTP carrier、study page materialization 和浏览器打开已物理下沉到 `progress_portal_parts/workspace_carrier.py`。active renderer 中旧 `conversation-*` timeline 样式残留已删除，focused test 防止 runtime conversation read model 或 conversation UI 回到 MAS Portal。carrier 子面只允许写 `artifacts/runtime/progress_portal/*` 与 `ops/mas/progress/*` display/read-model 文件，hosted package 暴露 `mas_progress_portal_workspace_carrier_boundary`、active callers 和 delete blockers；`medautosci workspace progress-portal`、`--serve` 与 `ops/mas/bin/start-web` 仍调用它，所以当前状态是 active diagnostic carrier delete-blocked，不是可物理删除队列。
- owner-route default-executor materialize / dispatch shell 的 action allowlist、forbidden / allowed write surfaces、source ref fields、request owner、request output surface 与 request packet ref 已收敛到 `default_executor_action_policy.py` 单一 policy module；`domain_action_request_materializer.py` 与 `domain_owner_action_dispatch.py` 只消费该 policy 并保留原有导出兼容。这是 `owner_route_reconcile_materialize_dispatch_shell` 的物理拆薄证据，不关闭 `standard_agent_purity_guard` 或 `domain_ref_consumer_physical_thinning`，也不授权删除 sidecar / controller / domain-action wrapper。
