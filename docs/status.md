...
- `physical_source_morphology_standardization` 现在单独列为结构治理口径：MAS 的 `supervisor` / `supervision_scheduler` / `runtime_supervisor_*` / `runtime_transport` / `runtime_lifecycle_store.py` / worker lease / turn runner / product cockpit / sidecar provider / runtime status projection 等历史命名已经被合同收薄为 domain bridge、receipt、typed blocker、refs-only adapter、diagnostic 或 direct domain handler target，但这不是最终物理形态。当前源码仍可见 `runtime_supervisor_*`、`supervision_scheduler.py`、`runtime_transport/mas_runtime_core.py`、`runtime_protocol/runtime_lifecycle_store.py`、`product_entry_parts/workspace_cockpit/` 和 `sidecar_provider.py` 等 active files；它们不能被写成 MAS-owned generic runtime，也不能被写成已经物理清零。后续应继续 rename、split、archive、delete 或 tombstone 到医学语义名，并把 developer repair/worktree 元数据外迁到 OPL Agent Lab / developer repair lane 或 explicit contract refs。
- 2026-05-19 的 OPL legacy cleanup 读取当前 MAS manifest 后返回 `plan_status=ready` / `lifecycle_apply.status=dry_run_ready`；随后 OPL refs-only lifecycle ledger 已 apply 空计划 closure batch receipt，`verify` 可读回 `verified_receipt_count=1`。MAS tombstone proof 已补齐 replacement parity refs、no-regression evidence refs、history refs 和 tombstone refs。该状态只证明 OPL cleanup gate / refs-only ledger 可安全消费 MAS cleanup proof，不表示 tracked runtime transport、supervisor 或 SQLite sidecar 已物理删除。
...
MAS 已完成 owner/contract/read-model 收薄，但 active source 仍保留历史平台化形状。下一轮 cleanup 要按 active caller、OPL parity、domain receipt parity、focused tests 与 no-forbidden-write proof 逐项处理：

- `runtime_supervisor_scan/consume/dispatch/reconcile`：改名或拆分为 domain route projection、owner action request、authority receipt dispatch、domain blocker reconcile 或 diagnostic probe。
- `supervision_scheduler*`：保留 OPL replacement / legacy diagnostic / tombstone refs，不能再表达 MAS local scheduler owner。
