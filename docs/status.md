...
- `runtime_backend_default_operation_contract`、product-entry manifest 与 sidecar export 声明默认 generic runtime owner 为 `one-person-lab`，generic substrate 为 `opl_provider_backed_stage_runtime`；`mas_runtime_core` 的角色是 `mas_domain_owner_receipt_adapter`，不是 MAS-owned generic runtime platform。
