...
- 2026-05-21 `runtime_transport/__init__.py` 的 package-level generic callable facade 已退役；默认 OPL provider 通过 explicit `runtime_backend` registry 验证，`runtime_transport` package 本身不再导出 `create_quest` / `resume_quest` / `schedule_turn` 这类 runtime callable。`runtime_transport/` 显式子模块、turn runner、worker lease 与 `runtime_lifecycle_store.py` 仍有 active domain / diagnostic caller；当前角色只能是 domain receipt adapter、refs-only lifecycle sidecar、guarded apply / typed blocker bridge 或 standalone diagnostic，不能写成已经物理删除。
- 2026-05-21 本轮继续把 `mas_runtime_core_turns.py` 的 JSON/state/queue/snapshot 壳拆入 `mas_runtime_core_turn_state.py`，主 turn lifecycle 文件从约 993 行降到约 944 行，并保留同名 public helper 委托。该 tranche 只关闭 turn lifecycle retained adapter 的文件边界收薄，不关闭 `runtime_transport/`、turn runner、worker lease、SQLite lifecycle sidecar 的 active caller、OPL parity、真实 paper-line owner receipt parity 或 physical delete gate。
...
- 2026-05-21 本轮继续把 turn authorization retained adapter 收薄：`mas_runtime_core_turn_authorization.py` 的 runtime prompt / controller action contract / AI reviewer redrive contract / claimed-message filtering 已拆入 `mas_runtime_core_turn_prompt.py`，主 authorization 文件从约 950 行降到约 725 行，继续只持有 runtime-state sanitization、controller currentness、owner-handoff arbitration 和 authorization sync。focused proof 覆盖 runtime core turn、blocked closeout、pause/stopped relaunch、worker watchdog、worker cache/env、OPL provider default 与 runtime backend 行为不变。该 tranche 只关闭 turn authorization prompt shell 的文件边界收薄，不关闭 `runtime_transport/`、turn runner、worker lease、SQLite lifecycle sidecar、真实 paper-line owner receipt parity、no-active-caller 或 physical delete gate。
- 2026-05-22 本轮继续把 `mas_runtime_core_turns.py` 的 user-message queue / claim / restore shell 拆入 `mas_runtime_core_turn_messages.py`，主 turn lifecycle 文件从约 944 行降到约 916 行，并保留同名 public helper 委托。`functional_privatization_audit.active_path_residue_cleanup_gates.runtime_turn_runner_closeout_adapter.current_paths` 已把新模块列入 retained turn adapter gate。该 tranche 只关闭 user-message delivery shell 的文件边界收薄，不关闭 `runtime_transport/`、turn runner、worker lease、SQLite lifecycle sidecar、OPL queue/provider parity、真实 paper-line owner receipt parity、no-active-caller 或 physical delete gate。
...
- 2026-05-21 本轮已物理退役 MAS repo-local `runtime watch --loop` / `run_watch_loop` / `run_managed_supervisor_loop` 长循环 shell；`ops/medautoscience/bin/watch-runtime` 新生成与 legacy upgrade 均只调用 one-shot `runtime watch` domain diagnostic tick。OPL provider/runtime manager 继续持有 cadence、wakeup、retry/dead-letter、attempt 与 operator projection；MAS 只保留 runtime health、owner-route、typed blocker、safe-action refs 与 AI doctor repair receipt 解释边界。该 tranche 不删除 `runtime_transport/`、turn runner、worker lease、SQLite lifecycle sidecar，也不声明真实 paper-line owner receipt parity 或 long-soak 完成。
...
- 2026-05-21 本轮已把 `sidecar_family_adapter` 的 dispatch 编排拆入 `sidecar_family_adapter_parts/dispatch_orchestration.py`，主 sidecar 文件只保留 export、projection 与薄 dispatch wrapper。focused proof 已覆盖 sidecar dispatch 只写 MAS dispatch receipt ref，不创建 `.ds/user_message_queue.json`、runtime quest root、controller decision truth、publication eval truth、`manuscript/current_package` 或 `current_package.zip`。这只关闭 `sidecar_dispatch_adapter` 的物理收薄与 no-forbidden-write focused proof，不关闭 active caller、OPL replacement parity、真实 paper-line MAS owner receipt parity 或 physical delete gate。
- 2026-05-21 本轮继续把 `runtime_lifecycle_store.py` 的 SQLite connection、schema、inspection、report-index 和 index-result 壳抽入 `runtime_lifecycle_store_parts/sqlite_sidecar.py`，主文件从 985 行降到约 728 行并保留 public API。focused proof 覆盖 runtime lifecycle store 与 OPL family persistence adapter 仍按原表结构、refs-only inspection 和 cleanup gate 输出工作。该 tranche 只关闭 SQLite lifecycle sidecar 的代码形态收薄，不关闭 `sqlite_lifecycle_sidecar_index` 的 active caller、OPL lifecycle index parity、真实 paper-line owner receipt parity、no-active-caller 或 physical delete gate。
