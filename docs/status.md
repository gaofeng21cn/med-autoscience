...
- 当前 family-shared substrate 复用真相：从 `2026-04-17` 起，`managed runtime contract` builder/validator 与 Hermes-hosted supervision 的通用 job/script/jobs.json 匹配逻辑，已改为直接复用 `one-person-lab` 持有的 `opl-harness-shared` 包；`MedAutoScience` 本仓只保留 medical-domain wrapper 与治理语义
