...
- Progress Portal 顶层源码已收薄为 MAS-owned payload / refs projection API；workspace 静态包写入、本地只读 HTTP carrier、study page materialization 和浏览器打开已物理下沉到 `progress_portal_parts/workspace_carrier.py`。该子面只允许写 `artifacts/runtime/progress_portal/*` 与 `ops/mas/progress/*` display/read-model 文件，hosted package 暴露 `mas_progress_portal_workspace_carrier_boundary`、active callers 和 delete blockers；`medautosci workspace progress-portal`、`--serve` 与 `ops/mas/bin/start-web` 仍调用它，所以当前状态是 active diagnostic carrier delete-blocked，不是可物理删除队列。
