...
| DM002/DM003 current owner handoff | Stage-native read-model 当前 owner 为 `publication_gate_owner / publication_handoff_owner_gate`，需要 `publication_handoff_owner_receipt_or_typed_blocker`；MAS repo callable / policy / route / focused tests 已落地，可 fail-closed 写出 stage-native handoff receipt 或 typed blocker。pause closeout 的 OPL queue hold / human gate 是 provider control truth，仍需从 OPL current-control fresh 读取；具体 study 是否已产出 live receipt / blocker 仍以 workspace owner evidence 为准。不得直接 tick、手写论文 truth，或把 stage-folder closure 写成 publication-ready。 |
