...
- 2026-05-21 `study_progress` 已把 current `domain_transition` 作为只读 progress read-model 字段透出，并让 `domain_transition_runtime_redrive` 优先于 submission metadata / package handoff / manual-finish parking 投影。该项只修 MAS 人读进度面与 owner-route 可见性，不生成 generic runtime attempt，也不把 OPL liveness、queue、retry/dead-letter 或 provider resume 下沉回 MAS。
