...
12. 当前 mainline 收口记录见 [Plan Completion Ledger](./program/plan_completion_ledger.md) 的 `2026-05-04-final-mainline-cleanup`、`2026-05-04-mas-mainline-closeout-cleanup`、`2026-05-04-v6-research-quality-and-test-hardening-closeout` 与 `2026-05-04-current-state-and-oar-delivery-authority-closeout`：OAR/DM002、v6 hardening、Developer Supervisor Mode、portable supervisor scheduler、storage governance/backfill/safe-cache/continuous-soak 相关 repo-level 能力保持 landed truth；重复 DM002 closeout 提交已进入远端历史，不做历史重写；`.sentrux` baseline refresh 继续按外部结构基线 lane 处理。当前 `open_auto_research_projection` 会把真实 study soak / delivery-journal usability guard 作为只读状态投影，明确 `submission_ready_authorized=false`、下一步仍是 AI reviewer / publication authority；OAR/DM002 soak 不替代 `publication_eval/latest.json`、`controller_decisions/latest.json` 或 `study_runtime_status`。截至本次 fresh audit，root `main` clean，相对 `origin/main` ahead 5：本轮新增 `67763b08 Guard OAR soak delivery authority`，其余 ahead 提交由外部/并发线持有；本轮未 push，外部 active worktrees 继续保留。
