...
| Workspace target-state cleanup | `ops/medautoscience/` 是唯一 MAS workspace ops 入口；`ops/mas/progress/` 只保留 Progress Portal 静态只读投影。`ops/mas/bin/*`、`ops/mas/config.env`、`ops/mas/README.md` 与 `ops/mas/behavior_equivalence_gate.yaml` 已退役为 legacy bridge surface；`workspace-target-state-cleanup --visual-clean` 可受控归档已知 study-local / ops residue，未分类 entry fail closed 到 blocker。该清理是 platform repair / topology hygiene，不写 study truth、paper、publication eval、controller decisions、artifact body、memory body、owner receipt 或 publication readiness。 |
