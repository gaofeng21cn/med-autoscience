...
- `sidecar export` 的 `domain_route/reconcile-apply`、`paper_autonomy/guarded-apply`、`publication_aftercare/*`、`domain_owner/default-executor-dispatch` pending task 和 stage-level owner handoff 已提供 `domain_dispatch_evidence_record_payload`。该 payload 只携带 MAS-owned typed blocker refs、owner-chain refs、no-regression evidence refs、identity fields、OPL 可记录的 receipt hint、success / typed-blocker payload path 与可直接提交给 `opl runtime action execute` 的 refs-only payload；typed blocker ref 与 ledger hint 保留 stage/source identity token，避免同类 stage attempt 在 refs-only ledger 中碰撞；`domain_owner_receipt_refs` 为空时只表示 owner receipt 尚未产生，不可读成 paper closure。当前 live guarded-apply sidecar dispatch 已能在真实 DM workspace 产出 MAS owner receipt refs；若 OPL refs-only record route 已先按 typed-blocker path verified，后续真实 owner receipt 只能通过新的或 superseding owner-payload route 被消费，不能回写改写已关闭的 OPL receipt。
...
- owner-chain dispatch ledger scaleout：`domain_dispatch_evidence_record_payload` 已显式输出 OPL owner-payload group 可消费的 `domain_owner_receipt_refs`、`owner_chain_refs`、`no_regression_evidence_refs`、`typed_blocker_refs`、return shapes 和 payload paths；真实 guarded-apply sidecar dispatch 已能产出 MAS owner receipt refs 与 no-forbidden-write guard。仍需让更多 owner-route / guarded-apply / aftercare / default-executor / stage-level handoff 在同一轮里完成 `MAS owner receipt or stable blocker -> OPL identity preflight -> refs-only ledger record/verify`，并在 mismatch 时 fail closed；已 verified 的 OPL typed-blocker receipt 不能被事后升级成 success receipt。
