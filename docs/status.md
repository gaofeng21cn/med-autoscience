...
- 当前 family-shared substrate 复用真相：从 `2026-04-17` 起，`managed runtime contract` builder/validator、Hermes-hosted supervision 的通用 job/script/jobs.json 匹配逻辑，以及 `product_entry_quickstart / product_entry_overview / product_entry_readiness` 的公共 payload helper，已改为直接复用 `one-person-lab` 持有的 `opl-harness-shared` 包；`MedAutoScience` 本仓只保留 medical-domain wrapper、research truth 与治理语义
