...
- `sidecar export` 的 `domain_route/reconcile-apply`、`paper_autonomy/guarded-apply`、`publication_aftercare/*`、`domain_owner/default-executor-dispatch` pending task 和 stage-level owner handoff 已提供 `domain_dispatch_evidence_record_payload`。该 payload 只携带 MAS-owned typed blocker refs、evidence refs、no-regression refs、identity fields、OPL 可记录的 receipt hint 与可直接提交给 `opl runtime action execute` 的 refs-only payload；study-level target 使用 `study_id`，stage-level target 使用 `stage_id`，不会为纯 stage target 伪造 `unknown-study`。`identity_binding` 明确 `stage_id`、`domain_source_fingerprint` 与 `stage_attempt_source_fingerprint` 参与 target identity 绑定：`domain_source_fingerprint` 绑定 MAS owner-route currentness，`stage_attempt_source_fingerprint` 绑定具体 OPL provider attempt，stale / mismatched attempt 必须 fail closed。profile path、artifact/memory/current-package body、paper body 和 domain truth body 不能作为 preflight 成功条件。
- owner-route 控制层已收敛为 `mas-owner-route-attempt-protocol.v1`：owner route 统一携带 reason registry、priority lattice、currentness contract 与 `owner_route_currentness_basis`；`domain_owner/default-executor-dispatch` pending task 导出完整 Owner-Route Attempt envelope 和 typed closeout completion boundary。AI reviewer record-production handoff 和 managed-runtime controller authorization 必须保留 runtime-health currentness basis；未注册 reason 或缺 work-unit/truth/runtime/source fingerprint 的 dispatch fail closed，不进入 OPL pending task。该协议只授权 OPL queue/attempt/provider/read-model transport；OPL provider completion 不等于 MAS owner receipt、AI reviewer pass、package freshness 或 submission authorization。
...
- route/stage 残留边界的机器面已收敛到当前 owner-route/read-model/receipt 名：`owner_route_reconcile`、`progress_projection`、`domain_health_diagnostic`、`domain_decision_authority`、`owner_receipt_lifecycle_ref_index` 与 `owner_route_dispatch_receipt`。旧 MAS 私有 runtime/route surface 名只保留为 tombstone/provenance；仍有 active caller 的当前实体继续受 no-active-caller、OPL replacement parity、MAS receipt parity、focused tests、no-forbidden-write proof 和 tombstone refs 删除门约束。
- T2E reporting / display legacy alias 已退役：MAS 不再把 `kaplan_meier_grouped` requirement key 改写为 `time_to_event_risk_group_summary`，也不再允许 `time_to_event_risk_group_summary` binding 继续 materialize `cumulative_incidence_grouped` grouped payload。旧 workspace 必须通过 `time_to_event_direct_migration` 重物化 canonical display input；未知旧 shape fail closed。
- submission-target 输入 alias 已退役：`default_submission_targets`、`study.yaml/quest.yaml submission_targets` 与 `paper/submission_targets.resolved.json primary_target` 不再接受 `publication_profile` 作为 exporter alias，必须显式写 `exporter_profile`；`publication_profile` 继续只作为已解析 package/export profile 输出字段和 submission manifest 领域字段存在。
...
MAS 已完成 owner/contract/read-model 收薄，并完成 domain route / domain SLO projection active source 命名收口。剩余差距统一读作 retained adapter / diagnostic 的物理删除门，不是 MAS generic runtime owner 复活，也不是已经清零：

| retained surface | 当前定位 | 删除门 |
| --- | --- | --- |
| `runtime_transport/`、`mas_runtime_core*`、turn runner、worker lease | 只能作为 domain receipt adapter、guarded apply / typed blocker bridge 或 standalone diagnostic；package-level generic callable facade 已退役。 | no-active-caller、OPL provider parity、真实 paper-line receipt parity、focused tests、no-forbidden-write proof、history/tombstone refs。 |
| `lifecycle_refs_adapter.py` 与 lifecycle refs parts | refs-only lifecycle refs index；不得写成 MAS generic persistence engine。 | OPL lifecycle index parity、MAS domain receipt parity、active caller 清零、focused tests、tombstone refs。 |
...
- domain dispatch owner-route / aftercare ledger scaleout：当前多批真实 workspace sidecar payload 已证明 OPL identity guard 能消费匹配 refs-only payload，并能 fail-closed 阻断错误回填；MAS sidecar 现在同时输出 `opl_runtime_action_execute_payload`、record/dry-run usage 和 `identity_binding`，减少 operator 把新 payload 记录到旧 target 的风险。这仍只是 payload 消费 / route-back hygiene，不替代 reviewer refresh 执行、runtime redrive 执行、writer direct-fix、owner receipt success、artifact/memory/human-gate receipt 或 long-soak。
