...
以下是 `2026-06-04` live audit、显式 wakeup 和 owner-chain follow-through 的当前文档摘要；具体 study owner、action、route key、stage attempt 和 blocker 必须重新运行 live 命令确认。

| Study | 已记录状态 | 当前读法 |
| --- | --- | --- |
| DM002 `002-dm-china-us-mortality-attribution` | storage 迁移后可经新 MAS runtime-state surface 定位；显式 wakeup 记录为 `truth-event-000036-fed012c6450e5df5`，当前 OPL dispatch task `frt_7e2bfa7f2c9960c1204127ba` 已批准并进入 running attempt `sat_5d11d86ff9a1ee5b379030ee`，action 是 `run_quality_repair_batch`。 | 这是 provider 正在执行 writer/prose-gate replay lane 的证据；仍不是 publication-ready、paper closure、artifact authority 或 `current_package` 更新。`study progress` 读面可能仍显示 queued，需以 owner-route / OPL attempt ledger fresh evidence 判定 runtime liveness。 |
| DM003 `003-dpcc-primary-care-phenotype-treatment-gap` | storage 迁移后可经新 MAS runtime-state surface 定位；显式 wakeup 记录为 `truth-event-000023-93bb65cc238e6517`，当前 OPL dispatch task `frt_bcfe9b417c7bff94787ea583` 已批准并进入 running attempt `sat_fe0c6a536fc5c9e21bca6389`，action 是 `run_gate_clearing_batch`。 | 这是 provider 正在执行 gate-clearing replay lane 的证据；仍不是 publication-ready、paper closure、artifact authority 或 `current_package` 更新。`study progress` 读面可能仍显示 queued，需以 owner-route / OPL attempt ledger fresh evidence 判定 runtime liveness。 |
...
| State index / restore-proof storage maintenance | MAS refs-only 小文件治理 pilot 已作为 opt-in storage maintenance surface 落到 `med_autoscience.runtime_protocol.refs_only_state_index_pilot`，通过 `runtime maintain-storage --refs-only-state-index-pilot`、`runtime maintain-storage --refs-only-state-index-pilot --refs-only-state-index-only` 或 `runtime storage-audit --apply --refs-only-state-index-pilot` 写 `artifacts/runtime/mas_refs_only_state_index_pilot.sqlite`。`runtime maintain-storage` 会先把 legacy `.ds/runtime_state.json` 物化为 canonical `artifacts/runtime/state/runtime_state.json`，再由 SQLite 将 canonical surface 记为 `lifecycle`、legacy `.ds` surface 记为 `legacy_lifecycle`；SQLite rebuild 只替换当前 `quest_root` 切片，避免多篇论文互相覆盖。该 SQLite 只保存 cursor/index/lifecycle/outbox/receipt ref 的路径、哈希、大小、mtime、index version 和 rebuild epoch，不保存 study truth、publication eval、controller decisions、manuscript、artifact body、memory body、owner receipt authority 或质量 verdict，也不计 stage completion / paper progress。`--refs-only-state-index-only` 是百万文件 `.ds` root 的首选索引 canary 入口：它显式跳过 legacy backend maintenance 和递归 size summary，只重建 refs-only SQLite pilot。`--restore-proof-canary --restore-proof-canary-entry-limit <n>` 是后续 restore-proof 机械验证入口：它只对每个选中 bucket 的 bounded 文件/链接样本写 archive、source manifest、restore proof、plan 和 receipt，保留源 `.ds` payload，记录 `actual_release_bytes=0`、`source_retained=true`、`mutated_runtime_payload=false`。`--restore-proof-compaction --restore-proof-bucket runs --restore-proof-bucket codex_homes --restore-proof-max-shards <n>` 是 stopped-cold runtime payload compaction 入口；它按 child shard 写 restore proof 和 refs-only archive refs 后删除已验证 source group；若只剩无 payload 空 child 目录，`nothing_to_archive` 路径会继续删除空 child 与空 bucket。`--legacy-ds-root <path/to/.ds> --restore-proof-compaction` 是 historical archive / nested bare `.ds` 的专用入口，要求 `.ds` 位于当前 profile workspace 内，并同样写 archive manifest、restore proof 与 refs-only archive refs。2026-06-04 full local historical closeout 覆盖四个已发现本地 MAS workspace 下的 active quest roots、legacy archive `.ds` 与 nested `.ds/python_pycache/**/.ds` residues：fresh physical check 读为没有剩余 `.ds/runs` 或 `.ds/codex_homes` bucket；archive refs final scan 为 `archive_ref_row_total=6025`、`archive_ref_quest_root_count=14`。同日 Yang active quest follow-through 物化了 DM-CVD 4 个、Obesity 1 个、NF-PitNET 4 个 active quest 的 canonical runtime state，并验证 workspace SQLite 分别覆盖 4/1/4 个 quest，`body_included_values=[0]`；Yang-wide physical check 读为没有 `.ds/runs`、`.ds/codex_homes` 或 `.ds/python_pycache` bucket。OPL Stage Folder closeout 位于 `medautoscience/state-index-full-migration/all-workspaces/historical-runtime-payload-compaction`，owner receipt ref 为 `mas-runtime-storage-full-payload-compaction:all-discovered-runtime-quests:20260604T044000Z`。这只关闭 discovered local historical runtime payload storage tail，不写 paper/source/memory/artifact truth，也不授权 publication-ready、stage completion、domain-ready 或 production-ready。 |
| Stage-native paper workspace materialization | `stage-artifact-materialize` 是 MAS-owned refs-only migration surface，用来把旧 study workspace 的 `study.yaml`、`paper/**`、controller artifacts、publication eval、reports 和 delivery refs 折叠为 `artifacts/stage_outputs/<stage_id>/` 下的 stage work-unit bundles、`stage_artifact_manifest.json` 和 `owner_receipt.json`。它只写 locator / hash / receipt，不复制 legacy body，不改 paper/package/publication truth，不授权 quality verdict、publication-ready 或 submission-ready。非终局 stage 的推进条件是 durable stage output + owner receipt / typed blocker / handoff；publishability 和 submission readiness 只属于 terminal publication handoff。2026-06-04 DM002 与 DM003 已各物化 8 个 stage、每篇 40 个 stage-native refs 文件，`domain_authority_refs.sqlite.paper_work_unit_receipts` 中各有 8 条 `stage_artifact_delta` receipt。Fresh `stage_artifact_index` 对全 stage materialized study 仍把 `current_stage` 投到 `08-publication_package_handoff`，并返回 `publication_handoff_owner_gate`，要求 `publication_handoff_owner_receipt_or_typed_blocker`；它不能因为 8 个 stage folder 都有 artifact delta 就返回 terminal ready 或清空下一 owner。旧 `paper/`、`artifacts/` 和 manuscript/package 文件仍作为 source truth 被 refs 指向；看见旧目录存在不表示仍按旧目录管理。`study_progress.current_stage=queued` 是 runtime/execution envelope，不是 stage folder kernel 的 current stage。 |
| Stage-native paper authority reader | `paper_authority_cutover/current_body` 是 paper body authority cutover 后的 reader authority surface。`medical_manuscript_blueprint` 的当前 reader、medical prose review request、study progress writing surface 和 AI reviewer request packet completion 必须在 legacy `paper/medical_manuscript_blueprint.json` 缺失、stage-native `artifacts/stage_outputs/_body_authority/paper_authority_cutover/current_body/paper/medical_manuscript_blueprint.json` 存在时使用 stage-native ref。writer/materializer 仍显式写 legacy canonical/source owner callable surface；不得因为 reader authority 已迁入 stage-native root，就把 AI-authorized payload 写回 `current_body` 或手工复制到旧 `paper/`。2026-06-04 DM002/DM003 live materialize 后，两篇 `artifacts/supervision/requests/ai_reviewer/latest.json` 的 `input_contract.required_refs.medical_manuscript_blueprint` 都指向 stage-native current body，`present=true`、`valid=true`。 |
| DM002/DM003 current owner handoff after stage-native blueprint repair | 2026-06-04 重新运行 Yang DM-CVD `domain-health-diagnostic --request-opl-stage-attempts --request-opl-owner-route-reconcile --apply`、`owner-route-reconcile --apply-safe-actions --developer-supervisor-mode developer_apply_safe`、`domain-action-request-materialize --apply`、`domain-owner-action-dispatch --apply` 后，DM002 和 DM003 都不再落到 `canonical_paper_inputs_rehydrate_required`。两篇当前 action queue 均为 `return_to_ai_reviewer_workflow`，reason 为 `paper_authority_clean_migration_required`，目标 surface 为 `artifacts/publication_eval/latest.json`。最新 dispatch execution 写于 `2026-06-04T14:11:12+00:00`，两篇均返回 typed blocker `opl_execution_authorization_required`，owner=`one-person-lab`，required input=`OPL provider attempt, lease, or closeout receipt binding`，`write_permitted=false`。这是真实下一 owner blocker，不是 stale paper path blocker、publication-ready、paper closure 或 MAS 私有执行授权。 |
