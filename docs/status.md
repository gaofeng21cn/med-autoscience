...
- `domain-handler export` 的 `domain_route/reconcile-apply`、`paper_autonomy/guarded-apply`、`publication_aftercare/*`、`domain_owner/default-executor-dispatch` pending task 和 stage-level owner handoff 已提供 `domain_dispatch_evidence_record_payload`。该 payload 只携带 MAS-owned typed blocker refs、owner-chain refs、stage expected receipt refs、monitor freshness refs、runtime event refs、no-regression evidence refs、identity fields、OPL 可记录的 receipt hint、success / typed-blocker payload path 与可直接提交给 `opl runtime action execute` 的 refs-only payload；typed blocker ref 与 ledger hint 保留 stage/source identity token，避免同类 stage attempt 在 refs-only ledger 中碰撞；`domain_owner_receipt_refs` 为空时只表示 owner receipt 尚未产生，不可读成 paper closure。provider-hosted guarded-apply 默认 target 现在只从当前 profile 中有 `study.yaml` 的 canonical live study id 派生，并限于默认 canary target 的 canonical match，避免继续导出 `DM003` 这类 retired shorthand identity。当前 OPL refs-only ledger 已记录 `DM003` stale guarded-apply attempt 和 DM002 default-executor attempt 的 MAS-owned typed blocker / owner-chain / no-forbidden-write refs，`domain_dispatch_evidence_workorder_count=0`；这只关闭对应 OPL refs-only record route，不关闭 paper、publication、artifact、memory 或 owner-chain live scaleout。
