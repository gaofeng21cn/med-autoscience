...
| Package | immutable SemVer `0.2.5`；`mas-scholar-skills` exact lock `0.2.2`（range `>=0.2.0 <0.3.0`）为硬依赖，并提供 reference verification 与 scientific search adapter modules；生命周期统一归 `opl packages` |
