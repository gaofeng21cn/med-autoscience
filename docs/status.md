...
0. `agent_lab_medical_manuscript_self_evolution`
   ...
   2026-05-19 follow-up 修复了 controller decision 到 runtime prompt 的 hard-methodology 字段丢失：`next_work_unit.hard_methodology`、`required_owner`、`required_next_work_unit`、`typed_blocker` 和 `route_options` 现在会保留到 `current_controller_authorization` 与 Codex runtime prompt，避免 DM002 在 `methodology_reframe_route_decision` 后退回普通 prose/source-documentation worker。
