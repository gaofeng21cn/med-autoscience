...
- 当前 family-shared substrate 复用真相：从 `2026-04-18` 起，`managed runtime contract` builder/validator、Hermes-hosted supervision 的通用 job/script/jobs.json 匹配逻辑、`product_entry_quickstart / product_entry_overview / product_entry_readiness / product_entry_start / product_frontdesk` 的公共 payload helper，以及 `family_orchestration` 的 preview shell / runtime companion / human gate / `program_id` 与 `active_run_id` 解析 helper，已改为直接复用 `one-person-lab` 持有的 `opl-harness-shared` 包；`MedAutoScience` 本仓只保留 medical-domain wrapper、research truth 与治理语义
