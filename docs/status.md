...
- 同一轮主线还把医学稿件初稿质量从“gate 后置拦截”前移为写作前合同：`medical journal prose`、IMRAD section contract、reporting-guideline obligations 与 first-draft generation model 必须在 study charter / quality OS 中可见。
...
- 当前 repo-side 已新增 `StudyTruthKernel` shadow layer：`study_runtime_status` 会构造 `study_truth_snapshot`，`study-progress` 会投影 `truth_epoch` 与 `study_truth_snapshot`。普通 read 不刷新 `artifacts/truth/latest.json`；materialized truth 只由显式 reconcile/controller tick 写入，避免 status/progress 读取路径继续制造 authority 漂移。
- 当前 repo-side 已新增 `RuntimeHealthKernel` shadow layer：`study_runtime_status` 会构造 `runtime_health_snapshot`，`study-progress`、`runtime_watch`、workspace-cockpit、product-frontdesk 与 MCP compact projection 会投影 `runtime_health_epoch` 与 runtime health 摘要。普通 read 不刷新 `artifacts/runtime/health/latest.json`；materialized health 只由 `runtime reconcile-health`、`runtime watch --apply` 或 controller tick 写入，避免 recovering / live / stale run handle 被多个 surface 重复解释。
- 当前 repo-side 已新增 medical journal prose 质量合同：first draft 必须是医学期刊可读的 manuscript-shaped prose，不能把 controller checklist、figure/table anchor、author-confirmation placeholder、内部 claim-boundary 标签或 operations/review 语言带入正文。
- 当前 repo-side 已把 workspace Git/storage 边界收紧为轻量外层 Git + generated/runtime/artifact 排除 + explicit storage audit repair；runtime-storage apply-mode release estimate 必须反映实际 apply strategy，而不是 gross candidate size。
...
- 当前 `skill-catalog` 的 domain projection 已暴露 `opl_runtime_manager_registration` v1：OPL Runtime Manager 可按这份 registration 索引 MAS 的 runtime continuity、research runtime control projection、artifact locator、attention/wakeup 与 runtime health 输入；其中高频 workspace/session/artifact/attention/runtime-health 索引可由 OPL Rust native helper 消费，并通过 `native_helper_consumption.proof_surface` 与 `contracts/opl-gateway/native-helper-contract.json` 证明 helper 只做 index-only 消费，MAS durable truth 仍是权威来源。
...
8. 把 `StudyTruthKernel`、`RuntimeHealthKernel` 与 first-draft prose quality contract 视为当前读状态与写作前门槛的核心文档面；维护者先读 [Study Truth Kernel Contract](./runtime/study_truth_kernel.md)、[Runtime Health Kernel Contract](./runtime/runtime_health_kernel.md)、[Medical Manuscript First-Draft Quality Policy](./policies/medical_manuscript_first_draft_quality.md) 和 [AI-first Quality Boundary Policy](./policies/ai_first_quality_boundary.md)。
