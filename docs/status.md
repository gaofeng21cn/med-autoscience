...
16. Runtime lifecycle storage 的当前落地面是 SQLite sidecar + 文件 authority 双轨：MAS 已把 runtime report store 与 workspace storage audit 登记进 `runtime_lifecycle.sqlite`，MDS 侧 runtime maintenance 继续持有 cold/archive/restore 判定。后续迁移优先推进 event/run/bash/codex history metadata、cursor pagination 与 retention ledger，原始 terminal/stdout/session payload、tar/gzip 冷归档、expanded worktree、paper/manuscript/package/dataset restore contract 继续保持文件或归档形态。
