...
14. Developer Supervisor Mode 已正式定义为开发环境外层巡检/排队能力：普通 MAS 运行默认保留 `internal_only`，portable 外层可用 `external_observe`，本机开发环境使用 `developer_apply_safe`。该模式只允许写 supervision/control/autonomy request、handoff packet、action queue 与 why_not_applied 投影；不得 patch `paper/current_package` 或 `manuscript/current_package`，不得放宽 publication/quality gate。`developer_apply_safe` 受 GitHub 用户门控保护，只有本机 `gh api user --jq .login == gaofeng21cn` 时启用，否则自动降级到 `external_observe`。
