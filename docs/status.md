...
- Progress Portal 顶层源码已收薄为 MAS-owned payload / refs projection API；workspace 静态包写入、本地只读 HTTP carrier、study page materialization 和浏览器打开已物理下沉到 `progress_portal_parts/workspace_carrier.py`。该子面只允许写 `artifacts/runtime/progress_portal/*` 与 `ops/mas/progress/*` display/read-model 文件，hosted package 暴露 `mas_progress_portal_workspace_carrier_boundary`；它仍不是完整 wrapper 删除，只是等待 OPL App 默认 progress carrier、workspace helper no-active-caller proof 与 focused tests 后可删除的 carrier 子面。
