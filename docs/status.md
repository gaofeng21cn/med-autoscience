...
- 当前 `skill-catalog` 的 domain projection 已暴露 `opl_runtime_manager_registration` v1：OPL Runtime Manager 可按这份 registration 索引 MAS 的 runtime continuity、research runtime control projection、artifact locator、attention/wakeup 与 runtime health 输入；其中高频 workspace/session/artifact/attention/runtime-health 索引可由 OPL Rust native helper 消费，但仍必须回指 MAS durable truth。
