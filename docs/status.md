...
同日已落领域侧 hosted authority 候选：`mas.paper-mission-authority-evaluate`
...
当前状态仍是 `candidate_pending_opl_v2_host_binding`；旧 `paper_mission`、
`domain_handler_export/dispatch` 与 `mainline_*` 不在本次候选中提前删除。
...
| Hosted authority candidate | `contracts/domain_handler_registry.json`；纯 `paper_mission` callable 已落，OPL v2 binding 与真实 terminal smoke 待完成 |
...
| `paper_mission` private discovery/readback/persistence transport | `partial` | OPL v2 绑定纯 authority callable，提供 typed refs、contained atomic output commit 与 readback；再迁默认 callers 并删除旧 transport |
| `mainline_status` / `mainline_phase` 私有产品状态 shell | `partial` | OPL hosted status/workbench 覆盖机器状态与阶段视图；generated/default callers 不再发布两个 MAS action |
