...
## Purpose-First Rendering Flow

Paper-facing figures and tables must not be generated by selecting a visually attractive template first.

The required sequence is:

1. state the artifact purpose: the claim, contrast, decision, or descriptive question that the figure or table must make easier to understand;
2. choose the display family and concrete template that best serves that purpose;
3. render through the audited Display Pack path;
4. confirm the sidecar/catalog records `figure_purpose`, `source_renderer`, and `rendered_title_policy`;
5. inspect the generated figure/table and the submission PDF page where it appears;
6. promote any repeated visual failure into renderer, template, or QC truth rather than repairing it only in the paper workspace.

The default title policy for paper figures is `figure_title_metadata_only_not_drawn_inside_plot`.
The manuscript caption carries the figure title. A renderer may still use panel labels, axis labels, legends,
or compact panel subtitles when they directly help interpretation, but it should not draw a duplicate overall
figure title inside the image.

If an artifact is partly a decision matrix, checklist, participant flow, or other non-axis display, the template
must use a layout that matches that purpose. It must not force long prose into a fake axis panel. Long rationale
