...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack` | 当前默认数据证据图只展示 canonical current R/ggplot2 模板；非数据设计/流程/graphical-abstract shell 作为独立 design Gallery 分区展示，但不混入 R/ggplot2 evidence 分区，也不承担统计证据 authority。具体 current canonical template、Gallery evidence figure、design / flow figure、non-visual inventory、retired alias 和 Python evidence 数量由生成的 [Display Pack Gallery status](../examples/display_pack_gallery_status.md) 给出。Python evidence 模板不作为隐藏库存、显式请求库存、Gallery 对比图、agent discover 或 runtime materialization 库存保留。共享 R helper 位于 `display-packs/fenggaolab.org.medical-display-core/rlib/medicaldisplaycore/`，display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
