...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack_external_source_with_mas_fallback_no_package_vendoring` | 当前默认数据证据图只展示 canonical current R/ggplot2 模板；cohort flow 作为 R/ggconsort-backed reporting flow 单独展示；graphical abstract 作为非统计设计壳展示。具体 current canonical template、Gallery evidence figure、design / flow figure、table preview、retired alias 和 Python evidence 数量由 ScholarSkills compact gallery review package 与 pack descriptor 给出。Python evidence 模板不作为隐藏库存、显式请求库存、Gallery 对比图、agent discover 或 runtime materialization 库存保留。通用 core pack source 由 `mas-scholar-skills/packs/medical-display-core/` 承接；MAS `external/display-packs/medical-display-core/` 只作为 source-tree temporary deprecated bundled migration fallback，不再由 `setup.py` 投影到 wheel/sdist 的 `med_autoscience.resources.display_pack_repo`。display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
