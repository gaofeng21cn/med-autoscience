...
当前完成度是 `MAS Display Pack v2 runtime lane complete, OPL Pack OS substrate landed externally, MAS refs-only boundary preserved`。
...
| Core pack R/ggplot2 template migration | `landed_p0` | 当前 22 个 `renderer_family = "r_ggplot2"` evidence templates 已迁到 `execution_mode = "subprocess"`，每个模板目录都有 template-local `render.R`，entrypoint 固定为 `Rscript render.R --request {request_json}`；共享 R helper 位于 `display-packs/fenggaolab.org.medical-display-core/rlib/medicaldisplaycore/evidence_renderer.R`，display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。P1/P2 分类见 core pack `renderer_migration_ledger.json` 与 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。 |
| Core pack R/ggplot2 candidate assets | `landed_p1_candidate_assets` | 33 个 P1 Python evidence templates 已有 template-local `render_candidate.R`，共享 helper 位于 `rlib/medicaldisplaycore/candidate_renderer.R`；`medautosci publication display-pack-render-candidate` 可对裸 `display_payload` 生成 request/stdout/stderr、PNG、PDF 和 layout sidecar，结果固定 `candidate_only=true`、`publication_readiness_verdict=false`。 |
...
当前 Display Pack v2 不是“模板市场已完成”，也不是“所有 98 个 catalog 模板都有 renderer/golden”。它表示 MAS 已有 paper-facing display pack descriptor、paper-level quality refs、R/ggplot2-first subprocess runtime 协议、22 个 core R/ggplot2 evidence templates 的一等 subprocess 默认入口、33 个 P1 Python evidence templates 的一等 R/ggplot2 candidate asset、Python plugin 兼容 renderer、multi-figure deterministic E2E render/QC/publication-manifest path、visual audit / polish lifecycle 和 submission refs preservation 的可验证下界；OPL 侧已有通用 Pack OS install、registry、cache、distribution、lock、validation 和 MAS consumer smoke，MAS 合同只保留外部 substrate refs 和 forbidden-authority 边界。core pack 的 P1 default renderer promotion 仍需 golden regression、visual audit 和 owner gate，不靠口头计划推进。
...
| 4 | `paper/figure_spec.json` / `paper/figure_specs.json` | MAS-native grammar 绑定 intent、template、figure kind 和医学语义；`figure_specs.json` 支持一次物化多张图。 | 不是 Vega-Lite runtime、renderer 或 publication verdict。 |
| 5 | deterministic render + QC | `medautosci publication display-pack-e2e` 生成 figure refs、PDF/PNG/layout sidecar、layout QC、artifact manifest 和 display pack lock；template 可用 `python_plugin` 或 `subprocess`。 | gate clear 只是下界，不等于视觉完成。 |
...
- 22 个 template 标为 `renderer_family = "r_ggplot2"`，并已使用 `execution_mode = "subprocess"`；
- 62 个 template 是 `renderer_family = "python"`；
- 22 个 core template 已使用 `execution_mode = "subprocess"`；
- 22 个 core template 目录已有 `render.R`；
- 33 个 P1 Python template 目录已有 `render_candidate.R`，可通过 `display-pack-render-candidate` 单独触发；
- `renderer_migration_ledger.json` 覆盖 84/84 evidence templates：P0 landed 22，P1 R candidate asset landed 33，P2 retained Python / later dual-stack 29，unclassified 0。

P0 迁移已经完成；P1 candidate asset 也已经完成。后续目标是按 ledger 对 P1 candidate 做 golden diff 和 visual audit promotion，对 P2 模板继续保留 Python 或按真实论文需求做局部双栈。结构评估和迁移清单见 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。
...
- template 必须存在 `render_candidate.R`，入口固定为 `Rscript render_candidate.R --request {request_json}`；
- 输出 JSON 固定 `candidate_only=true`、`publication_readiness_verdict=false`，并记录 request/stdout/stderr、PNG、PDF 和 layout sidecar；
- candidate receipt 不能替代默认 renderer、publication manifest、artifact authority、visual-audit clear、owner receipt 或 publication gate。
