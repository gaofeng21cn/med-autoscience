...
| Dependency Environment OS | `target_planned` | [Medical Display 依赖环境 OS 目标设计](./display_dependency_environment_os_target.md) 与 `contracts/opl-framework/dependency-environment-substrate-contract.json` 固定目标边界：Display Pack 只声明 renderer 需求；OPL 负责 resolve / lock / prepare / doctor / run-context / cache；MAS preflight/render 只消费 dependency lock 与 prepared receipt refs。当前 `renderer_dependency_profile.json` 仍只是 pack 局部需求声明，不能写成 OPL dependency substrate 已落地。 |
