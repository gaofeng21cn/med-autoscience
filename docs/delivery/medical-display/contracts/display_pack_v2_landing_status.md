...
| Agent-facing capability surface | `landed_orchestrate_first_adaptable_baseline_with_workflow_packet` | `src/med_autoscience/display_pack_agent.py` 暴露 `display-pack-capability-discover`、`display-pack-orchestrate`、`display-pack-figure-plan`、`display-pack-preflight`、`display-pack-render`；同一能力进入 CLI grouped commands、`MedAutoScienceDomainEntry` command contract、`contracts/action_catalog.json`、`contracts/agent_tool_arsenal.json` 和 `domain-handler export.display_pack_agent_capability`。`agent_tool_arsenal mode=resolve` 先以 soft discovery 保留 Display Pack 候选，再用 scored fit 暴露 `adaptable_baseline_not_exact_contract`，最后由 `invocation_gate` 对 `paper_root`、claim/data refs、allowed writes、forbidden authority 和 receipts fail closed。`orchestrate` 从 `current_owner_delta`、claim/data refs、paper target 和 intent 编译 `figure_intent` / `figure_request`，再返回推荐 plan、preflight、quality floor、typed repair routes、Nature-Skills-informed `figure_workflow_packet` 和下一步 callable。模板选择采用 `quality-floor baseline` 语义：exact match 直接用，接近匹配则输出 `adaptable_baseline_not_exact_contract`、adaptation hints、allowed/forbidden layers 和 minimum fit floor。Agent 默认走 resolve -> orchestrate，不要求用户或 Agent 手工理解模板目录。 |
| Paper-level figure quality refs | `landed` | `contracts/publication_figure_quality_contract.json` 索引 `figure_intent`、单图 `figure_spec`、批量 `figure_specs`、style refs、visual audit receipt、figure render receipt、figure polish lifecycle、`figure_workflow_packet` 和 AI illustration receipt。 |
...
| Dependency Environment OS | `consumer_slice_landed` | [Medical Display 依赖环境 OS 目标设计](./display_dependency_environment_os_target.md) 与 `renderer_dependency_profile.json` 固定 MAS consumer 边界：Display Pack 只声明 renderer 需求；OPL 主仓 canonical `contracts/opl-framework/runtime-environment-substrate-contract.json` 负责 runtime lock / materialization / cache / doctor / run-context 的 substrate 边界；MAS preflight/render 只消费 dependency lock、prepared receipt 与 run-context refs。缺 prepared receipt、profile mismatch 或 managed library 缺包时，MAS fail closed 到 OPL prepare/doctor，不在 MAS 内安装、修包或降级。 |
...
| Figure brief / storyboard workflow packet | `landed_nonblocking` | `src/med_autoscience/display_pack_agent/figure_workflow.py` 和 `paper/figure_workflow_packet.json` 把 Nature-Skills 的 core conclusion、evidence hierarchy、hero/support panel storyboard、render-inspect-revise loop 和 paper-use acceptance checklist 落为 MAS-native surface。该 packet 默认随 plan/render 返回，不能阻断普通 R/ggplot2 evidence progress，不能修改数据/统计，不能替代 visual audit、owner receipt 或 publication gate。 |
...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack_external_source_without_default_mas_fallback` | 当前默认数据证据图只展示 canonical current R/ggplot2 模板；cohort flow 作为 R/ggconsort-backed reporting flow 单独展示；graphical abstract 作为非统计设计壳展示。具体 current canonical template、Gallery evidence figure、design / flow figure、table preview、retired alias 和 Python evidence 数量由 ScholarSkills compact gallery review package 与 pack descriptor 给出。Python evidence 模板不作为隐藏库存、显式请求库存、Gallery 对比图、agent discover 或 runtime materialization 库存保留。通用 core pack source 由 `mas-scholar-skills/packs/medical-display-core/` 承接；MAS 默认 config、pack root resolver 和 repo-default tests 不再把 `external/display-packs/medical-display-core/` 当成 hidden bundled fallback。该 legacy tracked asset 已从 `MANIFEST.in`、focused direct-source tests 和 tracked files 中物理退役；display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
...
| Gallery packaging boundary | `externalized_to_scholarskills_compact_review_refs` | MAS 不再在 `docs/delivery/medical-display/examples/` 提交 gallery PDF、manifest、single-figure previews、layout sidecars、payloads 或 render cache。ScholarSkills 维护 compact review refs；MAS gallery builder 仅作为历史/support bridge 和 test fixture，不授权渲染 freshness 或 publication readiness。 |
...
| Next ideal operating model / Display Agent OS | `orchestrate_surface_landed_with_figure_workflow_packet_full_os_target_planned` | [Display Pack Agent OS 目标架构](./display_pack_agent_os_target.md) 描述 `current_owner_delta / claim refs / data refs -> figure_intent -> figure brief/storyboard -> medical figure spec -> template resolver -> OPL pack lock -> render -> QC -> visual audit -> typed repair router -> publication manifest` 的目标链路。当前 MAS main code surface 已落地 agent-native orchestrate 编译、推荐、预检、quality floor、figure workflow packet 和 typed repair route hints；OPL-hosted pack lock integration、完整 typed repair router、ordinary-path resolver receipt 和 publication manifest handoff 仍是 planned/target。 |

当前 Display Pack v2 不是“模板市场已完成”，也不是“所有 catalog 模板都有正式 checked-in golden”。它表示 MAS 已有 paper-facing display pack descriptor、paper-level quality refs、article-level style profile lock、R/ggplot2-first subprocess runtime 协议、canonical current template surface、Python evidence 清理边界、可发现/可描述/可 scaffold 试跑的 CLI surface、agent-facing orchestrate/discovery/plan/preflight/render receipt surface、adaptable-template baseline resolver、可 refresh/check 的 deterministic lower-bound golden surface、multi-figure deterministic E2E render/QC/publication-manifest path、visual audit / polish lifecycle 和 submission refs preservation 的可验证下界；ScholarSkills/OPL Display 侧承接通用 core pack source，OPL 侧已有通用 Pack OS install、registry、cache、distribution、lock、validation 和 MAS consumer smoke，MAS 合同只保留外部 source/substrate refs、bundled fallback 和 forbidden-authority 边界。当前模板数量以 ScholarSkills compact gallery review package 与 pack descriptor 为准；style profile lock、renderer migration、display lock、golden match、visual-audit clear、agent orchestration/render receipt 或 comparison receipt 仍不能代签 publication readiness、artifact authority、owner receipt 或 publication gate。
...
当前状态是 `orchestrate_surface_landed_adaptable_baseline_with_workflow_packet_full_os_target_planned`。它继承已落地的 Display Pack v2 descriptor、paper-level figure-quality refs、medical figure spec、style profile lock、E2E render/QC/manifest path、visual-audit receipt、polish lifecycle、figure workflow packet 和 OPL repo 外部 Pack OS substrate；MAS main code surface 已提供 `display_pack_agent.orchestrate`，能从 current owner delta 与 claim/data refs 编译 figure intent、推荐 exact template 或 adaptable baseline、预检 paper/style/runtime/QC/golden 下界、返回 Nature-Skills-informed brief/storyboard/render-inspect-revise packet、typed repair route hints 与下一步 callable。但 OS-level OPL-hosted pack lock integration、完整 resolver receipt、完整 typed repair router 和 publication manifest handoff 还没有完整落地。
...
当前 core pack 的 renderer inventory 应通过生成状态分开读：

- current canonical template、Gallery evidence figure、non-visual inventory、retired alias / duplicate id 和 Python evidence 数量，以 ScholarSkills compact gallery review package 与 pack descriptor 为准；
- Gallery evidence figure 必须是 `renderer_family = "r_ggplot2"` 和 `execution_mode = "subprocess"`；
- Python evidence template count 必须为 `0`，除非未来按 re-entry 规则以 current audited template 重新入库；
- retired alias / duplicate id 只用于显式旧 ID 迁移，不作为 current template、Gallery 卡片、agent discover 默认项、runtime materialization 库存或显式请求库存。

R-first 收口已经完成。后续目标不是保留 Python evidence，而是继续用真实论文 payload、visual audit 和 render-inspect-revise 提升 R 模板质量；未来 Python evidence 只有在证明相对 R/ggplot2 有明确优势并通过视觉审计后，才可作为新的当前模板进入 pack。结构评估和迁移清单见 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。
...
Candidate/comparison receipt 触发规则：
...
- template 必须存在当前默认 `render.R` 或明确的 candidate wrapper；comparison receipt 只能作为审计或迁移提案证据，不能替代默认 R renderer；
...
- dependency environment resolve / lock / prepare / doctor / run-context / cache；
