...
| Core pack R/ggplot2 template migration | `landed_p0_p1_default_with_ab_python_retention` | 当前 51 个 evidence templates 已是默认 R/ggplot2 subprocess renderer：P0 22 个和 P1 promoted 29 个均使用 template-local `render.R`，entrypoint 固定为 `Rscript render.R --request {request_json}`。33 个 P2 retained Python templates 继续走 `python_plugin`；其中 `risk_layering_monotonic_bars`、`time_to_event_landmark_performance_panel`、`time_to_event_threshold_governance_panel` 和 `time_to_event_multihorizon_calibration_panel` 因 A/B golden contract 依赖 Python 多 panel layout sidecar，R wrapper 暂只作为 comparison asset。共享 R helper 位于 `display-packs/fenggaolab.org.medical-display-core/rlib/medicaldisplaycore/`，display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。P1/P2 分类见 core pack `renderer_migration_ledger.json` 与 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。 |
...
- 51 个 template 标为 `renderer_family = "r_ggplot2"`，并已使用默认 `execution_mode = "subprocess"`；
- 33 个 template 是 `renderer_family = "python"`，均为 P2 retained Python / later dual-stack；
- 51 个 core template 目录已有默认 `render.R`；
- 33 个 template 目录保留 `render_candidate.R` legacy comparison wrapper，其中 29 个是 P1 promoted renderer，4 个是 A/B locked P2 retained Python 模板的 R parity comparison asset；可通过 `display-pack-render-candidate` 单独触发 comparison receipt；
- `renderer_migration_ledger.json` 覆盖 84/84 evidence templates：P0 landed 22，P1 default R/ggplot2 promotion landed 29，P2 retained Python / later dual-stack 33，unclassified 0。

P0 迁移已经完成；P1 default R/ggplot2 renderer promotion 也已经完成。后续目标是对 P1 promoted renderer 继续积累 golden diff、visual audit 和 baseline comparison evidence，对 P2 模板继续保留 Python 或按真实论文需求做局部双栈；A/B locked P2 模板必须先证明 R layout/QC parity，才可重新晋级默认 R renderer。结构评估和迁移清单见 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。
...
- template 必须存在 `render_candidate.R`；若该模板仍是 P2 retained Python，comparison receipt 只能作为 R parity evidence，不能替代 Python default renderer；
