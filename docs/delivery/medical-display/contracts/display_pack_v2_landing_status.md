...
当前 Display Pack v2 不是“模板市场已完成”，也不是“所有 98 个 catalog 模板都有 renderer/golden”。它表示 MAS 已有 paper-facing display pack descriptor、paper-level quality refs、R/ggplot2-first subprocess runtime 协议、22 个 core R/ggplot2 evidence templates 的一等 subprocess 入口、Python plugin 兼容 renderer、multi-figure deterministic E2E render/QC/publication-manifest path、visual audit / polish lifecycle 和 submission refs preservation 的可验证下界；OPL 侧已有通用 Pack OS install、registry、cache、distribution、lock、validation 和 MAS consumer smoke，MAS 合同只保留外部 substrate refs 和 forbidden-authority 边界。core pack 的 Python-to-R 后续迁移不再靠口头计划，而由 `display-packs/fenggaolab.org.medical-display-core/renderer_migration_ledger.json` 维护 P1/P2 分类。
...
- `renderer_migration_ledger.json` 覆盖 84/84 evidence templates：P0 landed 22，P1 R candidate 33，P2 retained Python / later dual-stack 29，unclassified 0。

P0 迁移已经完成；后续目标是按 ledger 对 P1 candidate 做双栈 R candidate、golden diff 和 visual audit promotion，对 P2 模板继续保留 Python 或按真实论文需求做局部双栈。结构评估和迁移清单见 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。
