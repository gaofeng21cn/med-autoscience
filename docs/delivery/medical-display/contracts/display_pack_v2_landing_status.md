...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack` | 当前默认数据证据图只展示 canonical current R/ggplot2 模板；cohort flow 作为 R/ggconsort-backed reporting flow 单独展示；graphical abstract 作为非统计设计壳展示。具体 current canonical template、Gallery evidence figure、design / flow figure、table preview、retired alias 和 Python evidence 数量由生成的 [Display Pack Gallery status](../examples/display_pack_gallery_status.md) 给出。Python evidence 模板不作为隐藏库存、显式请求库存、Gallery 对比图、agent discover 或 runtime materialization 库存保留。core pack 位于 `external/display-packs/medical-display-core/`；共享 R helper 位于 `external/display-packs/medical-display-core/rlib/medicaldisplaycore/`，display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
