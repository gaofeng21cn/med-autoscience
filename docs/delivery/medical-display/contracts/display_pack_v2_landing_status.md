...
| Core pack R/ggplot2 template migration | `planned` | 当前 22 个 `renderer_family = "r_ggplot2"` evidence templates 仍是 `execution_mode = "python_plugin"` 的 legacy bridge：Python plugin 调 `r_renderer.py`，再临时写 R script 并调用 `Rscript`。core pack 当前没有 `execution_mode = "subprocess"` 模板，也没有 template-local `render.R`。迁移计划见 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。 |
...
当前 Display Pack v2 不是“模板市场已完成”，也不是“所有 98 个 catalog 模板都有 renderer/golden”。它表示 MAS 已有 paper-facing display pack descriptor、paper-level quality refs、R/ggplot2-first subprocess runtime 协议、Python plugin 兼容 renderer、multi-figure deterministic E2E render/QC/publication-manifest path、visual audit / polish lifecycle 和 submission refs preservation 的可验证下界；OPL 侧已有通用 Pack OS install、registry、cache、distribution、lock、validation 和 MAS consumer smoke，MAS 合同只保留外部 substrate refs 和 forbidden-authority 边界。core pack 当前的 R/ggplot2 模板仍处于 legacy bridge，需要按 renderer migration plan 升级为一等 subprocess 模板。
...
- 22 个 template 标为 `renderer_family = "r_ggplot2"`，但仍通过 `python_plugin` 桥接到 Rscript；
- 62 个 template 是 `renderer_family = "python"`；
- 0 个 core template 已使用 `execution_mode = "subprocess"`；
- 0 个 core template 目录已有 `render.R`。

迁移目标是先把 22 个 legacy R/ggplot2 bridge 模板升级为 `execution_mode = "subprocess"`，再评估哪些 Python 模板适合转成 R/ggplot2。结构评估和迁移清单见 [Display Pack Renderer 分层与 R/ggplot2 迁移评估](../plans/display_pack_renderer_migration_assessment.md)。
