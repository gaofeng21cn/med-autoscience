...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack_external_source_with_mas_fallback` | 当前默认数据证据图只展示 canonical current R/ggplot2 模板；cohort flow 作为 R/ggconsort-backed reporting flow 单独展示；graphical abstract 作为非统计设计壳展示。具体 current canonical template、Gallery evidence figure、design / flow figure、table preview、retired alias 和 Python evidence 数量由生成的 [Display Pack Gallery status](../examples/display_pack_gallery_status.md) 给出。Python evidence 模板不作为隐藏库存、显式请求库存、Gallery 对比图、agent discover 或 runtime materialization 库存保留。通用 core pack source 由 `opl-scholarskills/packs/medical-display-core/` 承接；MAS `external/display-packs/medical-display-core/` 只作为 bundled migration fallback。display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
...
| Gallery packaging boundary | `landed_cached_artifact_surface` | `scripts/build-display-pack-gallery.py` 区分 renderer rebuild 与 artifact packaging。默认路径可按 render cache 增量渲染；`--package-only` 只从既有 `medical_display_gallery_assets` 重建 HTML/PDF/docs/manifest，必要时从 docs mirror 种子复制 assets，不调用 R/Python renderer，也不要求宿主机 R。该边界用于文档组织和快速打包，不授权渲染 freshness 或 publication readiness。 |
...
- current canonical template、Gallery evidence figure、non-visual inventory、retired alias / duplicate id 和 Python evidence 数量，以 [Display Pack Gallery status](../examples/display_pack_gallery_status.md) 为准；
