...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack` | 当前 55 个 evidence templates 全部是默认 R/ggplot2 subprocess renderer，均使用 template-local `render.R`，entrypoint 固定为 `Rscript render.R --request {request_json}`。Python evidence 模板不作为隐藏库存、显式请求库存、Gallery 对比图、agent discover 或 runtime materialization 库存保留。Python 只保留在 design/flow/graphical-abstract composition shell 和 table shell 路径。共享 R helper 位于 `display-packs/fenggaolab.org.medical-display-core/rlib/medicaldisplaycore/`，display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
...
- 旧 Python evidence template id 不维护为当前 pack inventory、Gallery 对比图、agent discover、runtime materialization 或显式请求库存；
- `renderer_migration_ledger.json` 只覆盖 55/55 current evidence templates。
