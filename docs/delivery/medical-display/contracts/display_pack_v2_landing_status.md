...
| Core pack R/ggplot2 template migration | `landed_r_first_current_pack` | 当前 55 个 evidence templates 全部是默认 R/ggplot2 subprocess renderer，均使用 template-local `render.R`，entrypoint 固定为 `Rscript render.R --request {request_json}`。Python evidence 模板不作为隐藏库存、显式请求库存或 Gallery 对比图保留；32 个原 Python evidence template id 仅在 `renderer_migration_ledger.json` / canonical catalog 中作为 retired history 记录。Python 只保留在 design/flow/graphical-abstract composition shell 和 table shell 路径。共享 R helper 位于 `display-packs/fenggaolab.org.medical-display-core/rlib/medicaldisplaycore/`，display lock 记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`。 |
...
- 55 个 evidence template descriptor 已注册；
- 55 个 template 标为 `renderer_family = "r_ggplot2"`，并已使用默认 `execution_mode = "subprocess"`；
- 0 个 evidence template 是 `renderer_family = "python"`；
- 55 个 core template 目录已有默认 `render.R`；
- 32 个原 Python evidence template id 仅作为 retired history 记录，不作为隐藏库存、显式请求库存或 Gallery 对比图保留；
- `renderer_migration_ledger.json` 覆盖 55/55 current evidence templates，并记录 retired Python evidence ids。
