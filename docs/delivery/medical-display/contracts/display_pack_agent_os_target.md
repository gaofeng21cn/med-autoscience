...
2026-06-20 analysis-responsibility follow-through：canonical template catalog 现在显式区分 `computed_in_template`、`validated_summary_required`、`illustration_shell` 和 `table_shell`。PCA/t-SNE/UMAP 这类模板可以从 `raw_feature_matrix` 在 renderer 内完成受限降维 workflow；ROC、PR、calibration、DCA、KM、time-dependent ROC、heatmap、forest、SHAP、omics 和模型审计等模板只接受上游已验证 display payload，不在 display renderer 内拟合模型、重算统计量、重跑差异分析、推断 SHAP 或调用变异。`display_pack_figure_plan` / `preflight` / `render` 遇到 raw analysis input 命中 summary-only 模板时 fail closed，typed blocker 为 `analysis_summary_required_before_display_render`，下一步路由到 `materialize_validated_analysis_summary_before_display_render` 或表格值物化，而不是继续出图。

2026-06-20 follow-through：`nature-figure` 的高价值模式已经落到 MAS-native `figure_contract_policy`，但按 MAS 目标态做了改造。每次 `display_pack_figure_plan` / `orchestrate` 现在先编译 `figure_contract`：core conclusion、claim/data evidence chain、figure archetype、backend policy、journal/export contract 和 visual QA requirements。query-only 请求会先经 `medical_figure_family_catalog` 做 loose family route，再用 family 的 `template_seed_ids` 给 canonical template scorer 加权；这避免把同类图拆成大量名字相近的模板，也避免 agent 手工浏览 Gallery。

这不是把 Nature-Skills 的 backend gate 原样搬进 MAS。上游缺 backend 时会问 `Python or R?` 并停止；MAS evidence path 默认 R/ggplot2，不用这个问题阻塞 autonomous agent 推进。只有显式非默认 backend 或 Python evidence 重新入库时，才需要 documented advantage、visual audit 和 owner-surface 证明。模板仍然只是 `quality-floor baseline`：AI 可以改 family、variant、layout、panel structure、palette、legend、scale、backend 和 multipanel composition，只要保留 claim/data/statistics refs、estimand、source traceability、visual-audit receipt 和 publication authority boundary。
