...
5. **renderer family 也是正式 contract。** 路由只回答“这是证据图修复还是说明性图绘制”；真正的渲染技术栈还必须在 `paper/figure_semantics_manifest.json` 里锁定。当前默认允许矩阵是：`evidence -> r_ggplot2`，`illustration -> python | r_ggplot2 | html_svg`。Python evidence 只有在新模板能证明相对 R/ggplot2 有明确优势、通过视觉审计并进入 current pack 后，才可作为显式例外；`html_svg` 永远不能用于证据型图。
...
- **适用边界对照。** 如果要改动的图是直接支撑 claim/结果的 artifact，应走 `figure_script_fix` 由 MAS artifact route 产生，并默认把 renderer family 锁定为 `r_ggplot2`；如果图只是说明性图，则走 `figure_illustration_program`，renderer family 可为 `python`、`r_ggplot2` 或 `html_svg`。旧的 `figure_illustration_domain-handler` 和任何外部绘图 domain-handler 路由都属于已废弃歧义入口，不应再使用。
