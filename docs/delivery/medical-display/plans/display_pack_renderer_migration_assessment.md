...
Machine boundary: 本文是人读结构评估。机器真相继续归 `external/display-packs/**/display_pack.toml`、`external/display-packs/**/templates/*/template.toml`、renderer source、`renderer_migration_ledger.json`、E2E runtime、tests、真实 paper artifacts、visual-audit receipt、owner receipt 和 publication gate。
...
```text
external/display-packs/medical-display-core/
  display_pack.toml
...
当前采用 `independent_display_pack_consumed_by_MAS`：模板库作为独立 display pack 维护，MAS 通过 pack descriptor、registry、catalog、agent capability 和 materialization runtime 调用它。这样比把模板散落在 MAS 业务源码中更容易维护：

- 模板包可以按 R package / ggplot2 extension 的方式组织 `theme`、`scale`、shared helper、template-local `render.R` 和 descriptor；
- MAS 只关心医学 figure intent、claim/data refs、paper style profile、artifact lock、visual audit 和 publication gate；
- Gallery 是 pkgdown/Quarto-like 的发布产物，可从 cached assets 重打包，也可在 release 或 cache invalidation 时全量重渲染；
- 依赖环境由 OPL substrate 统一准备，模板包只声明 requirement profile，不维护 installer、fallback 或第二套 lock/doctor。

需要新增模板时，优先判断它是否是新的医学表达任务或新的图型结构，而不是新的输入 schema 名称。PCA、t-SNE、UMAP 可以分开，因为计算流程和读者预期不同；热图、ROC、KM 等重复输入变体默认合并到同一 canonical template family，通过 payload、facet、annotation 或 paper-local adaptation 表达差异。
