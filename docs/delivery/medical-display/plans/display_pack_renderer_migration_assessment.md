...
State: `landed_p0_with_p1_p2_migration_ledger`
...
`R/ggplot2-first` 的方向已经在 core pack 的 P0 层落地：现有 22 个 `renderer_family = "r_ggplot2"` evidence templates 已升级为 template-local `render.R` + `execution_mode = "subprocess"`。
...
因此，现状应读作：**22 个 R/ggplot2 evidence templates 已经是一等 subprocess 路径；62 个 Python evidence templates 继续按 P1/P2 ledger 管理后续迁移或保留。**
...
1. P0 已解决 `r_ggplot2` descriptor 与 template-local `render.R` 不一致的问题；后续维护重点转为 R dependency profile、R-level golden 和 P1/P2 promotion。
2. Display lock 现在记录 `renderer_family`、`execution_mode`、`entrypoint` 和 `render_script_sha256`，可以按真实 runtime 分账审计。
...
4. **P1 候选双栈**：time-to-event composite、forest/effect、omics/SHAP 标准图等 33 个 Python templates 已进入 ledger candidate；后续必须以 R candidate + golden + visual audit promotion 方式落地。
...
6. **桥接退役边界**：core descriptor 已不再引用 `render_r_evidence_figure`；`r_renderer.py` 仅保留旧 host materialization 兼容调用器。
...
- Python-to-R 候选迁移：P1 `33` 个 candidate，P2 `29` 个 retained Python / later dual-stack，均写入 `renderer_migration_ledger.json`；
