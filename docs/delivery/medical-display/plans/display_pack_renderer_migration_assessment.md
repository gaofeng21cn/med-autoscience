...
`R/ggplot2-first` 的方向已经在 core pack 落成默认路径：P0 的 22 个 `renderer_family = "r_ggplot2"` evidence templates 已升级为 template-local `render.R` + `execution_mode = "subprocess"`；P1 的 29 个高价值 evidence templates 已从 Python 默认路线提升为默认 R/ggplot2 subprocess renderer。P1 的 Python 实现继续作为 baseline / legacy comparison provenance，不再是默认 renderer。
...
| `renderer_family = "r_ggplot2"` | 51 | P0 22 个和 P1 29 个已是默认 R/ggplot2 subprocess renderer。 |
| `renderer_family = "python"` | 33 | P2 retained Python / later dual-stack templates，主要是 MAS-specific composite、storyboard、custom layout code 或尚未达到 R layout parity 的 A/B locked composite。 |
| `execution_mode = "subprocess"` | 51 | P0 `render.R` 默认入口加 P1 promoted R/ggplot2 subprocess 默认入口。 |
| 模板目录内 `render.R` | 51 | 每个 P0/P1 default R/ggplot2 template 都有 local wrapper，source 到 shared R helper。 |
...
因此，现状应读作：**51 个 evidence templates 已经是一等默认 R/ggplot2 subprocess 路径，其中 P0 22 个来自原生 R 模板，P1 29 个来自已完成 promotion 的 R/ggplot2 renderer；33 个 P2 templates 继续保留 Python 或后续按真实论文需求双栈。**
...
1. P0 已解决 `r_ggplot2` descriptor 与 template-local `render.R` 不一致的问题；P1 已把达到默认执行门槛的高价值模板 promotion 为 R/ggplot2 default renderer。`risk_layering_monotonic_bars`、`time_to_event_landmark_performance_panel`、`time_to_event_threshold_governance_panel` 和 `time_to_event_multihorizon_calibration_panel` 因 A/B golden contract 依赖 Python 多 panel layout sidecar，继续归 P2 retained Python，直到 R layout/QC parity 落地。
...
这些模板原默认 renderer 是 Python，但医学论文和 R 生态已有成熟惯例，适合优先切换到 R/ggplot2 subprocess。P1 29 个模板已完成 default promotion：默认 renderer 口径已切到 R/ggplot2 subprocess，共享 helper 位于 `rlib/medicaldisplaycore/candidate_renderer.R`，`display-pack-render-candidate` 继续保留为 legacy comparison receipt surface。Python 实现保留为 baseline / legacy comparison，不再承担默认渲染路线。

| 图类 | 原 Python 模板 | 迁移理由 |
| --- | --- | --- |
| Time-to-event promoted subset | `time_dependent_roc_comparison_panel`, `time_to_event_discrimination_calibration_panel`, `time_to_event_risk_group_summary`, `time_to_event_stratified_cumulative_incidence_panel`, `time_to_event_decision_curve` | R 侧 survival / time-dependent ROC / calibration / DCA 生态更成熟；这些模板当前 R sidecar 已满足默认执行门槛。 |
...
| A/B locked time-to-event composites | `risk_layering_monotonic_bars`, `time_to_event_landmark_performance_panel`, `time_to_event_threshold_governance_panel`, `time_to_event_multihorizon_calibration_panel` | 这些模板已有 Python 多 panel / threshold card / calibration sidecar golden contract。当前 R wrapper 仍是 generic candidate plot，不能替代默认 renderer；需先补齐 R layout/QC parity，再重新晋级。 |
...
4. **P1 默认 R/ggplot2 promotion**：time-to-event composite、forest/effect、omics/SHAP 标准图等 29 个原 Python templates 已 promotion 为默认 R/ggplot2 subprocess renderer；Python 保留 baseline / legacy comparison；CLI/API comparison surface 仍可产出 request/stdout/stderr、PNG、PDF 和 layout sidecar。
5. **P2 保留边界**：33 个 Python templates 标为 retained Python / later dual-stack，原因是 MAS-specific composite、storyboard 或 custom layout 风险较高。
...
- core pack default R/ggplot2 subprocess templates：当前 `51`，包含 P0 22 个和 P1 promoted 29 个；
- Python-to-R 默认化：P1 `29` 个已切到默认 R/ggplot2 subprocess renderer，Python 保留 baseline / legacy comparison；P2 `33` 个 retained Python / later dual-stack，均写入 `renderer_migration_ledger.json`；
