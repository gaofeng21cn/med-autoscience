...
| `composition_recipe` | 组织多个 evidence primitives 成论文图页 | clinical triptych、schematic-led composite、asymmetric genomics figure、image plate plus quantification、single-cell atlas storyboard | 不能变成更多重复 Gallery 卡片，不能替代真实数据 evidence renderer。 |
...
`composition_recipe` 回答“这页论文图怎样组织多个 evidence primitives”。它来自对 Nature-Skills `nature-figure` 工作流的 clean-room 吸收：先写 figure brief，再确定 hero panel、supporting panel、shared legend / direct label 策略、render-inspect-revise 和 paper-use acceptance。

当前机器 catalog 已落地 6 个页面级 recipe：

| Recipe | 用途 | 核心启发 |
| --- | --- | --- |
| `clinical_triptych_prediction` | 把 discrimination、calibration、decision curve / clinical utility 组织成临床预测三联图 | clinical triptych、shared legends、direct labels |
| `schematic_led_composite` | 用机制图、流程图或研究设计图作为 hero，并附最少必要证据 panel | schematic-led composite、design shell art direction |
| `asymmetric_genomics_figure` | 用热图、oncoplot 或分子格局作为大 panel，右侧/下方承接 consequence、pathway、clinical association | asymmetric genomics figure、fixed color scale |
| `image_plate_plus_quantification` | 把影像/组织/空间图版和程序化量化 panel 组合 | dark image plate、scale/channel discipline |
| `single_cell_atlas_storyboard` | 组织 embedding / spatial hero、marker、composition、trajectory / interaction | atlas storyboard、direct cluster labels |
| `model_validation_dashboard` | 把泛化、解释、subgroup、threshold / complexity governance 组织成模型验证图页 | hero validation panel、shared model/cohort palette |

维护规则：

- 不因为一个输入数据类型不同就新增单图模板。热图、ROC、KM、forest 这类常见视觉结构应优先收敛成 canonical primitive。
- 当论文表达需要多个 primitives 的页面组织时，优先新增或改进 `composition_recipe`，而不是堆新的 Gallery 卡片。
- 只有计算流程或可复现数据处理有本质差异时，才新增单图 primitive，例如 PCA、t-SNE、UMAP 可以保留为不同 workflow，因为它们从数据处理到坐标生成不同。
- 设计/流程/graphical abstract 可以使用 SVG、Python composition 或 imagegen art direction；它们不能携带统计证据 authority。
- 数据证据 panel 必须由程序化 renderer 或声明清楚的 computed workflow / validated summary 控制，且必须保留 `source_data_and_statistics_refs`。

## Canonical Category 清单

当前机器 catalog 已落地为 12 个 category、70 个 figure family 和 72 个 starter recipe 定义。本文只展示人读索引；完整 family、variant、starter recipe、loose-match terms、style / palette / QA refs 和 external refs 以 `contracts/medical-figure-family-catalog/` 为准。后续若要增删或重命名，必须先更新机器 catalog、loader 和 tests，再同步本文。
...
| `contracts/medical-figure-family-catalog/composition_recipe_policy.json` 和 `composition_recipes/*.json` | 页面级 composition recipe 的机器真相；agent discovery、figure plan、workflow packet 和 Gallery manifest 都从这里读取。 |
