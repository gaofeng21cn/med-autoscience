...
- table shell：`7`
- retired Python evidence ids：`32`，只作为 provenance / migration ledger，不作为 Gallery、agent discover、runtime materialization 或显式请求库存
...
- 新增 evidence template 默认必须是 R/ggplot2；Python evidence 只有在证明相对 R/ggplot2 有明确优势、通过视觉审计并进入 current pack 后，才允许重新进入。
- 不为历史兼容保留重复模板；旧 ID 只能作为 canonical alias 或 retired provenance。
