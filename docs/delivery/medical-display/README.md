...
- `cohort_flow_figure` / Figure 1 类 reporting-flow 图必须服务 participant accounting / STROBE-CONSORT flow。图体内只保留短节点、计数和必要连接，不画整体标题、summary panel、context note、prose card、fake-axis prose panel 或长解释；endpoint、design boundary 和 interpretation caveat 进入 caption、legend、manifest 或正文；
