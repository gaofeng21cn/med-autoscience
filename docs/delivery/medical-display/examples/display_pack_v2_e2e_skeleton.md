...
target_owner = "OPL Framework"
public_stage_actions = ["manuscript_authoring", "review_and_quality_gate", "finalize_and_publication_handoff"]

[opl_handoff.external_opl_consumer]
status = "landed_in_opl_repo"
owner_repo = "one-person-lab"
surface = "stage_tool_affordance_boundary"
contract_ref = "mas:agent/stages/manifest.json"
boundary = "OPL discovers and executes display affordances; MAS retains medical quality and publication authority."

[opl_handoff.package_lifecycle]
status = "landed_in_opl_repo"
owner_repo = "one-person-lab"
package_id = "mas"
required_dependency_package_id = "mas-scholar-skills"
public_lifecycle_actions = ["install", "update", "uninstall"]
boundary = "OPL owns dependency closure, lock, lifecycle receipt, rollback and generated carrier materialization."
```
...
Python plugin 模板使用同一 descriptor contract，但只作为 host-native adapter、轻量 fixture 或内部 materializer 路径：

```toml
template_id = "roc_curve_binary_python"
full_template_id = "fenggaolab.org.medical-display-core::roc_curve_binary_python"
kind = "evidence_figure"
display_name = "ROC Curve (Python plugin adapter)"
paper_family_ids = ["A"]
audit_family = "Prediction Performance"
...
`paper/figure_spec.json` 是单图 grammar 面。存在 `figure_specs.json` 时，`manuscript_authoring` Stage executor 通过 Tool Affordance Boundary 把 paper-level figure request 交给当前 Display Pack capability；具体工具选择由 Codex/OPL runtime 完成，MAS 不维护 `medautosci publication ...` parser 或 direct display action。

## 4a. Template Discovery And Scaffold Smoke

模板资产先由 Stage executor 通过 OPL capability discovery/readback 查找和确认：

discovery tool result 读取当前启用 pack 的 template inventory、renderer family、execution mode、QC profile、exports、golden refs 和 authority boundary。调用方可按 `template_id` 在返回 inventory 中选择单个 descriptor；discovery 不执行 renderer，也不写 paper artifact。

开发或验收单个 template 时，可以用 payload JSON 直接生成最小 paper scaffold，然后走正式 E2E runtime：

开发或验收单个 template 时，先准备最小 paper scaffold，再由 `manuscript_authoring` Stage 内的 display affordance 执行 preflight/render，随后由 `review_and_quality_gate` 消费 `visual_audit_review`。这只用于模板试跑和开发验收，不替代真实论文的 claim/data/spec authoring，也不凭 renderer success 授权 publication ready。

需要固定 deterministic lower bound 时，用同一 payload 建立 golden：

Golden refresh/check 不在当前 V2 public Stage action catalog 中，只属于 repo test/fixture 维护面。旧 `display_pack_capability_discover`、`display_pack_preflight`、`display_pack_orchestrate`、`display_pack_render` 名称也只允许出现在 internal residue/provenance 中。默认 golden strict match 覆盖 PNG、layout sidecar、deterministic QC status 和 publication style profile hash；PDF bytes 因设备元数据可能变化，作为 observed-only artifact hash 记录。Golden match 是模板下限回归信号，不是 visual audit clear、publication readiness、artifact authority 或 owner receipt。

## 4b. Publication Style Profile

`paper/publication_style_profile.json` 是文章级风格 token 单一真相。模板 descriptor 的 `style_profile_ref` 表示模板可消费的风格族；paper JSON 才给出本篇文章实际锁定的配色、字体、线宽和网格值。
...
E2E runtime 会把该文件的 sha256 写入 render request、layout sidecar、每张 figure entry、`display_pack_lock.json` 和 publication manifest。该 profile 只约束视觉呈现，不承载数据、统计值、evidence mark mutation、publication verdict 或 submission readiness。
