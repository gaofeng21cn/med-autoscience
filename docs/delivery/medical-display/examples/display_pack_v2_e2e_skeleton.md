...
`paper/figure_spec.json` 是单图 grammar 面。存在 `figure_specs.json` 时，OPL generated `display_pack_orchestrate` / `display_pack_render` action 消费 paper-level figure request；具体 CLI/MCP/tool spelling 由 OPL 生成，MAS 不维护 `medautosci publication ...` parser。

## 4a. Template Discovery And Scaffold Smoke

模板资产先通过只读 discovery surface 查找和确认：

调用 generated `display_pack_capability_discover`，设置 `include_templates=true`，读取当前启用 pack 的 template inventory、renderer family、execution mode、QC profile、exports、golden refs 和 authority boundary。调用方可按 `template_id` 在返回 inventory 中选择单个 descriptor；该 action 不执行 renderer，也不写 paper artifact。

开发或验收单个 template 时，可以用 payload JSON 直接生成最小 paper scaffold，然后走正式 E2E runtime：

开发或验收单个 template 时，先准备最小 paper scaffold，再调用 generated `display_pack_preflight` 和 `display_pack_render`。`display_pack_render` 以 `paper_root` 为必需输入，可携带 `figure_request` 与 `visual_audit_review`；它只用于模板试跑和开发验收，不替代真实论文的 claim/data/spec authoring，也不凭 renderer success 授权 publication ready。

需要固定 deterministic lower bound 时，用同一 payload 建立 golden：

Golden refresh/check 不在 22-action public catalog 中，只属于 repo test/fixture 维护面。默认 golden strict match 覆盖 PNG、layout sidecar、deterministic QC status 和 publication style profile hash；PDF bytes 因设备元数据可能变化，作为 observed-only artifact hash 记录。Golden match 是模板下限回归信号，不是 visual audit clear、publication readiness、artifact authority 或 owner receipt。
