...
`paper/figure_spec.json` 是单图 grammar 面。存在 `figure_specs.json` 时，`medautosci publication display-pack-e2e` 默认物化其中所有 figure；可重复传 `--figure-id <id>` 选择子集。

## 4a. Template Discovery And Scaffold Smoke

模板资产先通过只读 discovery surface 查找和确认：

```bash
medautosci publication display-pack-templates \
  --repo-root <mas_repo> \
  --kind evidence_figure \
  --renderer-family r_ggplot2 \
  --audit-family "Prediction Performance"

medautosci publication display-pack-template \
  --repo-root <mas_repo> \
  --template-id roc_curve_binary
```

`display-pack-templates` 返回当前启用 pack 的 template inventory、renderer family、execution mode、QC profile、exports、`render.R` / `render_candidate.R` presence、golden refs 和 authority boundary。`display-pack-template` 返回单个 template 的 descriptor、source config、runtime path 和 asset hashes。这两个命令不执行 renderer，也不写 paper artifact。
...
该命令会写入最小 `figure_intent.json`、`figure_specs.json`、`publication_style_profile.json`、`display_overrides.json`、`figure_style_reference_bundle.json` 和 frozen payload copy，再调用 `display-pack-e2e` 等价路径生成 PNG/PDF/layout sidecar、QC、visual-audit receipt、polish lifecycle、display lock 和 publication manifest。它只用于模板试跑和开发验收，不替代真实论文的 claim/data/spec authoring。
...
默认 golden strict match 覆盖 PNG、layout sidecar、deterministic QC status 和 publication style profile hash；PDF bytes 因设备元数据可能变化，作为 observed-only artifact hash 记录。Golden match 是模板下限回归信号，不是 visual audit clear、publication readiness、artifact authority 或 owner receipt。
