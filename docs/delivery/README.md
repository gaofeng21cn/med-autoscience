...
Inspection package 只导出当前可检查的 manuscript、evidence、figure/table、review refs
与 blocked context，或指向已经由 controller 授权为 current 的 package。它始终是
`human_inspection_only`：不得写 publication eval/controller decision、清除 quality gate、
更新 current package，或用 export/zip/pointer 存在声明 publication/submission ready。
人工反馈必须路由回 MAS reviewer、write/delivery owner、publication gate 或 human gate。
