...
MAS 将 `mas-scholar-skills` 作为默认可随包提供的可选专业增强层。医学写作、独立审阅、统计审查、文献研究、图表与表格设计、投稿准备和数据治理等专业能力可由这一层增强；普通用户只需选择 MAS，OPL 可同时物化专业 Skills。Provider 缺失或不兼容时 MAS 继续工作并记录质量债务。OPL 分别管理各包的安装、版本、更新、修复和回滚，MAS 继续持有医学研究与质量判断。
...
`mas-scholar-skills` 可作为默认 bundled 的专业增强层随 MAS 一同工作，但不进入 MAS 的硬依赖闭包。用户从 MAS 进入，OPL 承接通用运行和界面，ScholarSkills 在可用时提供医学研究方法候选，MAS 始终对医学质量和权威结果负责。
