...
| `src/med_autoscience/controllers/stage_knowledge_plane/__init__.py` | MAS controller/knowledge-plane owner | Machine surface for stage knowledge packets, typed closeout, memory routing receipts, and recall index | `map_to_descriptor`; descriptor mirrors surface metadata, required fields, owner, and authority boundary |
| generated `family_stage_control_plane` | OPL generated descriptor owner; MAS semantic source owner | OPL-standard stage plane mapped to MAS routes and action catalog refs | `landed`; OPL may consume it as descriptor only |
| generated `family_stage_control_plane_descriptor` | OPL generated descriptor owner; MAS semantic source owner | Read-only deep family descriptor for `stage_led_autonomy` source refs, route snapshot, packet/read-model surfaces and authority boundary | `landed`; descriptor cannot become domain authority |
| generated `domain_memory_descriptor` | OPL generated descriptor owner; MAS memory locator/body owner | Read-only `publication_route_memory` locator for stage knowledge refs and writeback receipt projection | `landed`; OPL may consume locator/freshness/receipt refs only |
...
The standard `family_stage_control_plane` now links route-sensitive stages to this descriptor through `knowledge_refs`. OPL may display and inject the ref into provider attempts; MAS still owns actual retrieval, closeout normalization, router receipt, accepted/rejected writes, and publication route decisions. Human/operator inspection reads the canonical Markdown library plus workspace body-free inventory refs. The old `medautosci publication route-memory-inventory` command is retired provenance; no replacement command is implied until an action/schema is added to the catalog.
