...
OPL generated/hosted surface 可以把这些 Stage action 映射为 CLI/MCP/tool UI，但 MAS 不维护第二份 parser、transport 或 command template。唯一非 Stage action 是 host-only `paper_mission_authority_evaluate`；它通过 `contracts/domain_handler_registry.json` 的 closed binding 指向 `med_autoscience.authority_handlers.paper_mission:evaluate_paper_mission_authority`，没有 CLI/MCP/Skill/product user surface。
