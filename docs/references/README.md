...
- [ARK Research Workflow Intake](./mainline/ark_learning_intake.md)：`kaust-ark/ARK` 的 clean-room research workflow pattern 吸收记录；只吸收 review loop、goal anchor、issue repair validation、API-first citation、figure manifest、page adjustment 和 human-intervention UX 的 contract shape，不引入 ARK runtime、SQLite authority、conda project model、Telegram/webapp service、代码或依赖。
