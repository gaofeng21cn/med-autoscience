...
- `MAS Runtime OS` 是默认 runtime owner；默认 supervision scheduler owner 是 OPL `opl_provider_runtime_manager` / `opl_family_runtime_provider` replacement；MAS `local` adapter / LaunchAgent 只在显式 `--manager local` 下作为 legacy diagnostic / cleanup path，Hermes gateway cron 是 explicit optional adapter
