...
数据资产状态、startup readiness、asset update、impact与 release explanation通过当前 Stage、declarative policy与 MAS owner result处理。除非 catalog/schema正式增加 action id，否则不能把旧 `medautosci data ...` 文案当作可运行命令或恢复私有 authority wrapper。
