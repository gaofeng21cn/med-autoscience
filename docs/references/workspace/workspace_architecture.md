...
已有 workspace 如果出现外层 Git object store 膨胀，应交 OPL lifecycle owner 读取 Git health。旧 `medautosci runtime storage-audit` 已退役，只作为 maintenance provenance，不得恢复为执行入口。
低风险 hardening 也必须由 OPL owner surface 执行，并保留 authorization、inventory 与 readback。
...
### 4. Generated surface 必须 profile-driven

OPL generated/hosted surface 只从 profile/schema 读取 workspace locator，再 dispatch 到 catalog 声明的 MAS handler target。MAS 不再生成 workspace wrapper。

generated surface 不应硬编码：

- 某个具体疾病 workspace 下的绝对 runtime 路径
- 某个固定用户机器布局下的程序路径
- 某个旧版 legacy controller 路径

旧 `ops/medautoscience/bin/*` 和 `ops/med-deepscientist/bin/*` 如果存在，只是 legacy restore/import diagnostic、backend-audit reference 或 cleanup target；新 workspace 不生成，也不作为研究/runtime 入口。
...
- OPL-owned workspace/profile locator
  - 显式绑定 `profile_ref`、workspace refs 与 MAS package locator，不依赖 repo-local shell
- OPL hosted workbench / App display
  - 消费 MAS `study_progress`、`paper_mission` 与 refs-only projection；`ops/mas` 不再承载 runtime launcher config、static display、behavior gate、doctor/status/stop wrapper 或 supervision 控制面

当前阶段 `med_deepscientist_repo_root` 仍作为 profile TOML 输入兼容字段存在，但只在 `source_provenance`、`historical_fixture_ref` 或 `explicit_archive_import_ref` 这类 read-only reference 下暴露。它只服务 internal backend audit、upstream intake、parity oracle 等维护流程；旧 `backend-upgrade-check`/doctor command identity 已退役。该字段不意味着 workspace 正在直接从这个 repo 运行，也不是默认 product entry 或 executor owner。当前 repo-side runtime ref 是 `opl_runtime_ref=opl_hosted_stage_runtime`，对应 engine id 是 `opl-hosted-stage-runtime`；默认 generic runtime owner / topology 归 OPL provider-backed stage runtime，`mas_runtime_core` 只作为 retired provenance / delegated MAS domain adapter / diagnostic context。旧 behavior-equivalence gate 只作 legacy diagnostic/restore provenance，不得恢复成 active MAS runtime readiness 或 MDS default owner。
...
## Generated workspace surface 契约

OPL generated/hosted workspace surface 应满足：

- 只通过 profile/schema 和显式 locator 解析路径；
- dispatch 到 `contracts/action_catalog.json` 声明的 MAS handler target；
- 只有显式 legacy diagnostic/backend-audit/provenance 操作才读取 `MedDeepScientist` reference；
- 不依赖 workspace 内 legacy 程序副本或 repo-local wrapper。

旧 `ops/medautoscience/bin/*`、`ops/med-deepscientist/bin/*` 只能保留为历史/debug evidence 或通过 cleanup lane 移除。

Generated surface 不应继续做：
...
- 旧 wrapper 仍然依赖 workspace 私有脚本而不是 OPL generated surface + MAS handler target
...
- OPL generated surface 只从 profile/schema 取 workspace/runtime locator
- 不在 workspace scaffold 中散落硬编码 `ops/med-deepscientist/runtime/quests`
- 新 workspace 不生成 `ops/medautoscience` 或 `ops/mas` launcher/config/gate/doctor/status/stop bridge，也不生成 repo-local static display；用户可见 display 由 OPL hosted workbench 消费 MAS refs-only projection
...
`behavior_equivalence_gate.yaml` 不再是新 workspace 的长期 active artifact。当前 MAS-first workspace 的 readiness evidence 来自 OPL workspace/runtime readback、MAS domain authority refs、owner receipt、typed blocker 和真实 artifact；旧 `ops/mas/behavior_equivalence_gate.yaml` 与 `workspace-target-state-cleanup` wrapper 只作为 retired/archive provenance。若仍需审计旧字段，只能解释为 legacy behavior-equivalence provenance，不得作为 active MAS runtime readiness、外部 MDS 恢复或默认执行源。
