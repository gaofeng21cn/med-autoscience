...
- reviewer response、data、figure/source-grounded deliverable patterns 已作为 MAS-native stage / quality / review / evidence / publication surface 的学习模式吸收，并有 contract / prompt / descriptor / test surface 保护。
...
- `literature_search_source_pack`、`journal_policy_currentness_pack` 与 `citation_verification_pack` 已落到 stage quality pack descriptor：它们固定 source tiers、official/current policy refs、checked_at / expires_or_stale_after、citation verification output fields 与 forbidden authority。
...
- stage prompts / quality gate 已把 quality pack refs、durable output refs、closeout packet、gate owner 和 forbidden actions 接入主 stage skill / prompt surface。
