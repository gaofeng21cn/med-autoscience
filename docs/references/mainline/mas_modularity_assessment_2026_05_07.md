...
2026-05-08 复核：模块化治理后的关键结构信号没有漂移。Sentrux fresh scan 仍为 `quality_signal=6076`，DSM `above_diagonal=0`、`propagation_cost=63`。这些只作 dated evidence；当前结构检查统一读取 `opl quality details`、tracked-path hygiene 与 source-closure。
...
上述 Sentrux 数值不再是当前 gate；结构回归以 repo verification 与 OPL quality readback 为准。
