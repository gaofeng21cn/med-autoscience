...
| --- | --- | --- | --- | --- |
| daemon residency | `purpose_equivalent_with_different_timing` | resident HTTP/WebSocket daemon | 默认外层 cadence 由 OPL `opl_provider_runtime_manager` / `opl_family_runtime_provider` 持有；MAS local adapter 已物理退役为 tombstone/provenance refs；Hermes 是 legacy diagnostic adapter | drift detection and recovery can be scheduler-bound; no MAS resident daemon is claimed |
...
| daemon lifecycle controls | `not_equivalent_retired` | start/stop/restart MDS daemon | default OPL scheduler replacement projection; explicit legacy local cleanup/diagnostic job; Hermes cron as explicit legacy diagnostic adapter | no MAS-native MDS daemon control path is needed |
...
- `outer supervision latency`: 当前默认由 OPL scheduler replacement / provider SLO projection 解释；legacy local adapter 不再运行 300 秒 scheduler one-shot，也不再提供 status/remove cleanup path，Hermes gateway cron 只作 legacy diagnostic。它只影响 drift detection、stale recovery 和周期性刷新，不再影响正常 turn-to-turn continuation。
