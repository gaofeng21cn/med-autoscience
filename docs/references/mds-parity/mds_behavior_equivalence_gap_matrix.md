...
| WebUI/WebSocket/terminal streaming | `purpose_equivalent_with_different_timing` | React WebUI, WebSocket terminal attach, bash log stream | Progress Portal for progress plus MAS-authored Live Console session read model, static shell, snapshot/loopback SSE stream, and gated Portal pause/resume/stop runtime owner apply | users get MAS-owned progress, read-only runtime observation, and local-loopback authorized runtime control for pause/resume/stop; interactive terminal attach/input/resize/detach remains a future parity candidate, not retired requirement |
...
| workspace-local host service | `not_equivalent_retired` | historical MAS launchd/systemd/cron bridge | retired cleanup evidence; canonical owner is MAS scheduler contract; local adapter owns new OS backend semantics | old host services should be removed, not kept as active option |
...
- `outer_supervision_slo`: landed。它解释 outer supervision latency，默认由 MAS local scheduler 300 秒 tick 承担；`due/stale/missing/blocked` 都会投影到用户入口，并给出 canonical one-shot reconcile dry-run 推荐。Hermes optional adapter 必须输出同构 SLO，而不是让 Portal / OPL 读取 Hermes-specific path。
...
- `outer supervision latency`: 当前仍是 300 秒 scheduler one-shot；默认 adapter 是 local，Hermes gateway cron 是 optional。它只影响 drift detection、stale recovery 和周期性刷新，不再影响正常 turn-to-turn continuation。
...
| Live Console interactive terminal/control | 当前能看 session/run、terminal/log tail、SSE/snapshot 和 action intent；Progress Portal 在显式 `--serve --enable-actions` 下可执行 pause/resume/stop；仍不能 attach terminal、输入命令、resize/detach 或直接 publication/controller apply。 | read-only observation 已覆盖旧 WebUI 的“看状态/日志/终端尾部”目的；pause/resume/stop 已走 runtime owner apply；terminal input 仍触碰 terminal owner、authorization、idempotency 和审计边界。 | P1/P2：继续真实 workspace action receipt soak，再单独评估 interactive terminal attach。 |
| outer supervision stale/crash latency | 正常 turn-to-turn continuation 不等 scheduler tick；但 worker crash、stale recovery、drift detection 和部分 read-model refresh 仍可能等到 300 秒 MAS scheduler tick，或由 operator 触发 one-shot dry-run/reconcile。 | 这比旧 resident daemon 更可审计、fail-closed，且 `outer_supervision_slo` 已把 fresh/due/stale/blocked 和推荐命令投影给用户；默认 adapter 是 local，Hermes 只是显式 optional adapter。 | P1：对真实 workspace 继续收集 SLO evidence，必要时用安全 one-shot reconcile 或更短受控 local scheduler cadence，而不是恢复 resident MDS daemon。 |
...
后续完善顺序建议固定为：real-workspace `portal_console_soak` / source-ref polish -> route input completeness -> action receipt soak -> interactive terminal attach implementation gate。`portal-study-scoped-ia`、`portal-route-decision-trail`、`portal-stage-artifact-path`、visible `runtime-conversation-read-model` panel、study-scoped Live Console 和 pause/resume/stop authorized apply 已有 repo contract；下一步重点是让真实多论文 workspace 的用户体验稳定，同时不让 UI、connector 或旧 daemon 重新成为 runtime owner。
...
- 不能说：MDS resident WebSocket terminal attach 或 UI-issued runtime control 已经被放弃；准确口径是 Portal pause/resume/stop 已通过 MAS-native runtime owner apply 落地，terminal attach/input/resize/detach 仍必须通过 MAS-native safety / owner / audit gate。
