...
| workspace-local host service | `not_equivalent_retired` | historical MAS launchd/systemd/cron bridge | retired cleanup evidence; canonical default owner is OPL scheduler replacement; MAS local adapter is legacy diagnostic / cleanup only | old host services should be removed, not kept as active option |
...
- `outer_supervision_slo`: landed。它解释 outer supervision latency，默认由 OPL scheduler replacement / provider SLO projection 承担；显式 legacy MAS local scheduler 300 秒 tick 只服务 cleanup/diagnostic 与 no-active-caller proof。`due/stale/missing/blocked` 都会投影到用户入口，并给出 canonical one-shot reconcile dry-run 推荐。Hermes optional adapter 必须输出同构 SLO，而不是让 Portal / OPL 读取 Hermes-specific path。
...
- `outer supervision latency`: 当前默认由 OPL scheduler replacement / provider SLO projection 解释；显式 legacy local adapter 仍是 300 秒 scheduler one-shot，Hermes gateway cron 是 optional。它只影响 drift detection、stale recovery 和周期性刷新，不再影响正常 turn-to-turn continuation。
...
- 检测到旧 service file 或 loaded state 时，只作为 optional/legacy adapter status 中的 `retired_cleanup_evidence`；清理后回到 OPL scheduler replacement 默认 owner，MAS local adapter 只保留显式 legacy diagnostic / cleanup。
