...
2026-05-30 research lifecycle foldback：MDS parity 中“科研记录是否降级”的答案是 no。核心科研审计不随旧 MDS resident daemon、GitOps state management、interactive artifact mutation UX、WebSocket terminal attach 或 workspace-local service 退役而降级；这些 surface 只能作为 retired non-goal、historical fixture、parity oracle 或 OPL-owned product/runtime direction。MAS 当前最低科研审计链是 research evidence pack refs、negative / failed-path ledger refs、decision trace refs、artifact lineage / reproducibility refs、AI reviewer / auditor record、owner receipt 或 stable typed blocker。OPL runtime / lifecycle substrate 可以运输、索引和展示这些 refs，但不能替代 MAS medical authority、publication quality verdict、artifact mutation authorization、memory writeback decision 或 `current_package` freshness proof。

2026-06-23 PaperMissionTransaction foldback：MDS parity 的“论文严谨性不降级”现在按 `PaperMissionTransaction + StageTerminalDecision` 读取。旧 MDS / MAS 控制面中有价值的是 durable workflow、terminalizer、human interrupt、retry-catch、exit-handler、evidence/ref ledger 和 owner/auditor separation 这些共性；它们折叠进 MAS stage terminal decision，不恢复 MDS resident daemon、WebSocket terminal、GitOps state、owner-callable dispatch、domain diagnostic 或 PaperRecovery 作为默认论文路径。`PaperMissionRun` 只证明 candidate/readback shape 与 one-shot migration 兼容；只有 MAS stage terminalized decision 或其派生 route command 才能进入 OPL runtime transport。
...
| terminalizer / route command | runner completion 归一成继续、停止、人工介入、失败或下一轮。 | `done` | MAS stage 输出 `StageTerminalDecision`，OPL 只消费派生 route command；domain diagnostic/PaperRecovery 不再是默认 paper mainline。 |
...
| PaperMissionTransaction terminalization | `purpose_equivalent_with_authority_split` | runner / recovery loop normalized a quest turn into continuation, human stop, failure, or terminal state | MAS stage owns `PaperMissionTransaction` and emits `StageTerminalDecision`; OPL consumes only the derived route command for transport / retry / resume | preserves durable workflow and terminal decision rigor without restoring domain diagnostic / default dispatch / PaperRecovery as paper mainline |
...
| artifact interaction handoff | `partially_equivalent` | daemon artifact interactions | Artifact OS locator / package handoff / controller refs；interactive artifact mutation UX retired 或归 OPL-owned future product direction | package discovery covered; core research audit chain not degraded, but interactive mutation UX is not default MAS scope |
...
| artifact interactive mutation | package locator、artifact refs、current package discovery 已由 Artifact OS 持有；旧 MDS interactive artifact API 没有默认保留。 | MAS 选择 canonical-source-first，避免 UI 或 legacy artifact API 绕过 paper/package authority；这不是科研记录降级，因为 evidence pack、failed-path ledger、decision trace、artifact lineage / reproducibility refs 和 typed blocker 链继续由 MAS authority 要求。 | P2：仅在 Artifact OS authority 下增加 file browser / pickup / rebuild proof view，或由 OPL product/runtime owner 承担交互体验；不恢复任意 mutation API。 |
...
- 可以说：核心科研记录不降级；MAS 仍要求 research evidence pack、negative / failed-path ledger、decision trace、artifact lineage / reproducibility、AI reviewer / auditor record、owner receipt 或 stable typed blocker。
...
- 不能说：旧 MDS daemon、GitOps 或 artifact interaction UX 退役代表 MAS 科研审计链降级；准确口径是这些体验面退役或移交 OPL owner，MAS 医学 authority 证据链保持并进一步收紧。
