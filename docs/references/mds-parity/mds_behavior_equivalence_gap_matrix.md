...
| --- | --- | --- | --- | --- |
| daemon residency | `purpose_equivalent_with_different_timing` | resident HTTP/WebSocket daemon | 默认外层 cadence 由 OPL `opl_provider_runtime_manager` / `opl_family_runtime_provider` 持有；MAS local adapter 已物理退役为 tombstone/provenance refs；Hermes 是 optional adapter | drift detection and recovery can be scheduler-bound; no MAS resident daemon is claimed |
...
- `outer supervision latency`: 当前默认由 OPL scheduler replacement / provider SLO projection 解释；legacy local adapter 不再运行 300 秒 scheduler one-shot，也不再提供 status/remove cleanup path，Hermes gateway cron 是 optional。它只影响 drift detection、stale recovery 和周期性刷新，不再影响正常 turn-to-turn continuation。
...
- 检测到旧 service file 或 loaded state 时，只作为 optional/legacy adapter status 中的 `retired_cleanup_evidence`；清理后回到 OPL scheduler replacement 默认 owner，MAS local adapter 只保留 tombstone/provenance refs。
