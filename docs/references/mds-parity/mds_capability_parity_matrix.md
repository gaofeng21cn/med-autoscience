...
2026-05-26 Runtime Turn Lifecycle currentness correction 把旧 MDS daemon 的连续科研主循环读回 OPL stage runtime / MAS domain authority split：`runtime_core_daemon` 当前归类为 `retired_mas_runtime_surface`，`worker_runner_lifecycle` 只按 MAS domain authority refs / owner receipt / typed blocker consumer 读取，generic queue、attempt continuation、provider lifecycle、worker liveness 与 runtime drilldown 归 OPL `current_control_state` / provider attempt projection。MAS 不再声明 MAS-owned generic runtime core daemon、turn owner、worker residency 或 terminal owner。

2026-05-26 Runtime Continuity currentness correction 在 behavior matrix 中把 current owner 固定为 OPL `current_control_state` / provider attempt projection：OPL 投影 worker/session/freshness、provider attempt refs、typed blocker、owner receipt 和 runtime owner handoff；MAS 只把这些 refs-only handoff 投到 progress/cockpit/product-entry/Portal/MCP/OPL，不写 runtime truth 或 provider attempt state。该合同明确 `external_mds_repo_required=false`、`mds_daemon_required=false`，并固定 `quality_ready_authorized=false`、`publication_ready_authorized=false`、`submission_ready_authorized=false`。

2026-05-26 Live Console parity currentness correction 把旧 MDS WebUI 中有价值的论文/domain 观察能力收敛到 MAS-authored Progress Portal read-only projection；MAS private Live Console、conversation/session read model、terminal attach gate 和 Portal action endpoint 已从当前控制面物理退役，只保留 history/provenance。Progress Portal 继续提供 MAS-owned 静态进度 evidence、per-study page、route-decision read-only projection、source/artifact refs 和 owner receipt / typed blocker refs；pause/resume/stop 意图走 domain-handler / OPL owner-route handoff refs。运行对话、terminal/log/provider drilldown 与未来 attach/control 必须来自 OPL `current_control_state` 或 provider attempt projection。该 closeout 仍是 `purpose_equivalent_with_different_timing`，没有在当前 MAS landed scope 中实现旧 MDS resident WebSocket terminal attach、terminal input/resize/detach。旧 React bundle、产品身份、commit history 和 contributor metadata 仍不得导入 MAS。
...
2026-05-26 fresh assessment：当前 machine-readable behavior matrix 仍是 `17` 个 behavior surface，分类为 `2 behavior_equivalent / 6 purpose_equivalent_with_different_timing / 1 purpose_equivalent_with_authority_split / 3 partially_equivalent / 4 not_equivalent_retired / 1 historical_fixture_only`，且 `fully_equivalent_to_mds_daemon=false`。这不是退回到外部 MDS 依赖；它表示 MAS monolith 已经承接默认 domain progress、owner receipt、typed blocker、stage memory/literature 目的和 read-only progress projection，但 runtime drilldown / terminal / provider state 由 OPL `current_control_state` 承担。后续能力补齐应聚焦真实 workspace per-study Portal soak、route input completeness、OPL runtime drilldown join、stage injection soak 和 OPL terminal owner safety proof；connector background delivery、GitOps lifecycle、MDS daemon lifecycle controls 与 workspace-local host service 仍保持 retired，不进入默认 backlog。

## Capability Matrix

| Capability | Classification | MDS role | MAS owner | Parity proof | Medical quality authority |
| --- | --- | --- | --- | --- | --- |
| runtime execution | `mas_owned` | backend | MAS domain authority refs + OPL runtime handoff | runtime execution replay and recovery regression suite | blocked for MDS |
...
| --- | --- | --- | --- |
| runtime core daemon | `retired_mas_runtime_surface` | One Person Lab stage runtime | external source archive only |
| quest lifecycle | `mas_owned` | MAS domain authority refs | historical oracle fixture only |
| worker and runner lifecycle | `mas_owned` | MAS domain authority refs | external source archive only |
| channels, connectors, and transport | `opl_runtime_control_with_mas_refs` | MAS domain authority refs | explicit archive import ref only |
...
| supervision cadence | `purpose_equivalent_with_different_timing` | Use OPL scheduler replacement by default; MAS local 300s one-shot tick is explicit legacy diagnostic / cleanup, with Hermes gateway cron as explicit legacy diagnostic adapter. |
| turn completion continuation | `behavior_equivalent` | Use OPL stage runtime / current-control-state transport plus MAS owner receipt or typed blocker refs. |
| quest create/resume/pause/stop | `behavior_equivalent` | Use MAS domain intent / owner receipt refs plus OPL stage runtime lifecycle transport. |
| live worker/session tracking | `purpose_equivalent_with_different_timing` | Use OPL `current_control_state` / provider attempt liveness refs plus MAS domain refs. |
| crash recovery and auto-resume | `purpose_equivalent_with_different_timing` | Use OPL provider retry/reconcile with MAS owner receipt or typed blocker refs. |
...
| WebUI/WebSocket/terminal streaming | `purpose_equivalent_with_different_timing` | Use MAS Progress Portal for paper/domain progress and OPL `current_control_state` / provider attempt projection for runtime, terminal, log and provider drilldown; MAS Portal does not expose terminal attach/input/resize/detach or local action endpoints. |
...
| daemon lifecycle controls | `not_equivalent_retired` | Use OPL provider scheduler lifecycle; Hermes cron is only retired tombstone/provenance. |
| workspace-local host service | `not_equivalent_retired` | Clean retired service evidence; do not install. |

`turn_completion_continuation` 是本轮重新评估后的关键变化：旧 MDS 的 runner completion 后状态归一化和下一 turn 调度，已经落在 OPL stage runtime / current-control-state transport plus MAS owner receipt / typed blocker refs；MAS scheduler adapter 只剩 legacy diagnostic / provenance 语境。`runtime_continuity_completion` 是 OPL current-control-state handoff 与 MAS refs-only projection 的 landed refinement，但仍保留 scheduler-bound timing 差异，不宣称 resident daemon equivalence。
...
- `webui_websocket_terminal_streaming`: MAS 已实现 paper/domain progress 的 read-only purpose parity；MAS private runtime console、terminal attach gate 和 Portal action endpoint 已物理退役。runtime / terminal / log / provider drilldown 的差距应由 OPL `current_control_state` / provider attempt projection 与 App/workbench join 承接。
...
- MAS contract: OPL stage runtime / `current_control_state` 暴露 attempt continuation、queue hydration、provider lifecycle 和 runtime drilldown；MAS exposes domain intent, owner receipts, typed blockers, publication authority refs and progress/domain projection.
- MDS oracle: MDS `schedule_turn` / `_normalize_status_after_turn` 的语义被保留为 behavior oracle。
- Proof: OPL attempt completion / current-control-state must carry provider state and closeout refs; MAS may advance domain truth only after typed closeout, owner receipt, typed blocker, publication gate verdict or explicit MAS authority ref.

### worker and runner lifecycle

- MAS contract: OPL provider attempt / current-control-state records worker lease, run receipt, claimed user messages, idempotency key and per-attempt serialization; MAS consumes closeout refs and signs owner receipts or typed blockers.
- MDS oracle: MDS one-worker-per-quest turn worker 和 active run lifecycle 作为 behavior fixture。
- Proof: active worker 期间追加 turn 只进入 OPL pending/queue；provider completion must return typed closeout refs, MAS owner receipt or typed blocker before domain progress can advance; stale liveness is fail-closed in OPL current-control-state.
...
当前 `tests/test_standard_agent_boundary.py` 固定：
