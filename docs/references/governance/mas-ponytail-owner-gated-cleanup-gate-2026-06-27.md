...
- Physical delete landed only for `src/med_autoscience/controllers/paper_mission_owner_surface/owner_route.py`, an exact legacy re-export with no source active caller. `rg` found the old import path only in the removed compatibility test.
