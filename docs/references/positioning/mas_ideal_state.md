...
- `最短路径`：先迁移或退役 MAS-owned local scheduler / LaunchAgent / 300 秒 tick，再清理 Portal/workbench 中的 generic runtime owner wording，并把可复用 primitive 交给 OPL。
...
2026-05-16 校准：当前代码仍把 MAS standalone/local diagnostics 的外层监管实现为 MAS-owned scheduler surface。`src/med_autoscience/controllers/supervision_scheduler.py` 固定 `SCHEDULER_OWNER=mas_supervision_scheduler`、`DEFAULT_MANAGER=local`，默认 interval 来自 local adapter；`src/med_autoscience/controllers/supervision_scheduler_parts/local_adapter.py` 固定 `DEFAULT_INTERVAL_SECONDS = 5 * 60`，macOS backend 为 `local_launchd`，LaunchAgent `StartInterval` 使用该 interval，并生成依次执行 `watch-runtime`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch` 的 tick script；`src/med_autoscience/cli.py` 仍暴露 `runtime-supervision-status`、`runtime-ensure-supervision`、`runtime-remove-supervision`；`contracts/test-lane-manifest.json` 的 `outer-supervision-slo` 明确 `default_scheduler_owner=mas_supervision_scheduler`、`default_adapter_id=local_launchd_on_macos`、`default_tick_interval_seconds=300`。这不符合 MAS 作为纯 `Domain Knowledge / Authority Pack + thin program surface` 的理想终态。它只能被解释为当前 standalone/local diagnostic 迁移桥和 legacy supervision proof surface；理想终态要求周期唤醒、scheduler lifecycle、provider SLO、job registry 和通用 supervision cadence 由 OPL Framework / provider 层提供，MAS 只保留医学 owner route、paper progress SLO 语义、owner receipt、typed blocker、safe action 和质量/交付 authority。
...
- MAS 当前保留的 local supervision scheduler、LaunchAgent/cron job、300 秒 tick、job registry/latest run projection 和 `outer_supervision_slo` 输入属于尚未上收的功能层残留。它可以继续作为本机诊断和迁移桥运行，但不应被写成 MAS 长期产品内核；新增投入应优先把这类 cadence / scheduler / SLO 承载迁往 OPL provider，而不是继续加深 MAS 私有 scheduler。
