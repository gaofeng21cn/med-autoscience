...
- OPL App/operator 能把 stage expected receipt / monitor freshness 缺口转成 refs-only `stage_production_evidence_receipt_record|verify` route；MAS 只提供真实 owner receipt、typed blocker、no-regression、memory/artifact/human gate 或 long-soak refs，不维护私有 stage evidence ledger 或通用 App route。
