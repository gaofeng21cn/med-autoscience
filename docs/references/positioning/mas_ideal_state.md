...
- `目标态优先`：MAS 当前已经存在的 controller、scheduler、SQLite/lifecycle、workspace/source intake、memory/artifact transport、Portal/workbench、CLI/MCP/Skill/product-entry/sidecar/status wrapper 只能作为迁移输入，不是理想态约束。理想 MAS 是标准 OPL Agent：声明式 medical research pack + OPL generated/hosted surfaces + 最小医学 authority functions。
...
因此，MAS 的理想形态是医学研究 `Declarative Medical Research Pack + minimal authority functions`，不是一套独立 agent runtime platform，也不应长期手写一层可由 OPL 派生的薄程序面。MAS 可以保留 direct/local diagnostic path 和 MAS-owned controller truth，但 generic scheduler、generic queue、generic attempt ledger、generic memory locator、generic artifact lifecycle、generic state-machine runner、generic workbench shell、observability/SLO、CLI/MCP/Skill/product-entry/sidecar/status wrapper 和跨 domain App projection 都应由 OPL Framework / One Person Lab App 承载或生成。MAS 的 descriptor、contract/schema、domain transition spec/table、quality gate、artifact locator、receipt schema、tests 和 domain entry 只负责把医学 owner chain、typed blocker、owner receipt、artifact locator、handler target 和 domain transition spec 暴露给 OPL；手写 adapter/projection 只在无法声明化或仍处迁移桥时保留，并必须写明 `cannot_absorb_reason`。

AI-first quality gate 还要求 agent 角色分离。`Codex CLI` 或其他 executor agent 可以在 stage 内执行分析、写作、修复或交付任务，但 reviewer/auditor agent 必须被独立调用，读取 executor 产出的 artifact/source/evidence refs，并留下独立 context、task record、review/audit receipt。MAS 不接受“让同一个 agent 完成执行后再审阅自己”作为 AI reviewer-backed quality verdict、source readiness、artifact mutation authorization 或 publication gate closure。
...
当前差距必须拆成功能/结构差距与测试/证据差距。功能/结构差距至少包括：OPL generated surface 尚未生产生成和消费，MAS hand-written CLI/MCP/Skill/product-entry/sidecar/status/workbench/projection shell 的 active caller cutover 尚未完成，SQLite/lifecycle、paper outbox、storage、workspace-source、memory、artifact、Portal 和 runtime supervisor 仍需 refs-only 收薄或上收，local LaunchAgent/status/remove cleanup 与旧 workspace-local wrappers 仍需 physical retirement，OPL App drilldown 与 lifecycle locator/retention/restore ledger 仍需对账。测试/证据差距才包括真实 provider-hosted paper apply、publication-route memory receipt scaleout、artifact lifecycle receipt scaleout、human gate/resume 和 long provider SLO。
...
2026-05-17 MAS lane 语义等价校准：6 个功能面的 owner 边界已被重新定义，但仍要按目标态完成 cutover / 收薄 / 退役。`runtime_storage_maintenance` 与 `artifact_lifecycle_storage_audit_shell` 应只作为 refs-only adapter 输出 locator、audit、restore/retention receipt 和 artifact authority refs；`workbench_portal_generic_shell` 应只向 OPL generated-hosted workbench 提供 MAS domain projection refs；`runtime_supervisor_scan_consume_dispatch_shell` 应消费 OPL runtime manager loop，MAS 保留 owner-route guard、publication gate blocker、safe action refs 和 no-forbidden-write receipt；`generic_cli_mcp_product_wrappers` 应由 declarative pack / OPL generated surfaces 派生 wrapper metadata，MAS 只保留 domain handler、direct domain entry 和 owner receipt signer；`generic_queue_attempt_retry_dead_letter` 应消费 OPL queue/attempt/retry/dead-letter/worker lifecycle transport，MAS 只保留 stage closeout semantics、recovery owner decision 和 owner receipt。它们不得再被写成已完成后无需 cutover 的长期 MAS shell。
...
2026-05-17 consumer thinning 校准：上述功能残留已经在 MAS 单仓机器面完成第一轮分类与禁回流 guard，但这不能写成全部功能/结构已完成。剩余 generated surface active caller cutover、OPL App drilldown、lifecycle locator/retention/restore ledger 对账、refs-only adapter 收薄、local LaunchAgent/status/remove cleanup 与 workspace-local wrappers 的 physical retirement，都是 active plan 中的功能/结构差距。真实 paper-line provider apply、publication-route memory receipt scaleout、artifact lifecycle receipt scaleout、human gate/resume 和 provider SLO long soak，才属于测试/证据差距。

完善顺序必须服从这一边界：先完成 generated surface production consumption、active caller cutover、refs-only 收薄、legacy physical retirement、OPL App drilldown 与 lifecycle ledger 对账，再用真实 paper-line、provider SLO 和 human gate/resume 做证据验收。live soak 证明迁移后的目标形态成立，不应掩盖已知功能残留。
...
现有代码若落在这些功能族中，默认必须进入 `functional_consumer_boundary` / `functional_module_inventory` 分类，而不是因为“MAS 已经实现”就长期保留。2026-05-17 当前机器面已把已知项归类为 declarative pack / generated surface、refs-only adapter、minimal authority boundary 或 legacy cleanup gate，但分类本身不是功能/结构完成。只要 generated surface 尚未生产消费、active caller 尚未 cutover、refs-only adapter 尚未收薄、legacy cleanup surface 尚未 physical retirement，或 App/lifecycle 对账尚未完成，就仍属于功能/结构差距；真实 paper-line provider apply、memory/artifact receipt scaleout、human gate/resume 和 long provider SLO 才进入测试/证据门。
...
- MAS 仍保留的非知识代码均能归类为 declarative pack / generated surface handoff、refs-only adapter、minimal authority function 或 no-active-caller cleanup gate；每项都有 active caller、receipt/schema/ref 边界、cannot-absorb reason 或 cleanup gate。能由 OPL pack compiler 生成的 CLI/MCP/Skill/product-entry/sidecar/status/projection/harness 不再作为 MAS 长期私有实现扩展。
...
- Legacy MDS/Hermes/local/default-compat residue 完成 no-active-caller scan、replacement proof、history/provenance 分类与 physical retirement；`local_launchd_scheduler_install_path` 与旧 workspace-local service wrapper 不再保留为 status/remove compatibility surface，scheduler 管理只保留 canonical runtime CLI。
