...
- `最短路径`：默认 scheduler owner 已迁到 OPL replacement；接下来清理 Portal/workbench 中的 generic runtime owner wording，对显式 local scheduler / LaunchAgent legacy diagnostic path 做 no-active-caller proof 与物理退役。
...
- `功能消费方边界`：MAS 目标态以 `Domain Authority Pack + thin program surface` 消费 OPL generic scaffold、generic transition runner、memory/artifact locator、workbench shell 和 observability，不复制通用 scheduler、daemon、queue、attempt ledger、runner 或工作台。
...
因此，MAS 的理想形态是医学研究 `Domain Knowledge / Authority Pack + thin program surface`，不是一套独立 agent runtime platform。MAS 可以保留 direct/local diagnostic path 和 MAS-owned controller truth，但 generic scheduler、generic queue、generic attempt ledger、generic memory locator、generic artifact lifecycle、generic state-machine runner、generic workbench shell、observability/SLO 和跨 domain App projection 都应由 OPL Framework / One Person Lab App 承载。MAS 的 descriptor、contract/schema、sidecar、projection、runtime-facing surface、domain transition spec/table、quality gate、artifact locator、receipt schema、tests 和 domain entry 只负责把医学 owner chain、typed blocker、owner receipt、artifact locator 和 domain transition spec 暴露给 OPL，不承担平行通用运行框架职责。
...
- OPL scaffold：MAS 暴露 domain descriptor、stage/quality pack、owner receipt schema、sidecar bridge 和 domain entry；不再新增 MAS-owned generic scaffold 或通用 agent OS。
...
- OPL workbench primitive：MAS 输出 route map、stage review、quality/source/artifact refs 和 safe action receipt；OPL/App 负责通用导航、attention queue、drilldown 和 action transport。
...
- generic workbench navigation、attention queue、running/recent items、transition bridge evidence drilldown、notification 和 cross-domain dashboard；
