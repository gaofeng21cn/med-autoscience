...
2026-05-16 functional privatization 校准：MAS 非知识层私有功能不再按“当前在 MAS repo 内实现”判断 owner，而按长期 owner/authority 分类。A 类是 OPL-owned、MAS 只消费或暴露 adapter/receipt 的通用能力：runtime lifecycle SQLite reference adapter、runtime storage maintenance、artifact lifecycle/storage audit shell、workbench/portal generic shell、terminal attach transport、generic daemon/scheduler lifecycle、queue/attempt/retry/dead-letter 和 generic transition runner。B 类是 MAS 合理保留的 domain truth / authority：study truth、study runtime status、runtime_watch domain health、AI reviewer、publication gate、artifact authority、owner receipt、domain transition table、publication-route memory body、memory writeback decision、typed blocker 和 safe action refs。C 类是有 replacement 或 no-active-caller 后应退役的旧 surface：local scheduler install、workspace-local watch wrappers、MAS generic workbench shell、legacy scheduler aliases、daemon-ish terminal status owner claim 和 scheduler residue。`runtime_lifecycle.sqlite` 的理想角色因此是 MAS domain sidecar index / reference adapter，服务 OPL replacement audit 和 MAS receipt lookup，不是 MAS generic persistence engine。
