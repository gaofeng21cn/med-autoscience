...
- OPL App/operator 能把 stage expected receipt / monitor freshness 缺口转成 refs-only `stage_production_evidence_receipt_record|verify` route，并提供 payload workorder / preflight；MAS 只提供真实 owner receipt instance、typed blocker、no-regression、memory/artifact/human gate 或 long-soak refs，不维护私有 stage evidence ledger 或通用 App route，不把声明型占位 ref 或 body payload 当作成功证据。
