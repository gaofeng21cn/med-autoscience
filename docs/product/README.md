...
- domain-handler export：默认 task kind / action intent 为 `paper_mission/start_or_resume`，并携带 no-write authority boundary；旧 `domain_owner/default-executor-dispatch` 若出现，必须是 `default_paper_mission_entry=false` / `migration_diagnostic_only=true`。
...
- 若要判断论文推进，默认读 `artifact_first_mission_summary.paper_mission_run`、`medical_paper_product_entry` 和 domain-handler `paper_mission/start_or_resume` task；不要把 DHD/currentness/storage/dispatch/PaperRecovery diagnostics、no-write readback、focused tests 或 read-model clean 写成 paper progress。
