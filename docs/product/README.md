...
- `medautosci workspace progress-portal` 仍是 active no-App / diagnostic / evidence path，支持静态刷新、`--serve` 和显式 `--enable-actions`；它写 display/read-model artifact，不写 paper/package、publication gate、controller decision 或 provider runtime state。
...
| `../runtime/display/progress_portal.md` | MAS Progress Portal read-model/display 合同、workspace-local diagnostic path 和 OPL handoff refs。 | 不定义 OPL App 主工作台 UI，不写 study truth，不提供 generic runtime drilldown owner。 |
...
- 若要审计 Progress Portal，按 display contract 读取：Portal 是 MAS-owned read-model/display surface 和 no-App diagnostic path，长期主用户工作台归 OPL App。
