...
当前 product / workbench 默认叙事是 artifact-first paper mission：Progress Portal 和 study workbench 先展示 `artifact_first_mission_summary`，再展示 owner-route / current-owner-delta / runtime drilldown。`artifact_first_mission_summary.paper_mission_run` 对齐 `contracts/paper_mission_run_contract.json` / `paper-mission-run.v1`；DHD、currentness、storage、owner-route、dispatch 和 PaperRecovery 只作为 diagnostics / migration / provenance 读取，不能作为 product 或 domain-handler 的默认论文主线，也不能声明 paper progress、publication-ready、runtime-ready 或 DM002/DM003 完成。
...
- 若要判断论文推进，默认读 `artifact_first_mission_summary.paper_mission_run` 和 mission top-level fields；不要把 DHD/currentness/storage/dispatch/PaperRecovery diagnostics、focused tests 或 read-model clean 写成 paper progress。
