...
- 运行时层：`Med Auto Science` 已完成 monolith closeout，持有课题与工作区权威语义、默认运行、默认诊断、进度入口以及发表判断；默认执行继续继承本机 `Codex` 配置；`Hermes-Agent` 只作为可选 hosted runtime target / reference-layer 运行载体，并可由 OPL Runtime Manager 管理其 family-level adapter/projection；`MedDeepScientist` 不再是默认 operation / diagnostic / runtime root / WebUI 依赖，只保留为显式 backend audit、source provenance、historical fixture、explicit archive import reference、behavior parity oracle 与 upstream intake source。
