...
- 吸收 Light 等外部科研技能包时，只把 verified asset discipline、核心撞车检查、审稿拒稿预演、fresh-evidence gate 和可泛化 skill-engineering 方法写成 MAS-native advisory refs / runbook pattern；当前由 `medautosci study light-advisory-materialize` 自动物化到 stage advisory bundle，服务 stage-quality、knowledge、reviewer briefing 和 next owner delta。progress passport、checkpoint、references/scripts/assets 分层、PRISMA flow、style fingerprint、locator audit 和 bounded mode registry 只能作为 MAS-owned 方法；不安装外部 runtime，不复制外部 skill router，不把外部知识库或评分体系写成 MAS truth。
...
- 外部项目 intake 的当前边界是 pattern source / provenance / advisory briefing。Light 的 progress passport / checkpoint / locator audit / PRISMA / style fingerprint / mode registry 只能被重写成 MAS-owned advisory method；Light 的 `db09`、知识库、verification log、review score、checklist、API 表和 router 不拥有 MAS study truth、publication-route memory body、source readiness、AI reviewer verdict、publication gate、artifact authority、dispatch blocker 或 owner receipt。
