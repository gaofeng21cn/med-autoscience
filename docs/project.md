...
- `Hermes-Agent` 只出现在显式非默认 Agent executor adapter、reference-layer 或 proof lane 语境；MAS 的稳定 capability surface 和研究 owner 语义继续以 MAS surface 为准。当前只保证这类 executor 能接入、能产出回执并可审计，不保证行为或质量效果与 `Codex CLI` 等价。
