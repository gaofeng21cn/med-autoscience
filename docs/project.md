...
- 吸收 Light 等外部科研技能包时，只把 verified asset discipline、核心撞车检查、审稿拒稿预演和 fresh-evidence gate 写成 MAS-native advisory refs，服务 stage-quality、knowledge、reviewer briefing 和 next owner delta；不安装外部 runtime，不复制外部 skill router，不把外部知识库或评分体系写成 MAS truth。
...
- 外部项目 intake 的当前边界是 pattern source / provenance / advisory briefing。Light 的 `db09`、知识库、verification log、review score 和 router 不拥有 MAS study truth、publication-route memory body、source readiness、AI reviewer verdict、publication gate、artifact authority 或 owner receipt。
