...
- OPL 运行管理层：目标形态中，`OPL Runtime Manager` 位于 OPL product entry / family orchestration 与 Temporal-backed family runtime provider 之间，负责 provider profile/provisioning、task registration hydration、runtime status projection、doctor/repair/resume、native helper catalog 与高频状态索引；Temporal 是 OPL production online runtime 的必需 substrate，Hermes 只在显式 provider / Agent executor adapter 或诊断语境中出现，local provider 只作为 dev/CI/offline diagnostic baseline。MAS 研究 truth、scheduler kernel、session store、memory store 和 domain quality owner 继续由 MAS-owned surface 持有。
