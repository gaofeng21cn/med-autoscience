...
- 论文推进 SSOT 层：`PaperMissionRun` 是 MAS 论文推进的默认运行单元和人读 / product / skill / domain-handler 默认读面。`paper-mission inspect|start|resume|consume-candidate`、`medical_paper_product_entry.default_action_intent=domain_route/start-or-resume`、domain-handler `domain_route/start-or-resume` 和 `artifact_first_mission_summary.paper_mission_run` 都必须回到同一份 mission truth。旧 domain diagnostic、owner-route、owner-callable dispatch、PaperRecovery 和 legacy blocker 只允许作为 `legacy_truth_import_pack`、mission input、decision constraint、diagnostic、migration 或 provenance 读取，不能重新成为默认执行状态。
...
- 让论文推进默认从 `PaperMissionRun.current_mission`、`next_owner`、`required_output`、`consume_candidate_status` 和 artifact delta / owner decision packet 读取；旧 blocker 被保留为输入约束和 non-degradation evidence，而不是继续支配当前执行状态。
- 吸收 Co-Scientist 论文启发时，只把候选假设组合、evidence pack、negative / failed-path ledger、decision trace、advisory ranking/proximity 和 reviewer/human gate closeout 写成 MAS-native 研究推进形状；不引入外部 runtime，也不让 ranking/Elo/proximity 关闭质量或发表 gate。
- 吸收 Light 等外部科研技能包时，只把 verified asset discipline、核心撞车检查、审稿拒稿预演、fresh-evidence gate 和可泛化 skill-content 方法写成 MAS-native advisory refs / companion templates / runbook pattern；当前由 `medautosci study light-advisory-materialize` 自动物化到 stage advisory bundle，服务 stage-quality、stage knowledge、reviewer briefing、display QA 和 next owner delta。progress passport、checkpoint、references/scripts/assets 分层、source map / literature matrix、experiment matrix / reproducibility plan、manuscript / rebuttal modes、citation locator、PRISMA flow、figure export / integrity / panel template、style fingerprint、argument review 和 bounded mode registry 只能作为 MAS-owned optional refs 或 companion 方法；不安装外部 runtime，不复制外部 skill router，不把外部知识库、期刊规格表、font defaults、示例项目或评分体系写成 MAS truth。
...
- 按 [MAS / OPL Agent OS 目标运行架构与重构计划](./runtime/designs/mas_opl_agent_os_target_operating_architecture.md) 收敛下一层重构：OPL 是 Agent OS / runtime substrate，MAS 是 Medical Research Pack + Medical Authority Kernel，外部科研系统能力进入 Scientific Capability Registry 或 refs-only advisory worker；新增 capability、worker、read-model、lifecycle 或 workbench 面都必须服务 `current_owner_delta` 主线，不新增默认前置流程或第二 authority。
- 按 [MAS / OPL 进度运行时理想蓝图](./runtime/designs/mas_opl_progress_runtime_ideal_blueprint.md) 和 `contracts/paper_progress_transition_runtime_completion_audit.json#/blueprint_l0_l7_functional_acceptance` 读取当前 progress runtime 完成度：repo/source/control-plane L0-L7 功能面已作为源码、合同、readback shape、replay fixture、projection demotion、legacy retirement/rebuild surface 和 live-claim harness 落地；live runtime / paper-line acceptance 仍另账，不声明 runtime ready、provider running、paper-line ready、publication-ready 或 production-ready。
- 按 [MAS 理想目标态差距与完善计划](./active/mas-ideal-state-gap-plan.md) 的“一步到位落地矩阵与完成度审计”读取顶层规划验收：OPL 基座对应 `TransitionRuntime`、`Outbox`、`StageRunKernel`、`StateIndex`、`Tool Arsenal` 和 `Workbench`；MAS 侧只保留 `Declarative Medical Research Pack` 与 `Medical Authority Kernel`。后续完成度百分比必须把 `repo/source/control-plane` 与 `live acceptance` 分账，不能用 docs、contract、tests、descriptor、queue empty、projection clean 或 refs-only candidate 替代 live/runtime/owner evidence。
...
- `OPL` 可以托管、唤醒和投影 MAS mission refs，但不拥有 `PaperMissionRun` 的 paper-progress truth，不签 MAS owner receipt，不创建 MAS typed blocker，不授权 publication quality、artifact mutation、current package 或 submission readiness。
...
- 外部项目 intake 的当前边界是 pattern source / provenance / advisory briefing。Light 的 progress passport / checkpoint / source map / literature matrix / experiment matrix / reproducibility plan / manuscript mode / rebuttal mode / locator audit / PRISMA / figure export / integrity / panel template / style fingerprint / argument review / mode registry，只能被重写成 MAS-owned optional advisory refs 或 stage skill companion content；Light 的 `db09`、知识库、verification log、review score、checklist、API 表、期刊规格表、font defaults、示例项目和 router 不拥有 MAS study truth、publication-route memory body、source readiness、AI reviewer verdict、publication gate、artifact authority、dispatch blocker 或 owner receipt。
