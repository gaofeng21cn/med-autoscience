...
- 操作与集成层：`CLI`、`MCP`、`controller`，以及 repo-tracked 的 workspace commands / scripts / contracts，共同构成 MAS 对外稳定 capability surface；`product-entry manifest`、`OPL handoff` 与其他机器可读桥接负责 family-level session/runtime/projection 编排和 shared modules/contracts/indexes，研究 owner 身份继续归 MAS。
