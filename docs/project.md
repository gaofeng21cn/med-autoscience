...
- 论文推进 SSOT 层：`PaperMissionRun` 是 MAS 论文推进的默认运行单元和人读 / product / skill / domain-handler 默认读面。`paper-mission inspect|start|resume|consume-candidate`、`medical_paper_product_entry.default_action_intent=paper_mission/start_or_resume`、domain-handler `paper_mission/start_or_resume` 和 `artifact_first_mission_summary.paper_mission_run` 都必须回到同一份 mission truth。旧 domain diagnostic、owner-route、owner-callable dispatch、PaperRecovery 和 legacy blocker 只允许作为 `legacy_truth_import_pack`、mission input、decision constraint、diagnostic、migration 或 provenance 读取，不能重新成为默认执行状态。
