...
- `MAS supervision scheduler contract`：当前仍是 MAS standalone/local diagnostics 的外层 supervision cadence、job identity、tick receipt、SLO / drift projection 和 adapter migration owner；默认 adapter 是 MAS-owned `local` scheduler，macOS backend 已落地为 LaunchAgent。该 surface 只能解释为 direct/local diagnostic 迁移桥和当前残留，不能写成 MAS 长期 generic runtime 内核。OPL Full online runtime 的 family-level wakeup 由 OPL family runtime provider 触发，再通过 `medautosci sidecar dispatch` 进入 MAS owner surface；后续应把 scheduler lifecycle、cadence SLO 和 job registry 上收到 OPL provider/runtime manager。
