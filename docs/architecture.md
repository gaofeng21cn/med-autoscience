...
2. 操作与集成层
   ...
   - `OPL Runtime Manager` 是 OPL 侧的薄运行管理/投影层：它接收 MAS 暴露的 task registration、runtime_control projection、status/artifact locator 与 approval/wakeup 边界，再把这些信息挂到 OPL 的 profile、task、resume、doctor 与索引面。Provider-backed family topology 中，OPL Full online runtime 由已配置的 family runtime provider 提供 durable stage attempt、wakeup、signal/query、retry/dead-letter、resume 与 transport；Temporal 是 production online runtime 的必需 substrate，也是 MAS hosted autonomous runtime 的默认 provider。Hermes 只在显式非默认 executor/proof lane、hosted proof backend 或诊断语境中出现。MAS 侧通过 `medautosci sidecar export|dispatch` 暴露受控桥接。高频文件/状态索引可由 OPL Rust native helper 加速，MAS 侧通过 `native_helper_consumption.proof_surface` 和 `contracts/opl-framework/native-helper-contract.json` 固定 index-only 边界；研究真相来源继续是 MAS truth surface。
...
3. 运行时与持久真相层
   ...
   - `MAS Runtime OS` 持有医学研究 truth、paper-progress SLO 语义、owner receipt、typed blocker、safe action refs、no-forbidden-write evidence、runtime event refs、guarded apply 与 diagnostic surface。默认外层监督调度已经由 OPL replacement 持有；`MAS supervision scheduler contract` 只保留 local scheduler tombstone/provenance refs，不再安装、刷新、状态检查、清理或触发本机 LaunchAgent。`Progress Portal` / `Live Console` / `study-progress` / cockpit 只做 Product Projection。
   - `Stage` 是一次大型研究步骤；Agent executor 是 stage 内最小执行单位，`Codex CLI` 是当前第一公民 concrete executor，继续继承本机 Codex 配置。OPL Full 的长期在线 substrate 由 OPL family runtime provider 承担。该 provider 负责唤醒、signal/query、delivery/approval transport、retry/dead-letter、resume 与 family queue tick；MAS study truth、publication judgment、quality gate 和 artifact authority 继续由 MAS 持有。
...
`family_action_catalog` 是这些 callable action 的 MAS-owned metadata 单一声明面；repo-root OPL standard pack 现在以 `agent/` 作为 canonical repo-source semantic pack，把 stage prompt、stage policy、skill policy、knowledge refs、quality gate refs、action catalog、memory/artifact/receipt contract 和私有功能审计一起声明为 OPL pack compiler 输入。`OPL` 从该 pack 生成统一 CLI / MCP / Skill / product-entry / tool descriptor bundle，用于 stage-runtime discovery/export/parity；MAS 现有 CLI、MCP、single app skill、product-entry、sidecar 和 controller 不再各自维护通用接口真相，而是作为 generated descriptors 指向的 action target / authority function。MAS action execution、study truth、runtime controller truth、AI reviewer judgement、publication gate 和 artifact authority 留在 MAS。
...
- `OPL scheduler replacement`：当前默认 outer supervision scheduler owner 已迁到 `opl_provider_runtime_manager`，默认 adapter 是 `opl_family_runtime_provider`。`runtime-supervision-status`、`runtime-ensure-supervision` 和 `runtime-remove-supervision` 默认只投影或委托 OPL provider/runtime manager replacement；MAS 保留 paper-progress SLO 语义、owner receipt、typed blocker、safe action refs 和 no-forbidden-write evidence。MAS-owned `local` adapter / LaunchAgent install path 已物理退役；公开 CLI 不再暴露 `--manager local`，MAS 只保留 tombstone/provenance refs。
- `Hermes scheduler cleanup`：显式 Hermes 只保留 `runtime-supervision-status --manager hermes` 与 `runtime-remove-supervision --manager hermes`，用于读取或移除旧 cron job/script/session/gateway evidence。`runtime-ensure-supervision --manager hermes` 不再是公开入口，controller direct-call 只返回 retired tombstone，不写 tick script、不 create/edit/resume/run cron job。
- `Workspace runtime layout migration`：当前论文 workspace 的 MAS domain runtime binding / quest snapshot 默认指向 MAS-owned `runtime/` 与 `runtime/quests/<quest_id>`，active default backend 标识为 `opl_provider_backed_stage_runtime` / `opl-provider-backed-stage-runtime`，delegated MAS adapter 标识为 `mas_runtime_core` / `mas-runtime-core`；generic runtime owner 与 provider substrate 归 OPL，MAS adapter 只表示 domain adapter / receipt / diagnostic surface。旧 `ops/med-deepscientist`、legacy backend id、legacy diagnostic 和旧 baseline 绝对路径只能作为 `historical_fixture_ref` / `explicit_archive_import_ref` 的 read-only provenance 保存，不能重新进入 active runtime、controller 或 paper truth。
- `Legacy physical cleanup gate/apply`：物理删除或归档旧 `ops/med-deepscientist` 根前必须通过 `workspace-legacy-physical-cleanup-audit`。该 gate 只读 profile、runtime binding / quest snapshot、monolith migration ledger、delivery manifest、controller decision / publication eval 等 targeted surface；它证明 replacement 是否仍成立、旧 workspace-local service wrapper 是否还存在、旧 MDS 根是否仍被 current truth/delivery/provenance refs 引用。`workspace-legacy-physical-cleanup-apply --dry-run|--apply` 只在 gate 成立后执行 archive move、tombstone 与 targeted provenance ref rewrite，不写 paper truth、controller truth、submission package、runtime SQLite 或 generic runtime state。当前 5 个真实 profile 已完成 archive/tombstone 与 no-active-reference proof；后续该 surface 主要承担 drift guard 和新增 ref cleanup。
...
- `OPL family runtime provider`：OPL Full online family runtime 的外部 durable substrate / hosted carrier / delivery transport；Temporal 是 production online runtime 的必需 substrate，Hermes 只在显式非默认 executor/proof lane、hosted proof backend 或诊断语境中出现。MAS 当前第一公民 concrete executor、MAS domain truth、MAS quality 和 artifact owner 继续由 MAS 持有；非默认 executor 只承诺接入、回执和审计，不承诺行为效果等价。
...
- `Runtime OS` 的当前落点是 MAS domain operations state、runtime health、human gate、controller-owned resume action 与 MAS-owned Runtime Turn Lifecycle Kernel。Kernel 持有 `schedule_turn`、`complete_turn_and_normalize`、`inspect_turn_lifecycle`、user message queue、worker lease、runner monitor、delayed auto-continue timer 和 turn receipt；这些只作为 domain receipt / runtime event refs / guarded apply 输出被 OPL generic runtime 消费。generic retry budget、attempt ledger、queue、worker residency、transition runner、provider wakeup 与 lifecycle/index owner 归 OPL。
- 因此 MAS/MDS 行为差异现在按两层判断：内层 `turn completion continuation` 已达到旧 MDS 自动科研连续跑的 domain adapter 行为等价；外层 resident daemon 特征按 OPL/Temporal provider cadence / OPL generic runtime + MAS domain receipt 边界处理，监督刷新、drift detection、stale recovery、retry 与 resume 由 OPL replacement 承担。Terminal/log observation 已由 `live-console-parity` 落为 MAS-owned read-only purpose parity；Progress Portal 和 MAS Live Console 按 MAS 当前架构表达，Hermes cron 只保留历史/provenance 语境。WebSocket terminal attach / UI-issued runtime control 是后续 gated parity lane。
...
后续增强建议先通过 [Program Portfolio Consolidation](./active/program_portfolio_consolidation.md) 归并评估；旧 [MAS/MDS Unified Enhancement Program](./history/program/mas_mds_unified_enhancement_program.md) 作为历史 backlog 参考。真实 workspace soak、PI 动作投影、投稿结果 calibration、provider 运营、journal-family pack、legacy upgrade queue、delivery traffic-light、backfill blocker、audit compaction 和结构治理都服务对应 owner surface；系统状态解释继续回到 `StudyTruthKernel`、`RuntimeHealthKernel`、AI reviewer-backed `publication_eval/latest.json`、`controller_decisions/latest.json` 和 canonical artifact proof。

Runtime lifecycle 小文件治理和 quest Git 退役的执行入口固定为 [Runtime Lifecycle SQLite Migration Program](./active/runtime_lifecycle_sqlite_migration_program.md)。该 program 把 SQLite 定义为 runtime lifecycle authority 和 read-model layer，把 Git 固定为 repo source control，把 latest authority、restore metadata、paper/manuscript/package/dataset 交付物继续固定为文件或归档形态；它属于 Runtime OS / maintainability lane，study truth、publication truth 和 artifact authority 留在对应 MAS owner surface。
...
live artifacts 不属于本治理面的写集。`publication_eval/latest.json`、`controller_decisions/latest.json`、`progress_projection`、`domain_health_diagnostic`、`current_package`、submission package、runtime lifecycle SQLite、restore archives 与真实 workspace 产物都继续由 stable runtime / controller / canonical artifact flow 持有。Boundary governance 只报告 repo-tracked source/test/docs 的维护风险，不触碰或重生成 live study artifact。
...
- 迁移、解构、切换和历史推进记录继续留在 `docs/active/`、`docs/runtime/`、`docs/references/` 和 `docs/history/`。
