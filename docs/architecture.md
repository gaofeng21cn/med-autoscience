...
- `Workspace runtime layout migration`：当前论文 workspace 的 MAS domain runtime binding / quest snapshot 默认指向 MAS-owned `runtime/` 与 `runtime/quests/<quest_id>`，active backend 标识为 `mas_runtime_core` / `mas-runtime-core`，但它只表示 domain adapter / receipt / diagnostic surface；generic runtime owner 与 provider substrate 归 OPL。旧 `ops/med-deepscientist`、legacy backend id、legacy diagnostic 和旧 baseline 绝对路径只能作为 `historical_fixture_ref` / `explicit_archive_import_ref` 的 read-only provenance 保存，不能重新进入 active runtime、controller 或 paper truth。
