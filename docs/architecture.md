...
3. 运行时与持久真相层
   ...
   - `MedDeepScientist` 不再是默认 operation 或默认 diagnostic 依赖；它只保留为 frozen source archive、historical fixture、explicit legacy restore/import/backend-audit diagnostic 与 provenance reference，不是用户入口，也不是第二 owner。
...
- `MedDeepScientist`：frozen source archive、historical fixture、explicit legacy restore/import/backend-audit diagnostic 与 provenance reference；不承担默认运行依赖、默认诊断依赖、用户入口或第二 owner 身份。
...
- 因此 MAS/MDS 行为差异现在必须按两层判断：内层 `turn completion continuation` 已达到旧 MDS 自动科研连续跑的行为等价；外层 resident daemon 特征仍按 scheduler-bound MAS Runtime OS 处理，300 秒 tick 只负责监督刷新、drift detection 和 stale recovery。WebSocket terminal/log streaming 由 `live-console-parity` staged contract 继续推进，不把 Progress Portal 或 Hermes cron 写成旧 MDS WebUI / resident daemon 的 1:1 替代。
...
- `MedDeepScientist` 只保留 frozen source archive / historical fixture / explicit legacy diagnostic / provenance reference 角色。
