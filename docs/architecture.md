...
3. 运行时与持久真相层
   ...
   - 默认 concrete executor 继续继承本机 `Codex` 配置；OPL Full 的长期在线 substrate 由 OPL family runtime provider 承担。该 provider 只负责唤醒、signal/query、delivery/approval transport、retry/dead-letter 与 family queue tick，不持有 MAS study truth、publication judgment、quality gate 或 artifact authority。
