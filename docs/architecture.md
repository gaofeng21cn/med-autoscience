...
2. 操作与集成层
   ...
   - `OPL` 在当前定位中是完整智能体运行框架，不只是入口聚合层：它提供 stage attempt、queue、wakeup、approval/retry/dead-letter、trace/projection 和 shared module/index 能力，可作为 MAS 外部依赖运行；但医学研究 owner 仍留在 MAS。
...
3. 运行时与持久真相层
   ...
   - `Stage` 是一次大型研究步骤；`Codex CLI` 是 stage 内默认 concrete executor 和最小执行单元，继续继承本机 Codex 配置。OPL Full 的长期在线 substrate 由 OPL family runtime provider 承担。该 provider 只负责唤醒、signal/query、delivery/approval transport、retry/dead-letter 与 family queue tick，不持有 MAS study truth、publication judgment、quality gate 或 artifact authority。
...
- `OPL stage-led framework`：完整智能体运行框架，负责 MAS stage descriptor discovery、attempt/queue/approval/retry/dead-letter/trace/projection 支撑，可作为外部依赖运行 MAS stage；不生成医学结论、不替 MAS 执行 publication 或 quality verdict。
...
- `OPL family runtime provider`：OPL Full online family runtime 的外部 durable substrate / hosted carrier / delivery transport；Temporal 是生产目标，Hermes 是迁移期 legacy/optional provider 或 executor/proof lane。它不改写 MAS 默认 concrete executor，不持有 MAS domain truth，也不替代 MAS quality 或 artifact owner。
