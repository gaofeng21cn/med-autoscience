...
`family_stage_control_plane_descriptor` 是 Stage-Led Autonomy 的 MAS-owned descriptor projection；它从 `agent_entry_modes.yaml` 和 `stage_knowledge_plane_contract` 暴露 route snapshot、stage packets、memory closeout/router/recall、evidence/review/controller/publication refs 与 `authority_boundary`。`OPL` 可以读取它做 family-level indexing、display、freshness check 和 MAS-exported task discovery，但不能用它派生 route、写 study truth、提升 memory 为 evidence，或授权 publication quality / submission readiness。
