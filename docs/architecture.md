...
- `Legacy physical cleanup gate/apply`：物理删除旧 `ops/med-deepscientist` 根之前，必须先通过 `workspace-legacy-physical-cleanup-audit`。该 gate 只读 profile、runtime binding / quest snapshot、monolith migration ledger、delivery manifest、controller decision / publication eval 等 targeted surface；它证明 replacement 是否仍成立、旧 workspace-local service wrapper 是否还存在、旧 MDS 根是否仍被 current truth/delivery/provenance refs 引用。`workspace-legacy-physical-cleanup-apply --dry-run|--apply` 只在 gate 成立后执行 archive move 与 targeted provenance ref rewrite，不写 paper truth、controller truth、submission package、runtime SQLite 或 generic runtime state。只要 current delivery/controller refs 或 retained provenance refs 仍指向旧物理根，MAS 只能输出 blocked/dry-run archive/tombstone plan，不执行物理删除。
