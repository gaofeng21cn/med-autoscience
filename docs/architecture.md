...
当 OPL App 需要统一进度看板、长期托管、跨域唤醒或统一状态面时，`OPL Runtime Manager` 消费 MAS progress portal payload refs、HTML refs 和现有 MAS projection，并把结果索引为 family-level dashboard / runtime status；study truth、publication judgment 和 evidence ledger 继续由 MAS 持有。最佳集成形态见 [Progress Portal OPL App Integration](./references/integration/progress_portal_opl_app_integration.md)。
...
- `gateway / harness` 继续保留为内部架构边界语言；对外第一身份是 `Med Auto Science` domain agent。
