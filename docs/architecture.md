...
- `Legacy physical cleanup provenance`：旧 `ops/med-deepscientist` 根归档、tombstone 与 targeted provenance ref rewrite 已完成真实 profile closeout；专用 `workspace-legacy-physical-cleanup-*` controller / CLI 已物理退役。当前只保留 `init-workspace` 的 retired-entry cleanup、legacy tombstone contract 和 history/provenance refs；MAS 不再提供旧物理根 cleanup/apply active command。
