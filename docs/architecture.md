...
- `OPL family runtime provider`：OPL Full online family runtime 的外部 durable substrate / hosted carrier / delivery transport；Temporal 是 production online runtime 的必需 substrate，Hermes 只在显式 provider / Agent executor adapter 或 proof lane 语境中出现。MAS 当前第一公民 concrete executor、MAS domain truth、MAS quality 和 artifact owner 继续由 MAS 持有；非默认 executor 只承诺接入、回执和审计，不承诺行为效果等价。
