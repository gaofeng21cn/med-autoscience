...
- `Research Integrity Layer` 是投稿前 integrity gate input 层：`reference_verification_attestation` 核实文献真实性与撤稿/更新信号，`claim_citation_support_matrix_v2` 绑定 claim、citation、evidence 与 attestation，`manuscript_consistency_meta_review` 检查全文数字、单位、人群、时间窗、display-to-claim、逻辑和 reporting guideline 一致性。OPL 持有 Crossref / PubMed / OpenAlex / Semantic Scholar / publisher / Crossmark connector、cache、rate limit、attestation store 和 generic gate runner；MAS 持有医学解释、publication quality consumption、owner receipt、typed blocker、human gate 与 artifact / submission authority。该层只产出 evidence / gate input / blocker candidate，不能写 `publication_eval/latest.json`、`controller_decisions/latest.json`、`current_package` 或声明 publication-ready / submission-ready。
