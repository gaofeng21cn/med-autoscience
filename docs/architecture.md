...
3. 运行时与持久真相层
   ...
   - OPL provider-backed stage runtime 持有 generic runtime owner / substrate：durable attempt、queue、wakeup、retry/dead-letter、worker residency、transition runner、provider transport 和 generic lifecycle/index。
   ...
   - `Stage` 是一次大型研究步骤；Agent executor 是 stage 内最小执行单位，`Codex CLI` 是当前第一公民 concrete executor，继续继承本机 Codex 配置。OPL Full 的长期在线 substrate 由 OPL family runtime provider 承担。该 provider 负责唤醒、signal/query、delivery/approval transport、retry/dead-letter 与 family queue tick；MAS study truth、publication judgment、quality gate 和 artifact authority 继续由 MAS 持有。
...
- 因此 MAS/MDS 行为差异现在按两层判断：内层 `turn completion continuation` 已达到旧 MDS 自动科研连续跑的 domain adapter 行为等价；外层 resident daemon 特征按 OPL provider cadence / OPL generic runtime + MAS domain receipt 边界处理，监督刷新、drift detection 和 stale recovery 由 OPL replacement 或显式 operator action 承担。Terminal/log observation 已由 `live-console-parity` 落为 MAS-owned read-only purpose parity；Progress Portal、Hermes cron 和 MAS Live Console 按 MAS 当前架构表达。WebSocket terminal attach / UI-issued runtime control 是后续 gated parity lane。
