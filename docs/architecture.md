...
3. 运行时与持久真相层
   ...
   - MAS domain runtime authority surface 持有医学研究 truth、paper-progress SLO 语义、owner receipt、typed blocker、safe action refs、no-forbidden-write evidence、runtime event refs、guarded apply 与 diagnostic refs。默认外层监督调度和 generic runtime control 已由 OPL replacement 持有；`MAS supervision scheduler contract` 只保留 local scheduler tombstone/provenance refs，不再安装、刷新、状态检查、清理或触发本机 LaunchAgent。`Progress Portal` / `study-progress` / cockpit 只做 Product Projection；旧 MAS 私有 Live Console 只保留 history/provenance。
...
- `Workspace runtime layout migration`：当前论文 workspace 的 MAS domain runtime binding / quest snapshot 默认指向 OPL provider-backed stage runtime 与 MAS domain authority refs；active default backend 标识为 `opl_provider_backed_stage_runtime` / `opl-provider-backed-stage-runtime`。历史 `mas_runtime_core` / `mas-runtime-core` 字段只能作为 retired provenance / migration input 读取；generic runtime owner 与 provider substrate 归 OPL，MAS 只输出 domain authority refs、owner receipt、typed blocker、artifact/source/quality refs 和 diagnostic refs。旧 `ops/med-deepscientist`、legacy backend id、legacy diagnostic 和旧 baseline 绝对路径只能作为 `historical_fixture_ref` / `explicit_archive_import_ref` 的 read-only provenance 保存，不能重新进入 active runtime、controller 或 paper truth。
...
- `Runtime OS` 的当前落点是 OPL-owned stage/runtime control plane 加 MAS domain authority refs。OPL 持有 attempt、queue、wakeup、provider query、typed closeout、retry/dead-letter、worker residency、human gate transport、current_control_state 和 lifecycle/index；MAS 只持有 DomainIntent / owner route、owner receipt、typed blocker、artifact/source/quality refs、guarded apply receipt、paper-progress SLO 解释与 diagnostic explanation。旧 `schedule_turn`、turn runner、worker lease、runner monitor、delayed auto-continue timer、runtime session read model 和 recovery intent 面已经按 no-alias physical retirement 处理，不再作为 MAS Runtime OS 子系统保留。
- 因此 MAS/MDS 行为差异现在按 OPL stage runtime 与 MAS domain authority boundary 判断：provider cadence、supervision refresh、drift detection、stale recovery、retry、resume、terminal/log observation、provider attempt drilldown 和 UI-issued runtime control 归 OPL `current_control_state` / runtime workbench layer；MAS Progress Portal 只表达 domain progress、route/decision、owner receipt、typed blocker 和 artifact/source refs。Hermes cron、旧 Console parity、旧 turn lifecycle kernel 和旧 runtime read-model 只保留历史/provenance 语境。
