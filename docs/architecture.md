...
3. 运行时与持久真相层
   ...
   - `MAS Runtime OS` 持有 Runtime Core；当前 `MAS supervision scheduler contract` 仍持有 direct/local diagnostics 的外层监督调度语义，这是本仓仍未完全上收到 OPL 的功能层残留；`Progress Portal` / `Live Console` / `study-progress` / cockpit 只做 Product Projection。
