...
- `OPL current control plane`：当前默认 outer supervision、stage attempt、provider liveness、retry/dead-letter 和 operator runtime projection 由 OPL `current_control_state` / provider attempt ledger 持有。MAS 不再暴露 scheduler lifecycle CLI、manager alias 或 workspace-local service wrapper；只保留 paper-progress SLO 语义、owner receipt、typed blocker、safe action refs、no-forbidden-write evidence 与 tombstone/provenance refs。
- `Hermes scheduler cleanup`：Hermes 只作为历史 cron/script/session/gateway evidence 的 provenance 来源或显式非默认 proof lane；MAS 不再提供 Hermes scheduler status/remove/ensure 入口，不写 tick script，不 create/edit/resume/run cron job，也不把 Hermes 投影成当前 runtime truth。
