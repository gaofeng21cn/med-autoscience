...
MAS 对 OPL 的标准化适配由三个 domain-owned machine profile 收口：`contracts/domain_route_profile.json` 归一化 runtime / autonomy / aftercare task kind、route identity、terminal sync discriminator、receipt labels 与 forbidden writes；`contracts/domain_projection_profile.json` 把 current-work-unit 收敛为 registry-driven generic projection，并把 paper/study 展示隔离在 OPL 不解释的 `domain_display`；`contracts/display_pack_opl_adapter.json` 把 Display Pack v2 先转换成通用 pack descriptor。active runtime task kind 只使用 `domain_route/*` 与 `domain_autonomy/*`；`paper_mission/*`、`paper_autonomy/*`、`publication_aftercare/*` 只能作为 MAS-owned normalization input、历史 evidence 或 domain-internal vocabulary。OPL 不得从 legacy 名称、`study_id`、paper mission payload 或 Display Pack domain fields 推断框架语义。
