...
2. 操作与集成层
   ...
   - 在 OPL stage-led family framework 中，MAS stage descriptor、handoff、receipt 和 projection 可以被 OPL 发现和托管；stage 内部的研究拆解、分析探索、写作、审核和路线判断由 MAS prompt/skill、AI reviewer、controller truth 和被选中的 Agent executor 执行，当前第一公民 executor 是 `Codex CLI`。
...
- `Workspace runtime layout migration`：当前论文 workspace 的 active runtime binding / quest snapshot 默认指向 MAS-owned `runtime/` 与 `runtime/quests/<quest_id>`，active backend 标识为 `mas_runtime_core` / `mas-runtime-core`。旧 `ops/med-deepscientist`、legacy backend id、legacy diagnostic 和旧 baseline 绝对路径只能作为 `historical_fixture_ref` / `explicit_archive_import_ref` 的 read-only provenance 保存，不能重新进入 active runtime、controller 或 paper truth。
