...
- `Agent Tool Arsenal` 是 Runtime OS 与 Capability Registry 之间的 agent-facing invocation ABI。MAS 的 action catalog、MCP tools、owner callable、stage skills、sidecar 和 native helpers 需要被编译成 `ToolArsenalIndex`、`ToolUseCard`、`CapabilityInvocationPlan`、`ToolResultEnvelope` 和 audit trail，让 autonomous agent 从 `current_owner_delta` 自动发现、选择、调用和验证工具；人类用户只管理目标、授权和异常，不学习底层 CLI/MCP/helper 拼装。
