...
- DHD dry-run：`managed_study_actions`、dispatch/admission 状态、当前 safe action、blocker refs；
...
Supervisor 必须把缺少 touchpoint protocol 的 package 判为 `unanchored_foreground_candidate`，不得计入 mission progress；下一步必须 steering executor 先补 fresh `study progress` / DHD / legal owner path precheck / post-write readback，而不是继续扩写 package。
...
- package 只有手工文件，没有 fresh `study progress` / DHD / legal owner path precheck / post-write readback；
