...
- `current_owner_identity_unavailable_for_guard` 只能说明 guarded no-write precheck 没有拿到 current owner identity；它可以证明没有越权写，但不能证明 B002 owner path 已修好。下一步应追 current owner identity 为什么没有从 PaperMission / StageOutcome、MAS owner answer 或 OPL `StageAttemptReceipt` 稳定产出 / 传递；该缺口只阻止当前 wrong-target mutation，不阻止 Codex 启动其他 declared stage。`current_work_unit` 只能作为 legacy diagnostic / currentness 对照，不能补出权威下一步。
...
- `paper-mission inspect` / `study progress` fresh readback：study id、StageOutcome、nonbinding Codex route context、MAS owner answer / typed blocker / human gate / route-back、source refs；legacy `current_work_unit` / `current_executable_owner_action` 只能作为 diagnostic drilldown 对照；
...
2026-06-21 的 B002 / B003 incident replay 只保留为 provenance，不是当前 truth。B002 当时的 AI reviewer successor action、B003 当时的 owner-gate / domain blocker projection、fingerprint、source record 和 refs 细节不再保存在本 active runbook；需要追溯时读 [Program 历史归档](../history/program/README.md)、对应 workspace receipt 或 git history。

当前 B002 / B003 owner、action、blocker、successor 或 human decision 必须 fresh 读取 `paper-mission inspect` / `study progress`、Codex-selected stage、StageOutcome、MAS owner answer、OPL `StageAttemptReceipt`、owner receipt、typed blocker、human gate 或 route-back。缺 route context 时 Codex 仍可从任一 prior artifact/diagnostic 启动 declared stage；不得从 dated successor action、loose `current_work_unit` 或 historical `paper_recovery_state` 补出权威下一步。foreground sprint可以继续产出MAS/OPL-anchored paper-facing artifact、diagnostic、human decision packet、blocked revision alternatives或manuscript/package patch proposal。
