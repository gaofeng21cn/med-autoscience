...
当前 B002 / B003 owner、action、blocker、successor 或 human decision 必须 fresh 读取 `paper-mission inspect` / `study progress`、StageOutcome / `NextActionEnvelope`、MAS owner answer、OPL `StageAttemptReceipt`、owner receipt、typed blocker、human gate 或 route-back。缺 canonical envelope 时不得从 dated successor action、loose `current_work_unit` 或 historical `paper_recovery_state` 补出下一步；foreground sprint 只能产出 MAS/OPL-anchored paper-facing candidate、human decision packet、blocked revision alternatives、manuscript/package patch proposal 或明确保持 blocker 的理由，并等待 owner surface 消费。
