...
2026-05-22 MAS 已把 `domain_dispatch_evidence_record_payload` 接入 `domain_route/reconcile-apply`、`paper_autonomy/guarded-apply` 和 `publication_aftercare/*` pending task。该 payload 是 body-free refs-only handoff：`typed_blocker_refs` 表示 MAS owner-chain closeout 仍待真实 owner receipt 或 live paper-line stable blocker，`evidence_refs` 指向 owner-route / controller / aftercare / production-acceptance refs，`no_regression_refs` 指向 no-forbidden-write proof ref，`receipt_ref` 供 OPL `agents evidence apply` 记录。它只关闭 OPL domain-dispatch workorder 的可提交 payload 缺口；即使被 OPL record / verify，也只能关闭为 `closed_by_domain_owned_typed_blocker` 或 refs-only receipt，不授权 publication quality、artifact mutation、memory writeback、domain ready 或 production ready。
