...
| Package / ScholarSkills enhancement | `contracts/opl_agent_package_manifest.json` 声明 MAS SemVer `0.2.18`、可选增强范围 `>=0.2.12 <0.3.0` 和兼容 ABI/exports/modules；出版版式由 ScholarSkills 单一目录提供，MAS 只消费 selection ref | 缺失或不兼容只形成诊断/quality debt，不阻断 MAS；exact installed resolution、lock 与 activation 归 OPL package readback |
