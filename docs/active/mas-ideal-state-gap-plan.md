...
- 2026-05-21 本轮把 sidecar dispatch 编排从 `sidecar_family_adapter.py` 抽到 `sidecar_family_adapter_parts/dispatch_orchestration.py`，并用 focused test 锁定 dispatch 只产出 MAS dispatch receipt ref 和 forbidden-write guard，不写 `.ds` queue、runtime quest root、controller/publication truth 或 current package。该 tranche 是 `sidecar_dispatch_adapter` 的物理收薄与 no-forbidden-write proof，不是 physical delete、OPL default caller cutover、真实 paper-line owner receipt parity 或 App/workbench closeout。
