...
| Area | 当前进度 | 当前读法 |
| --- | --- | --- |
| Standard OPL Agent source shape | `strict_purity_cutover_pending` | `agent/` pack、stage/action/memory/artifact contracts、generated surface handoff 与 production acceptance 已成为 MAS 当前机器面；但 product/status/workbench/sidecar/controller/progress shell 仍有 repo-local active caller，必须迁到 OPL generated/default caller 后删除。 |
...
| Domain owner evidence shape | `shape_landed_scaleout_pending` | guarded apply、owner-route、aftercare、body-free evidence packet、typed blocker shape、逐 paper-line owner-chain result 与 OPL owner-payload group return shapes 已可供 OPL refs-only ledger 消费；真实 paper-line owner receipt / reviewer record / artifact movement 仍需扩面。 |
...
| Functional privatization boundary | `classified_with_delete_gates` | `contracts/functional_privatization_audit.json`、`contracts/production_acceptance/mas-production-acceptance.json`、`docs/status.md` | 未分类 generic owner 回流已被识别；但 classified retained adapter 不再算完成态。MAS retained surfaces 最终只能是 declarative pack、domain authority refs、minimal authority function、domain handler target 或 necessary medical helper；repo-local generic shell 满足 cutover 后删除。 |
| Default runtime owner | `opl_owned_default` | runtime default operation contract、product-entry manifest、sidecar export/read-model | 默认 runtime ref / substrate 是 `opl_hosted_stage_runtime`，engine id 是 `opl-hosted-stage-runtime`；domain adapter 是 `mas_domain_intent_adapter`；`default_autonomous_runtime` 默认启用，provider 为 `temporal`，wakeup/retry/resume owner 为 OPL。历史 `mas_runtime_core` 只能按 retired provenance / migration input 读取。 |
...
| Domain dispatch evidence payload | `return_shape_landed_scaleout_pending` | `domain_dispatch_evidence_record_payload`、focused body-free evidence refs tests、sidecar owner-route/export dispatch tests、live guarded-apply sidecar receipt、OPL `domain_dispatch_evidence_workorder_count=0` | MAS payload 现在显式输出 `domain_owner_receipt_refs`、`owner_chain_refs`、`stage_expected_receipt_refs`、`stage_monitor_freshness_refs`、`stage_runtime_event_refs`、`no_regression_evidence_refs`、`typed_blocker_refs`、success / typed-blocker payload path、required return shapes 和 legacy OPL action aliases；默认无 owner receipt 时仍 fail-closed 生成 MAS-owned typed blocker，有真实 owner receipt refs 时生成 `refs_only_domain_owned_success_payload` 且清空 typed blocker refs。paper-line provider canary closeout 现在会把真实 owner receipt 或 stable blocker 同步投影成 OPL 可 record/verify 的 `domain_dispatch_evidence_record_payload`，但 success path 仍只记录 owner-chain refs，不声明 domain ready、publication ready、artifact mutation authorization 或 `current_package` 更新。typed blocker ref 与 ledger hint 保留 stage/source identity token，避免同类 stage attempt 在 refs-only ledger 中碰撞；空 owner receipt refs 不代表 paper closure。stage evidence handoff 只把 expected receipt、monitor freshness 与 runtime event refs 绑定到同一 identity-bound payload，供 OPL stage evidence record/verify 消费，不声明 domain ready。provider-hosted guarded-apply 默认 target 只从当前 profile 的 canonical live study id 派生，并限于默认 canary target 的 canonical match，避免继续导出 retired shorthand identity；OPL refs-only ledger 已记录旧 `DM003` guarded-apply attempt 和 DM002 default-executor attempt 的 MAS-owned typed blocker / owner-chain / no-forbidden-write refs。后续真实 owner receipt 必须通过新的或 superseding owner-payload route 消费，不能回写改写已关闭的 typed-blocker receipt。 |
| Body-free evidence packet shape | `landed_shape_scaleout_pending` | `paper_line_guarded_apply_evidence`、`body_free_evidence_packets`、`paper_line_owner_chain_results`、production acceptance contract | progress delta、AI reviewer/gate、artifact movement、human gate/resume、stable typed blocker 和 no-forbidden-write proof 已有 refs-only packet shape；canary closeout 现在逐 study identity 绑定 owner receipt / stable blocker / progress / artifact / human-gate refs，供 OPL/App 记录同一论文线的 refs-only evidence。真实关闭仍需要 MAS owner chain 在真实 workspace 产出 owner receipt 或 stable typed blocker。 |
...
| Progress Portal workspace carrier thinning | `active_diagnostic_carrier_delete_blocked_by_callers` | `progress_portal_parts/workspace_carrier.py`、`mas_progress_portal_workspace_carrier_boundary`、focused Progress Portal materialization tests、active caller/delete blocker fields | Progress Portal 顶层只保留 MAS-owned payload / refs projection API；workspace 静态包、本地只读 HTTP carrier、study page materialization 和浏览器打开已抽到 carrier 子模块；active renderer 中旧 `conversation-*` timeline 样式残留已物理删除，并由 focused test 防止 runtime conversation read model / conversation UI 回到 MAS Portal。该 carrier 只写 display/read-model refs，但 `medautosci workspace progress-portal`、`--serve` 和 `ops/mas/bin/start-web` 仍是 active caller；当前 focused tests 证明 blocked boundary，物理删除仍被 OPL App 默认 carrier 未证实和 workspace helper no-active-caller proof 缺失阻塞。这不等于 `workbench_sidecar_status_cutover` 或 `domain_ref_consumer_thinning` 全部关闭。 |

## 当前功能/结构差距

以下项目按 strict standard-agent purity 读取为仍需推进的功能/结构差距。真实 evidence tail、provider completion、repo tests 或 refs-only ledger 消费结果不能把这些 active shell 改写成长期合理组成。

| 差距 | 当前实际 | 完成门槛 |
| --- | --- | --- |
| `workbench_sidecar_status_cutover` | product-entry / status / workbench projection shell、sidecar export/dispatch 与 owner-route handoff 仍承担 direct MAS path、OPL handoff 输入或 diagnostic read model；输出限于 domain projection refs、owner receipt、typed blocker、authority refs 或 diagnostic refs。 | OPL generated/default product/status/workbench/sidecar caller 接管默认路径；MAS 侧 wrapper / facade / alias 无 active caller 后直接删除或仅保留 history/provenance。 |
| `domain_ref_consumer_thinning` | `progress_projection`、`owner_route_dispatch_receipt`、`domain_authority_refs_index`、storage/artifact/memory locator 和 runtime storage maintenance 仍是 current domain-ref consumer；runtime storage maintenance live reports 已显式暴露 refs-only adapter boundary。 | 只保留必须由 MAS 生产的 body-free owner refs、receipt、typed blocker、authority boundary 和 medical helper；generic locator/index/projection shell 迁到 OPL primitive。 |
...
| owner-chain dispatch ledger scaleout | `domain_dispatch_evidence_record_payload` 已具备 OPL owner-payload group 所需 return shapes 与 refs-only payload path；本轮把 paper-line provider canary closeout 接到同一 payload builder：真实 MAS owner receipt 进入 success refs path，stable blocker 进入 typed-blocker path，二者都保留 no-forbidden-write proof 且不声明 domain ready。OPL 当前 `domain_dispatch_evidence_workorder_count=0` 只说明没有 open refs-only record route；仍需把更多真实 owner-route / guarded-apply / aftercare / default-executor / stage-level handoff 做成同轮闭环：先取得 MAS owner receipt 或 stable blocker，再经 OPL identity preflight record/verify；若 OPL 已关闭 typed-blocker receipt，不能事后升级为 success receipt。 |
...
- `docs/active/mas-ideal-state-gap-plan.md`、`docs/status.md`、核心五件套、`agent/` pack、MAS contracts、owner-route / sidecar / product-entry / progress projection source、focused tests，以及只影响 MAS owner receipt / typed blocker / body-free evidence path 的 runtime/controller surfaces。
...
- live CLI / read-model：`scripts/verify.sh`、focused pytest、sidecar export/dispatch、owner-route reconcile / dispatch、`study_progress`、OPL `agents interfaces --repo-dir <this-repo> --json`、OPL framework readiness / App drilldown / evidence-worklist outputs when the lane consumes OPL refs.
...
- 对 owner-chain dispatch ledger，下一轮应优先选择能先产生 MAS owner receipt 或 stable blocker 的 live sidecar/domain owner path，再把同一 identity 的 success/blocker payload 提交给 OPL refs-only ledger；旧 `DM003` guarded-apply 与 DM002 default-executor workorder 已按 typed-blocker path 关闭，不能事后升级为 success receipt。
...
- 满足 no-active-caller、OPL replacement parity、MAS owner receipt parity、focused tests 和 tombstone/provenance proof 后，直接删除旧 runtime/workbench/scheduler/alias/product/status/sidecar/controller/progress residue；需要来龙去脉只保留 history/provenance，不新增 compatibility shim 或 repo-local tombstone code path。
...
- 触及 source/contracts/runtime：`rtk scripts/verify.sh`；按触及面追加 focused `scripts/run-pytest-clean.sh ...`、`make test-meta` 或对应 owner-route / sidecar / product-entry smoke。
