...
| `real_paper_line_canary` | 从 proof/conformance 进入真实论文线 owner-chain | 选取一条真实 paper line，从 OPL provider attempt 进入 MAS sidecar dispatch，再由 MAS owner surface 产出 receipt、delta、gate replay、route decision、human gate、stop-loss 或 typed blocker；整个链路不写 OPL 不该写的 truth/body/package | `contracts/agent_lab_handoff.json` 已声明 `mas_production_evidence_suite` 中的 real-paper-line task，交由 `opl-meta-agent` materialize suite / developer work order、OPL Agent Lab run/evolve；canary 结果只能证明该链路可运行，不能直接写成 publication-ready、medical-ready 或 `current_package` authority |
