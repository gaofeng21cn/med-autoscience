...
| StageRun Kernel foundation | Stage Native 目录产出思路保留为 artifact/evidence plane；MAS `3eb66cbe` 已新增 `contracts/stage_run_kernel_profile.json`、`src/med_autoscience/controllers/stage_run_kernel.py` 和 `study_progress` StageRun projection binding，用最小 StageRun 状态壳表达 stage folder / manifest / receipt / blocker 派生状态；focused AI reviewer canary tests 已覆盖 owner receipt、typed blocker 与 provider terminal 不等于 domain accepted。MAS 只用 `OwnerReceipt` / `TypedBlocker` 关闭 Stage 和授权 route；`latest.json`、progress projection、Portal/workbench 只能是 receipt-derived read model。 |
...
| StageRun Kernel foundation | `profile_projection_canary_landed_real_owner_chain_evidence_open` | MAS `3eb66cbe`：`contracts/stage_run_kernel_profile.json`、`contracts/stage_artifact_kernel_adoption.json#/stage_run_kernel_profile_binding`、`src/med_autoscience/controllers/stage_run_kernel.py`、`src/med_autoscience/controllers/study_progress_parts/stage_kernel_projection.py`、`tests/stage_run_kernel_cases/test_ai_reviewer_stage_run_kernel.py`；OPL `631aeb6b` refs-only StageRun substrate | 已关闭 StageRun Kernel 的 MAS/OPL foundation：stage folder / manifest / role artifact / owner receipt / typed blocker 是当前 transition 口径；provider completion、file presence、`latest.json`、read-model 和 active_run_id 不能关闭 Stage。真实 DM002 / DM003 fresh receipt/blocker、补偿链退役和 real owner-chain evidence 仍开放。 |
...
| Co-Scientist-inspired Stage / Route 重构 | 执行规格已归入 [Co-Scientist Stage / Route 重构设计与执行规格](../runtime/designs/coscientist_stage_route_restructure.md)，当前只冻结目标、并行 lane、验收门和 forbidden authority boundary。 | 按规格把 Stage-native scientific work system 落到 `agent/` semantic pack、machine-readable contracts、route supervisor/read-model、focused tests、runtime/operator docs，并在真实或 synthetic owner-chain evidence 下证明不把 advisory ranking/proximity、platform repair 或 provider completion升级为 MAS authority。 |
| StageRun Kernel 收敛 | 顶层设计和 foundation landing 已归入 [MAS / OPL Stage Native 状态机设计](./mas-opl-stage-native-state-machine.md)。MAS `3eb66cbe` 已有 StageRun profile contract、adoption binding、stage folder projection helper、`study progress` projection binding 和 AI reviewer synthetic canary tests；OPL 主干已加入 refs-only StageRun substrate contract / primitive / focused test。OPL family active truth 仍把下一层归为 domain adoption canaries / real owner-chain evidence tail。 | 恢复 DM002 / DM003 前先保持 queue hold / human gate 事实；恢复后以 AI reviewer publication eval rebuild 为 live canary，产出 fresh MAS owner receipt 或 stable typed blocker，并证明 OPL StageRun status、MAS receipt/blocker、stage folder manifest 和 `study progress` 四者一致。通过 live canary 后，再退役同一 work unit 上的重复 reconcile/materialize/dispatch 补偿路径。 |
...
- Verification commands: docs-only 维护运行 `rtk git diff --check`、
  `rtk rg -n "^(<<<<<<<|=======|>>>>>>>)" docs README.md README.zh-CN.md` 和必要的
  docs/path inventory；触及 source/contracts/runtime 时运行 `rtk scripts/verify.sh`
