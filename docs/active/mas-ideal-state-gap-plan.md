...
| Domain dispatch evidence payload | `return_shape_landed_scaleout_pending` | `domain_dispatch_evidence_record_payload`、focused body-free evidence refs tests、sidecar owner-route/export dispatch tests、live guarded-apply sidecar receipt、OPL `domain_dispatch_evidence_workorder_count=0` | MAS payload 现在显式输出 `domain_owner_receipt_refs`、`owner_chain_refs`、`no_regression_evidence_refs`、`typed_blocker_refs`、success / typed-blocker payload path、required return shapes 和 legacy OPL action aliases；typed blocker ref 与 ledger hint 保留 stage/source identity token，避免同类 stage attempt 在 refs-only ledger 中碰撞；空 owner receipt refs 不代表 paper closure。provider-hosted guarded-apply 默认 target 只从当前 profile 的 canonical live study id 派生，并限于默认 canary target 的 canonical match，避免继续导出 retired shorthand identity；OPL refs-only ledger 已记录旧 `DM003` guarded-apply attempt 和 DM002 default-executor attempt 的 MAS-owned typed blocker / owner-chain / no-forbidden-write refs。后续真实 owner receipt 必须通过新的或 superseding owner-payload route 消费，不能回写改写已关闭的 typed-blocker receipt。 |
