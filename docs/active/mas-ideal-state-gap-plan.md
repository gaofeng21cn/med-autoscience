...
| MAS / OPL Agent OS target architecture | `docs_target_plan_landed_registry_runtime_abi_landed_evidence_tail_open` | [MAS / OPL Agent OS 目标运行架构与重构计划](../runtime/designs/mas_opl_agent_os_target_operating_architecture.md) 已把外部成熟工程经验转译成可落地 lane：OPL Agent OS、MAS Medical Research Pack、MAS Authority Kernel、Scientific Capability Registry、Pack Compiler、StageRun / State Index / Route Reconciler、evidence lineage、AI-first Quality OS、Agent Tool Arsenal、Workbench UX 和 production evidence soak。当前已关闭方向入口、Agent Tool Arsenal contract 和 Scientific Capability Registry repo runtime ABI；真实 hosted consumption / paper-line / provider / reviewer / human gate / artifact lifecycle soak 仍是证据尾项。 |
| Agent Tool Arsenal / Capability Invocation OS | `agent_tool_arsenal_and_registry_runtime_abi_landed_live_soak_tail_open` | `contracts/agent_tool_arsenal.json` 已把 action catalog / MCP registry / owner callable / plugin skill / stage refs / scientific capability registry 统一成 machine-readable agent invocation ABI，并由 MCP `agent_tool_arsenal` 只读工具暴露 index、card、plan 和 result envelope schema。`scientific_capability_registry` 已作为 action catalog、MCP runtime、CLI/product-entry 和 Agent Tool Arsenal card 落地，支持 `index / resolve / invoke`，把 external-learning sidecar、Light advisory、Evo sidecar、Co-Scientist affordance 和 Display Pack capability 作为 current-owner-delta-bound refs-only capability 消费。MAS 声明 current-delta applicability、authority boundary、allowed writes、forbidden authority、output refs、owner receipt / typed blocker / reviewer receipt 晋级门；剩余差距是 hosted OPL ordinary-path consumption、direct/hosted parity、current_owner_delta live soak 和 production evidence tail。该 landing 不新增 MAS 私有 selector、always-on sidecar、默认 preflight 或用户手动工具学习负担。 |
...
| `agent_tool_arsenal_generation` | `contracts/agent_tool_arsenal.json`、MCP `agent_tool_arsenal` 和 public runtime `scientific_capability_registry` 已把 action catalog / MCP registry / owner callable / plugin skill / stage refs / external scientific capability refs 收成 agent-facing tool cards、invocation plan、structured result envelope schema 和 current-delta-bound capability `index / resolve / invoke` ABI。 | 新 agent-facing 工具必须继续从 `current_owner_delta` 解析适用性，提供 risk annotations、input/output schema、allowed writes、forbidden authority、human gate 条件、idempotency/fingerprint policy、receipt/blocker shape 和 failure class；不得把人类 runbook 当作 agent tool-use ABI。后续尾项是 hosted OPL ordinary-path consumption 和 live soak，不是 MAS 私有 selector。 |
