...
- 本文是 MAS 当前唯一 single Active Truth plan：维护当前唯一真相、目标态、已落地状态、功能/结构差距、测试/证据差距、近期完善计划和历史索引。
...
1. 先核对 `contracts/functional_privatization_audit.json`、`contracts/production_acceptance/mas-production-acceptance.json`、`contracts/stage_control_plane.json`、product-entry manifest、sidecar export/read-model 和相关 tests，确认 retained adapter / diagnostic / direct handler target 的 active caller。
2. 优先处理 `workbench_sidecar_status_retirement` 和 `remaining_domain_ref_consumers`：对已满足 no-active-caller、OPL parity、MAS receipt parity、focused tests、no-forbidden-write proof 和 tombstone/provenance refs 的旧 wrapper、alias、facade、compat test 或文档入口直接删除、archive 或 tombstone。
3. 对 `runtime_transport/`、`mas_runtime_core*`、turn runner、worker lease、`lifecycle_refs_adapter.py` 和 legacy human-gate / display / submission-target alias 继续只做 no-resurrection guard；发现 current caller 或 current wording 复活旧控制面时，改回 OPL provider stage runtime、MAS owner receipt、typed blocker 或 tombstone/provenance。
4. 对仍需要保留的 sidecar、owner-route、progress、workbench/status、storage、artifact、memory 或 terminal-related refs 面，补清 active caller、authority boundary、body-free output、不能上收原因、receipt/blocker/ref 输出和删除门，不新增 compatibility shim。
5. 结构删除门关闭后，用真实 `real_paper_line_canary` 进入 evidence tail：成功标准只能是 MAS owner receipt、progress delta、gate replay、route decision、human gate、stop-loss、artifact/memory/lifecycle receipt 或 stable typed blocker。
6. canary 形成稳定 owner receipt 或 typed blocker 后，再扩展 publication-route memory、artifact lifecycle、human gate/resume 和 provider SLO long-soak。该阶段只关闭测试/证据差距，不回写成结构未完成或 publication-ready。

### 接续 `/goal` 摘要

继续治理 `/Users/gaofeng/workspace/med-autoscience` 的 single Active Truth plan。写入范围仅限 MAS 文档；先读 AGENTS、TASTE、核心五件套、本文、current development lines、关键 contracts/source/tests/read-model。优先核实并收口 `workbench_sidecar_status_retirement`、`remaining_domain_ref_consumers` 和 `physical_source_morphology_guard`。不要改代码、contracts、真实 study workspace artifact、paper body、`publication_eval/latest.json`、`controller_decisions/latest.json`、`current_package`、memory body 或 artifact body。验证至少跑 `git diff --check`；docs-only 变更按 documentation review 处理，若触及机器合同或 runtime 语义再追加 repo-native focused tests / `make test-meta` / `scripts/verify.sh`。
