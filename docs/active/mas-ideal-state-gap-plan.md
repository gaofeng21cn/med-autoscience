...
| Physical source morphology | `open_functional_structure_tail` | no-alias retired paths: `runtime_transport/`、`mas_runtime_core*`、turn runner、worker lease、`lifecycle_refs_adapter.py`；remaining current shells: workbench/status/owner-route handoff domain-ref surfaces | 旧 runtime / lifecycle writer 已按 no-alias physical retirement 关闭；剩余 active domain-ref consumers 尚未全部清零，删除仍必须等 OPL parity、MAS receipt parity、focused tests、no-forbidden-write proof 和 tombstone refs。 |
