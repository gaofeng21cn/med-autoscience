...
| H2 | 纯 MAS-owned `paper_mission` authority callable、strict input/output schemas、registry candidate | `done_candidate` | `src/med_autoscience/authority_handlers/paper_mission.py`、focused terminal/no-effect tests |
| H3 | OPL v2 callable binding、SHA-bound input、contained atomic output commit、refs-only ledger | `partial` | 等待 OPL hosted runtime ABI 与真实 callable smoke |
| H4 | 迁移默认 callers 并退役 v1 `source_command`、`domain_handler_export/dispatch`、`mainline_status/phase` 与 owner-surface queue/daemon shell | `partial` | H3 完成后做 source-closure、default-caller 与 no-behavior-loss cutover |

H2 只表示领域侧安全候选已落地。旧 transport 和医学行为在 H3/H4 关闭前继续保留，避免用尚未绑定的合同或测试替代可运行能力；候选不拥有 profile/path discovery、通用 I/O、queue/session/DAG、spawn、OPL/Codex 调用或 runtime ledger。
