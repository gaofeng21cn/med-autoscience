...
| Agent Tool Arsenal / Capability Invocation OS | `target_design_landed_implementation_open` | 当前审计确认 MAS 已有 action catalog、MCP registry、owner callable registry、plugin skill 和 pack compiler input，但它们还没有统一成给 autonomous agent 使用的 invocation ABI。目标态是 OPL 从 MAS pack / action / callable / MCP / stage refs 生成 `ToolArsenalIndex`、`ToolUseCard`、`CapabilityInvocationPlan`、`ToolResultEnvelope` 和 audit trail；MAS 只声明 current-delta applicability、authority boundary、allowed writes、forbidden authority、output refs、owner receipt / typed blocker / reviewer receipt 晋级门。该设计不新增 MAS 私有 selector、always-on sidecar、默认 preflight 或用户手动工具学习负担。 |
...
| `agent_tool_arsenal_generation` | 当前 MAS action catalog / MCP registry / owner callable / plugin skill 仍是多处 surface；目标是由 OPL generated surfaces 编译成 agent-facing tool cards 和 structured result envelope。 | 新 agent-facing 工具必须从 `current_owner_delta` 解析适用性，提供 risk annotations、input/output schema、allowed writes、forbidden authority、human gate 条件、idempotency/fingerprint policy、receipt/blocker shape 和 failure class；不得把人类 runbook 当作 agent tool-use ABI。 |
