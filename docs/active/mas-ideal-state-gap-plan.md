...
- [MAS / OPL Stage Native 状态机设计](./mas-opl-stage-native-state-machine.md) 是 Stage Native 后的下一层顶层设计：保留 stage folder artifact/evidence 面，把 Stage 状态收敛到最小 StageRun Kernel，把 Stage transition authority 收敛到 MAS owner receipt / typed blocker。它不新增 MAS controller system，不替代本文，也不声明实现已落地。
...
| StageRun Kernel target | Stage Native 目录产出思路保留为 artifact/evidence plane；下一层目标是用最小 `StageRun.spec/status/observed_generation` 表达 Stage 状态，MAS 只用 `OwnerReceipt` / `TypedBlocker` 关闭 Stage 和授权 route。`latest.json`、progress projection、Portal/workbench 只能是 receipt-derived read model。 |
...
| StageRun Kernel 收敛 | 顶层设计已归入 [MAS / OPL Stage Native 状态机设计](./mas-opl-stage-native-state-machine.md)，当前只冻结 `stage folder + manifest + role artifact + receipt/blocker` 的目标口径。 | 先以 DM002/DM003 AI reviewer publication eval rebuild 为 canary，证明 OPL StageRun status、MAS owner receipt / typed blocker、stage folder manifest 和 `study progress` 四者一致，且不需要人工反复 reconcile。 |
