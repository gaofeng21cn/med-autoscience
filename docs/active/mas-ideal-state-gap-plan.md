...
生成或托管通用 interface/runtime；MAS 只持有医学语义声明、专业能力依赖与三个
无法上收的最小纯 authority functions。
...
| 最小 authority function 边界 | `done` | 100% | closed registry、strict schemas、terminal/no-effect focused tests |
...
| Package/ScholarSkills 闭包 | `done` | 100% | MAS `0.2.12`、ScholarSkills exact lock `0.2.3`（range `>=0.2.0 <0.3.0`）、required capability ABI 与两个 package-owned Connect adapter modules |
...
- package init；
- `authority_handlers/` 下的 candidate admission、paper mission、self-evolution closeout 及共享纯校验 helper；
- CSL assets。
