...
| Progress Portal workspace carrier thinning | `carrier_submodule_split_delete_ready_pending` | `progress_portal_parts/workspace_carrier.py`、`mas_progress_portal_workspace_carrier_boundary`、focused Progress Portal materialization tests | Progress Portal 顶层只保留 MAS-owned payload / refs projection API；workspace 静态包、本地只读 HTTP carrier、study page materialization 和浏览器打开已抽到 carrier 子模块。该 carrier 只写 display/read-model refs，等待 OPL App 默认 carrier、workspace helper no-active-caller proof 和 focused tests 后删除；这不等于 `workbench_sidecar_status_cutover` 或 `domain_ref_consumer_thinning` 全部关闭。 |
