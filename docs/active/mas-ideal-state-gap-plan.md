...
| Physical source morphology | `open_functional_structure_tail` | no-alias retired paths: `runtime_transport/`、`mas_runtime_core*`、turn runner、worker lease、`lifecycle_refs_adapter.py`；remaining current shells: workbench/status/owner-route handoff domain-ref surfaces | 旧 runtime / lifecycle writer 已按 no-alias physical retirement 关闭；product-entry/status/workbench/owner-route operator command 投影已消费 public group-first CLI surface，并用 focused tests/search 防止旧 flat command recommendation 复活；剩余 active domain-ref consumers 尚未全部清零，删除仍必须等 OPL parity、MAS receipt parity、focused tests、no-forbidden-write proof 和 tombstone refs。 |
...
2. `domain_authority_refs_thinning`
   ...
   `opl_unique_control_plane_handoff` 进一步把历史 runtime transport 与 domain route 代码路径逐项约束为 OPL-owned generic runtime 的 domain bridge / diagnostic，不允许它们重新声明 MAS-owned queue、attempt ledger、worker residency、transition runner 或 persistence engine。
...
- `product_entry_parts/workspace_cockpit/`、product-entry manifest/status 与 runtime status projection 仍承担 direct MAS path、OPL handoff 输入或 diagnostic read model；当前这些 read-model / operator action command 已投影为 public group-first CLI 形式，旧 flat command id 只作为 grouped dispatcher 的内部 handler target 保留。generic sidecar provider lifecycle CLI 已退役，owner-route handoff active bridge 只剩 `owner_route_handoff` 的 export/dispatch owner-route。default caller 迁移后，只保留 MAS domain handler target、receipt signer、typed blocker 和 authority refs，其余删除、archive 或 tombstone。
...
| `product_entry_parts/workspace_cockpit/`、status/progress/workbench projections | direct MAS path / OPL handoff 输入仍可见；operator command 投影已统一到 group-first public CLI 形式，旧 flat command recommendation 由 focused tests/search 防复活。 | OPL generated product/status/workbench 成为 default caller 后，只保留 MAS truth refs；删除旧 cockpit/workbench shell 和只保护旧字段的测试。 |
