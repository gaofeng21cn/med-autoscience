...
| Area | 当前进度 | 当前读法 |
| --- | --- | --- |
| Standard OPL Agent source shape | `landed_with_evidence_tail` | `agent/` pack、stage/action/memory/artifact contracts、generated surface handoff 与 production acceptance 已成为 MAS 当前机器面；这不等于真实论文线 production ready。 |
...
以下项目是当前标准 OPL Agent 源码形态下仍需守住的长期边界。它们不是本轮未完成清理项；真实 evidence tail、provider completion、repo tests 或 refs-only ledger 消费结果也不能改变这些 owner 边界。

| 边界 | 当前实际 | 持续门槛 |
| --- | --- | --- |
| `workbench_sidecar_status_boundary` | product-entry / status / workbench projection shell、sidecar export/dispatch 与 owner-route handoff 只承担 direct MAS path、OPL handoff 输入或 diagnostic read model；输出限于 domain projection refs、owner receipt、typed blocker、authority refs 或 diagnostic refs。 | 不复制 OPL App/workbench owner，不承担 queue、attempt、retry/dead-letter、worker residency、terminal transport 或 lifecycle owner；若 OPL generated surface 接管默认 caller，MAS 侧 wrapper / facade / alias 直接删除、archive 或 tombstone。 |
| `domain_ref_consumers` | `progress_projection`、`owner_route_dispatch_receipt`、`domain_authority_refs_index`、storage/artifact/memory locator 和 runtime storage maintenance 仍是 current domain-ref consumer；runtime storage maintenance live reports 已显式暴露 refs-only adapter boundary。它们不是 MAS generic runtime owner。 | retained surface 只输出 body-free refs、locator、receipt、typed blocker 或 authority boundary；OPL primitive 消费 refs 后仍不能写 MAS truth。 |
...
- 满足 no-active-caller、OPL replacement parity、MAS owner receipt parity、focused tests 和 tombstone proof 后，直接删除、archive 或 tombstone 旧 runtime/workbench/scheduler/alias residue，不新增 compatibility shim。
...
- MAS repo 源码形态按 standard OPL Agent 收口：current docs、contracts、source 和 tests 只保护 declarative pack、OPL generated/hosted surfaces、minimal authority functions、domain authority refs、owner receipt、typed blocker 和 refs-only evidence。
...
- 仍未闭合的真实 paper-line provider apply、publication-route memory、artifact lifecycle、human gate/resume 和 provider SLO long-soak 保留在“当前测试/证据差距”，不回写成结构差距。
