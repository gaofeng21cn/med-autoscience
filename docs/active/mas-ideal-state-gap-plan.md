...
| Area | 当前进度 | 当前读法 |
| --- | --- | --- |
| Standard OPL Agent source shape | `strict_purity_cutover_pending` | `agent/` pack、stage/action/memory/artifact contracts、generated surface handoff 与 production acceptance 已成为 MAS 当前机器面；但 product/status/workbench/domain-handler/controller/progress shell 仍有 repo-local active caller，必须迁到 OPL generated/default caller 后删除。 |
...
| 差距 | 当前实际 | 完成门槛 |
| --- | --- | --- |
| `workbench_domain_handler_status_cutover` | product-entry / status / workbench projection shell、domain-handler export/dispatch 与 owner-route handoff 仍承担 direct MAS path、OPL handoff 输入或 diagnostic read model；输出限于 domain projection refs、owner receipt、typed blocker、authority refs 或 diagnostic refs。 | OPL generated/default product/status/workbench/domain-handler caller 接管默认路径；MAS 侧 wrapper / facade / alias 无 active caller 后直接删除或仅保留 history/provenance。 |
...
| 证据差距 | 需要看到的证据 |
| --- | --- |
| 真实 paper-line provider apply | OPL provider -> MAS domain-handler -> MAS owner chain 在真实论文线上留下 attempt query、typed closeout、MAS owner receipt 或 stable typed blocker，并暴露 progress delta、AI reviewer/gate receipt、artifact movement、human gate/resume、route decision、stop-loss 或 no-forbidden-write refs。 |
| owner-chain dispatch ledger scaleout | `domain_dispatch_evidence_record_payload` 已具备 OPL owner-payload group 所需 return shapes 与 refs-only payload path；真实 MAS owner receipt 进入 success refs path，stable blocker 进入 typed-blocker path，二者都保留 no-forbidden-write proof 且不声明 domain ready。本轮已把 open OPL record route 经 MAS-owned typed blocker path 记录并验证到 `domain_dispatch_evidence_workorder_count=0`。仍需把更多真实 owner-route / guarded-apply / aftercare / default-executor / stage-level handoff 做成同轮闭环：先取得 MAS owner receipt、stage receipt、monitor freshness receipt 或 stable blocker，再经 OPL identity preflight record/verify；若 OPL 已关闭 typed-blocker receipt，不能事后升级为 success receipt。 |
...
- 对 owner-chain dispatch ledger，下一轮应优先选择能先产生 MAS owner receipt 或 stable blocker 的 live domain-handler/domain owner path，再把同一 identity 的 success/blocker payload 提交给 OPL refs-only ledger；旧 `DM003` guarded-apply 与 DM002 default-executor workorder 已按 typed-blocker path 关闭，不能事后升级为 success receipt。
...
- 满足 no-active-caller、OPL replacement parity、MAS owner receipt parity、focused tests 和 tombstone/provenance proof 后，直接删除旧 runtime/workbench/scheduler/alias/product/status/domain-handler/controller/progress residue；需要来龙去脉只保留 history/provenance，不新增 compatibility shim 或 repo-local tombstone code path。
...
- MAS repo 目标源码形态按 standard OPL Agent 收口；当前仍有 active caller 的 product/status/workbench/domain-handler/controller/progress shell 是 strict source-purity tail，不能再写成结构完成态。
