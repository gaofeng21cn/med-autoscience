...
2026-05-15 live evidence 更新：MAS 已补齐 OPL-hosted guarded-apply 的 task source fingerprint 与 source-keyed dispatch receipt。刷新 OPL Temporal production proof 后，DM002 provider-hosted guarded-apply task 的 export fingerprint 为 `a22e142f0c6ceab0`，OPL Temporal attempt 为 `sat_f1059df1570a71ce93641555`，MAS receipt 为 `artifacts/runtime/opl_family_sidecar/dispatch_receipts/97137e3f890436a41a18.json`。该 receipt 仍返回 `typed_blocker` / `blocked_no_mas_owner_apply_receipt`，并证明 `writes_domain_truth=false`、`can_write_domain_truth=false`、no forbidden writes。它关闭的是 OPL/MAS thin adapter evidence-refresh gap，不关闭 paper-line publication / artifact authority。
...
| OPL-hosted paper-line apply | OPL provider proof 可被 MAS 读取；sidecar dispatch 能回到 MAS owner chain 并返回 source-keyed receipt / typed blocker；DM002 已证明 source fingerprint rehydrate 后生成 Temporal attempt `sat_f1059df1570a71ce93641555` 与 receipt `97137e3f890436a41a18.json` | 真实 provider-hosted paper-line guarded apply 还没有连续产出 MAS owner apply receipt、artifact delta、AI reviewer update、route decision 或 human gate/resume 证据；当前最新结果仍是 `blocked_no_mas_owner_apply_receipt` | 用 DM002、DM003、Obesity 等真实 paper line 跑 guarded apply；每条线返回 owner receipt、typed blocker、stop-loss 或 human gate，而不是把 provider completion 写成 paper closure。 |
...
3. `domain_transition_table_hardening`
   维护已落地的 MAS-owned domain transition table / read-model oracle，把更多真实 paper-line transition receipt 纳入 coverage。已纳入的真实回归包括：002 的 reviewer coverage closeout 完成态消费、003 的 AI reviewer provenance 优先级、Obesity 的 publication gate 真阻断与 stale owner_handoff 授权消费、以及单 study projection error fail-closed 隔离。OPL runner 可用后，只接入这份 MAS-owned spec，不让 OPL 重写医学判断。
