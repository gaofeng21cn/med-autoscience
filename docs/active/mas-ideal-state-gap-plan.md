...
| Domain dispatch evidence payload | `return_shape_landed_scaleout_pending` | `domain_dispatch_evidence_record_payload`、focused body-free evidence refs tests、sidecar owner-route/export dispatch tests、live guarded-apply sidecar receipt | MAS payload 现在显式输出 `domain_owner_receipt_refs`、`owner_chain_refs`、`no_regression_evidence_refs`、`typed_blocker_refs`、success / typed-blocker payload path、required return shapes 和 legacy OPL action aliases；typed blocker ref 与 ledger hint 保留 stage/source identity token，避免同类 stage attempt 在 refs-only ledger 中碰撞；空 owner receipt refs 不代表 paper closure。真实 guarded-apply sidecar dispatch 已能产出 MAS owner receipt refs；若 OPL refs-only record route 已先按 typed-blocker path verified，后续真实 owner receipt 不能事后改写该已关闭 receipt，必须通过新的或 superseding owner-payload route 消费。 |
...
- 对 owner-chain dispatch ledger，下一轮应优先选择能先产生 MAS owner receipt 或 stable blocker 的 live sidecar/domain owner path，再把同一 identity 的 success/blocker payload 提交给 OPL refs-only ledger；避免先用 placeholder-free typed-blocker path 关闭 route 后再发现真实 owner receipt 已存在。
