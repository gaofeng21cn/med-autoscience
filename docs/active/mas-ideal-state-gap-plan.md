...
- Required actions:
  ...
  2. 对 read-model/currentness/dispatcher/lifecycle apply 类问题，先判断是否只是
  ...
  3. 对 owner-chain dispatch ledger，只处理 fresh OPL worklist 中可绑定 MAS current
     ...
     production-ready。
  4. 对 publication-route memory receipt，优先取得真实 accepted/rejected/blocked
  ...
  5. 对 artifact lifecycle receipt，优先从 stable typed blocker refs scaleout 前进到
