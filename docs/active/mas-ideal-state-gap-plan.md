...
canonical id 是 `mas`。OPL 从 MAS pack、action catalog、schemas 与 handler registry
...
| Package/ScholarSkills 闭包 | `done` | 100% | MAS `0.2.1`、ScholarSkills `>=0.2.0 <0.3.0`、required capability ABI |
...
- packaged `resources/stage_route_contract.yaml`；
- CSL assets。

`agent/` 持有 rich declarative pack；`contracts/` 持有 machine inputs、schema 与
