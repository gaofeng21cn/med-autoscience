...
- [MAS / OPL Stage Native 状态机设计](./mas-opl-stage-native-state-machine.md) 是 Stage Native 后的下一层顶层设计与当前 foundation landing 记录：保留 stage folder artifact/evidence 面，把 Stage 状态收敛到最小 StageRun Kernel，把 Stage transition authority 收敛到 MAS owner receipt / typed blocker。它不新增 MAS controller system，不替代本文；MAS 主干已落地 StageRun profile / projection / focused synthetic canary foundation，并在 canonical path follow-through 中把 stage folder 文件名收敛到 `stage_manifest.json`、`receipts/owner_receipt.json`、`receipts/typed_blocker.json` 等当前口径，但不声明 DM002 / DM003 live paper line 已完成。
- [Co-Scientist Stage / Route 重构设计与执行规格](../runtime/designs/coscientist_stage_route_restructure.md) 是下一轮 `/goal` 的 runtime-facing 执行规格：先把 Stage-native scientific work system 的目标、并行 lane、验收门和吸收规则冻结，再按该规格落到 `agent/`、contracts、source、tests、runtime/docs。它不替代本文的 single Active Truth owner，也不表示任何实现已经完成。
...
| DM002 / DM003 anti-stall control loop | `paused_for_stage_run_platform_optimization` | `always resolve to next owner`、AI reviewer currentness consumption、owner-route/read-model/liveness 优先级、closeout ingestion、gate-clearing receipt consumption 和 stale provider-attempt invalidation 已进入当前口径；2026-06-05 两篇已按用户要求 queue hold，暂停前 next owner 为 `write/run_quality_repair_batch`，但 publication truth 仍是 `blocked/partial`。后续先优化 StageRun / receipt-blocker / currentness arbitration，不继续论文 tick。 |
...
| StageRun Kernel foundation | `profile_projection_controlled_canary_fixture_landed_real_owner_chain_evidence_open` | MAS `contracts/stage_run_kernel_profile.json`、`contracts/stage_run_canary_evidence.json`、`contracts/stage_artifact_kernel_adoption.json#/stage_run_kernel_profile_binding`、`src/med_autoscience/controllers/stage_run_kernel.py`、`src/med_autoscience/controllers/study_progress_parts/stage_kernel_projection.py`、`src/med_autoscience/controllers/stage_artifact_materializer.py`、`src/med_autoscience/controllers/stage_artifact_index.py`、`tests/stage_run_kernel_cases/test_ai_reviewer_stage_run_kernel.py`、`tests/test_stage_artifact_index.py`、`tests/test_stage_run_kernel_profile_contract.py`；OPL refs-only StageRun substrate / CLI / conformance 以 OPL repo live state 为准 | 已关闭 StageRun Kernel 的 MAS foundation：stage folder / manifest / role artifact / owner receipt / typed blocker 是当前 transition 口径；canonical stage folder 文件名已采用 `stage_manifest.json`、`owner_receipt.json`、`typed_blocker.json`、`consumed_artifact_refs.json`、`prov.json` 和 `current_owner_delta.json`；provider completion、file presence、`latest.json`、read-model 和 active_run_id 不能关闭 Stage。Co-Scientist candidate generation、grounded reflection、comparative selection、evolution / revision、meta-review 和 independent quality gate 已进入 controlled canary evidence fixture，全部是 refs-only / advisory boundary；旧 scheduler / runner / session store / status shell / workbench wrapper 只能是 migration input、diagnostic projection 或 provenance。真实 DM002 / DM003 fresh receipt/blocker、补偿链退役和 real owner-chain evidence 仍开放。 |
...
| Co-Scientist-inspired Stage / Route 重构 | 执行规格已归入 [Co-Scientist Stage / Route 重构设计与执行规格](../runtime/designs/coscientist_stage_route_restructure.md)；StageRun profile 已补充 canary 级机器边界：candidate generation、reflection、review、meta-review、ranking、proximity、evolution 只能作为 Stage 内 advisory evidence refs，不能定义硬编码 workflow、关闭 quality gate 或 promote stage。 | 按规格把 Stage-native scientific work system 继续落到 `agent/` semantic pack、machine-readable contracts、route supervisor/read-model、focused tests、runtime/operator docs，并在真实或 synthetic owner-chain evidence 下证明不把 advisory ranking/proximity、platform repair、provider completion 或旧 wrapper/status shell 投影升级为 MAS authority。 |
| StageRun Kernel 收敛 | 顶层设计和 foundation landing 已归入 [MAS / OPL Stage Native 状态机设计](./mas-opl-stage-native-state-machine.md)。MAS 已有 StageRun profile contract、controlled canary evidence fixture、adoption binding、stage folder projection helper、`study progress` projection binding、AI reviewer synthetic canary tests 和 canonical stage folder path follow-through；OPL 侧 StageRun substrate / CLI / conformance 以 OPL repo live state 为准。OPL family active truth 仍把下一层归为 domain adoption canaries / real owner-chain evidence tail。 | 恢复 DM002 / DM003 前先保持 queue hold / human gate 事实；恢复后以 AI reviewer publication eval rebuild 为 live canary，产出 fresh MAS owner receipt 或 stable typed blocker，并证明 OPL StageRun status、MAS receipt/blocker、stage folder manifest 和 `study progress` 四者一致。通过 live canary 后，再退役同一 work unit 上的重复 reconcile/materialize/dispatch 补偿路径。 |
| DM002 / DM003 paused paper line | 2026-06-05 已执行 OPL queue hold，当前无 running provider attempt；dated 证据见 [DM002 / DM003 paper-line pause closeout 2026-06-05](../history/program/dm002_dm003_paper_line_pause_closeout_2026_06_05.md)。 | 恢复前必须先 release hold / human approval，并通过 StageRun 或 current owner-route surface 重新生成当前 work unit；不得用全局 tick 或手写 workspace truth 恢复。 |
...
- Required actions:
  1. 优先扩面真实 paper-line owner delta：至少一条线产出 paper/manuscript delta、
     artifact/package delta、independent reviewer/auditor record、publication-route
     memory receipt、artifact lifecycle receipt、human gate/resume receipt、provider
     long-soak owner evidence、family transition live receipt 或 stable typed blocker。
  2. 维护 Co-Scientist-inspired Stage / Route landed surface：`agent/` Stage-native
     ...
     runtime docs 已作为当前 Stage/Route 形态输入；后续只扩面真实 owner-chain
     evidence，不能把 advisory ranking/proximity、platform repair 或 provider
     completion 升级为 MAS authority。
