...
| External-learning adoption closure | `machine_policy_runbook_skill_landed_not_external_runtime` | `contracts/progress_first_safety_envelope.json#/external_learning_adoption_closure_policy`、[External Learning Adoption Closure Runbook](../runtime/control/external_learning_adoption_closure.md)、`docs/decisions.md`、`docs/status.md`、reference intake 边界和 `$mas-contract-first-external-intake` skill 已固定：外部自动研究框架的 `adopt_contract`、reference intake、design doc、score、ranking、checklist 或 skill inventory 不能写成 functional landed；必须标注 `owner_surface_landed`、`read_model_landed`、`sidecar_or_worker_landed`、`contract_projection_landed` 或 `contract_only_gap` / `projection_only_gap` / `history_only_gap` / `not_landed_gap`。该面只约束 intake 口径和 implementation scaleout，不引入外部 runtime，也不让缺外部 advisory 阻断当前可执行 owner action。 |
