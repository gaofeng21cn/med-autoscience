...
| `legacy_physical_cleanup` | `functional_follow_through_gate` | 用 legacy audit 和 tombstone contract 指导旧 active-path 删除或归档。当前 workspace-local service wrapper 切片执行删除优先口径：不再生成旧 wrapper，`init-workspace` 删除旧生成物，测试验证删除和 canonical runtime CLI。 | stale scan、no default caller proof、replacement proof、无 fixture/provenance dependency、focused cleanup tests。 |
...
| `P2.4` | `legacy_retirement_after_replacement` | MAS 已提供 no-active-default-caller tombstone proof 和 physical tombstone contract；有替代证据后，继续删除 scheduler/Hermes/MDS/legacy manager/UI wording 与代码，只有 archive/provenance/parity 必需 reader 保留。当前 active contract 已把 Hermes 表述收窄为 explicit optional executor adapter，旧 manager 表述只保留为 retired cleanup evidence。 | retired path evidence、tombstone contract、删除旧入口后的 scaffold/init tests 和更新后的 diagnostics/fallback docs |
