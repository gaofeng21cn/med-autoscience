...
| `progress_first_stage_throughput` | [Progress-first Stage 推进吞吐审计](./progress_first_stage_throughput_audit.md) + [Stage / Route / Handoff 标准](../runtime/stage_route_handoff_standard.md) | `throughput_read_model_gate` | `read_model_landed_real_delta_tail_pending` | 维护 6 个 runtime-guard Stage、10 个 domain route、minimum forward delta、next forced delta、owner pickup、closeout semantics、no-op budget 和 telemetry 完整性的系统排查入口；ready owner action target specificity、pickup latency 排序、terminal closeout 语义、route knowledge/memory obligations 已进入可排序、可验证 read-model。 | 每个 Stage attempt 必须留下 deliverable delta、typed blocker、human gate 或 stop-loss；平台修复不计 deliverable progress；generic target / unconsumed handoff / closeout semantic gap 现在可见；真实 paper-line owner receipt / stable blocker / no-forbidden-write proof 仍是 production evidence tail。 |
