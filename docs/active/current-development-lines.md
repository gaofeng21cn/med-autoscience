...
| `state_contract_thinning` | [MAS 状态与 Contract 收薄计划](./mas-state-contract-thinning-plan.md) + [MAS 理想目标态差距与完善计划](./mas-ideal-state-gap-plan.md) | `active_control_plane_thinning_lane` | `active_plan_landed_followthrough_in_progress` | 把稳定控制面收敛为 `macro_state + owner_route + receipt_or_blocker + evidence_refs`；长 runtime/status reason、supersession reason、publication supervisor phase 和 operator/workbench 文案只保留为 diagnostic/read-model detail。 | macro-state focused tests、runtime contract docs、owner-route consumer focused tests 证明执行授权只来自 current owner ticket、receipt/blocker 和 evidence refs，不来自 projection-local long reason。 |
