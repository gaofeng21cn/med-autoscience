...
| `publication_route_memory_management` | [Markdown-first Publication Strategy Memory Policy](../policies/study-workflow/publication_route_memory_policy.md) + [Study Workflow](../policies/study-workflow/README.md) | `functional_follow_through_gate` | `markdown_strategy_memory_prompt_and_seed_landed_receipt_scaleout_pending` | 保持 Markdown-first strategy body、stage packet reference-only prompt block、workspace init seed apply、body-free descriptor、router/writeback refs 和 maintainer review discipline；`publication_route_memory` 只作为兼容实现族名；OPL 只消费 locator/index/projection/receipt，不读取正文、不裁决或控制路线。 | 多 paper-line `stage-memory-closeout-route -> memory_write_router_receipt -> inventory/export` proof，覆盖 accepted、rejected、route-back lessons、grouping 和 review summary；proof 只证明 refs/receipt 链，不把 memory 升级为 evidence gate、route scorer 或 controller decision source。 |
