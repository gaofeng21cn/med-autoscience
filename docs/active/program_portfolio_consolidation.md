...
| --- | --- | --- | --- |
| [MAS 理想目标态差距与完善计划](./mas-ideal-state-gap-plan.md) | 唯一当前 truth、目标态、已落地状态、功能/结构差距、测试/证据差距、完善顺序、历史索引和禁止误写口径。 | `active_plan` | 不保存 dated closeout、receipt 流水、旧 phase checklist。 |
...
| [Domain Authority Refs Index Guard](./runtime_lifecycle_sqlite_migration_program.md) | `domain_authority_refs_index`、retired-runtime SQLite/file provenance、quest/root Git retirement、restore proof 和 stale runtime-control drift guard。 | `landed_foundation_guard` | 不作为 broad runtime refactor board，不授权 study/publication/artifact truth，不恢复 MAS-owned runtime lifecycle/read-model/scheduler。 |
