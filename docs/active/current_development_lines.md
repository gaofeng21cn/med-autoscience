...
| `domain_transition_table_hardening` | [MAS 理想目标态](../references/positioning/mas_ideal_state.md) + OPL master docs | `functional_follow_through_gate` | `domain_transition_read_model_landed; opl_runner_integration_pending` | 继续把更多真实 paper-line owner surfaces 纳入 MAS-owned domain transition table / oracle fixtures，并接入 OPL generic runner / matrix runner。 | `study-state-matrix` 已投影 JSON/Markdown `domain_transition_table`，覆盖 publication gate blocker、AI reviewer re-eval、artifact delta guarded apply、human gate、stop-loss、truth conflict/fail-closed、active runtime watch、delivered package handoff 和 unclassified fail-closed；OPL runner 只执行 MAS spec，不产生 publication-ready verdict。 |
