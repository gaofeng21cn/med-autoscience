...
2026-05-14 transition table follow-through：`study-state-matrix` 现在投影 `domain_transition_table`，作为 MAS-owned domain transition spec/read model/oracle。该 projection 将 study runtime status、macro state、publication eval、controller decision 与 artifact delta evidence 的常见组合映射到 `decision_type`、`route_target`、`next_work_unit`、`controller_action`、`owner`、typed blocker 和 guard boundary；它只读 repo/workspace durable refs，不写 live workspace、runtime SQLite、current_package、publication_eval 或 controller_decisions，也不实现通用 state-machine runner。OPL 仍持有通用 runner / attempt / queue；MAS 持有 domain route、quality gate、artifact authority 与 fail-closed 判断。
...
| `stage_surface_standardization` | [MAS Stage Surface Standardization Program](./stage_surface_standardization_program.md) | `landed_foundation_with_live_apply_gate` | `functional_projection_landed; transition_read_model_landed; stage_surfaces_landed_live_apply_followthrough_pending` | 维护 skill-change guard、live review/index follow-through、standard skeleton slot discipline；新增 stage/prompt/skill 必须继续消费 machine-derived refs。 | `mas_functional_closure_status_projection` 暴露 standard skeleton / stage surfaces 与 live provider owner-chain typed blocker；`study-state-matrix` 暴露 MAS-owned `domain_transition_table`，把 publication gate、AI reviewer、artifact delta、human gate、stop-loss、active runtime、delivered package 和 fail-closed 相邻状态投影成 owner/action/guard read model；live paper apply 前不宣称 production closure。 |
| `p1_app_runtime_workbench` | [OPL App MAS Runtime Workbench Program](./opl_app_mas_runtime_workbench_program.md) | `functional_follow_through_gate` | `functional_projection_landed; transition_read_model_landed; opl_app_drilldown_pending` | OPL App 读 MAS workbench projection、stage review/index、memory refs、provider refs、transition table、安全 action receipt 和 terminal attach gate。 | `mas_functional_closure_status_projection` 暴露 workbench reference projection 与 App drilldown typed blocker；`domain_transition_table` 给 App/Workbench 提供 per-study owner、next work unit、typed blocker 和 guard boundary；action 只返回 MAS typed receipt。 |
...
OPL 层面的主参考是 `one-person-lab/docs/active/opl-family-development-reference.zh-CN.md`；production functional closure umbrella plan 是 `one-person-lab/docs/active/production-functional-closure-plan.zh-CN.md`。MAS 这里不再维护一份平行的 production-functional closure 大计划；MAS 的下一批可立即工程落地工作应作为该 umbrella plan 的 MAS implementation lane 并行推进。
