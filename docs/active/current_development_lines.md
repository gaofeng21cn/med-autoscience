...
| `domain_transition_table_hardening` | [MAS 理想目标态](../references/positioning/mas_ideal_state.zh-CN.md) + OPL master docs | `functional_follow_through_gate` | `domain_spec_pending_opl_runner_integration_pending` | 把 publication gate、bundle-stage、AI reviewer、claim/evidence/display blocker、task intake、controller authorization、owner priority 和 runtime liveness 收口成 MAS-owned domain transition table / oracle fixtures，并接入 OPL generic runner / matrix runner。 | matrix tests 证明相邻状态不会误转；OPL runner 只执行 MAS spec，不产生 publication-ready verdict。 |
