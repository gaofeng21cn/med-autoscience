...
| 落点 | 当前职责 |
| --- | --- |
| [MAS 理想目标态差距与完善计划](./mas-ideal-state-gap-plan.md) | 持有当前唯一 active truth、剩余 production evidence tail 和禁止误写口径。 |
| [当前状态](../status.md) | 摘要化当前控制面口径，说明 consumer/dispatch/product-entry/OPL handoff 的授权来源。 |
| [MAS 当前开发线路](./current-development-lines.md) | 把 `state_contract_thinning` 归为 folded guard，不维护第二 backlog。 |
...
- 不记录真实 paper-line、memory/artifact/lifecycle、human gate/resume、provider SLO long-soak 或 OPL worklist 计数；这些只保留在 [MAS 理想目标态差距与完善计划](./mas-ideal-state-gap-plan.md) 的 production evidence tail。
