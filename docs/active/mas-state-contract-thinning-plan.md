...
| `consumer_followthrough` | owner-route、study-progress、product-entry、domain-handler 或 default-executor focused tests | 后续 lane 只消费 `macro_state + owner_route` 决定执行，不再依赖 projection-local long reason。 |

## 当前可一次落地的范围

本轮先完成前三条 lane，并把第四条登记为后续 focused code follow-through。这样能立即降低新增状态的扩散风险，同时不在同一轮大范围改动 owner-route、domain-handler、product-entry 和 OPL handoff 的活跃控制面。
...
- 补 focused tests，证明长 reason 不提升为宏观控制状态。
...
- 不大规模删除 reason enum、typed blocker reason 或 owner-route reason；这些需要 consumer follow-through 和兼容迁移证据。
...
- Focused verification 通过：docs lint / conflict marker check，macro-state focused pytest，必要时 runtime docs diff check。
- 并行 worktree 的改动已吸收回 `main`，临时 worktree 和分支已清理；未吸收项必须明确保留原因。

## 后续 direct implementation gate

下一轮代码收薄优先处理 consumer follow-through：

1. `study_progress`、product-entry、domain-handler export 和 default-executor dispatch 读取同一个 materialized/shadow macro state 与 owner route。
