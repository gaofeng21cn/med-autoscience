...
| Executor-first 目标架构 | [MAS executor-first 重构目标与迁移计划](./mas_executor_first_rearchitecture_program.md) | 当前 MAS 论文推进效率问题的目标态设计：把默认路径改成 artifact-first paper mission loop，MAS 收薄为医学 authority kernel，OPL 持有 runtime/session，旧 DHD / owner-route / dispatch / recovery 链只作为迁移输入，先立后破后退役。 |
