...
- OPL hosted projection：MAS-local `stage_run_kernel.py` 与 `stage_artifact_index` 通用 ref/text helper 已物理退役。MAS 只通过 `contracts/stage_run_kernel_profile.json`、stage pack、owner receipt / typed blocker validator 和最小 authority function 声明 domain semantics；通用 StageRun / artifact status、index、current pointer 与 Workbench projection 由 OPL Stagecraft / Workspace 从 stage folder、manifest 和 receipt refs 重建。
...
- MAS legacy taxonomy declaration：`contracts/mas-paper-study-stage-pack.json#/legacy_taxonomy_migration` 继续持有 domain-owned legacy stage id 到 stage-native stage id 的 mapping；它只作为 OPL generated/hosted projection 的 declarative input，不再由 MAS 私有 index/projector 解释或写回 current pointer、owner receipt、study truth、paper、publication eval 或 controller decision。
- MAS controlled canary：`contracts/stage_run_canary_evidence.json` 与 `tests/test_stage_run_kernel_profile_contract.py` 只验证 domain profile、controlled fixture、owner receipt / typed blocker refs 和 overclaim boundary；通用 StageRun reducer/projector 的行为回归归 OPL Framework，不在 MAS 复制第二套 implementation canary。
...
- MAS readiness follow-up：DM002 / DM003 live workspace 当前已产出 `medical_paper_readiness_not_ready` typed blocker，`projection/current_owner_delta.json` 指向 `MedAutoScience / complete_medical_paper_readiness_surface`。该 owner surface 只消费 paper-mission artifact/diagnostic、StageOutcome、owner receipt、legal typed blocker 与 currentness evidence；能力面由当前 owner evidence 明示，不从 `NextActionEnvelope`、action queue 或程序 selector 推导。普通 payload / surface 缺口形成质量债并允许 Codex 继续，只有真实 authority/safety/permission/identity/currentness、不可逆动作或显式 human decision 才写 stable hard blocker。2026-06-07 follow-through 已补齐 `literature_provider_runtime`、`study_line_selection`、`archetype_analysis_contract`、`bounded_analysis_candidate_board` 和 `stop_loss_memo` 五类 owner-authored payload / materializer；这些路径只写 readiness canonical surface、action result、readiness projection 和 owner blocker，不写 paper / package / publication truth。DM002 / DM003 的具体 readiness、active attempt 和 owner evidence 属于 live study truth，必须从 fresh `paper-mission inspect`、`study_progress`、workspace artifact、OPL current-control、owner receipt 和 typed blocker refs 读取；本文只保留 owner surface 规则和误用边界。`study progress` 仅可把旧 `current_executable_owner_action` 投影为 refs-only legacy diagnostic / provenance。
- MAS current execution diagnostic read-model：legacy `current_execution_envelope` 只保留为 diagnostic / operator drilldown，不能再作为默认执行 authority 或 next-work-unit producer。默认执行状态与 running proof 只能来自 nonbinding Codex route context 绑定的 OPL StageAttemptReceipt / StageRun live readback，并且必须与同一 StageOutcome / work-unit identity 匹配；parked / executable projection、action queue、runtime-health hint 或旧 envelope residue 只能解释历史状态，不能生成下一步、provider admission 或论文进展。
...
`outputs/` 是 artifact/progress evidence，`receipts/` 是强 claim 与 authority evidence，`projection/` 是人机读面。Codex CLI 是唯一 transition authority；没有 receipt 只降低 lineage/claim 质量，raw/partial/negative output 或 no-output diagnostic 仍可进入下一 declared Stage。
...
```text
any prior-stage artifact / diagnostic -> Codex-selected declared stage
  -> optional owner projection/current_owner_delta drilldown
  -> current medical stage prompt and goal
  -> executor produces concrete paper/evidence/reviewer/gate delta
  -> MAS records artifact, ProgressDeltaReceipt, OwnerReceipt, legal TypedBlocker or no-output/failure diagnostic
  -> Codex advances, skips, repeats, reverses or routes back
```
...
这些 sidecar 可以提示 hard-gate candidate，但真正的 hard gate 只允许来自 executor unavailable、MAS owner 确认的安全/权限/authority、wrong-target identity/currentness、不可逆动作或显式 human decision；OPL 不拥有语义 transition authority。
...
7. 默认 operator API 读取 Codex-selected stage、artifact/diagnostic 与可选 owner summary；legacy `NextActionEnvelope` 和 `current_owner_delta` 只作 nonbinding route context / audit drilldown，不能成为 planning root、dispatch selector 或 blocker。
...
2. Stage 语义 route 只有一个 owner：Codex CLI。receipt、blocker、`latest.json` 和 read model 都只是输入；raw/partial/negative/diagnostic 也可被 Codex 带入任一 declared Stage。
