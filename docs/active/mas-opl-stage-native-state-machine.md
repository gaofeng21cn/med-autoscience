...
- MAS projection：`src/med_autoscience/controllers/stage_run_kernel.py` 从 stage folder / manifest / receipt / blocker 派生 refs-only StageRun projection；`study_progress/stage_kernel_projection.py` 在 physical stage folder 存在时优先采用 manifest-backed `current_owner_delta`。
