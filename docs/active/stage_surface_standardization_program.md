...
Status: `stage prompt surfaces landed / ideal-state proof surfaces landed / route-memory grouping landed / provider live apply pending`
...
| `stage prompt / policy` | `agent/stages/` + `agent/prompts/` 是标准 OPL Agent stage 主提示词与策略的 canonical source；Codex `SKILL.md`、overlay template 或 `.codex/skills/medical-research-*` 只能是运行时发现/兼容投影。 |
...
| `agent/skills/` | MAS app skill / generated projection refs | stage skill policy / app-skill refs；不承载专业 specialist skill 正文。 |
...
本表的 `independent skill` 指 MAS stage 主提示词/策略的独立人读面，canonical repo source 仍是 `agent/stages/` + `agent/prompts/`；Codex `SKILL.md` 和 `.codex/skills/medical-research-*` 是投影，不是 professional specialist skill 源头。
