...
Status: `stage prompt surfaces landed / stage runtime projection boundary landed / ideal-state proof surfaces landed / route-memory grouping landed / provider live apply pending`
...
State: `stage_skill_surfaces_landed; stage_runtime_projection_boundary_landed; provider_residency_read_model_landed; guarded_apply_harness_landed; review_index_workspace_locator_proof_landed; memory_receipt_inventory_grouping_review_landed; workbench_ref_projection_landed; runtime_owner_followthrough_landed; opl_production_proof_ingested; standard_agent_purity_projection_landed; production_provider_live_apply_pending`
Machine boundary: 本文是人读规划与维护合同。机器真相继续归 canonical route / contract / runtime surfaces：`stage_route_contract.yaml`、`research_memory_contract.py`、stage quality contract、MAS controller/domain-authority refs surfaces、OPL current-control-state、product-entry manifest、domain-handler receipts、AI reviewer artifacts、publication gate、evidence/review ledgers 和真实 workspace artifact proof；Markdown 只解释和导航，不成为机器 truth。
...
2026-07-03 之后，本文中旧有 `skill surface` / `independent skill` 只能按 MAS stage 主提示词 / policy 的 Codex runtime projection 读取，不表示标准 OPL Agent 需要长期 `stage-bound skill` 概念。Stage source 留在 MAS 的 `agent/stages/`、`agent/prompts/` 与 route / knowledge / quality contracts；MAS-local overlay template 已物理退役。专业 specialist skill 由 owning domain pack 单源维护；当前医学论文 active module 的专业 Skill 由外部 `mas-scholar-skills` 维护，并通过 OPL Connect 同步到 workspace / quest 本地 `.codex/skills` 后供 Codex 调用；`medical-figure-style` / `medical-figure-composer` 是 Display module 子 Skill，不新增 active module。
...
2026-06-03 更新：MAS 已在 `stage_artifact_index` 接入 OPL Stage-Native Artifact Contract 的 machine-readable 派生投影。每个 stage state 现在显式暴露 `stage_folder_contract`、`manifest_requirements`、`receipt_requirements` 与 `artifact_classification`，并按 `current`、`historical`、`missing_manifest_or_receipt`、`orphan`、`broken`、`missing` fail-closed 分类。2026-06-03 physical kernel follow-through：`contracts/mas-paper-study-stage-pack.json` 现在声明 MAS 对 OPL physical Stage Folder Kernel 的 locator：`domain_id=med-autoscience`、`program_id=mas-paper-study`、`topic_id={study_id}`、`deliverable_id=paper-study`；`stage_artifact_index` 会在 `OPL_STATE_DIR/runtime-state/domains/med-autoscience/deliverables/mas-paper-study/<study_id>/paper-study` 下消费 OPL physical `current.json`、stage `latest` pointer、attempt `manifest.json`、receipt file、manifest hash refs、lineage/conformance refs。2026-06-03 next-layer completion：physical folder 只有在 current pointer 指向 latest attempt、semantic owner / decision receipt 成立、consumability gate 全项通过、lineage events / graph 可定位、retention / restore refs 覆盖时才算 current artifact progress；stale pointer、缺 domain receipt、缺 restore contract、缺 lineage 或缺 hash 都 fail closed。`stage_artifact_index` 仍是 derived refs-only index，不是 artifact truth、study truth、quality verdict 或 publication readiness authority；legacy declared refs 只保留为 migration / historical projection，缺 OPL physical manifest、receipt、hash 或 pointer 时不能算 current artifact delta。

2026-07-11 更新：MAS Stage-Native 继续保留 semantic receipt validation、current pointer promotion audit、lineage/retention drilldown 与 cross-domain soak 只读摘要；本地 StateIndex rows projection 已物理删除。stage artifact refs 由 `med_autoscience.opl_domain_pack.state_index_source_refs` 以 body-free metadata 发给 OPL StateIndex owner，`stage_kernel_projection` 不再生成第二套 StateIndex read model。保留的 operating-layer helper 不写 MAS truth、paper body、publication eval、controller decisions、memory body、artifact body、submission package 或 current pointer，只投影 fail-closed reason、typed blocker candidate、next owner / repair diagnostic 或 Workbench drilldown。
...
| `stage prompt / policy` | `agent/stages/` + `agent/prompts/` 是标准 OPL Agent stage 主提示词与策略的 canonical source；不新增长期 `stage-bound skill` 概念。Codex carrier 和 OPL Connect materialized skill 只服务运行时发现，不成为 MAS stage source。 |
...
| --- | --- | --- |
| `agent/stages/` | MAS Stage-Led Autonomy | stage policies、route summaries、stage-to-route map。 |
| `agent/prompts/` | MAS stage prompt canonical source | executor-neutral stage 主提示词、stage operating strategy 和 handoff prompt。 |
| `agent/skills/` | MAS app skill / generated projection refs | stage runtime projection / app-skill refs；不承载专业 specialist skill 正文，也不作为 stage source of truth。 |
...
| `contracts/runtime/domain-handler/` | Runtime OS + OPL bridge | domain-handler export/dispatch、guarded task receipt、forbidden-write proof。 |
| `runtime/artifact_locator/` | Artifact OS | workspace/runtime artifact root locator refs，不保存 artifact body。 |

短期不需要一次性物理搬目录；当前更重要的是先统一概念、字段和入口，再按低风险方式逐步迁移。`src/med_autoscience/overlay/templates/*.SKILL.md` 已物理退役；`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep` 和 `medical-data-governance` 属于外部 `mas-scholar-skills` 专业增强包，不是 MAS stage 主提示词源头。调用链只闭合到 `OPL Connect 同步 -> workspace/quest .codex/skills -> Codex discovery -> specialist candidate refs -> MAS owner gate`，不新增 MAS loader / registry，也不复制外部专业 Skill 正文。
...
- `machine source`：`agent/stages/stage_route_contract.yaml`、`agent/stages/*.policy.md`、`agent/prompts/*.md`、stage knowledge / quality contracts、generated `docs/runtime/contracts/stage_surfaces.md`、product-entry / OPL descriptor 和 standard skeleton anchors 持有可验证 stage shape；Markdown 只解释这些 surfaces。
- `human stage surface`：主 stage 已有 canonical stage prompt / policy 人读面，新增或修改 stage/prompt/skill 投影时必须继续消费 machine-derived route、knowledge、quality、tool、closeout 和 OPL boundary refs。
...
| --- | --- | --- |
| Production provider-hosted live apply 仍未闭合 | 只能说 stage prompt / projection surfaces landed、descriptor / adapter ready、provider projection 与 typed blocker proof landed；不能说 production-hosted paper automation 已完成。 | 真实 provider attempt 在 paper line 上留下 `attempt query -> typed closeout -> MAS owner receipt -> artifact delta / gate replay / human gate / stop-loss / typed blocker`。 |
| OPL production provider residency proof 已落地，MAS managed-state read model 已收口，真实 domain activity soak 仍未收口 | 当前 OPL production proof 已显示 `production_residency_proven`，MAS product-entry / domain-handler ingestion 可把 provider availability 切到 available，并暴露 managed service / worker state consistency projection；剩余风险是真实 MAS domain activity 长时运行证据，以及 OPL App/status 面消费同一 projection。 | OPL `family-runtime status --provider temporal` 与 MAS `managed_temporal_state_consistency` 同口径显示；真实 domain activity attempt 能 re-query、restart、retry/dead-letter，并串到 MAS domain-handler dispatch receipt。 |
| Stage prompt、policy、quality rubric 仍需要持续守住 owner boundary | 新增或修改 stage prompt / policy / Codex 投影时必须继续消费 stage card、canonical route contract、stage knowledge obligations、quality pack refs、RH clean-room gates 和 MAS owner closeout packet，不能回退为 Markdown-only 规则。 | agent-stage / agent-entry asset validation 加上 focused route/knowledge/quality contract tests；新增 Codex runtime projection 必须带 machine-derived surface block。 |
| AI-first verdict boundary 已落地，真实 reviewer/auditor receipt scaleout 仍需扩展 | `pack_compiler_input`、stage quality contract、product-entry stage descriptor 与 focused tests 已把 verdict surface 固定为独立 reviewer/auditor agent 的 AI-first stage gate；程序只做 validator / materializer / receipt signer / guard，不直接输出 ready/pass。 | 更多真实 paper-line verdict surface 能追到独立 reviewer/auditor invocation、task/context record、reviewer / quality-pack record、evidence refs、reviewer OS trace、route-back 或 typed blocker；继续防止 self-review、mechanical projection、regex、固定分支或普通脚本替代 quality judgment。 |
...
| 旧 MDS / Hermes / local scheduler residue 已归入 history/provenance | current product-entry / domain-handler / read-model 默认只暴露 `standard_agent_purity`、domain refs、owner receipts 和 typed blockers；旧 residue/compat/standard Agent purity 细节不再作为 active/default surface。legacy tombstone contract 只在明确 legacy 引用、旧状态输入降级测试或 provenance 语境中读取。 | generated/hosted parity + MAS receipt parity + focused boundary tests + no-forbidden-write proof。 |
...
| --- | --- | --- | --- |
| `skill_change_guard` | `landed_foundation` | `planned; machine_derived_surface_block_landed` | 新增或修改 stage prompt / Codex projection 时继续消费 generated stage card、route contract、knowledge obligations、quality pack refs、RH clean-room gates 和 MAS closeout refs。 |
...
| `ai_first_verdict_alignment` | `landed_foundation` | `contract_validator_landed; receipt_scaleout_pending` | `publication_quality_verdict`、`ai_reviewer_quality_decision`、`source_readiness_verdict` 等已在 contract / validator / focused tests 中绑定到独立 reviewer/auditor agent 的 AI-first stage quality gate；真实 paper-line 仍需继续产生 reviewer/auditor receipt、route-back 或 typed blocker。 |
...
| OPL-hosted execution | `production_proof_ingested_guarded_harness_landed_live_apply_pending` | descriptor、domain-handler、typed closeout、forbidden-write proof、provider residency read model、OPL production proof ingestion、MAS provider availability 和 guarded apply harness 已有；还缺真实 domain activity soak 与 provider-hosted paper apply soak。 |
...
| Legacy references | `history_provenance_only` | 默认路径已迁走；旧词汇、alias surface、cleanup gate 和 tombstone 细节只在 history/provenance 或明确 legacy contract 语境中读取，不能重新进入 current product-entry / domain-handler / read-model 默认面。 |

因此，开发者的理解可以调整为：在 stage 内部，确实应主要维护 `agent/stages/` + `agent/prompts/` 中的自然语言主提示词/策略、知识包、工具边界和质控，并按需要调用外部 professional skill；但在 stage 与运行框架、workspace artifact、publication authority、human gate、provider live apply 的交界处，仍必须关心程序 contract、owner receipt 和验证证据。理想的分工不是“开发者完全不关心程序”，而是“日常科研语义维护尽量落在人读 stage source，跨边界动作必须由 machine-owned MAS / OPL contract 兜住”。
...
| `Allowed Tools` | 只列 MAS owner callable surface，例如 `stage-knowledge-packet`、`stage-memory-closeout-route`、`paper-mission`、`stage-closure-terminalizer`、`ai-reviewer`、`publication_gate` 等。 |
...
本表的 `independent skill` 是历史短语，当前只指 MAS stage 主提示词/策略的独立人读面与 Codex runtime projection；canonical repo source 仍是 `agent/stages/` + `agent/prompts/`。Codex `SKILL.md` 和 `.codex/skills/medical-research-*` 是投影，不是 professional specialist skill 源头，也不是长期 `stage-bound skill` source。
...
`Biajin-PKU/research-harness@006ab44` / `v0.4.0` 对 MAS 的当前价值不是引入外部 runtime、SQLite schema、MCP server、dashboard、parser backend 或 auto-runner，而是提醒本 program 不要只停在 generated stage card。MAS 已经落地 stage card、knowledge / closeout obligations、stage quality pack contract、numeric trace / claim-evidence gate、OPL descriptor projection，并已把这些 machine surfaces 消费进主 stage prompt / projection surface；真实 production provider-hosted live apply 仍需沿 MAS paper owner receipt 闭合。

可继续学习并整合的内容只按 MAS owner surface 落地：

| RH pattern | MAS current surface | next integration |
| --- | --- | --- |
| typed research checkpoint / evidence gate | `stage_surface_contract.py`、`research_memory_contract.py`、`stage_quality_contract.py`、`publication_eval/latest.json`、publication gate | `baseline`、`experiment`、`analysis-campaign`、`review` 独立 skill 已显式消费 stage card ref、research-memory obligations、quality pack refs、durable output refs、closeout packet 和 gate owner。 |
...
- 不新增 Markdown-only 规则来替代 `stage_route_contract.yaml`、MAS research-memory authority / refs contract、stage quality contract、publication gate、AI reviewer artifact、evidence/review ledger 或 controller decision。
...
当前 landed evidence 是：`stage_quality_contract.py` 提供 quality pack contract；主 stage prompt / Codex projection surface 消费 stage card、knowledge obligations、quality pack refs、closeout packet 和 forbidden actions；product-entry manifest、family stage control-plane descriptor、standard skeleton quality locator 和 stage deliverable index 暴露只读 refs；focused tests 覆盖 stage quality contract、stage surface contract、stage route assets、product-entry descriptor parity、citation/data/figure/display/reviewer-response、manuscript argument/statistical reporting 与 stage review portal surface。剩余证据尾巴是 live paper-line：真实论文线必须留下 owner receipt、evidence/review ledger refs、AI reviewer-backed `publication_eval/latest.json`、controller decision、artifact delta / gate replay / human gate / typed blocker 和 no external authority proof。
...
| `P1` | `landed` | 把 append-block 主 stage 升级为独立可读 stage prompt / policy surface，并消费 stage card / knowledge / quality pack / closeout refs | baseline / experiment / analysis-campaign / review stage prompt / projection | route / knowledge / quality contract focused tests + agent entry asset tests |
| `P1` | `landed` | 补齐 research-memory / closeout obligations | 更新 `research_memory_contract.py` 与 canonical YAML 中的 obligations | `tests/test_research_memory.py` + `tests/test_stage_knowledge_refs_consumer.py` + `tests/test_stage_route_assets.py` |
...
| `P3` | `history_provenance_only` | 退役旧 alias vocabulary 或移入 history/reference；不进入 current product-entry / domain-handler / read-model 默认面 | legacy contract / history refs + no default caller + replacement proof | focused boundary tests + `rg` stale scan |
...
| --- | --- | --- | --- | --- |
| `provider_residency_and_activity_soak` | `proof_ingestion_landed; domain_activity_soak_pending` | OPL production provider / Temporal-backed runtime | `provider_runtime_residency_read_model` 已能表达四类必需 receipt 和 typed blocker；OPL proof 可被 MAS product-entry / domain-handler ingestion 消费。下一步是真实 domain activity 长时运行、worker restart/re-query 可见性和 retry/dead-letter proof。 | OPL ledger / status 同时显示 provider 长驻、attempt 可查询、失败可重试/进 dead letter，真实 domain activity attempt 串到 MAS domain-handler receipt，且没有 MAS forbidden write。 |
...
| `publication_route_memory_receipt_scaleout` | `body_free_grouping_review_landed; more_receipts_needed` | MAS research-memory authority / workspace memory pack | OPL/Aion body-free receipt inventory、operator grouping 和 stale/deprecated review summary 已落地；下一步从更多真实论文线收集 accepted / rejected route lessons。 | `publication-route-memory-inventory` 显示多条真实 receipt、ref-only grouping、review summary 和 freshness，OPL/Aion 不持有 memory body 或 acceptance authority。 |
| `skill_change_guard` | `ongoing; companion_block_guard_landed` | OPL Connect generated stage surface + stage route/knowledge/quality contracts | 对后续任何 stage prompt / Codex projection 变更保持 machine-derived stage surface block、canonical route refs、knowledge obligations、quality pack refs、closeout refs、required companion block 和 forbidden actions。 | OPL Connect sync/readback 与 generated stage surface focused tests 通过，required companion block 被物化并纳入 readiness audit，且新增 prose 没有替代 machine truth。 |
| `standard_agent_purity_projection` | `standard_agent_purity_source_shape_landed` | P2 migration / P3 provenance guard | current product-entry / domain-handler / read-model 默认只暴露标准 Agent 口径、domain refs、owner receipts 和 typed blockers；旧 active-path、cleanup gate、compat alias 和 retired runtime action 细节只在 history/provenance、旧状态输入降级测试或明确 legacy contract 语境中读取。 | generated/hosted parity + MAS receipt parity + focused boundary tests + no-forbidden-write proof；文档不再暗示旧 runtime 是默认形态。 |
...
- stage prompt / policy / generated projection 更新：OPL Connect sync/readback、generated stage surface、stage knowledge 和 stage quality focused tests。
...
- 不把 overlay template、`.codex/skills/medical-research-*`、professional skill 或工具/connector 写成 stage 主提示词 authority；三者默认都是 no-authority projection / specialist / execution surface。
- 不新增 stage-bound skill loader、registry 或第二套 stage/professional skill catalog；分工通过 MAS stage source、OPL Connect 同步 readback 和外部 `mas-scholar-skills` source refs 表达。
