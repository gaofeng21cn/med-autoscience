...
State: `stage_skill_surfaces_landed; provider_residency_read_model_landed; guarded_apply_harness_landed; review_index_workspace_locator_proof_landed; memory_receipt_inventory_grouping_review_landed; workbench_ref_projection_landed; runtime_owner_followthrough_landed; opl_production_proof_ingested; legacy_residue_tombstone_surface_retired; production_provider_live_apply_pending`
...
| 旧 MDS / Hermes / local scheduler residue 已有 active-path tombstone | current product/sidecar manifest 不再暴露旧 residue audit surface；no-default-caller、replacement proof、retained reference 与 tombstone disposition 由 `legacy_retirement_tombstone_proof` 和 `functional_consumer_boundary.retired_legacy_residue_tombstones` 持有。仍需按无 active reference 与无 fixture/provenance dependency 逐项删除可删代码。 | stale scan + tombstone proof + no-default-caller proof + focused cleanup tests。 |
...
| `legacy_residue_retirement` | `tombstone_surface_retired; selective_cleanup_pending` | P2 migration / P3 provenance guard | current legacy audit surface 已删除；legacy active-path tombstone 与 retired tombstone machine surface 已持有 current role、replacement proof、no-default-caller 和 disposition。下一步只按 no-active-reference 与 fixture/provenance 需要删除可删代码或保留 provenance。 | stale scan + no-default-caller proof + focused cleanup tests；文档不再暗示旧 runtime 是默认形态。 |
