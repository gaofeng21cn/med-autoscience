...
| 旧 MDS / Hermes / local scheduler residue 已有 active-path tombstone | current product/sidecar manifest 不再暴露旧 residue audit surface；no-resurrection、replacement proof、provenance reference 与 tombstone disposition 由 `legacy_retirement_tombstone_proof` 和 `functional_consumer_boundary.retired_legacy_residue_tombstones` 持有。仍需按 stale-surface 与无 fixture/provenance dependency 逐项删除可删代码。 | stale scan + tombstone proof + no-resurrection proof + focused cleanup tests。 |
