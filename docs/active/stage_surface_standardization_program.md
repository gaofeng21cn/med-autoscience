...
2026-06-04 更新：MAS Stage-Native 的下一层 OPL Artifact Operating Layer 已在 repo-side 可用面落地。`stage_kernel_projection` 现在投影 `state_index`、`promotion`、`lineage_retention`，Workbench 的 `stage_operating_layer` 直接消费这些 read-only drilldown，并新增 `cross_domain_soak` 只读摘要。新增 helper 是 `opl_state_index_kernel.py`、`mas_stage_semantic_receipts.py`、`opl_stage_promotion_runtime.py`、`opl_stage_lineage_retention.py`；它们分别承担 refs-only sidecar index projection、body-free semantic receipt validation、current pointer promotion audit、lineage / retention / restore gate drilldown。所有 helper 都不写 MAS truth、paper body、publication eval、controller decisions、memory body、artifact body、submission package 或 current pointer；它们只把缺口转成 fail-closed reason、typed blocker 候选、next owner / repair diagnostic 或 Workbench drilldown。
...
| `stage prompt / policy` | `agent/stages/` + `agent/prompts/` 是标准 OPL Agent stage 主提示词与策略的 canonical source；不新增长期 `stage-bound skill` 概念。Codex `SKILL.md`、overlay template 或 `.codex/skills/medical-research-*` 只能是运行时发现/兼容投影。 |
...
短期不需要一次性物理搬目录；当前更重要的是先统一概念、字段和入口，再按低风险方式逐步迁移。`src/med_autoscience/overlay/templates/*.SKILL.md` 与运行时 `.codex/skills/medical-research-*` 只负责 Codex 自动发现和兼容投影；`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep` 和 `medical-data-governance` 属于外部 `mas-scholar-skills` 专业增强包，不是 MAS stage 主提示词源头。调用链只闭合到 `MAS 初始化 / OPL Connect 同步 -> workspace/quest .codex/skills -> Codex discovery -> specialist candidate refs -> MAS owner gate`，不新增 loader / registry，也不复制外部专业 Skill 正文。
...
| `P1` | `landed` | 把 append-block 主 stage 升级为独立可读 stage prompt / policy surface，并消费 stage card / knowledge / quality pack / closeout refs | baseline / experiment / analysis-campaign / review stage prompt / projection | overlay installer tests + agent entry asset tests |
