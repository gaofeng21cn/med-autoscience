...
| `skill_change_guard` | `ongoing; companion_block_guard_landed` | overlay installer + stage route/knowledge/quality contracts | 对后续任何 stage prompt / Codex projection 变更保持 machine-derived stage surface block、canonical route refs、knowledge obligations、quality pack refs、closeout refs、required companion block 和 forbidden actions。 | overlay / agent-entry focused tests 通过，required companion block 被物化并纳入 readiness audit，且新增 prose 没有替代 machine truth。 |
...
- stage prompt / policy / overlay 更新：overlay installer、agent-entry asset、stage surface、stage knowledge 和 stage quality focused tests。
