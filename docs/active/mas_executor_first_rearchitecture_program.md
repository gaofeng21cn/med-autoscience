...
| OPL runtime carrier bridge | `paper-mission inspect` 与 `study_progress` 顶层现在投影 `opl_runtime_carrier.surface_kind=mas_domain_progress_transition_request`。该 carrier 由 `PaperMissionTransaction.opl_route_command` 派生，包含 `paper_mission_transaction_ref`、`stage_terminal_decision_ref`、`opl_route_command_ref`、aggregate identity、route / attempt / request idempotency、`required_postcondition` 和 `DomainProgressTransitionRuntime` contract ref。DM002 fresh readback 为 `advance/start_next_stage -> publication_gate_replay`，carrier `carrier_status=waiting_for_opl_runtime_live_readback`；DM003 fresh readback 为 `typed_blocker/stop_with_typed_blocker -> current_owner_route_superseded_by_existing_typed_blocker`，carrier 同样等待 OPL live readback。 | 可以声明 MAS 已给 OPL 提供 request-only carrier，OPL 不再需要从 DHD/owner-route/default dispatch 猜下一步；不能声明 carrier 已变成 OPL outbox、event、StageRun、provider attempt、running proof、owner receipt 或 paper progress。 |
