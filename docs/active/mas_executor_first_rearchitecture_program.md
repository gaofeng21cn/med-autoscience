...
| Foreground candidate package usable | `done_for_no_write_candidate_package_surface` | `paper-mission package-candidate` 输出 package manifest、paper mission readback、candidate manifest、mission candidate artifact delta、owner decision packet、foreground owner-decision summary、`mission_executor_handoff.json` 和 `paper_facing_candidate_delta.json`，全部固定 no-authority/no-runtime/no-Yang-authority writes。2026-06-24 fresh DM002 package 写出 8 个非 authority 文件；当前 route-back handoff 为 `status=ready_for_mission_executor`、`next_owner=mission_executor`，paper-facing candidate delta 为 `status=candidate_ready_for_mas_consume`，并在 `candidate_manifest.candidate_artifact_refs` 中引用 manuscript patch plan、claim/evidence ledger delta、figure/table/caption delta、reviewer/gate response draft 与 owner decision packet 的候选 ref。 | 这是让 DM002 当前 `route_back -> mission_executor` 能继续 owner-consumption-first 的可消费候选 delta 切片；真实论文产物仍需后续 mission executor 写出具体 manuscript / claim-evidence / figure-table / reviewer response 内容，并由 MAS authority consume / route-back / human gate / typed blocker 或 governed artifact delta 接收。 |
