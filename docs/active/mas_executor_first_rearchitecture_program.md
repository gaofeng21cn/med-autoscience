...
| Foreground candidate package usable | `done_for_no_write_candidate_package_surface` | `paper-mission package-candidate` 输出 package manifest、paper mission readback、candidate manifest、mission candidate artifact delta、owner decision packet、foreground owner-decision summary、`mission_executor_handoff.json`、`paper_facing_candidate_delta.json` 和 `paper_facing_candidate_artifacts/*.json`，全部固定 no-authority/no-runtime/no-Yang-authority writes。2026-06-24 fresh DM002 package 写出 13 个非 authority 文件；当前 route-back handoff 为 `status=ready_for_mission_executor`、`next_owner=mission_executor`，paper-facing candidate delta 为 `status=candidate_ready_for_mas_consume`，并引用 5 个具体候选内容 JSON。该 package 经 `paper-mission consume-candidate --output-root` 可生成 5 个 governed consume ledger 文件，fresh readback 为 `consume_status=accepted_candidate`、`route_handoff_status=ready_for_opl_route_command`、`route_command_kind=route_back`、`next_owner=mission_executor`、`can_submit_to_opl_runtime=true`、`can_claim_paper_progress=false`。 | 这是让 DM002 当前 `route_back -> mission_executor` 能继续 owner-consumption-first 的可消费候选内容 bundle 与 OPL route handoff evidence；真实论文进度仍需后续 mission executor 或 MAS authority 把这些候选内容转成 governed manuscript / claim-evidence / figure-table / reviewer response artifact delta，并由 MAS authority consume / route-back / human gate / typed blocker 接收。 |
...
2. 下一轮真正论文推进必须从 DM002 当前 `route_back -> mission_executor` handoff 进入 MAS stage transaction，生成新的 paper candidate delta、owner decision packet 或明确 owner blocker；DM003 必须先关闭 missing OPL stage-route readback，或由 `one-person-lab` 给出 stable typed blocker / route-back / human gate。
