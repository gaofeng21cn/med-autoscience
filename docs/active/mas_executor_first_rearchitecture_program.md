...
| 旧 default-executor ordinary queue 复活 | `done` | `domain_handler_export` 现在把 legacy dispatch 从 ordinary tasks 分离到 `legacy_dispatch_diagnostics`；`pending_family_tasks` 默认保留 `paper_mission/start_or_resume`，不再把 DHD/default-executor/PaperRecovery action 当作可调度论文主线。 | legacy diagnostics 可保留解释旧状态；不得被 OPL 普通队列当成论文推进 carrier。 |
