...
scripts/run-python-clean.sh -m med_autoscience.cli runtime domain-diagnostic-report \
  --profile /Users/gaofeng/workspace/Yang/DM-CVD-Mortality-Risk/ops/medautoscience/profiles/dm-cvd-mortality-risk.local.toml \
...
| --- | --- | --- | --- |
| DM002 `002-dm-china-us-mortality-attribution` | `current_stage=queued`、`paper_stage=publishability_gate_blocked`、`active_run_id=null`。`study progress` 指向 `analysis-campaign/run_quality_repair_batch`，但 domain diagnostic dry-run 消费到 `accepted_closeout_typed_blocker`。 | 当前 work unit 已落到 `typed_blocker`，blocker 为 `stage_outcome_authority_execution_count_zero`：provider attempt 收到 OPL refs，但 MAS `stage-outcome-authority` apply 选中 0 个 execution，`gate_clearing_batch` owner receipt 未写出。 | MAS progress 说 owner action ready；domain diagnostic 说 accepted typed blocker；runtime health 说 `missing_live_session` / `quest_marked_running_but_no_live_session`；authority snapshot 又要求 OPL lifecycle readback。论文主线被 owner dispatch、typed blocker 和 runtime handoff 分裂卡住。 |
| DM003 `003-dpcc-primary-care-phenotype-treatment-gap` | `current_stage=queued`、`paper_stage=analysis-campaign`、`active_run_id=null`。`study progress` 指向 `paper_recovery_domain_blocked`，next safe action 为 `honor_stable_typed_blocker`。 | owner gate 已接受 `deny_and_stable_typed_blocker`，work unit 仍围绕 `medical_prose_write_repair` / `publication_gate_replay` 等 recovery/action identity 旋转。 | progress 表面显示 human / authority blocker 已成为 current truth；domain diagnostic 又保留 OPL transition request / provider-admission readback 要求。系统能解释为什么不动，但未把解释变成新的论文 delta、owner decision packet 或可消费修稿。 |
...
| L2 cross-surface evidence | DM002 的 progress owner action、domain diagnostic typed blocker、runtime missing live session、authority lifecycle readback 互相指向不同 owner；DM003 的 stable blocker 与 OPL transition request 同时存在。 |
...
| L4 prevention | 把“paper-facing artifact delta or accepted owner blocker”作为 primary progress metric，把 currentness / read-model / domain diagnostic / storage / admission 修复列入 sidecar platform repair，不再让平台修复默认中断论文主线。 |
...
| OPL runtime carrier bridge | `paper-mission inspect` 与 `study_progress` 顶层现在投影 request-only `opl_runtime_carrier.surface_kind=mas_domain_progress_transition_request`，并用独立 `opl_runtime_carrier_readback` / `opl_runtime_readback_status` 消费本地 closeout 或 live OPL terminal closeout readback。carrier 由 `PaperMissionTransaction.opl_route_command` 派生，包含 `paper_mission_transaction_ref`、`stage_terminal_decision_ref`、`opl_route_command_ref`、aggregate identity、route / attempt / request idempotency、`required_postcondition` 和 `DomainProgressTransitionRuntime` contract ref。2026-06-24 DM002 fresh readback 已从 stale `waiting_for_opl_runtime_live_readback` 修正为 `opl_runtime_terminal_readback_observed` / `terminal_closeout_observed`，live OPL task 为 `frt_4eed4fe9278da12fa57b336a`、stage attempt 为 `sat_ef496f3ff06c1873ed121dc9`、closeout receipt status 为 `accepted_typed_closeout`。该 closeout 折叠为 operator-facing `terminal_owner_gate`：`owner=mas_authority_kernel`、`gate_kind=domain_gate`、`blocked_reason=paper_mission_stage_route_domain_gate_pending`、`typed_blocker_ref=typed-blocker:opl_runtime_live_readback_required`、`can_claim_paper_progress=false`、`authority_materialized=false`。`terminal_owner_gate` 会生成 readback-only `terminal_owner_gate_owner_answer_readback`，并把当前顶层 `paper_mission_transaction` / `stage_terminal_decision` / `opl_route_command` / `transaction_state` 覆盖为 `route_back`，`next_owner_or_human_decision.next_owner=mission_executor`、`write_plan.written_files=[]`、`consume_candidate_status=route_back`。`consume-candidate` dry-run / ledger 路径不启用 OPL live probe，避免 candidate consume 被 live terminal task 抢占。DM003 没有 matching OPL stage-route terminal task，因此不生成 `terminal_owner_gate` / owner-answer packet；它仍通过 `stage_terminal_decision=typed_blocker` / `next_owner_or_human_decision.next_owner=one-person-lab` 表达当前 owner gate，且未错误触发 provider start。 | 可以声明 MAS 已把 OPL terminal closeout readback 与 carrier request ABI 分离，provider terminal observed 不再被误读成 domain completion，并且 CLI / study_progress 的 current terminal decision / route command 已明确为 route-back handoff。不能声明 carrier 已变成 OPL outbox、event、StageRun、provider attempt、running proof、owner receipt、typed blocker authority file、human gate、domain-ready 或 paper progress。 |
...
5. 从 `platform repair on critical path` 改为 `platform repair sidecar`。OPL lease、provider admission、currentness、domain diagnostic scope、projection drift、storage/index/read-model 问题默认是 repair lane。除非它直接解除当前 paper mission 的合法执行阻断，否则不能抢占论文 executor。
...
| `OPL route command` | OPL runtime substrate | 只消费 `StageTerminalDecision` 派生的 route command，负责 attempt、queue、resume、retry/dead-letter、provider lifecycle、stdout/log 和 current-control readback；不消费 domain diagnostic/owner-route/default dispatch/PaperRecovery 的内部状态作为 paper mission progress。 |
...
目标：停止继续在旧 domain diagnostic / owner-route / provider-admission 链上补 guard 作为主线。
...
- Progress Workbench 优先显示 mission artifact delta，而不是 domain diagnostic/currentness 字段。
...
- `domain-handler export` 不再默认导出旧 `stage_outcome/opl-handoff` 粒度任务；改导出由 `StageTerminalDecision` 派生的 `paper_mission/start_or_resume` / route command。
- 旧 owner-route/domain diagnostic 只作为 diagnostic / migration consumer / provenance input，不能再生成主线 next action。
...
- domain diagnostic/currentness/OPL readback 只显示在 platform diagnostics。
...
- old domain diagnostic / owner-route / dispatch / recovery surface 没有 default product-entry、domain-handler、MCP、skill 或 OPL active caller。
...
- active docs、product docs、skill docs 和 progress portal 不再把旧 domain diagnostic/dispatch/recovery 链写成主执行方式。

2026-06-23 tombstone slice：`contracts/runtime/legacy-active-path-tombstones.json` 新增 `domain_diagnostic_owner_route_dispatch_paper_recovery_default_paper_mainline`，把旧 domain diagnostic / owner-route / dispatch / PaperRecovery 默认主路径降为 `diagnostics_migration_provenance_only`，并禁止 product/default domain-handler mainline、paper progress、publication-ready、runtime-ready、provider-running、DM002 complete 和 DM003 complete claim。该 tombstone 关闭默认叙事边界，不等于旧物理 caller 全部删除。

2026-06-23 caller proof status：有界 caller/search evidence 已证明旧路径仍有 repo 内 references。`domain-diagnostic-report` 仍作为 runtime diagnostic / authority consumer 暴露；`paper_recovery_state` / PaperRecovery 仍被 `current_work_unit`、`study_progress`、domain diagnostic report 和 provider-admission projection 消费；旧 handoff/task 词汇在 active contract 中只允许以 tombstone marker、migration diagnostic 或 history provenance 语境出现，不再作为 OPL StageRun ABI、legacy fixture 或 transition carrier。`contracts/runtime/legacy-active-path-tombstones.json#/no_active_default_caller_proof` 现在显式列出 default-surface proof scope：product entry、domain-handler dispatch / pending tasks、family action catalog、MCP tool manifest 和 plugin skill ordinary path；focused tests 断言默认 paper mission replacement 为 `paper_mission/start_or_resume`。当前能声明的是“默认 product/domain-handler/MCP/skill/action-catalog surface 不再把旧链路暴露为 paper mission mainline”；不能声明所有旧 diagnostic / provenance references 已物理删除。

2026-06-23 clean paper mission slice：旧 `stage_outcome/opl-handoff` 不再进入 ordinary `pending_family_tasks`。`domain_handler_export` 只把它收集为 `legacy_dispatch_diagnostics`，默认 paper loop task 只出现在 `/paper_mission_default_tasks`；`/pending_family_tasks` 只保留 explicit owner handoff / migration compatibility 任务，所有非默认任务固定 `default_paper_mission_entry=false`、`can_select_next_paper_stage=false`、`can_authorize_provider_admission=false`、`counts_as_paper_progress=false`。手工向 `domain-handler dispatch` 传入 legacy task 会 fail closed，reason 固定为 `legacy_owner_callable_dispatch_tombstoned`。`domain_handler_dispatch` 在 action catalog 和 agent tool arsenal 中降为 read-only / descriptor-only boundary，不再声明可产生 owner receipt、typed blocker、domain truth、publication quality、artifact gate 或 current package 写入。新增 `paper-mission package-candidate` 将物化 `PaperMissionRun` 打包为 foreground candidate package，固定写入非权威 `ops/medautoscience/paper_mission_candidate_package/...`，输出 `paper_mission_readback`、`candidate_manifest`、`mission_candidate_artifact_delta`、`owner_decision_packet` 和 foreground summary；该 package 只服务 owner-consumption-first，不是 authority truth、runtime truth、paper body patch、owner receipt、typed blocker、human gate、publication eval、controller decision 或 current package。
...
本节回答“旧历史残留是否全面彻底清理干净，防止二次污染”。结论是：默认论文主线污染口已经 `done`，全物理删除仍是 `partial`。旧 MAS / MDS / domain diagnostic / owner-route / dispatch / PaperRecovery 相关残留只能作为 diagnostics、migration input 或 provenance 读取；不得再作为 ABI carrier 或 fixture 保留。凡要重新进入默认论文推进，必须重新经过 `PaperMissionTransaction`、`StageTerminalDecision`、MAS authority consume path、no-forbidden-write guard 和 OPL route-command/readback。
...
| 旧 domain diagnostic / owner-route / dispatch / PaperRecovery 主线污染 | `done` | tombstone `domain_diagnostic_owner_route_dispatch_paper_recovery_default_paper_mainline` 将旧链路降为 `diagnostics_migration_provenance_only`，并禁止 paper progress、publication-ready、runtime-ready、provider-running、DM002 complete、DM003 complete claim。 | 旧面可继续作为诊断、迁移输入或 provenance；不得作为 ABI/fixture 或默认 executor mainline。 |
| 旧 owner-callable ordinary queue 复活 | `done` | `domain_handler_export` 现在把 legacy dispatch 从 ordinary tasks 分离到 `legacy_dispatch_diagnostics`；默认 `paper_mission/start_or_resume` 只在 `/paper_mission_default_tasks`，`pending_family_tasks` 不再承载默认 paper loop，也不再把 domain diagnostic/owner-callable/PaperRecovery action 当作可调度论文主线。 | legacy diagnostics 可保留解释旧状态；不得被 OPL 普通队列当成论文推进 carrier。 |
| 手工 legacy dispatch 复活 | `done` | `domain-handler dispatch` 对 `stage_outcome/opl-handoff` 返回 fail-closed receipt，reason=`legacy_owner_callable_dispatch_tombstoned`，并指向 replacement `paper_mission/start_or_resume` / MAS authority consume。 | 这不是 legacy task 的兼容执行；operator 不能靠手工 dispatch 绕回旧主路径。 |
...
| 旧 physical caller / ABI / fixture 全部删除 | `partial` | 有界 caller evidence 仍命中 domain diagnostic、PaperRecovery 与旧 handoff 词汇，但 active contracts 不再保留 owner-callable ABI carrier / legacy fixture 作为允许类别。 | 剩余命中应继续物理删除或迁入 history/provenance；若要清零，需逐项证明 replacement parity、no-active-caller、no-forbidden-write 和 tombstone/provenance。 |
...
| OPL terminal closeout readback overlay | `done_for_current_route_back_readback_boundary` | `paper-mission inspect` 与 `study_progress` 在 `opl_runtime_carrier` 之外投影 `opl_runtime_carrier_readback`。同一 study / transaction / route command / work unit / fingerprint 且 `record_only_surface=true` 的 closeout 才能给出 `opl_runtime_terminal_readback_observed`；带 provider/domain completion 或 domain-ready claim 的 closeout fail closed。DM002 readback 为 live OPL terminal closeout observed + `paper_mission_stage_route_domain_gate_pending`，并由 MAS owner-answer packet 覆盖顶层 current transaction：`consume_candidate_status=route_back`、`transaction_state=route_back`、`stage_decision=route_back`、`route_command=route_back`、`next_owner=mission_executor`、`written_files=[]`；DM003 仍为 `stage_decision=typed_blocker`、`route_command=stop_with_typed_blocker`、`next_owner=one-person-lab`、missing OPL stage-route readback。 | 这是 readback/currentness 与 route-back handoff 修复，不是 MAS authority materialization、OPL runtime readiness、paper artifact delta、owner receipt、typed blocker file、human gate、publication-ready 或 paper progress。 |
| OPL owns runtime | `done_for_route_handoff_intake_and_stage_attempt_admission_smoke_live_provider_tail_open` | `/tmp` 隔离 OPL state / fixture readback 显示 DM002 `route_back` handoff 被 OPL `family-runtime intake` 接收为 `paper_mission/stage-route` task：`enqueued_count=1`、`blocked_count=0`、handoff `status=accepted_for_runtime_intake`、`command_kind=route_back`、`route_target=gate_clearing_claim_evidence_repair`、`can_submit_to_opl_runtime=true`、`can_claim_paper_progress=false`。随后 `family-runtime tick` 创建 task `frt_6bb396b4aa3835a04e791e3e` / StageAttempt `sat_fc307ea9e11ac7b0f47acc1b`，`stage_run_created=true`、`provider_attempt_requested=false`、`provider_running=false`；attempt list 为 `status=queued`、`provider_kind=local_sqlite`。 | 这只证明 MAS route command 能进入 OPL runtime intake / StageAttempt admission 边界。还缺生产 OPL hosted provider-running / terminal closeout / real session readback；不能从隔离 `/tmp` smoke、dry-run inspect、queue empty、read-model clean 或 domain diagnostic dry-run 推导 runtime-ready、paper progress、domain-ready 或 publication-ready。 |
| Platform repair demoted | `done_for_projection_boundary` | `artifact_first_mission_summary.platform_diagnostics.counts_as_paper_progress=false`，product/docs/status 把 domain diagnostic/currentness/storage/dispatch/PaperRecovery 折叠为 diagnostics / migration / provenance。 | 这只关闭默认读面和完成口径，不说明平台修复已经全部完成。 |
| Old path retired | `done_for_default_surface_retirement` | Tombstone contract 禁止默认 mainline claim；`no_active_default_caller_proof.proof_scope` 覆盖 product entry、domain-handler dispatch / pending tasks、family action catalog、MCP tool manifest、plugin skill ordinary path；domain-handler export 不再把 legacy dispatch 放入 ordinary pending queue，手工 legacy dispatch fail closed；action catalog/tool arsenal 把 dispatch 面降为 read-only descriptor；focused tests 锁住 replacement / forbidden claims / allowed legacy reference classes。 | 这只证明旧链路不再是默认 paper mission 主线；有界 caller evidence 仍命中 domain diagnostic、PaperRecovery、owner-callable-adapter 的 diagnostic / authority readback / OPL ABI / fixture / migration / provenance references，不能把它们说成全部物理删除。 |
| Foreground candidate package usable | `done_for_submission_milestone_candidate_package_surface` | `paper-mission package-candidate` 输出 `submission_milestone_candidate`：package manifest、paper mission readback、candidate manifest、mission candidate artifact delta、owner decision packet、foreground owner-decision summary、`mission_executor_handoff.json`、`paper_facing_candidate_delta.json`、`owner_consumption_request.json`、`owner_blocker_packet.json`、`submission_milestone_checklist.json` 和 `paper_facing_candidate_artifacts/*.json`，全部固定 no-authority/no-runtime/no-Yang-authority/no-paper-body writes。2026-06-24 fresh live DM002/DM003 package 各写出 16 个非 authority 文件；两者 `package_manifest.milestone_kind=submission_milestone_candidate`、`package_manifest.counts_as_paper_progress=true`、`paper_facing_candidate_delta.counts_as_paper_progress=true`、`can_claim_submission_ready=false`、`can_claim_publication_ready=false`。DM002 当前为 `route_back` + `owner_consumption_request.status=ready_for_mas_authority_consume`；DM003 当前为 `typed_blocker` + `owner_consumption_request.status=owner_blocker_packet_required`。2026-06-27 follow-up：`owner_consumption_request`、`owner_blocker_packet` 和 `submission_milestone_checklist` 显式列出待 owner 消费的 authority materialization refs 与 independent reviewer quality gate refs。 | 这是 DM002/DM003 可接力的投稿里程碑候选产物，不再把 route-back / typed-blocker 停成纯状态说明；但它仍不是 MAS authority materialization、submission-ready、publication-ready、owner receipt、typed blocker authority file、human gate、publication eval、controller decision、current package、runtime queue/provider attempt 或 canonical paper body mutation。 |
| Scientific capability / ScholarSkills module surface | `done_for_repo_capability_surface_and_refs_only_owner_gate_request_readback` | `scientific_capability_registry` 已作为 CLI / MCP / product-entry / Agent Tool Arsenal capability ABI 暴露 `index / resolve / invoke / owner-consumption`；ScholarSkills 十模块 descriptor consumer、module required ref families、refs-only execution receipt candidate consumer、file-materialized package manifest consumer、candidate artifact readback、owner-gate-request / handoff readback 和 no-authority fail-closed 边界已形成 repo capability closure。 | 这只证明能力发现、descriptor 消费、文件化 package refs 消费和 owner gate request readback 已落地；不能声明 OPL Capability Runtime live invocation、hosted provider run、OPL outbox / StageRun / provider attempt、MAS owner gate 接受、owner receipt、typed blocker authority file、human gate、publication eval、controller decision、paper truth、`current_package` 或 production-ready。 |

`100%` 只能在上述 evidence 全部新鲜可读时声明。文档、plan、schema、focused tests、domain diagnostic dry-run、queue empty、read-model clean、contract/tombstone landed、CLI no-write readback、one-shot migration package 或 old surface deletion 均不能单独支撑目标态完成；DM002/DM003 只有在 authority consume readback 或等价 owner-answer readback 指向真实 artifact delta、owner receipt、route-back、human gate、stable typed blocker 或 rejection，并且所需 authority/runtime owner 明确消费后，才可计入目标态 paper completion。
...
2. 下一轮真正论文推进必须优先消费或继续修订 DM002/DM003 已生成的 `submission_milestone_candidate`：MAS authority 应接受、拒绝、route-back、human-gate 或 typed-blocker；OPL owner path 应为 DM003 提供 missing live readback 或可消费 owner answer。不能再把 typed_blocker / waiting readback 当成停止目标。
