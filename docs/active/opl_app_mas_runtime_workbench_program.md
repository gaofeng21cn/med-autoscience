...
- `mas_opl_runtime_workbench_projection.studies[*]` 不再输出 generic `actions` alias；只保留 `operator_intent_projection` refs-only 面，且每个 intent 固定 `allowed=false`、`can_execute=false`、`command=null`，必须经 OPL current-control / owner-route handoff readback 和 MAS owner receipt 才能成为真实动作；
