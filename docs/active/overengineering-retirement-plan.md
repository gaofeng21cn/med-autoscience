...
| P1 | Display Pack bundled fallback | 默认 Display Pack 解析只走外部 `mas-scholar-skills/packs/medical-display-core` 或显式 paper/repo config；停止 repo-default loader、lock、agent preflight 和 focused tests 对 `external/display-packs/medical-display-core` hidden fallback 的依赖。物理删除 legacy tracked asset 前，先迁走不在本 lane 写集内的 `MANIFEST.in` graft 和 direct-path renderer/R-style tests。 | `config/display_packs.toml` 无 fallback source；`display_pack_paths.py` 不再优先探测 `external/display-packs/medical-display-core`；focused display-pack tests；`rg external/display-packs` 只剩 legacy direct-path/tests/docs refs。 |
...
- 当前 source/test morphology 完成口径：本轮目标文件均退出 clear `>1500` 超线范围，`paper_mission_commands.py` 与 `paper_mission_receipt_owner_consumption.py` 已回到 preferred boundary 内；`scripts/line_budget.py` 仍报告 37 条 preferred `>1000` advisories，作为后续 source-governance backlog，不是 runtime / publication / domain-ready blocker。
