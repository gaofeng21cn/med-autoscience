...
- `tests/test_cli_cases/paper_mission_command_cases/drive_and_route_handoff.py` 已把 consumption ledger readback、domain transition route、direct next action / OPL stage route case 拆到 `tests/test_cli_cases/paper_mission_command_cases/drive_and_route_handoff_cases/`；原入口从 3168 行降到 965 行，保留顶层 thin import 入口维持既有 nodeid/discovery。`--collect-only` 仍收集 32 个原 nodeid；当前 focused pytest 为 29 passed / 3 failed，其中首个失败可由 `HEAD` 原始单文件复现同一 route-checkpoint recursion，归属既有 PaperMission drive 行为缺口，不在本轮 test-only 写集内修复。
