...
Status: `long-line execution program; L1 default workspace layout and repo-level quest lifecycle replacement landed; physical absorb, real workspace cutover and compatibility retirement pending`
Date: `2026-05-06`
...
2026-05-06 当前进度判断：Git 退役已经完成 repo-side contract、默认 layout、SQLite authority 边界、workspace Git boundary guard、current eligible disease workspace 的 restore-proof storage compaction、SQLite lineage/snapshot/allocation writer、SQLite-only reader projection、plain quest materializer CLI 和 compatibility-retirement guard。还没有完成旧 quest `.git` 的 inventory/import/archive/projection proof、真实 workspace active-path cutover ledger，也没有完成 MDS compatibility reader / import / restore interface 的退役。后续论文项目需要优先完成这条 fast-track，使新项目默认不再接触 MDS/DS 路径、quest `.git` 或 `.ds/worktrees` 日常 lifecycle。
...
1. 先完成 runtime lifecycle program 的 `Q1-Q3`：repo-level schema/writer/read-model/plain materializer 已落地；剩余工作是把所有 legacy MDS Git writer、Git diff/log/branch/Canvas reader 和 hidden fallback 调用点实际切到 SQLite-backed lineage / allocation / snapshot / projection。
2. 再完成 `Q4-Q5`：对 registry-discovered 真实 workspace 做 Git-era inventory、import、restore-proof archive、projection verification 和 controller-authorized cutover ledger；已经完成 storage compaction 的 workspace 仍要补齐 quest `.git` active-path cutover 证明。
3. 最后完成 `Q6`：删除默认 Git writer、隐式 Git diff/log/worktree reader、旧 MDS product/runtime compatibility fallback；只保留显式 `restore_legacy_git_archive` / `import_legacy_git_archive` 诊断入口。
4. 完成 `Q6` 后，后续论文项目才能按 MAS-only workspace 规则展开：新 scaffold 无 `ops/med-deepscientist`、无 `.ds` 默认目录、无 quest `.git` 日常 lifecycle、无 MDS/DS 命名作为用户或医生可见路径。
