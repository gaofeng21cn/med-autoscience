...
2026-05-08 closeout：近期模块化治理、runtime/control 边界和 progress projection 收口不要求调整本 program 的终局目标，也不需要新增“全仓重构”计划。`workspace_layout_de_mds_ds` 已处理 still-visible MDS/DS surfaces：`profiles/workspace.profile.template.toml` 改为 MAS-owned `runtime/quests` / `runtime`，doctor/show-profile 把旧 `med_deepscientist_*` 字段显示为 historical import/provenance 或 backend-audit ref，bootstrap/quickstart/agent runtime 文档也对齐 no root Git / no quest Git / SQLite lifecycle / explicit diagnostic only。`profile_entry_compat_retirement` 已把默认入口继续收紧：MCP `doctor_audit` 只接受 `backend_audit`，旧 `med_deepscientist_upgrade` mode fail-closed；profile JSON 顶层不再直出 `med_deepscientist_*`，只在 `legacy_diagnostic.read_only` 暴露；workspace contract 改用 `managed_runtime_*` 与 `controlled_backend_*`；product-entry / progress 的默认 executor owner 改为 `controlled_research_backend`。最终 `no_history_physical_absorb` campaign 已把 source provenance、author audit、capability parity fixtures、MAS-owned retained capability absorb、external runtime dependency retirement 和 docs closeout 落到 repo-level guard；外部 `med-deepscientist` checkout 不再是 MAS 默认 study/status/progress/cockpit operation 的运行必需依赖，只保留为显式 backend audit、legacy restore/import diagnostic、upstream intake 和 parity oracle。
...
2026-05-08 current behavior reassessment：此前“日常科研运行目的等价 + 部分实时/交互能力退役或替换”的结论需要细化。当前 MAS 已把最关键的 `continuous turn loop` 从目的等价推进到行为等价：runner 返回后由 MAS kernel 归一化状态、清理 live flags、优先 drained queued user messages，并按 `continuation_policy=auto` 低延迟调度下一 turn；这条内生主循环不再受 `Hermes gateway cron` 的 300 秒 tick 限制。300 秒 tick 仍然存在，但现在只作为 outer supervision、drift detection、stale recovery 和 read-model refresh 的默认 cadence。仍未等价或有意替换的范围是 WebSocket terminal/log streaming、connector background delivery、旧 in-memory session API 和 MDS daemon lifecycle controls。
...
- 当前项目的 SQLite-backed runtime cutover 已完成到 quest/root Git retired 口径；MDS Git compatibility reader 不再是默认安全阀。后续只保留显式 legacy restore/import diagnostic、upstream intake 和 parity oracle。
...
| `M3b_live_console_parity` | live-console contract、runtime read model、stream bridge、Portal refs、real workspace soak | 当前为 `implementation_in_progress`：docs/test-lane/soak contract 已固定，Lane D/E read-only UI shell 与 Portal thin link/ref 边界已落地。关闭前不得把 WebSocket/terminal streaming 改成等价；Progress Portal 继续只负责进度可视化和入口导航。 | 不导入旧 MDS WebUI code/bundle/history；UI 不直接执行 controller action；soak 不写 paper/package/publication/controller/runtime authority。 |
...
| `workspace-monolith-migration` | migration dry-run pending | workspace profile migration readiness、legacy diagnostic evidence、migration skipped/appliable reason | dry-run 先行；apply 另需 Lane 4 migration apply 合同和 restore proof。 |

完成口径：只有三条 focused lane 均有真实 workspace evidence，且 read-only harness 明确报告没有真实 workspace mutation，才能把 `paper_autonomy_stability` 从 `evidence_pending` 提升到 `landed`。functional monolith、profile/entry retirement 和 no-history absorb 只能作为前置条件，不能替代该层证据。
...
3. `M3b_live_console_parity` 是当前唯一明确仍可推进的 MDS 行为差异 replacement lane；它只处理 read-only live console/terminal/log 观察，不恢复 MDS WebUI 或 resident daemon。当前 UI shell 可以独立生成 `ops/mas/live-console/index.html`，Portal 只接 thin link/ref。
4. connector background delivery、MDS team service、GitOps runtime lifecycle、MDS daemon lifecycle controls 和 in-memory session API 继续保持 retired / fixture / diagnostic 口径；除非未来有新 product requirement 和 owner proof，不作为默认 MAS operation backlog 重开。
5. 后续新能力只能作为 MAS-owned surface、explicit legacy diagnostic、upstream intake 或 parity oracle 进入，不再重开默认 MDS dependency lane。
