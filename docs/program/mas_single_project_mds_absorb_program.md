...
4. 后续论文项目按 MAS-only workspace 规则展开：新 scaffold 无 `ops/med-deepscientist`、无 `.ds` 默认目录、无 quest `.git` 日常 lifecycle；用户/医生可见路径去 MDS/DS 化仍作为 workspace layout 后续工作推进。
