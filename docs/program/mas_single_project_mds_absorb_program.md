...
2026-05-08 behavior equivalence correction，2026-05-09 scheduler wording correction：本 program 的 monolith 完成口径进一步收紧为 `default_independence_not_full_behavior_equivalence`。MAS 可以不运行 MDS repo、MDS daemon、MDS runtime root 或 MDS WebUI 完成默认日常 operation；但旧 MDS resident daemon 的低延迟会话、WebSocket terminal、connector background delivery、daemon lifecycle control 和 GitOps runtime lifecycle 没有被声明为 1:1 等价实现。当前 active scheduler adapter 是 `Hermes gateway cron`，每 300 秒执行 MAS-owned supervision tick script；desired script 依序运行 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`。旧 workspace-local launchd/systemd/cron/docker service scaffold 已退役，发现后只作为 cleanup evidence，不作为 active scheduler 选项。
...
2026-05-08 WebUI user parity correction，2026-05-09 更新：Progress Portal 的固定入口已经 landed，per-study/per-paper 工作台、Route / Decision Trail、Conversation read model、study-scoped Live Console link、authorized action receipts 和 terminal attach fail-closed gate 已进入 repo contract。旧 MDS WebUI 的强点仍是 per-project/per-quest workspace，用户按一篇论文/quest 查看研究路线、stage、文件、执行对话和 terminal；当前 MAS 的剩余差距已从“没有机器合同”转为真实多论文 workspace 的长期刷新、route input 完整性、source ref 可读性和交互控制安全门禁。`focused_lanes.portal-route-decision-trail` 固定 `mas_progress_portal_route_decision_trail` 为 read-only projection，缺 controller/evidence/runtime lineage 输入时必须 fail-closed。调研与后续 polish lane 见 `docs/references/mds_webui_user_parity_gap_review.md`。
