...
2026-05-08 current behavior reassessment：此前“日常科研运行目的等价 + 部分实时/交互能力退役或替换”的结论需要细化。当前 MAS 已把最关键的 `continuous turn loop` 从目的等价推进到行为等价：runner 返回后由 MAS kernel 归一化状态、清理 live flags、优先 drained queued user messages，并按 `continuation_policy=auto` 低延迟调度下一 turn；这条内生主循环不再受 `Hermes gateway cron` 的 300 秒 tick 限制。300 秒 tick 仍然存在，但现在只作为 outer supervision、drift detection、stale recovery 和 read-model refresh 的默认 cadence。旧 resident WebSocket terminal attach、connector background delivery、旧 in-memory session API 和 MDS daemon lifecycle controls 仍不作为默认 MAS 行为恢复；terminal/log observation 已由 MAS-owned read-only Live Console 承接为 purpose parity。
...
3. `M3b_live_console_parity` 已关闭为 MAS-owned read-only purpose parity；它只处理 live console/terminal/log 观察，不恢复 MDS WebUI 或 resident daemon。当前 Runtime OS read model、stream bridge 和 UI shell 已支撑独立 `ops/mas/live-console/index.html`；Portal 只接 thin link/ref。后续只能做 polish、soak 或入口体验改进，不得把它扩大成 Portal-owned 状态解释或旧 daemon 复刻。
