...
Status: `long-line execution program; L1 default workspace layout and repo-level quest lifecycle replacement/inventory/diagnostic/cutover surfaces landed; physical absorb and remaining real workspace cutover pending`
Date: `2026-05-06`
...
2026-05-06 当前进度判断：Git 退役已经完成 repo-side contract、默认 layout、SQLite authority 边界、workspace Git boundary guard、current eligible disease workspace 的 restore-proof storage compaction、SQLite lineage/snapshot/allocation writer、SQLite-only reader projection、plain quest materializer CLI、quest Git inventory、默认 fallback retirement、legacy runtime reader cutover、显式 legacy restore/import diagnostic 和 safe quest Git cutover CLI。真实 workspace active-path cutover ledger 已推进到 NF-PitNET、AS biologics、HeRR verified；DM-CVD 退役 3 个、剩余 DM002/DM003 两个 active/null-run quest `.git` 继续 audit-only/skipped。后续论文项目需要优先完成这条 fast-track，使新项目默认不再接触 MDS/DS 路径、quest `.git` 或 `.ds/worktrees` 日常 lifecycle。
...
2. 当前优先完成 `Q5`：对 registry-discovered 真实 workspace 做 Git-era inventory、safe cutover dry-run/apply、restore-proof archive、projection verification 和 controller-authorized cutover ledger；已经完成 storage compaction 的 workspace 仍要补齐 quest `.git` active-path cutover 证明。
3. 最后完成项目级 `Q6` closeout：所有 current projects cutover verified 后，删除剩余 stale Git compatibility docs/tests，并只保留显式 `restore_legacy_git_archive` / `import_legacy_git_archive` 诊断入口。
4. 完成项目级 `Q6` 后，后续论文项目才能按 MAS-only workspace 规则展开：新 scaffold 无 `ops/med-deepscientist`、无 `.ds` 默认目录、无 quest `.git` 日常 lifecycle、无 MDS/DS 命名作为用户或医生可见路径。
...
| `L4_artifact_and_storage_absorb` | repo-level storage/Git retirement surfaces partly landed; workspace cutover pending | Artifact OS、runtime lifecycle SQLite、storage migration tests | 吸收 artifact inventory、storage audit、cold archive / restore proof，同时保持 canonical paper truth 优先；quest Git inventory / safe cutover CLI 已落地，真实 workspace Q5 ledger 待完成。 |
...
| `L8_real_workspace_cutover` | `codex/mas-real-workspace-layout-cutover` | real workspace migration ledgers only | NF-PitNET、DM-CVD、DPCC 等 workspace dry-run、apply、compat export、restore proof；active/unknown quest `.git` 只能 audit-only/skipped。 |
