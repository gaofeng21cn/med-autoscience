...
| `M3_visual_status_replacement` | progress portal、workspace scripts、OPL handoff docs/tests | MAS Progress Portal 成为默认可视化入口；旧 MDS WebUI 的默认 `start-web` path 被改成 MAS Portal 或 explicit legacy diagnostic，不再让用户在两个进度看板之间判断 truth。 | 不导入上游 WebUI commit history、branding 或 contributor footprint。 |
