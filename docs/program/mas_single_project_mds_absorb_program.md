...
- 已有 root Git 如有 commits、dirty state、remotes、stashes 或 linked worktrees，进入 maintenance/audit，不自动删除或重写历史；空 root Git 只能作为显式 safe cleanup 候选。
...
- 新 workspace root Git 默认 no-Git，显式 opt-in 后仍只是 maintenance-only 边界。
