...
Status: `long-line execution program; L1 default workspace layout and repo-level quest lifecycle replacement/inventory/diagnostic/cutover surfaces landed; current-project quest Git retired; physical absorb pending`
Date: `2026-05-07`
...
2026-05-07 当前进度判断：Git 退役已经完成 repo-side contract、默认 layout、SQLite authority 边界、workspace Git boundary guard、current eligible disease workspace 的 restore-proof storage compaction、SQLite lineage/snapshot/allocation writer、SQLite-only reader projection、plain quest materializer CLI、quest Git inventory、默认 fallback retirement、legacy runtime reader cutover、显式 legacy restore/import diagnostic 和 safe quest Git cutover CLI。真实 workspace active-path cutover ledger 已推进到 NF-PitNET、AS biologics、HeRR、DM-CVD verified；DM-CVD 剩余 DM002/DM003 已在无 live worker 窗口经 `pause-runtime` 进入 safe state 后 archive/remove。后续论文项目默认 no root Git / no quest Git，不再接触 MDS/DS 路径、quest `.git` 或 `.ds/worktrees` 日常 lifecycle；已有 workspace root Git 进入 runtime lifecycle 子计划的 full retirement lane，按 restore-proof inventory/archive/remove/verify 处理。剩余大项是 workspace 用户可见 layout 去 MDS/DS 化、profile/entry compatibility retirement 和 MDS 物理代码 no-history absorb。
...
- 当前所有项目完成 SQLite-backed runtime cutover 前，MDS Git compatibility reader 和 restore diagnostic 不能删除；完成后不再维护 quest-level Git 作为日常 lifecycle 基础设施。
...
4. 后续论文项目按 MAS-only workspace 规则展开：新 scaffold 无 `ops/med-deepscientist`、无 `.ds` 默认目录、无 quest `.git` 日常 lifecycle；workspace root Git 默认不创建，只有显式 `--with-git` 才作为 maintenance-only root Git 初始化；用户/医生可见路径去 MDS/DS 化仍作为 workspace layout 后续工作推进。
...
当前执行队列由 portfolio 固定为：workspace root Git full retirement -> workspace layout de-MDS/DS -> profile/entry compat retirement -> no-history physical absorb。本文件负责后三项的总执行计划；第一项由 runtime lifecycle 子计划执行并把 ledger / proof 回写到本 program 的 workspace cutover gate。
...
| `L1_workspace_layout_contract` | landed via `codex/mas-workspace-layout-v3-default` | workspace layout helpers、runtime protocol layout tests、docs/runtime | 新 workspace 默认写 MAS layout、portfolio memory、paper truth 与 runtime sidecar；旧 `.ds` / `ops/med-deepscientist` 只读兼容。 |
...
| `L8_real_workspace_cutover` | quest Git cutover landed for current projects; broader layout de-MDS pending | real workspace migration ledgers only | NF-PitNET、DM-CVD、DPCC 等 workspace dry-run、apply、compat export、restore proof 已完成 quest `.git` active-path retirement；后续聚焦 workspace 用户可见 layout 去 MDS/DS 化。 |
...
- 不默认初始化 workspace root Git；只有维护者显式 `--with-git` 时才创建 root Git，且该 Git 只覆盖轻量 scaffold、contracts、portfolio registry 与 source contract，不接管 runtime、quest、delivery 或 publication truth。
...
- 已有 root Git 如有 commits、dirty state、remotes、stashes 或 linked worktrees，进入 runtime lifecycle full retirement lane；只有完成 inventory、archive、restore command、sha256 和 verify ledger 后，才允许执行 remove。
...
- 新 workspace 默认 no root Git / no quest Git；已有 root Git 必须经 restore-proof full retirement lane 退出默认 workspace 状态面。
