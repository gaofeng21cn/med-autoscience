...
| `paper-progress-loop-closeout` | landed repo-level contract after 2026-05-10 integration | Paper Progress SLO fields in user-visible projection、owner callable registry、submission authority/delivery closure transaction、old worktree path normalization、worker wrapper workspace config allowlist、dirty patch supersession ledger、focused tests and repo gates | repo/controller/read-model change only；不写 live study `current_package`、submission package、`publication_eval/latest.json`、`controller_decisions/latest.json`、runtime SQLite 或 restore archive。 |
