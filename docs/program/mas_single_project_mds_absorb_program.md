...
| `M3b_live_console_parity` | live-console contract、runtime read model、stream bridge、Portal refs、real workspace soak | 当前为 `implementation_in_progress`：docs/test-lane/soak contract 已固定，Runtime OS read model、只读 stream bridge 和 Lane D/E read-only UI shell / Portal thin link-ref 边界已落地。关闭前不得把 WebSocket/terminal streaming 改成等价；Progress Portal 继续只负责进度可视化和入口导航。 | 不导入旧 MDS WebUI code/bundle/history；UI 不直接执行 controller action；soak 不写 paper/package/publication/controller/runtime authority。 |
...
3. `M3b_live_console_parity` 是当前唯一明确仍可推进的 MDS 行为差异 replacement lane；它只处理 read-only live console/terminal/log 观察，不恢复 MDS WebUI 或 resident daemon。当前 Runtime OS read model、stream bridge 和 UI shell 已可支撑独立 `ops/mas/live-console/index.html`；Portal 只接 thin link/ref。
