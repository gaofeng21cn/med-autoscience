...
Status: `long-line execution program; L1 default workspace writer and repo-level quest lifecycle replacement/inventory/diagnostic/cutover surfaces landed; current-project quest/root Git retired; doctor-visible layout and profile/entry retirement pending; physical absorb pending`
Date: `2026-05-08`
...
2026-05-08 校准：近期模块化治理、runtime/control 边界和 progress projection 收口不要求调整本 program 的终局目标，也不需要新增“全仓重构”计划。下一步只需要把执行口径压实：`workspace_layout_de_mds_ds` 先处理 still-visible MDS/DS surfaces，例如 `profiles/workspace.profile.template.toml` 的旧 `ops/med-deepscientist` 默认路径、doctor/show-profile 里的 `med_deepscientist_*` 直出、bootstrap/quickstart/agent runtime 文档里的 legacy root Git 口径，以及旧 `.ds` / `ops/med-deepscientist` reader 的显示语义。`L1 default workspace layout landed` 只表示新 scaffold writer 已经 MAS-first，不表示所有静态 profile、用户文档和兼容字段都已去 MDS/DS 化。物理 no-history absorb 仍不能提前开始；必须等 source provenance、author audit、capability parity fixtures、rollback surface 与 profile/entry retirement gate 成立。
...
4. 后续论文项目按 MAS-only workspace 规则展开：新 scaffold 无 `ops/med-deepscientist`、无 `.ds` 默认目录、无 quest `.git` 日常 lifecycle，也无 workspace root Git 默认要求；用户/医生可见路径去 MDS/DS 化仍作为 workspace layout 后续工作推进。
...
当前执行队列由 portfolio 固定为：workspace layout de-MDS/DS -> profile/entry compat retirement -> no-history physical absorb。workspace root Git full retirement 已由 runtime lifecycle 子计划执行并把 ledger / proof 回写到本 program 的 workspace cutover gate。
...
| `L1_workspace_layout_contract` | default writer landed via `codex/mas-workspace-layout-v3-default`; doctor-visible cleanup remains in `workspace_layout_de_mds_ds` | workspace layout helpers、runtime protocol layout tests、docs/runtime、profile template、doctor/show-profile docs | 新 workspace 默认写 MAS layout、portfolio memory、paper truth 与 runtime sidecar；旧 `.ds` / `ops/med-deepscientist` 只读兼容；静态 profile 和医生可见输出仍需在下一 lane 去 MDS/DS 化。 |
...
| `L8_real_workspace_cutover` | quest Git and workspace root Git cutover landed for current projects; broader layout de-MDS pending | real workspace migration ledgers only | NF-PitNET、DM-CVD、DPCC 等 workspace dry-run、apply、compat export、restore proof 已完成 quest `.git` active-path retirement；NF-PitNET、DM-CVD / DPCC 和 AS biologics 已完成 workspace root Git restore-proof retirement，HeRR 原本 no root Git；后续聚焦 workspace 用户可见 layout 去 MDS/DS 化。 |
