...
| `M3b_live_console_parity` | live-console contract、runtime read model、stream bridge、Portal refs、real workspace soak | 当前为 `implementation_in_progress`：docs/test-lane/soak contract 已固定，目标是 MAS-authored clean-room read-only session/log/terminal replacement。关闭前不得把 WebSocket/terminal streaming 改成等价；Progress Portal 继续只负责进度可视化。 | 不导入旧 MDS WebUI code/bundle/history；UI 不直接执行 controller action；soak 不写 paper/package/publication/controller/runtime authority。 |
...
3. `M3b_live_console_parity` 是当前唯一明确仍可推进的 MDS 行为差异 replacement lane；它只处理 read-only live console/terminal/log 观察，不恢复 MDS WebUI 或 resident daemon。
