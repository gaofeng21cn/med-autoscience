...
2026-05-08 closeout：近期模块化治理、runtime/control 边界和 progress projection 收口不要求调整本 program 的终局目标，也不需要新增“全仓重构”计划。`workspace_layout_de_mds_ds` 已处理 still-visible MDS/DS surfaces：`profiles/workspace.profile.template.toml` 改为 MAS-owned `runtime/quests` / `runtime`，doctor/show-profile 把旧 `med_deepscientist_*` 字段显示为 legacy diagnostic / backend-audit alias，bootstrap/quickstart/agent runtime 文档也对齐 no root Git / no quest Git / SQLite lifecycle / legacy diagnostic only。`profile_entry_compat_retirement` 已把默认入口继续收紧：MCP `doctor_audit` 只接受 `backend_upgrade`，旧 `med_deepscientist_upgrade` mode fail-closed；profile JSON 顶层不再直出 `med_deepscientist_*`，只在 `legacy_diagnostic.read_only` 暴露；workspace contract 改用 `managed_runtime_*` 与 `controlled_backend_*`；product-entry / progress 的默认 executor owner 改为 `controlled_research_backend`。最终 `no_history_physical_absorb` campaign 已把 source provenance、author audit、capability parity fixtures、MAS-owned retained capability absorb、external runtime dependency retirement 和 docs closeout 落到 repo-level guard；外部 `med-deepscientist` checkout 不再是 MAS 默认 study/status/progress/cockpit operation 的运行必需依赖，只保留为显式 backend audit、legacy restore/import diagnostic、upstream intake 和 parity oracle。

2026-05-08 functional monolith correction：上面的 closeout 不是函数级、模块级或 workflow 级 1:1 吸收完成。当前 parity matrix 只固定 `runtime_execution`、`artifact_inventory`、`paper_contract_health`、`manuscript_coverage`、`prompt_stage_discipline`、`memory_and_lesson_store` 六类 retained capability；`runtime_core_ingest`、旧 MDS WebUI replacement / retirement、MDS daemon / quest / runner / connector / MCP / TUI / Web / gitops / skills / team 等 remaining surface 的 MAS-owned 归宿仍未完成。后续目标从“MAS 是唯一入口”升级为“MAS 是实际 monolith”：默认 operation、可视化、运行 core、诊断、学习和保留能力都由 `med-autoscience` 内部 MAS-owned modules 承接；外部 MDS repo 只可作为 source archive / provenance reference，不作为运行、WebUI 或 contributor source。
...
| `L2_mds_inventory_and_classification` | no-history provenance and parity harness landed | migration inventory tooling、capability matrix、source snapshot manifest | MDS source provenance 已固定到真实 ref/hash，能力已标记 `absorb` / `oracle` / `retire` / `compat`。 |
...
| --- | --- | --- | --- |
| `M1_remaining_capability_inventory` | source provenance、capability matrix、tests/fixtures | 重新清点 MDS daemon、quest、runners、channels/connectors、MCP、TUI/Web、gitops、skills、team 等 remaining surface，标记 `absorb` / `rewrite` / `oracle` / `retire` / `external_source_only`；不再用 unique function name 数量作为完成指标，但要覆盖 workflow 语义。 | 只记录 source ref/hash/license/provenance，不导入上游 history。 |
...
4. 若未来从外部 `med-deepscientist` 或上游 `DeepScientist` 学习新能力，必须重新记录 source ref/hash、capability classification、MAS owner、authority boundary、tests 和 no-history contributor audit。
...
3. 写或更新 `source_provenance.json`，包括 upstream repo/ref、snapshot sha256、license refs、capability classification；当前 closeout snapshot 为 `med-deepscientist@35976b7d6e3b99b15b57ec44ff5f5d959b342ecc`，archive sha256 为 `f8dc31822dc52ecc6e073f54c8b5c95cd46646e299a67cd1c1f6f7f3764e0d5b`。
