...
2026-05-08 WebUI user parity correction：Progress Portal 的固定入口已经 landed，但用户体验等价仍不完整。旧 MDS WebUI 的强点是 per-project/per-quest workspace，用户按一篇论文/quest 查看研究路线、stage、文件、执行对话和 terminal；当前 MAS Portal 默认 workspace overview 会把多篇论文放在同一页，容易混淆，也还没有把路线分支、失败/阻塞原因、转向理由和 active/winning path 投影成单篇论文 decision trail。后续 Portal UX parity 的 P0 是 study-scoped IA、per-study deep link 和单篇论文 Overview / Route/Decision Trail / Path / Runtime / Conversation / Terminal / Artifacts 工作台。调研与后续 lane 见 `docs/references/mds_webui_user_parity_gap_review.md`。
...
2026-05-09 fresh reassessment：MAS monolith 当前已经不需要外部 MDS repo / daemon / WebUI 承担默认 operation，但行为等价不是满分。机器矩阵显示 `2/17` behavior surface 已完全等价，`6/17` 目的等价但 timing/实现不同，`4/17` 部分等价，`4/17` 已退役，`1/17` 只作历史 fixture。当前真实剩余工作不是继续复制 MDS，而是把用户可见 UX 和受控交互补齐到 MAS-native：per-study/per-paper Portal 工作台、runtime conversation read model、study-scoped Live Console、authorized UI control，以及带安全门禁的 terminal attach 设计。connector background delivery、GitOps runtime lifecycle、MDS daemon lifecycle controls 和 workspace-local host service 是有意退役，不作为 monolith 能力降级处理。
