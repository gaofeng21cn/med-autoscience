...
Domain memory 这条线现在可以回到主线任务推进：`publication_route_memory` 已经具备 policy/index、repo seed fixture、workspace memory pack、stage entry refs、typed closeout proposal 和 MAS router receipt。当前最合适继续落地的是人读 inventory、更多真实 paper-line writeback receipt、OPL/Aion ref-only 展示和 App/workbench 分组；不应推进 recipe engine、winning-route scorer 或 OPL-owned memory body store。
...
| `2a` | `publication_route_memory_management` | [Publication Route Memory Policy](../policies/study-workflow/publication_route_memory_policy.md), [Study Workflow](../policies/study-workflow/README.md) | `workspace_apply_closure_ready; human_inventory_documented; operator_ui_pending` | 继续把可复用论文路线经验写成自然语言 memory card：维护 workspace `publication_route_memory/memory_pack.json`、migration/writeback receipts、人读 inventory 和 OPL/Aion ref-only projection；下一步优先做只读 inventory/export、更多真实论文线 accepted/rejected receipt、按 workspace/stage/route family/status 分组展示。暂缓 recipe engine、自动 winning-route scorer 和 OPL-owned memory body。 |
...
| `4` | `stage_surface_standardization` | [MAS Stage Surface Standardization Program](./stage_surface_standardization_program.md) | `stage_skill_surfaces_landed; provider_live_apply_pending` | 已落地 generated stage cards、缺失 knowledge/closeout obligations、stage quality pack contract、product-entry / family descriptor locator、`baseline` / `experiment` / `analysis-campaign` / `review` 独立 stage skill surface、既有 skill 的 stage surface / quality pack / RH clean-room gate 消费，以及 provider projection / typed blocker proof。下一步仍是 production provider-hosted live apply。 |
...
| `publication_route_memory_management` | 人类用户能从 policy/index 进入 workspace memory pack 与 receipt/proposal locator；OPL/Aion 只显示 consumed refs、writeback receipt refs、rejected reason 和 freshness；更多真实论文线通过 MAS router receipt 生成 accepted/rejected reusable lessons。 |
