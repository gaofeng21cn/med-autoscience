...
| `publication_route_memory_management` | [Publication Route Memory Policy](../policies/study-workflow/publication_route_memory_policy.md) + [Study Workflow](../policies/study-workflow/README.md) | `functional_follow_through_gate` | `implemented; body_free_inventory_grouping_review_landed` | 扩展更多真实 accepted / rejected / route-back receipts，补跨 workspace inventory smoke 和 maintainer review discipline。 | `publication-route-memory-inventory` body-free receipt inventory + operator grouping + stale/deprecated review summary；多 paper-line `stage-memory-closeout-route -> memory_write_router_receipt -> inventory/export` proof 仍由真实 workspace receipt 扩展。 |
...
| `legacy_residue_retirement` | legacy active-path tombstone contract 已落地，后续按 finding 删除或保留 reference。 | P2 / P3 / P3a | stale scan、no default caller proof、replacement proof、focused compatibility tests。 |
