...
5. **已落地基础层**：P3 和 P3a 已完成主要实施，当前角色是 owner/provenance/archive/parity guard。它们保留在 `docs/program/`，是因为仍承载 monolith、MDS provenance、restore proof、runtime lifecycle、quest/root Git retirement 和后续 source-intake / archive / parity 判断的当前 owner 文档，不再代表活跃实现队列。
