...
关键顺序是 `A1 -> A2 -> A3 -> A4`。当前 quest Git、workspace root Git、workspace 用户可见 layout、profile/entry compatibility retirement 和 no-history physical absorb repo-level closeout 已对 repo contract / new scaffold 完成到 verified / default-retired / landed。后续只保留 recurring upstream learning / intake support lane；如果把 SQLite 当成 paper truth，或把 legacy backend diagnostic 当成默认研究入口，会破坏 MAS publication authority。
