...
2026-05-11 OPL stage-led family framework 校准：本文仍是 MAS 仓内部 program portfolio 入口；但涉及 OPL 作为 Codex-first、stage-led 完整智能体运行框架、Temporal provider、执行语言、OPL-hosted path、domain-agent skeleton 和跨仓旧面退役的总入口，统一回到 `/Users/gaofeng/workspace/one-person-lab/docs/references/runtime-substrate/opl-stage-led-agent-framework-roadmap.zh-CN.md`。MAS 的 [OPL Temporal MAS Runtime Retirement Program](./opl_temporal_mas_runtime_retirement_program.md) 是 domain-side 子计划：OPL master P1 的 Temporal provider core 已在 repo/test 面证明 workflow/activity/signal/query 和 provider receipt 可表达；MAS 仍要等真实 Temporal worker/server deployment、Codex long-running activity runner、human-gate 操作闭环和 provider-domain soak 后，才进入真实 paper-line guarded soak 与物理退役。它不替代 `ai_first_paper_autonomy_closure` 的医学论文主循环，也不得把 OPL attempt completion 当成 MAS paper progress 或 publication quality verdict。`Stage` 是大型任务步骤，`Codex CLI` 是 stage 内默认 concrete executor 和最小执行单元；OPL provider 只能承载 attempt，MAS 继续持有 domain truth。
