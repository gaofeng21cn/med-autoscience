...
2026-05-08 functional monolith 校准：`no_history_physical_absorb=landed` 不能解释为“MDS 所有模块、函数和 WebUI 工作流已经在 MAS 内 100% 等价实现”。它只表示默认 MDS checkout dependency、入口兼容、历史导入、contributor footprint 和 6 类 retained capability proof 已关闭。用户目标中的 MAS monolith 仍需要一条新的 `functional_monolith_completion` queue：以 MAS-authored/no-history 方式把仍有产品价值的 MDS runtime core、状态可视化、执行 orchestration、诊断和学习能力吸收到 MAS-owned modules，并把旧 MDS WebUI 退为 legacy diagnostic 或删除。该 queue 不增加 `med-autoscience` default-branch contributor graph；所有来自外部 MDS / DeepScientist 的材料只能通过 source provenance、snapshot hash、license refs、capability classification、parity proof 和 MAS maintainer-authored commit 进入。
...
| `4` | `functional_monolith_completion` | `mas_single_project_mds_absorb_program.md`、`../runtime/runtime_core_convergence_and_controlled_cutover.md` | `not_landed`: 完成 MDS remaining capability inventory、runtime core ingest、MAS-native progress/status surface replacement、legacy MDS WebUI retirement、OPL App family-level dashboard handoff、compat/oracle shrink-to-diagnostic 和 post-push contributor audit；不得把外部 MDS checkout 恢复成默认 operation dependency，也不得让 OPL App 重解释 MAS study truth。 |
...
| `A6_functional_monolith_completion` | 不再满足于“MAS 是唯一入口”；把仍有长期价值的 MDS runtime core、status/visualization、orchestration、diagnostic 和 learning capabilities 以 MAS-owned/no-history 方式收进同一 repo，并退役旧 MDS WebUI default path。OPL App 集成只作为 family-level projection/deep-link layer，不成为 MAS progress authority。 | `mas_single_project_mds_absorb_program.md`、`../runtime/runtime_core_convergence_and_controlled_cutover.md` | 可以并行做 inventory、runtime core、visual surface、OPL handoff contract、compat shrink、contributor audit；吸收必须按 owner/parity/rollback gate。 |
...
- functional monolith completion 明确保留为未完成队列项，且必须区分“默认入口/依赖已退役”和“所有 MDS 功能已被 MAS-owned 模块吸收”。
