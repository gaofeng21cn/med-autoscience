...
| `4` | `functional_monolith_completion` | `mas_single_project_mds_absorb_program.md`、`../runtime/runtime_core_convergence_and_controlled_cutover.md` | `not_landed`: 完成 MDS remaining capability inventory、runtime core ingest、MAS-native progress/status surface replacement、legacy MDS WebUI retirement、compat/oracle shrink-to-diagnostic 和 post-push contributor audit；不得把外部 MDS checkout 恢复成默认 operation dependency。 |
...
| `A6_functional_monolith_completion` | 不再满足于“MAS 是唯一入口”；把仍有长期价值的 MDS runtime core、status/visualization、orchestration、diagnostic 和 learning capabilities 以 MAS-owned/no-history 方式收进同一 repo，并退役旧 MDS WebUI default path。 | `mas_single_project_mds_absorb_program.md`、`../runtime/runtime_core_convergence_and_controlled_cutover.md` | 可以并行做 inventory、runtime core、visual surface、compat shrink、contributor audit；吸收必须按 owner/parity/rollback gate。 |
