...
当前 active 入口固定为五件加一个 OPL 对齐子计划：
...
- [OPL Temporal MAS Runtime Retirement Program](./opl_temporal_mas_runtime_retirement_program.md)：OPL 成为完整 stage-led family agent runtime 后，MAS scheduler / watchdog / legacy runtime surface 的退役、降级与迁移子计划；它从属于 OPL master roadmap，不替代 MAS 医学论文主循环。
...
DeepScientist latest-update learning 这类 learning/intake 工作仍由对应 reference policy/protocol 触发，并由 `MAS` 直接面向 upstream DeepScientist 执行。`docs/history/program/` 里的 dated 文件是单轮快照，不表示 recurring lane 已退役；它们也不恢复 MDS 默认 runtime、WebUI、diagnostic 或 quality dependency。
