...
- repo 内已经把 `MedAutoScience gateway -> upstream Hermes-Agent target outer substrate -> controlled research backend` 的最小正式链路冻结完成
- 对当前开发宿主，external runtime bring-up 与 repo-side `F2 / real adapter cutover` 已经落下代码、合同与回归 proof
...
- `doctor`、`hermes-runtime-check` 与 `backend-upgrade-check` 会把 gate 结果收口成结构化 blocking verdict
...
uv run --python 3.14 python -m med_autoscience.cli doctor backend-upgrade --profile <profile> --refresh
...
- `./manual_runtime_stabilization_checklist.md`
  - 收口 `doctor -> hermes-runtime-check -> backend-upgrade-check` 的手工验证顺序
- `./med_deepscientist_deconstruction_map.md`
...
- 对当前开发宿主，external runtime bring-up 已完成，repo-side `F2 / real adapter cutover` 也已落地；剩余 blocker 已主要转为 `F3 real study soak / recovery proof`、external workspace truth 与 human gate
