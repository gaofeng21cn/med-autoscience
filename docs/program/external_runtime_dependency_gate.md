...
- `doctor`、`hermes-runtime-check` 与 `med-deepscientist-upgrade-check` 会把 gate 结果收口成结构化 blocking verdict
...
uv run --python 3.14 python -m med_autoscience.cli hermes-runtime-check --profile <profile>
...
- `./manual_runtime_stabilization_checklist.md`
  - 收口 `doctor -> hermes-runtime-check -> med-deepscientist-upgrade-check` 的手工验证顺序
- `./med_deepscientist_deconstruction_map.md`
