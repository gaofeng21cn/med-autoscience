...
- `macOS Advisory` 为重 lane 预留 `MAS_TEST_LANE_SUMMARY_PATH`，并以非阻塞 run log 摘要和 artifact 上传 lane summary，用于观察 duration drift 和失败前后的耗时变化
...
耗时预算只用于观察和提醒，不作为 push 阻塞条件。预算漂移应该先进入 advisory run log、artifact、run summary 或人工排查；除非另有 repo 决策，不能把 duration guard 回灌到 `macOS CI` 的 `quick-checks`。
...
- `full`：目标是正式发布前完整覆盖，允许显著慢于 smoke / ci-preflight；耗时漂移通过 advisory / full summary 观察，不能替代质量失败或变成日常 push 门禁。
