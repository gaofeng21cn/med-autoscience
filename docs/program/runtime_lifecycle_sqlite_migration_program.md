...
- compatibility retirement：默认 CLI/MCP/controller/product entry 不再走 MDS Git compatibility fallback；保留入口必须显式命名为 explicit archive import reference，并 fail-closed 到只读。当前 repo 已完成默认 lifecycle read fallback retirement 与 historical reader cutover；current projects Q5/Q6 ledger 已验证，只保留显式诊断入口。
...
| `Q6_compat_layer_retirement` | landed for default runtime path; explicit archive import reference retained | MDS compatibility adapter、tests、docs/status | 默认 lifecycle fallback 已删除，historical reader 默认不扫 `.ds/worktrees`，保留只读 historical fixture / explicit archive import reference；`quest_git_retired_for_current_projects=true` 已进入 current status 口径。 |
...
2. `profile_entry_compat_retirement`：已落地为默认 CLI/MCP/controller/product entry 不回退到 MDS Git reader 或 MDS product entry；archive import 只能通过显式 reference 入口读取 archive。
3. `no_history_physical_absorb` 和 `functional_monolith_completion` 已关闭为 landed guard：后续只允许显式 explicit archive import reference、historical fixture、source provenance 或 MAS-owned capability proof 读取旧 MDS 材料。
