...
`/Users/gaofeng/workspace/Yang/NF-PitNET` 是无功能垂体瘤 / nonfunctioning pituitary neuroendocrine tumor 的 canonical workspace。`/Users/gaofeng/workspace/Yang/无功能垂体瘤` 是本机轻量中文 alias/scaffold，约 392 KiB、74 个文件、没有 `.ds` quest root；它不进入独立 runtime lifecycle migration accounting，也不得和 NF-PitNET 重复统计。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| NF-PitNET | `20260505T054758Z-nfpitnet-selected-study-apply` | `parked_controller_stop` | NF003 `.ds`: 347,275 files / 8,405,233,664 bytes; NF004 `.ds`: 346,801 files / 39,168,921,600 bytes; NF002 `.ds`: 146 files / 2,625,536 bytes | scoped cache cleanup only: 17,300,667 bytes; prior NF003 runtime maintenance released 8,049,453 bytes | NF001 paused audit-only; NF003/NF004 parked or stop-loss/owner-sensitive; dirty workspace; no restore proof for deeper compaction | no meaningful file-count reduction; NF003 file count slightly increased versus earlier scan because index/ledger/runtime sidecars were added while deep payload stayed online | absent | workspace SQLite sidecar exists; compatibility export exists for storage audit; old authority files remain file-shaped and readable |
| DM-CVD / DPCC | `20260505T052743Z-dm-cvd-live-audit` | `live_active` | DM002 `.ds`: 493,606 files / 15,354,638,336 bytes; DM003 `.ds`: 206,511 files / 8,405,336,064 bytes | prior safe cache cleanup only: 312,732,597 bytes; latest ledger is dry-run/audit | 001/002/003 live or escalated; 004 paused without restore proof; dirty workspace | no runtime file-count reduction; live quest payload remains audit-only | absent | workspace and quest lifecycle SQLite sidecars are ready; latest file authority remains the reader source of truth |
| AS biologics | `20260505T053738Z-as-biologics-selected-study-apply` | `stopped_cold` ledger, but effective runtime remains parked/audit-only | AS001 `.ds`: 172,868 files / 5,948,604,416 bytes after selected-study apply | earlier cache cleanup released 210,389,648 bytes; latest selected-study run released 0 runtime bytes | pinned/recent worktrees and existing cold archives; no restore proof for destructive runtime compaction | no file-count reduction from latest selected-study apply | absent | workspace SQLite sidecar is ready; restart remains controller-owned, with current scientific/task blockers outside storage migration |
...
结论：当前已经完成的是 `SQLite sidecar + ledger + compatibility read/export` 的可用层，以及对 live/paused/owner-sensitive quest 的 fail-closed 保护；当前没有完成的是百万级 `.ds` 文件数压降终态。真正的文件数量下降仍取决于后续 stopped-cold restore-proof lane：先生成 rollback plan、archive ref、checksum 和 restore proof，再对 `bash_exec`、`runs`、`codex_history`、`codex_homes`、`worktrees`、`cold_archive` 这类桶做安全归档与 prune。
