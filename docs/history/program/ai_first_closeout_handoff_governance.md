...
每条非冲突计划完成记录必须落在 `docs/history/program/plan_completion_ledger.md`，并使用同一组 ledger 字段。记录只说明计划项、落地证据、验证证据、push/cleanup 状态、live surface 验收和剩余缺口；不得把 narrative closeout 升级为机械质量 gate。
