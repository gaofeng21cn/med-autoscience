...
旧 execution map、phase ladder、integration activation 与 Hermes continuation
逐项正文已从 tracked tree 退役；精确历史读取 Git history。当前边界读取：
...
这条 5 阶段梯子的旧详细口径已从 tracked tree 退役；精确历史读取 Git history。
...
旧 outer-loop wakeup 机制正文已从 tracked tree 退役；精确历史读取 Git history。
...
5. 本文写作时的最小 repo-tracked activation artifact 是 Integration Harness Activation Package；该旧正文现从 Git history 读取。
...
- Hermes Backend Continuation Board 与 Hermes Backend Activation Package 已从 tracked tree 退役；精确历史读取 Git history。
