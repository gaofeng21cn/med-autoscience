...
| Executor-first rearchitecture program compression | The old executor-first active support doc no longer stores DM002/DM003 command transcripts, readback tables, candidate package ledgers, residual-risk audit rows or completion-proof tables as an active process log. It now keeps only target architecture, owner split, current boundary, migration route and immediate next work. | Current active gaps stay in `docs/active/mas-ideal-state-gap-plan.md`; next-action semantics stay in `docs/runtime/control/next_action_control_plane.md`; historical evidence detail remains in git history, runtime/workspace receipts and machine surfaces. |
