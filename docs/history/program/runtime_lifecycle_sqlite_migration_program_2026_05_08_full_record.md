...
近期 MAS 模块化治理、Runtime Control 和 Progress Projection 收口没有改变本 program 的职责。当前 Git/runtime 子计划已经进入 guard 维护口径：current projects 的 quest `.git` 和 workspace root Git 都已退为 restore-proof archive，新 workspace 默认 no root Git / no quest Git。后续不再把 root Git 写成可选 workspace 维护模式；如果外部或旧 workspace 再出现 root `.git`，处理方式是显式 inventory / archive / remove / verify，而不是 opt-in source-control mode。

因此，本文剩余工作只服务 guard 维护：`workspace_layout_de_mds_ds`、`profile_entry_compat_retirement`、`no_history_physical_absorb` 和 `functional_monolith_completion` 已消费本文提供的 Git/runtime 事实边界、只读 historical fixture / explicit archive import reference、provenance、author audit 与 parity proof 输入。不得把新增 live writer drift、profile 文案、MDS product entry、no-history import gate 或 functional monolith 口径误写成 SQLite/runtime 子计划未完成。
...
3. `remove`：只对 ledger 判定可退役的 root Git 执行 archive/remove；runtime truth、paper truth、publication truth、controller truth 和 delivery package 不参与这个迁移。
...
- 旧 workspace cutover ledger：每个 registry-discovered workspace 有 Git-era inventory、SQLite import、restore-proof archive、compat export、projection equivalence 和 `quest_git_active_path_retired=true` 或明确 skipped reason；repo CLI 已提供 `runtime lifecycle-quest-git-inventory` 与 `runtime lifecycle-quest-git-cutover`。
...
- compatibility retirement：默认 CLI/MCP/controller/product entry 不再走 MDS Git compatibility fallback；保留入口必须显式命名为 explicit archive import reference，并 fail-closed 到只读。当前 repo 已完成默认 lifecycle read fallback retirement 与 historical reader cutover；current projects Q5/Q6 ledger 已验证，只保留显式 archive 入口。
- doctor-facing layout：实际 workspace 和 README/progress projection 不再向医生展示 MDS/DS 路径或 root Git 作为默认要求；旧路径只在 migration ledger、restore proof 或 maintainer diagnostic 中出现。

### 2026-05-06 / 2026-05-07 real workspace quest Git cutover ledger

本轮已把 Q5 从纯 planning / dry-run 推进到 safe apply。执行入口是 `runtime lifecycle-quest-git-cutover --mode apply`；该入口只处理 `paused/parked/stopped/completed/failed/cancelled` 且 `active_run_id=null`、`worker_running=false` 的 quest `.git`，对 `active/running/live/unknown` fail-closed 为 audit-only。随后用 `runtime lifecycle-ledger --mode verify --write` 写入 workspace ledger；migration ledger 会读取最新 cutover record，并把 archive manifest、sha256、restore command 与 remaining skipped paths 纳入 `git_lifecycle_cutover`。

| workspace | apply result | verify ledger | remaining gate |
| --- | --- | --- | --- |
| DM-CVD / DPCC | 2026-05-06 `3/5` retired；2026-05-07 追加 DM002、DM003，最终 `5/5` retired | `runtime-lifecycle-20260507-quest-git-cutover-dm-cvd-final-ledger`，`git_lifecycle_cutover.status=verified`；Q6 ledger `runtime-lifecycle-20260507-quest-git-cutover-dm-cvd-q6-retirement`，`compatibility_retirement.allowed=true` | none for current active-path quest Git；DM002/DM003 因本次手动暂停进入 explicit wakeup contract，后续论文执行需正式 wakeup / task intake。 |
| NF-PitNET | `3/3` retired | `runtime-lifecycle-20260506-quest-git-cutover-nf-pitnet.json`，`git_lifecycle_cutover.status=verified` | none for current active-path quest Git. |
| AS biologics | `1/1` retired | `runtime-lifecycle-20260506-quest-git-cutover-as-biologics.json`，`git_lifecycle_cutover.status=verified` | none for current active-path quest Git. |
| HeRR | `0/0` targets | `runtime-lifecycle-20260506-quest-git-cutover-herr.json`，`git_lifecycle_cutover.status=verified` | none. |

因此 current project 口径已经完成 active-path quest Git retirement：NF-PitNET、AS biologics、HeRR 和 DM-CVD 均 verified；显式 explicit archive import reference 保留为只读恢复入口，不再是默认 compatibility fallback。

## 结论

引入 SQLite 可行，也有必要。目标形态是 `SQLite runtime authority + file truth authority + Git source control`：Git 继续管理源码、合同、文档和可审阅配置；MAS 的 publication/study authority surface、restore metadata、paper/manuscript/package/dataset 交付物继续保持文件形态；SQLite 负责 runtime lifecycle 的状态、lineage、workspace allocation、snapshot metadata、索引、历史、游标、摘要、retention ledger 和 compact projection。旧 MDS 路径只作为 explicit archive import reference 或 historical fixture 被读取。
...
Program portfolio 口径：本文是 [MAS Single-Project MDS Absorb Program](../../active/mas_single_project_mds_absorb_program.md) 的 runtime persistence / quest Git retirement 子计划。它接管 MDS Git-era 的 runtime lifecycle 语义，但不单独拥有产品入口、MDS 代码吸收、no-history import、functional monolith closeout 或 compatibility layer 退役的最终授权；这些已由 MAS absorb program、runtime backend contract、capability parity matrix 与 owner-boundary contract 统一 gate。当前 program portfolio 分类与归档策略见 [Program Portfolio Consolidation](../../active/program_portfolio_consolidation.md)。

## 一步到位目标

本 program 的目标不是只补一个索引文件，也不是只清理某个异常 workspace。完成后的可用状态必须同时满足：

- MAS repo 已有统一 lifecycle schema、读写 contract、compatibility export、migration CLI / MCP surface、guardrail 和验证入口。
...
- 默认新 scaffold：`runtime/quests/` 承接 live quest root，`runtime/archives/` 与 `runtime/restore_index/` 承接归档和恢复索引，`artifacts/runtime/` 承接 SQLite runtime DB 与 migration ledger；root Git 默认不创建，也不再作为 MAS workspace 的推荐 opt-in 维护模式。外部或旧 workspace 若仍带 root Git，必须走显式 retirement diagnostic，并排除 generated/runtime 面。
...
- Workspace 外层 Git 不再默认创建，也不再作为 MAS 论文 workspace 的可选日常维护模式。未来若维护者接入外部或旧 workspace 并发现 root `.git`，必须把它归类为 legacy maintenance diagnostic：先 inventory、authority classification、archive、restore command、sha256 和 verify ledger，再 remove 或显式保留在 MAS runtime/status/truth 之外；不得让它重新承担 status、runtime lifecycle、paper truth、delivery truth 或 Agent lookup。
...
| --- | --- | --- | --- |
| SQLite runtime authority | MAS runtime | lifecycle state、lineage graph、workspace allocation、checkpoint metadata、diff summary、cursor、retention、archive refs、Canvas read-model | `artifacts/runtime/runtime_lifecycle.sqlite` 与 quest/workspace-local DB |
| Memory / runtime knowledge | MAS runtime + agent memory | interaction summary、decision rationale、resume packet、agent handoff、session/codex/bash summaries | SQLite rows + compact JSON export；大 payload archive ref |
...
| `Q4_project_git_to_sqlite_migrator` | landed via `codex/quest-git-inventory-cutover` plus prior SQLite lanes | migration CLI、fixture tests、real workspace ledger | Repo-level inventory and migration ledger integration 已落地；现有 quest `.git` 可被 inventory 自动纳入 skipped/audit-only，blocked active path 会阻止 verified cutover。真实 workspace import/projection equivalence 仍需在 Q5 ledger 中按项目写入。 |
| `Q5_current_workspace_cutover` | landed for current projects | 真实 workspace migration ledger；不改 paper truth | Repo-level safe cutover CLI 已落地；NF-PitNET、DM-CVD/DPCC、AS biologics 和 HeRR current active-path quest `.git` 已 archive/remove 或 verified zero-target。 |
| `Q6_compat_layer_retirement` | landed for default runtime path; explicit reference retained | MDS compatibility adapter、tests、docs/status | 默认 lifecycle fallback 已删除，historical reader 默认不扫 `.ds/worktrees`，保留只读 explicit archive import reference；`quest_git_retired_for_current_projects=true` 已进入 current status 口径。 |

吸收策略：`Q0-Q6` 对 current projects 已吸收；`Q5` 只写真实 workspace ledger 和 archive，不改医学/论文 truth。后续新增或重开 workspace 进入同一 gate，不再恢复 quest-level Git 日常 lifecycle。
...
- `docs/status.md` 明确记录 `quest_git_retired_for_current_projects=true` 后，才允许删除 compatibility tests 和 old Git lifecycle docs；workspace root Git full retirement 已对 current projects 完成，未来新发现的 root Git 只走同一显式 retirement diagnostic，不再形成新的 active backlog。
...
2. `profile_entry_compat_retirement`：已落地为默认 CLI/MCP/controller/product entry 不回退到 MDS Git reader 或 MDS product entry；explicit archive import 只能通过显式 reference 入口读取 archive。
3. `no_history_physical_absorb` 和 `functional_monolith_completion` 已关闭为 landed guard：后续只允许显式 archive import reference、historical fixture、source provenance 或 MAS-owned capability proof 读取旧 MDS 材料。

compatibility reader 不再是默认迁移安全阀；只剩 archive import reference。
