...
Program portfolio 口径：本文是 [MDS absorb guard history](./mas_single_project_mds_absorb_guard_2026_06_07.md) 的 runtime persistence / quest Git retirement 子计划。它接管 MDS Git-era 的 runtime lifecycle 语义，但不单独拥有产品入口、MDS 代码吸收、no-history import、functional monolith closeout 或 compatibility layer 退役的最终授权；这些已由 MAS absorb history、runtime backend contract、capability parity matrix 与 owner-boundary contract 统一 gate。当前 program portfolio 分类与归档策略见 [Program Portfolio Consolidation](../../active/program_portfolio_consolidation.md)。
