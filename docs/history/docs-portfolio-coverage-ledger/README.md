...
| Full tracked docs scope closure 2026-06-12 | Fresh OPL Doc readback covers root `README*`, repo-source support `README.md` files, every tracked `docs/*.md` and every tracked `docs/**/*.md`. Current owner routing is through `docs/README.md`, `docs/docs_portfolio_consolidation.md`, core docs, the single Active Truth plan, directory indexes, runtime/delivery/source/policy/reference/history owners, this coverage ledger, history/program closeouts, contracts, source, tests and CLI/read-model output. Historical bodies remain provenance unless a current owner explicitly promotes a subsection. | No uncovered MAS docs-governance scope is currently carried in this ledger. Remaining work is implementation/evidence-tail work under active owners: live paper-line owner evidence, reviewer/auditor records, publication-route memory receipts, artifact lifecycle receipts, human gate/resume, provider SLO long-soak, current work-unit owner closeout and physical-delete authorization. |
| Status current-summary compression 2026-06-12 | `docs/status.md` was rechecked against `docs/docs_portfolio_consolidation.md`, `docs/active/mas-ideal-state-gap-plan.md`, runtime/stage-route owner docs and history/program provenance. Its top dated “recently recorded” list and DM002/DM003 follow-through blocks were compressed into current owner anchors plus a stable runtime-truth reading. Current state now points to live study truth, active gap plan, Progress-First runbook, Stage-route design/contract, Domain Authority Refs Index Guard and history provenance instead of keeping timeline evidence in the active status page. | This is docs-only SSOT compression. It changes no source, contract, workflow, package, CLI/API, runtime state, study workspace, owner receipt or typed blocker, and it authorizes no physical stale-surface deletion. Future status updates must keep dated proof, command output, worklist counts and receipt ids in history/runtime/workspace/git provenance. |
| Active-support reviewed tranche 2026-06-12 | `docs/active/current-development-lines.md`, `docs/active/program_portfolio_consolidation.md`, `docs/active/mas-opl-stage-native-state-machine.md`, `docs/active/opl_app_mas_runtime_workbench_program.md`, `docs/active/stage_surface_standardization_program.md` and `docs/active/ai_first_paper_autonomy_closure_program.md` were checked as active support / active plan-index docs under the lifecycle rules in `docs/docs_portfolio_consolidation.md`. Their current roles remain support, design, acceptance or execution-map roles; single Active Truth still belongs to `docs/active/mas-ideal-state-gap-plan.md`, current-state summary still belongs to `docs/status.md`, and process proof remains history/runtime/workspace/git provenance. | This tranche authorizes no source, test, workflow, package, CLI/API or runtime deletion. Physical stale-surface retirement still requires replacement owner, no-active-caller proof, tombstone/provenance and relevant owner receipt / typed blocker / owner decision evidence. |
| Active-doc legacy vocabulary retirement 2026-06-12 | `docs/decisions.md`、`docs/status.md` 与 `docs/runtime/control/progress_first_stage_outcome.md` 已按 `contracts/stage_route_reconcile_contract.json`、`docs/active/mas-ideal-state-gap-plan.md` 和 Stage-route currentness 文档复核。当前人读 prose 只把 SSOT 写成 Codex executor route policy 与 DM002/DM003 recovery acceptance policy；旧 `frontdoor` 词面只作为 contract key / history vocabulary 读取，不再作为 active MAS user-facing terminology。 | 这是 docs-only semantic consolidation。不重命名 contract key、source symbol、CLI/API、test 或 runtime surface，也不授权任何 physical stale-surface deletion。若后续要改机器 key，必须作为单独的 contract/source/test migration，并带 caller proof。 |
...
2. Return to MAS only after fresh intake shows a new MAS docs body, a live owner-surface foldback need, an active/history owner conflict, or a concrete stale code/test/workflow/package retirement candidate with replacement-owner and no-active-caller proof.
