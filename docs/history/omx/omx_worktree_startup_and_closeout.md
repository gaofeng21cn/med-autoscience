...
当前读法：以下正文是 dated OMX-era 操作记录，不是当前 MAS 执行手册。正文里的“当前”“推荐”“必须”“正确用法”等词只在 OMX 已作为活跃本地工具时成立；2026-04-11 后，项目级 `.omx` / `.codex` 已退役，`docs/history/omx/` 只能作为 provenance / tombstone / no-resurrection guard。不要据此新建或恢复项目级 `.omx` state、root hook 扫描、tmux/session 指针、旧 owner worktree 常驻面或 OMX prompt/report state。
...
这份文档与 [主线集成与清理节奏](../../policies/repo-ops/mainline_integration_and_cleanup.md) 配套：

- `../../policies/repo-ops/mainline_integration_and_cleanup.md` 负责定义“代码主线怎么吸收和清理”；
...
- 读取 `docs/capabilities/medical-display/{portfolio,board,contracts,catalogs,plans,provenance}/medical_display_*` 的 tracked 真相；
