...
| [MAS private surface physical retirement](mas-private-surface-retirement.md) | MAS 私有控制面物理退役 tombstone；记录旧 module / CLI alias / workspace wrapper / compat shim 的 repo-source 删除、合法残留角色和 live-runtime readiness 分账边界。 | Git history、[status](../../status.md) 与 [MAS ideal-state gap plan](../../active/mas-ideal-state-gap-plan.md)。 |
...
| legacy next-action surfaces: PaperRecovery, `current_executable_owner_action`, domain transition, provider admission, OPL queue / attempt, current-work-unit / current-execution-envelope selectors | [Next Action Control Plane](../../runtime/control/next_action_control_plane.md) and [legacy active path tombstones](legacy_active_path_tombstones.md) | 只能作为 provenance、diagnostic、migration input、receipt/readback evidence 或 no-resurrection guard；默认 next action 只能来自 `StageOutcome -> NextActionEnvelope`，`OPL TransitionReceipt` 只作 transport receipt-only evidence 和 MAS owner-consumption input，缺 envelope 时 fail closed。 |
| MAS private runtime substrate, local carrier persistence, owner-callable/default-executor carriers, runtime health/workbench/capability projections, runtime lifecycle/storage maintenance | Git history and [MAS private surface physical retirement](mas-private-surface-retirement.md) | repo-source retirement 与 live-runtime readiness 分账读取；旧 module / CLI alias / wrapper / compat shim 不再作为 active public surface。允许残留只限 OPL-authorized adapter、MAS minimal authority adapter、body-free projection、tombstone/provenance 或 no-resurrection guard。 |
