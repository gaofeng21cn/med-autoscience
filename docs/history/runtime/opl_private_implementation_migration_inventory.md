...
| `src/med_autoscience/controllers/opl_unique_control_plane_boundary/consumer_migration_inventory.py` | 当前 functional module inventory source；只记录 15 个标准 Agent surface。 | `build_functional_consumer_boundary()`、`contracts/functional_privatization_audit.json`、`contracts/test-lane-manifest.json`。 | 三类标准 Agent 分类、domain authority refs、minimal authority functions、generated surface handoff。 | deleted helper、cleanup registry、generic runtime owner、scheduler owner、worker lease owner。 |
