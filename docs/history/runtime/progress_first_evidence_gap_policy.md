...
- Stage / default executor policy: `agent/stages/manifest.json`、OPL-generated stage control plane、`stage_throughput_contracts` 和 `owner_callable_action_policy` 的 missing-evidence 口径统一为 `classify_with_evidence_gap_decision_then_progress_first`；普通 evidence gap 不再直接 fallback 到 typed blocker。
