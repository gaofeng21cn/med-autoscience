...
- 最近一次已吸收完成的 capability cluster：
  - `H / broader QC / batch-effect / transportability follow-on beyond the first bounded center-coverage transportability shell`
...
- Family cluster：`F`
- Capability cluster：`multi-group grouped-local or decision-path follow-on beyond the current support-domain lower bound`
- Owner worktree：`未开启`
- 状态：`brief_needed`
...
> 在 `shap_grouped_local_explanation_panel`、`shap_grouped_decision_path_panel`、`feature_response_support_domain_panel`、`partial_dependence_interaction_slice_panel`、`partial_dependence_subgroup_comparison_panel` 与 `accumulated_local_effects_panel` 已经入库的前提下，如何把更宽的 multi-group grouped-local or decision-path manuscript-facing explanation evidence 收束成新的 bounded template，使论文能够稳定回答“多组之间的局部解释差异是否清楚、共享特征顺序与基线是否仍可读、support-domain 下限之外是否还需要更强的 explanation scene”，并继续保持投稿面可审计、可回归、可复用。

### Fresh Route 收敛

当前 reroute 已明确：

1. `H` 家族 broader QC / batch-effect / transportability follow-on 已经用 `transportability_recalibration_governance_panel` 正式收口。
2. 下一轮切回 `F` 家族 explanation follow-on，继续坚持 bounded manuscript-facing template 路线，直接服务 multi-group grouped-local / decision-path 的进一步合同化。
...
1. 固定 `F` 家族当前 owner 候选为 `multi-group grouped-local or decision-path follow-on`；
2. 在 `F` 家族当前 round 里，继续复用现有 grouped-local / decision-path / support-domain vocabulary、shared feature-order discipline 与 bounded explanation contract；
...
- 对应 grouped-local / decision-path regression tests
...
1. `multi-group grouped-local or decision-path follow-on` 已正式入库为单一 bounded explanation template；
