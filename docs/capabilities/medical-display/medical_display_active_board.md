...
- Family cluster：`H`
- Capability cluster：`broader transportability and center-coverage follow-on beyond the current recalibration-governance lower bound`
- Owner worktree：`未开启`
- 状态：`brief_needed`
...
> 在 `multicenter_generalizability_overview`、`generalizability_subgroup_composite_panel`、`center_coverage_batch_transportability_panel`、`transportability_recalibration_governance_panel` 与 `broader_heterogeneity_summary_panel` 已经入库的前提下，如何把更宽的 transportability / center-coverage / manuscript-facing study-governance synthesis 继续收束成新的 bounded `H` follow-on template，使论文能够稳定回答“哪些中心级泛化与治理结论足以进入正文主叙事”，并继续保持投稿面可审计、可回归、可复用。

### Fresh Route 收敛

当前 reroute 已明确：

1. `C/H` broader heterogeneity summary follow-on 已经用 `broader_heterogeneity_summary_panel` 正式收口。
2. 下一轮切到 `H` broader transportability and center-coverage follow-on，继续坚持 bounded manuscript-facing template 路线。
3. `F` 更高阶 explanation scene 与 `D/E/G` 更高阶 manifold / multi-view atlas follow-on 继续留在后继 reroute 池。
...
1. 固定 `H` 家族当前 owner 候选为 `broader transportability and center-coverage follow-on`；
2. 在 `H` 当前 round 里，继续复用现有 multicenter generalizability / bounded transportability shell / recalibration-governance / broader heterogeneity synthesis vocabulary 与 manuscript-facing study-governance contract；
...
- 对应 `H` follow-on renderer 文件
- 对应 `H` / cross-paper golden regression tests
...
1. `broader transportability and center-coverage follow-on` 已正式入库为单一 bounded manuscript-facing template；
...
1. `F / stronger explanation-panel readability or higher-order explanation scene only if new real-paper demand proves the current grouped-local + multigroup decision-path lower bound insufficient`
