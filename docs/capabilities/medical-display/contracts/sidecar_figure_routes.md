...
- **适用边界对照。** 如果要改动的图是直接支撑 claim/结果的 artifact，应走 `figure_script_fix` 由 MAS artifact route 产生，并把 renderer family 锁定为 `python` 或 `r_ggplot2`；如果图只是说明性图，则走 `figure_illustration_program`，renderer family 可为 `python`、`r_ggplot2` 或 `html_svg`。旧的 `figure_illustration_sidecar` 和任何外部绘图 sidecar 路由都属于已废弃歧义入口，不应再使用。
