...
| `2026-04-13` | `C/H` 家族 compact effect-estimate 面板正式入库 | 真实论文 compact-estimate manuscript follow-on 交付需求 + `C/H` bounded lower-bound hardening 决策 | 把多个预设调整规格 / cohort 定义下的 OR/HR / effect estimate 从论文现场并排 forest / 补充说明收敛成共享 `reference_value`、共享 row order 的紧凑多 panel 模板；新增 `compact_effect_estimate_panel_inputs_v1`、python renderer、layout QC 与 `C/H` golden regression 锁定，并把 bounded `2-4` panel count、panel identity、interval finite contract、support_n 正值约束与 outboard row-label containment 做成正式契约 | `C/H` / `compact_effect_estimate_panel` | 已正式入库，作为 post-baseline rolling expansion 的第二十六个 capability cluster |
...
| `2026-04-13` | `H` 家族 baseline-balance + missingness + QC 三联壳层正式入库 | 真实 cohort-quality / manuscript-facing 数据质量摘要需求 + `H` 家族 bounded shell hardening follow-on | 把基线平衡、缺失模式与质控摘要从分散的表格/补充说明需求收敛成固定三联 manuscript-facing 图面；新增 `baseline_missingness_qc_panel_inputs_v1`、python renderer、layout QC 与 shell-catalog materialization 路径，并把 balance threshold、变量唯一性、missingness grid 完整性、QC card containment 与固定 `A/B/C` panel 治理做成 fail-closed contract | `H` / `baseline_missingness_qc_panel` | 已正式入库，作为 post-baseline rolling expansion 的第三个 bounded illustration-shell follow-on，也是第二十五个 capability cluster |
...
| `2026-04-14` | `F` 家族 feature-response support-domain explanation panel 正式入库 | 真实解释型论文交付需求 + support-domain / subgroup-legend / extrapolation reminder 合同化收口 | 把 feature-response support-domain explanation 从“已有 PDP / subgroup / ALE lower bound，但 support 语义仍分散在多个模板里”的 follow-on 候选提升为正式 bounded multi-panel 模板；新增 `feature_response_support_domain_panel_inputs_v1`、python renderer、layout QC 与 `F` 家族 golden regression 锁定，并把 response-curve 区间、support-kind 受控词表、full-domain coverage、reference guide 与 support-domain segment / label containment 做成正式契约 | `F` / `feature_response_support_domain_panel` | 已正式入库，作为 post-baseline rolling expansion 的第二十七个 capability cluster |
