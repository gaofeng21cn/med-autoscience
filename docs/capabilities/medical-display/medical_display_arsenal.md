...
当前生效统计口径以 registry / template catalog / audited guide 的一致真相为准，现行库存是 `46`。
...
- 历史冻结快照（仅用于 provenance）为 `31`：
  ...
  - 插图壳层模板 `2`
  - 表格壳层模板 `5`
- 当前 strict registry / template catalog 工程口径统一为 `46`：
  - 证据型图模板 `39`
  - 插图壳层模板 `2`
  - 表格壳层模板 `5`
- 截至 `2026-04-12`，post-baseline rolling expansion 已在冻结基线上正式收口十二个 evidence capability cluster：
...
| `F. 模型解释` | 特征归因、解释性摘要与复杂度审计 | `shap_summary_beeswarm`、`shap_dependence_panel`、`shap_waterfall_local_explanation_panel`、`shap_force_like_summary_panel`、`partial_dependence_ice_panel`、`model_complexity_audit_panel` | 已具备首个审计基线，并把 dependence、patient-level waterfall、representative-case force-like summary 与 bounded PDP+ICE baseline 一并提升为正式 pack 资产；当前剩余主缺口转向 richer partial-dependence variants、feature-importance 与 grouped-local-explanation follow-on | `001/003` 锚点论文 + `npj Digital Medicine` `2025` SHAP dependence exemplar + `npj Digital Medicine` `2025` UMORSS 图 6 + `JBJS Open Access` `2025` PARITY 图 6A/B + SHAP force plot 经典解释图式 + PDP / ICE 经典解释图式 + `F` 家族视觉审计决策线 |
...
- `F` 家族虽已从 `shap_summary_beeswarm` 扩到 `shap_dependence_panel`、`shap_waterfall_local_explanation_panel`、`shap_force_like_summary_panel` 与 `partial_dependence_ice_panel`，但 richer partial-dependence variants、feature-importance 与 grouped-local-explanation follow-on 仍需继续滚动 hardening；
