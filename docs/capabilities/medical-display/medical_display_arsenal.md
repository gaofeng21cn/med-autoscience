...
当前生效统计口径以 registry / template catalog / audited guide 的一致真相为准，现行库存是 `42`。
...
- 历史冻结快照（仅用于 provenance）为 `31`：
  ...
  - 插图壳层模板 `2`
  - 表格壳层模板 `5`
- 当前 strict registry / template catalog 工程口径统一为 `42`：
  - 证据型图模板 `35`
  - 插图壳层模板 `2`
  - 表格壳层模板 `5`
...
以下素材已经进入绘图主线视野，但当前仍不计入“已正式落地的新增军火库能力”。其中 atlas exemplar 暴露出的 `embedding + signature heatmap` 与 `embedding + composition + marker/program overview` 子能力，已在 `2026-04-08` 正式入库为 `celltype_signature_heatmap` 与 `single_cell_atlas_overview_panel`；更大 atlas / spatial / trajectory 变体仍留在候选池中：
