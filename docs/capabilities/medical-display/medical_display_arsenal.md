...
当前生效统计口径以 registry / template catalog / audited guide 的一致真相为准，现行库存是 `72`。
...
- 历史冻结快照（仅用于 provenance）为 `31`：
  ...
  - 插图壳层模板 `2`
  - 表格壳层模板 `5`
- 当前 strict registry / template catalog 工程口径统一为 `72`：
  - 证据型图模板 `60`
  - 插图壳层模板 `7`
  - 表格壳层模板 `5`
- 截至 `2026-04-18`，post-baseline rolling expansion 已在冻结基线上正式收口三十四个 capability cluster：
...
| `真实论文 compact-estimate / coefficient-path / broader-heterogeneity follow-on 交付需求 + bounded lower-bound hardening` | 先把多个预设 estimate slices 从论文现场并排 forest / 补充说明收敛成共享 reference line、共享 row order 的紧凑多 panel effect-estimate 图面，再把预设模型步骤下的 coefficient direction / magnitude stability 收束成 bounded `path + summary` manuscript-facing 合同，最后进一步上提到逐行 manuscript verdict 收口的 broader heterogeneity summary；同时把 panel count、panel identity、interval containment、step coverage、slice vocabulary、verdict state 与 summary-panel alignment 做成正式契约 | `C/H` / `compact_effect_estimate_panel`、`coefficient_path_panel`、`broader_heterogeneity_summary_panel` |
