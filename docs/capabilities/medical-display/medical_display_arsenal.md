...
当前生效统计口径以 registry / template catalog / audited guide 的一致真相为准，现行库存是 `98`。
...
- 历史冻结快照（仅用于 provenance）为 `31`：
  ...
  - 插图壳层模板 `2`
  - 表格壳层模板 `5`
- 当前 strict registry / template catalog 工程口径统一为 `98`：
  - 证据型图模板 `84`
  - 插图壳层模板 `7`
  - 表格壳层模板 `7`
- 截至 `2026-04-20`，post-baseline rolling expansion 已在冻结基线上正式收口五十五个 capability cluster：
  ...
  - `atlas_spatial_trajectory_multimanifold_context_support_panel`：把 `D/E/G` 从“已有六块式 context-support baseline”继续推进到正式的七块式 dual-manifold context-support baseline，并把 `UMAP + PHATE atlas geometry + spatial state topography + trajectory progression + region-wise state composition + kinetics heatmap + state-by-context support heatmap` 固化为单一 bounded composite contract；
  ...
  - `diffusion_map_scatter_grouped`：把 `D` 从“PCA / UMAP / t-SNE / PHATE 四投影 baseline”继续推进到正式的 `Diffusion Map` grouped manifold slice，并把 grouped embedding scatter 的 projection family 扩成五投影一致口径；
  - `celltype_marker_dotplot_panel`：把 `D/E/G` 从“已有 atlas overview / bridge / niche / trajectory / multimanifold context-support lower bound，但仍缺一个高频正文 companion atlas marker scene”继续推进到正式 bounded dotplot 模板，并把 shared `celltype_order`、shared `marker_order`、完整 `panel x celltype x marker` 网格、effect/size 双标尺语义与 up-to-two-panel manuscript-facing 版式治理固化为单一模板；
  - `confusion_matrix_heatmap_binary`：把 `A/E` 从“已有 ROC / PR / calibration / decision curve / performance heatmap，但仍缺一个高频正文二分类诊断矩阵表达”继续推进到正式 bounded confusion-matrix heatmap 模板，并把显式 `2x2` 网格、`row/column/overall` normalization、`metric_name`、行列顺序和百分比标注语义固化为单一模板；
  ...
  - `shap_signed_importance_local_support_domain_panel`：把 `F` 从“已有 signed importance、单病例 waterfall 与 support-domain follow-on，但缺一张能把全局重要性、单病例局部解释与数据域支持统一进正文主叙事的 bounded 复合图”继续推进到正式 `1 + 1 + 2` explanation scene，并把 global feature order、local contribution subset-preserving order、exactly-two support panels、panel-label 全局唯一性与 local-to-support feature follow-through 一并固化为单一 manuscript-facing 模板。
...
| --- | --- | --- | --- | --- |
| `A. 预测性能与决策` | 模型效果、校准、决策阈值与临床可用性 | `roc_curve_binary`、`pr_curve_binary`、`calibration_curve_binary`、`decision_curve_binary`、`clinical_impact_curve_binary`、`binary_calibration_decision_curve_panel`、`time_to_event_decision_curve`、`time_to_event_landmark_performance_panel`、`time_to_event_threshold_governance_panel`、`time_to_event_multihorizon_calibration_panel`、`confusion_matrix_heatmap_binary` | 已形成真实论文证明的核心能力，并已把 `Brier/error-oriented` landmark 治理、threshold summary + grouped survival calibration governance、binary clinical-impact counting、binary confusion-matrix heatmap，以及 multi-horizon grouped calibration governance 一并提升为正式模板资产 | `001/003` 锚点论文 + `A/B/H` 回归加固 + `Nature Communications` `2021` 动态复发风险 exemplar + `Nature Medicine` / `npj Digital Medicine` `2025` 阈值与校准 exemplar + clinical-impact counting 与 confusion-matrix follow-on |
...
- `二分类混淆矩阵热图`
  用途：把真阳性、假阳性、真阴性、假阴性的比例直接放进固定 `2x2` 矩阵里，回答“模型到底错在什么地方、错误分布集中在哪一格、当前指标对应的分类代价长什么样”。
- `时间依赖 ROC（单时间点）`
...
- `Diffusion Map 分组散点图`
  用途：补齐 grouped manifold 的第五种常见低维投影，适合展示连续几何结构在 diffusion map 视角下的分离与过渡。
- `细胞类型嵌入 + 签名热图`
...
- `细胞类型-marker 点图面板`
  用途：把 `cell type × marker` 的 mean expression 和 detection rate 压成一张 atlas companion dotplot，适合回答“哪些 marker 真正定义了这些状态、这个定义在 discovery / validation atlas 里是否一致”。
...
- `atlas-spatial-trajectory multimanifold context-support 面板`
  用途：在六联 context-support 的基础上再加入第二个 atlas manifold 视角，把 `UMAP + PHATE` 两种几何表达、空间定位、轨迹进展、区域组成、程序动力学和上下文支持度压成一张七联图，回答“同一状态结构在不同流形投影下是否稳定，并且能否继续被空间、轨迹和程序证据链共同支持”。
...
- `二分类混淆矩阵热图`
  用途：把固定 `2x2` confusion matrix 做成正文可直接使用的热图，适合在矩阵层面快速说明 TP、FP、TN、FN 的比例结构，以及当前 normalization 下错误主要堆积在哪一块。
- `相关性热图`
...
- `细胞类型-marker 点图面板`
  用途：把 marker 的平均表达和阳性比例同时压进 `cell type × marker` 点阵里，适合用一张正文 companion 图说明 atlas 里的状态定义是否清楚、不同 cohort / panel 之间是否一致。
- `细胞类型嵌入 + 签名热图`
...
- `atlas-spatial-trajectory multimanifold context-support 面板`
  用途：在已有 context-support 复合矩阵基础上，再加入第二个 atlas manifold 视角，适合回答“换一种流形几何以后，状态-上下文证据链是否依旧成立”。
...
- `SHAP signed importance + local waterfall + support-domain 复合面板`
  用途：左上先给出全局带方向的重要性排序，右上再解释一个代表病例的局部贡献路径，下排用两个 matched support-domain panel 补充关键特征在真实数据域里的响应曲线与支持区间，适合论文一张图同时回答“全局最重要的是谁、这个病例为什么会这样、这些解释在什么范围内站得住”。
- `PDP + ICE 面板`
...
- `细胞类型-marker 点图面板`
  用途：把 atlas 里的 marker mean expression 和 detection rate 压成一张正文 companion 图，适合说明不同细胞状态是由哪些 marker 支撑的，以及这些定义在 discovery / validation atlas 之间是否一致。
- `组学 volcano 面板`
...
- `genomic alteration multiomic consequence 面板`
  用途：把关键 driver alteration 的 consequence 继续扩到 proteome、phosphoproteome、glycoproteome 三层，适合回答“关键异常在多层组学里是否同步传导、哪一层最强、哪一层更像补充证据”。
- `genomic alteration pathway-integrated composite 面板`
  用途：把 gene-level alteration landscape、三层 multiomic consequence 和三层 pathway enrichment 绑定进一张大图，适合回答“driver alteration 是否同时改写了 pathway 层面的证据链”。
...
- `atlas-spatial-trajectory multimanifold context-support 面板`
  用途：把双 atlas manifold 几何、空间定位、轨迹进展、区域组成、程序动力学和上下文支持度一起放进同一张七联复合图里，适合回答“同一组学状态在不同流形表达下是否仍然能被整合证据链稳定支撑”。
...
- `治疗变化对齐图`
  用途：把不同治疗阶段或不同策略切换前后的关键指标对齐放在一张图里，适合交代研究流程中的 treatment shift 是否和主要结果方向一致。
- `实用因素点图`
  用途：把几个直接面向临床或实际部署的问题因子压成一张规整点图，适合快速交代“哪些现实因素最值得关注、它们大概有多强”。
- `偏好类别敏感性图`
  用途：把不同偏好类别或不同决策口径下的结果敏感性放进一张图里，适合说明“换一种临床偏好或分类方式，结论是否还稳”。
- `workflow fact sheet`
...
| `Nature Medicine` `2024` spatial niche exemplar + `Nature Communications` `2025` spatial atlas exemplar + `Nature Biotechnology` `2023` trajectory exemplar + dual-manifold atlas geometry follow-on real-paper demand | 把 atlas、空间与轨迹三条已经推进到六联 context-support baseline 的证据链继续上提为七联 dual-manifold composite，并把 `UMAP + PHATE` 双 atlas 几何、空间状态拓扑、轨迹进展、区域组成、程序动力学与 state-by-context support heatmap 一并收进单一 bounded manuscript-facing 契约 | `D/E/G` / `atlas_spatial_trajectory_multimanifold_context_support_panel` |
