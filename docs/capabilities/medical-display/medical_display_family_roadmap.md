...
- celltype/program/kinetics/support/context composite matrices beyond the current atlas-overview / atlas-spatial-bridge / spatial-niche / trajectory / storyboard / density-coverage / context-support baseline
...
- higher-order explanation scenes only when real paper demand proves the current summary + bar-importance + signed bar-importance + multicohort bar-importance + dependence + waterfall + force-like + grouped-local + grouped decision-path + grouped-local support-domain composite + bounded PDP/ICE baseline + bounded pairwise interaction-contour lower bound + interaction slice + subgroup comparison + ALE lower bound + support-domain lower bound is insufficient
- more complex grouped-local scenes only when real paper demand proves the current up-to-three-panel grouped-local lower bound should expand
- stronger explanation-panel readability, subgroup-legend, and annotation contracts now that grouped-local support-domain composite has absorbed the current paper-facing explanation-scene gap
...
- the current omics-native lower bound now includes `GSVA/ssGSEA` heatmaps, a shared-order pathway-enrichment dotplot baseline, a bounded up-to-two-panel volcano baseline, a bounded oncoplot mutation-landscape baseline, a bounded CNV recurrence-summary baseline, a bounded mutation-plus-CNV genomic-alteration landscape baseline, and a bounded driver-centric genomic-alteration consequence follow-on baseline; broader omics composites and richer enrichment compositions remain follow-on slices
...
- expand beyond the current GSVA + enrichment + volcano + oncoplot + CNV-summary + genomic-alteration lower bound into enrichment-bar and broader genomic-composite families as real paper demand appears
