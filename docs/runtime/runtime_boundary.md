...
这份文档定义 `MedAutoScience`、`Hermes gateway cron`、`MAS Runtime OS` 与 `MedDeepScientist` 在当前架构下的边界。

## 一句话版本

`MedAutoScience` 是唯一研究入口与 research gateway，外层由单一 MAS app skill 承接稳定 callable surface；`MAS Runtime OS` 是默认 runtime/backend owner；`Hermes gateway cron` 是当前已实现的默认 scheduler adapter，不是研究运行中心；`MedDeepScientist`（仓库名 `med-deepscientist`）只作为 frozen source archive、historical fixture、explicit archive import reference / backend audit reference；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。
...
| Supervisor Scheduler | 多久触发一次外环检查、stale / crash 后谁叫醒 MAS | 当前实现是 `Hermes gateway cron` adapter；目标语义是可替换 scheduler contract |
...
最优方向不是新增一个 MDS 式 resident daemon，也不是继续把 `Hermes` 写成架构中心。当前应把 canonical 语义收敛为 `MAS supervision scheduler contract`：

- MAS 定义唯一 tick payload、运行顺序、幂等性、并发 / 去重、失败投影和 status schema。
- `Hermes gateway cron` 只是该 contract 的当前 adapter。
- 后续如果要消除 Hermes 对本地运行的必要依赖，应新增正式 `local_os_scheduler` adapter，而不是复活旧 workspace-local host service；该 adapter 必须与 Hermes adapter 写同一类 status / SLO / latest-run projection。
- 若 Hermes 继续作为默认 adapter，它的产品价值必须来自跨 workspace registry、失败可见性、session history、统一 dashboard / OPL 集成、remote / hosted runtime 和权限审计，而不是“实现 cron”本身。
