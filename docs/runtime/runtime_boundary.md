...
`MedAutoScience` 是唯一研究入口与 research gateway，`Hermes` 是默认 outer runtime substrate owner，`MedDeepScientist`（仓库名 `med-deepscientist`）是 controlled research backend；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。
