...
`MedAutoScience` 是唯一研究入口与 research gateway，外层由单一 MAS app skill 承接稳定 callable surface；`Hermes gateway cron` 是默认 supervision scheduler owner；`MAS Runtime OS` 是默认 runtime/backend owner；`MedDeepScientist`（仓库名 `med-deepscientist`）只作为 frozen source archive、historical fixture、explicit legacy diagnostic / backend audit reference；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。

Current reading note：这里的 `Hermes` 默认指 `Hermes gateway cron` 对 MAS one-shot tick 的托管 owner；optional external `Hermes-Agent` hosted target 必须另有 readiness proof，不能被写成默认 study truth 或 runtime truth。
...
- `MedDeepScientist`
  - 只负责 source provenance、historical behavior fixture、explicit backend audit / legacy diagnostic
