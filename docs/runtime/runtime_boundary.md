...
这份文档定义 `MedAutoScience`、`MAS Runtime OS`、`MAS supervision scheduler contract`、当前 `Hermes gateway cron` adapter 与 `MedDeepScientist` 在当前架构下的边界。

## 一句话版本

`MedAutoScience` 是唯一研究入口与 research gateway，外层由单一 MAS app skill 承接稳定 callable surface；`MAS Runtime OS` 是默认 runtime/backend owner；`MAS supervision scheduler contract` 是外层监管调度 owner；`Hermes gateway cron` 是当前 active scheduler adapter，不是研究运行中心；`MedDeepScientist`（仓库名 `med-deepscientist`）只作为 frozen source archive、historical fixture、explicit archive import reference / backend audit reference；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。

Current reading note：这里的 `Hermes` 默认指当前 `Hermes gateway cron` scheduler adapter。optional external `Hermes-Agent` hosted target 必须另有 readiness proof，不能被写成默认 study truth、runtime truth、session truth 或默认 executor。
...
| Supervisor Scheduler | 多久触发一次外环检查、stale / crash 后谁叫醒 MAS | `MAS supervision scheduler contract`；当前实现是 `Hermes gateway cron` adapter，目标默认是 MAS-owned local scheduler adapter |
...
- `Hermes gateway cron`
  - 负责当前默认外层定时唤醒：注册、启停和执行 MAS 生成的 supervision tick script
  - 负责提供当前 adapter 的 job registry、schedule state、latest cron session record 与 gateway service liveness
...
截至 `2026-05-09` 的 repo 实现，`Hermes` 在默认 MAS 运行路径里的事实如下：
...
- `med_autoscience.runtime_transport.hermes` 当前是 optional hosted/runtime target 的 readiness / binding contract。默认 `create_quest`、`resume_quest`、`pause_quest`、`stop_quest`、`get_quest_session`、`inspect_quest_live_runtime` 仍 fail-closed，并提示使用 `mas_runtime_core`；它没有成为默认 hosted executor 或 runtime truth。
- `runtime-ensure-supervision --manager systemd|cron|launchd|docker` 当前返回 `retired_workspace_local_service_manager`，不安装 OS service / cron template，也不写 install proof。也就是说，本仓当前没有可用的 local OS scheduler adapter；只有 Hermes cron adapter 是 active scheduler adapter。
...
- `Hermes gateway cron` 只是该 contract 的当前 active adapter。
- 后续要消除 Hermes 对本地运行的必要依赖，应新增正式 `mas_local_scheduler` adapter，而不是复活旧 workspace-local host service；该 adapter 必须与 Hermes adapter 写同一类 status / SLO / latest-run projection。
- Hermes 后续的产品价值应来自 optional hosted / remote runtime、non-GPT executor routing、跨 workspace registry、统一 dashboard / OPL online-management 和权限审计，而不是“实现 cron”本身。
- 在 local adapter 落地前，默认运行可以继续使用 Hermes cron，因为它已在真实 workspace 注册并能投影 job / session / latest run 状态；但文档和 UI 不得把 `Hermes-hosted` 泛化成 MAS 的研究运行 owner。

一步到位的 scheduler contract 与迁移计划见 [Supervision Scheduler Contract](./supervision_scheduler_contract.md)。
