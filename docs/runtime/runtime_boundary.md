...
Current reading note：这里的 `Hermes` 指向外部 `Hermes-Agent` 目标、repo-side outer-runtime seam 与 optional hosted target 的 owner 语义；它不持有 MAS study truth，也不表示当前仓内已经存在独立 external runtime workspace truth。
...
- `Hermes`
  ...
  - 不负责 study governance、publication judgment 或外部 runtime workspace truth 的仓内归属
- `MedDeepScientist`
