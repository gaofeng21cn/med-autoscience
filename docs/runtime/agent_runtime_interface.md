...
- `MAS supervision scheduler contract` = outer supervision scheduler owner；当前 active adapter 是 `Hermes gateway cron`，目标默认 adapter 是 MAS-owned local scheduler
...
Current reading note：本文件里的 `Hermes` 默认读作当前 `Hermes gateway cron` scheduler adapter；它不是 MAS study truth owner、runtime truth owner、session owner 或默认 executor owner。study truth 继续由 `MedAutoScience` 的 study-owned surfaces、controller decisions 与 publication gate 持有。
...
- `main` 合并门与现网切换门：[`merge_and_cutover_gates.md`](../policies/merge_and_cutover_gates.md)
...
- external runtime blocker package：[`../policies/external_runtime_dependency_gate.md`](../policies/external_runtime_dependency_gate.md)
- external gate 未清除前的手工测试与 repo-side 稳定化清单：[`manual_runtime_stabilization_checklist.md`](../policies/manual_runtime_stabilization_checklist.md)
...
在当前架构里，`MedAutoScience` 是唯一研究入口和 `Research Ops Gateway`，`MAS supervision scheduler contract` 是 outer supervision scheduler owner，当前 active adapter 是 `Hermes gateway cron`，`MAS Runtime OS` 是默认 runtime/backend owner；`MedDeepScientist`（仓库名 `med-deepscientist`）只在 frozen source archive、historical fixture、显式 archive import / provenance reference 和上游 intake 里出现。`DeepScientist` 只在上游比较、兼容审计和历史命名里单独出现。
...
- Supervision scheduler tick：`runtime-ensure-supervision --profile <profile>` 注册或刷新当前 adapter 的 MAS-owned tick script；当前 active adapter 是 `Hermes gateway cron`，desired script 依序运行 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`
- Scheduler 注册/刷新入口：`runtime-ensure-supervision --profile <profile>`；当前实现默认刷新 Hermes adapter，后续 local adapter 应保持同构 status / receipt / SLO
...
同一条升级路径会把旧 workspace-local host-service 漂移识别为 retired cleanup evidence。旧 launchd/systemd/cron/docker service 不再作为 active runtime option 保留；当前 `runtime-ensure-supervision` 负责清理旧 service evidence 并注册/刷新 Hermes adapter，后续 MAS-owned local adapter 必须复用同一 scheduler contract。
...
- Scheduler adapter（当前为 `Hermes gateway cron`）或显式 `runtime-supervisor-reconcile --mode developer_apply_safe --apply` 负责持续刷新 outer supervisor evidence；没有新鲜 tick / reconcile evidence，`study-progress` 必须诚实降回 `managed_runtime_supervision_gap`
...
历史上真实 workspace 曾使用 workspace-local launchd service 验证监管刷新；该路径现在只作为历史/debug 事实保留。当前 active contract 以 MAS supervision scheduler contract 为准，当前 adapter 是 Hermes gateway cron one-shot tick，旧 launchd/systemd/cron/docker service 出现时应被清理，不再作为“产品态常驻路径”或替代 scheduler 推荐给 Agent。
...
- `MAS supervision scheduler contract` 负责周期唤醒 MAS one-shot tick；当前 adapter 是 `Hermes gateway cron`
...
当前 closeout 已把 MAS Runtime OS / `mas_runtime_core` 切成默认 runtime/backend owner。`Hermes gateway cron` 的意义是按固定间隔调用 MAS-owned supervision tick script；它不把 external Hermes repo、external MDS repo 或旧 MDS daemon 重新变成默认 runtime truth。
...
当前代码事实也限制了优化方式：`runtime-ensure-supervision --manager systemd|cron|launchd|docker` 仍是 retired fail-closed path，并不是可用替代 scheduler。若要降低 Hermes 对本地运行的必要性，应新增正式 local scheduler adapter，让它调用同一个 MAS tick script 并投影同构 status / SLO，而不是让用户或 workspace 重新维护旧 host service。
