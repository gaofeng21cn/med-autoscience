...
### 2.2 `request_opl_stage_attempt(...)`

- MAS domain refs / controller authorization 入口
- 负责把 `progress_projection(...)` 给出的运行时意图收敛成 MAS owner receipt、typed blocker、domain refs 或 OPL owner-route handoff
- 不直接写 OPL queue、attempt ledger、provider worker、retry/dead-letter、worker liveness 或 current-control-state truth
...
`request_opl_stage_attempt(...)` 的完成语义必须按 owner 顺序解释：
...
- MAS controller action 请求收回当前运行写权，并输出 owner authorization、typed blocker 或 OPL current-control-state handoff refs
- OPL runtime owner 执行实际 pause / worker / queue / current-control-state 写入；MAS 不直接写 generic runtime queue、runtime chat、provider worker 或 worker liveness truth
- quest 保持当前 identity
...
- MAS controller action 请求终止当前 quest 的运行，并输出 owner authorization、typed blocker 或 OPL current-control-state handoff refs
- OPL runtime owner 执行实际 stop / terminal attempt / worker / queue / current-control-state 写入；MAS 不直接停止 provider worker 或改写 generic runtime state
...
- 典型形态是：
  - quest 已 `paused`
  - gates clear
  - `request_opl_stage_attempt(...)` 给出 `RESUME`
...
- `request_opl_stage_attempt(...)` 只有在 caller 显式传入 stopped relaunch 许可时，才允许把它推进到正式 relaunch 动作
...
- direct controller entry:
  - `request_opl_stage_attempt(..., allow_stopped_relaunch=True)`

约束：

- `progress_projection(...)` 不因为 caller 有 relaunch 意图而改变其只读真相
- 显式 relaunch 仍需通过 startup boundary 与 runtime reentry gate
- MAS 只输出 `RELAUNCH_STOPPED` 意图、owner authorization、typed blocker 或 owner-route handoff refs；provider terminal-state release、attempt hydration、queue/retry/dead-letter 与 worker relaunch 归 OPL current-control-state / runtime manager
- 若 OPL runtime owner 接受 relaunch，controller-facing launch report / runtime binding / projection 必须把该动作记为 `relaunch_stopped`，不能降格为普通 `resume`
...
- 任何试图让 `request_opl_stage_attempt(...)` 自动恢复 `stopped` quest 的行为
...
4. `request_opl_stage_attempt(...)` 只能从该恢复点继续，不得根据旧队列或旧 launch summary 隐式重放。
...
dashboard / API / logs / status 都是 observability-only surface：只能读取 orchestrator/controller state，例如 `progress_projection`、`domain_diagnostic_report`、attempt record、runtime event、controller decision artifact 和 publication gate projection。
...
- `domain_diagnostic_report` / status API / dashboard 可以显示 `generated_at`、running/retrying/released counts、`active_run_id`、`session_id`、`worker_host`、`workspace_root`、`run_attempt_phase`、`attempt_count`、last event、last heartbeat、`failure_reason`、`backoff_until`、accepted absolute token totals、runtime seconds 和 rate-limit snapshot。
...
允许的边界是：外部系统可以作为人工可见的 mirror、通知源、排队器或审计引用，但它不能替代 `progress_projection` / `request_opl_stage_attempt` / `study_outer_loop_tick` / `study_decision_record`，也不能成为 study truth 或 publication authority。
...
- 上述 action 是 MAS controller/domain action 名称，不是 MAS 直接 provider-control 写入口；实际 provider pause/stop/relaunch/current-control-state mutation 归 OPL runtime owner
...
- `./study_runtime_orchestration.md`
  - 负责 `progress_projection(...)` / `request_opl_stage_attempt(...)` 的状态机与执行 contract
- `../contracts/runtime_event_and_outer_loop_input_contract.md`
