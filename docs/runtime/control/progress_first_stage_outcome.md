...
监督心跳 / automation 每次只能输出 `root_cause_to_resolution` 记录，不能把 `why_not_progressing`、queue empty、typed blocker 名称、DHD reason 或 `active_run_id=null` 当作结论。记录必须至少包含 `blocker_to_owner_map`、合法入口、预期产物、验证方式和停止条件，并把 root cause 归入以下一类：

| root-cause class | blocker-to-owner map | 合法入口 | 预期产物 | 验证方式 | 停止条件 |
| --- | --- | --- | --- | --- | --- |
| `paper_content_gap` | 对应医学 owner、写作 owner、分析 owner、AI reviewer 或 publication gate。 | MAS owner callable、domain-handler dispatch 或 OPL StageRun provider attempt。 | canonical paper / artifact delta、owner receipt、reviewer / gate delta，或命名缺口的 stable typed blocker。 | fresh `study progress`、DHD readback、owner receipt / changed surface / reviewer record / gate receipt。 | 缺口被 owner 产物消费、转成下一 owner handoff、升级为 human gate，或形成 stable typed blocker。 |
| `gate_evaluator_or_currentness_bug` | MAS controller / read-model / evaluator owner；需要 OPL readback 时映射到 OPL runtime owner。 | DHD apply / MAS authority consumer、evaluator replay、current-control reducer 或 OPL transition runtime readback。 | current work-unit reducer 修正、evaluator / gate replay receipt、currentness readback、route-back evidence 或 typed blocker。 | same identity 的 work unit、route identity、attempt idempotency key、source/eval currentness basis 和 post-apply readback 对齐。 | 当前误判被消费，或输出下一 owner / human gate / stable typed blocker；不得把 parser/test green 写成 paper progress。 |
...
预算关系固定为 mission-first：第一次同签名 route-back 是 handoff；第二次同签名 route-back 仍没有 semantic delta 时，OPL redrive 预算耗尽，不能靠 worker ready、queue present、queue empty、stage attempt id、provider liveness、DHD clean、focused tests 或 docs/contract diff 重置。下一 owner 必须升级为 MAS-owned `paper_mission_semantic_progress_executor` Codex stage，输出 mission delta、stable typed blocker、human gate、带 successor work unit 的 route-back evidence、`CarryForwardRiskReceipt` 或 repair-lane proposal。若升级 stage 也只能发现 transport/control-plane defect，报告必须按 root-cause-to-resolution 表映射 owner、allowed/forbidden write set、验证方式和停止条件；不能把 defect 命名本身当作 paper progress。
...
不能被写成 governed recovery 的路径包括：未声明 manual/fast-lane mode 的 paper-local edit、把 foreground edit 直接声明为 MAS/OPL recovery、重放旧 provider admission、同 work-unit default-executor dispatch、旧 gate replay、用 `active_run_id` / provider completion / queue empty 证明恢复、手写 runtime/study artifact，或把 docs-only 结论当 live paper progress。
