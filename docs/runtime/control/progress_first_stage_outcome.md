...
Artifact-first MVP 是 Progress-first 的第一推进口径：非终局 stage 首先要留下可定位、可审阅、可接力的 stage artifact refs，并汇入 `stage_artifact_index` / Stage Deliverable Index 这类 refs-only projection。typed closeout、read-model/currentness hygiene、evidence-tail 分类和 provider liveness 是增强、审计和调度层；它们证明 artifact delta 是否可消费、是否应进入下一 owner、是否暴露 typed blocker 或是否等待 provider admission，但不能替代 artifact refs 本身，也不能把平台修复写成论文或交付进展。
...
Artifact-first 分账进一步约束 stage MVP：`stage_artifact_index` / Stage Deliverable Index 更新本身只算 locator/projection，除非它引用了新的 canonical paper / artifact delta、owner receipt、reviewer/gate delta、stable typed blocker、human gate、route-back 或 stop-loss。仅刷新 currentness、telemetry、read-model、receipt reconcile、provider liveness 或 evidence-tail 分类时，记为 platform repair / observability；不能写成 artifact-first MVP landed。
