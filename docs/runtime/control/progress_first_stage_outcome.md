...
本 runbook 用于 DM002 / DM003 这类 Progress-first / AI-first 论文线停滞判断：系统已经能看到 receipt、currentness、read-model、provider handoff 或 canonical `NextActionEnvelope`，但 operator 需要判断当前 stage 是否真正前进、是否应把 canonical next action 提交给 OPL transition runtime 或 MAS owner callable，还是仍卡在 hard gate。
...
| `ready_for_owner_action` | canonical `NextActionEnvelope` 给出唯一 owner/action/work-unit，hard gate 缺席，但尚未观察到 provider running proof。 | 提交 canonical envelope 到 OPL transition runtime 或 MAS owner callable；若只写出 handoff，下一 owner 是 OPL transition runtime。 |
...
当 canonical `NextActionEnvelope` 已同时具备以下字段，且 hard gate 缺席时，Progress-first operator 必须把下一步提交给 OPL transition runtime 或 MAS owner callable：
