...
repair-progress follow-up 的 provider admission 必须使用同一 current work-unit identity。`study_progress.current_executable_owner_action.source=repair_progress_projection.mas_owner_repair_execution_evidence` 时，顶层 action 必须直接携带 `work_unit_fingerprint` / `action_fingerprint`，并且 OPL current-control action queue 只允许在 current-control action / owner-route currentness 匹配 MAS current work-unit identity、dispatch 为 ready、action/owner 和 dispatch authority 都合法时生成 provider-admission candidate。ready dispatch 是 callable carrier，不是新的 work-unit truth；旧 typed blocker envelope、stale readiness queue 或同 study 旧 dispatch 不得遮蔽 `return_to_ai_reviewer_workflow -> ai_reviewer` 与 `run_gate_clearing_batch -> gate_clearing_batch` 这两个已由 repair evidence 派生的下一 owner work unit。

`owner-route-reconcile` 写 `opl_current_control_state_handoff` 时必须把当前 `action_queue` 同步物化为 root `provider_admission_candidates[]`，并挂到对应 study 的 `provider_admission_candidates[]`。该物化不是新的 authority：它复用 MAS provider admission gate，只有 current-control action / owner-route currentness 与 MAS current work-unit identity 匹配，且 dispatch ready、action/owner 和 dispatch authority 都合法时才输出 OPL 可消费 candidate。OPL intake 因而可以直接从 root candidates 启动 queue / attempt / provider transport，不再从旧 sidecar task、旧 gate replay 或 stale top-level queue 猜测下一步。
