...
repair-progress follow-up 的 provider admission 必须使用同一 current work-unit identity。`study_progress.current_executable_owner_action.source=repair_progress_projection.mas_owner_repair_execution_evidence` 时，顶层 action 必须直接携带 `work_unit_fingerprint` / `action_fingerprint`，并且 OPL current-control action queue 只允许在 action、owner、ready dispatch、dispatch authority 和 fingerprint 全部匹配时生成 provider-admission candidate。当前允许的 family 是 `return_to_ai_reviewer_workflow -> ai_reviewer` 与 `run_gate_clearing_batch -> gate_clearing_batch`；旧 typed blocker envelope、stale readiness queue 或同 study 旧 dispatch 不得遮蔽这两个已由 repair evidence 派生的下一 owner work unit。
