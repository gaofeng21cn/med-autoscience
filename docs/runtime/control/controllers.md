...
`domain-health-diagnostic --apply` 的 command-side obligation authority 是 `domain_health_diagnostic_obligation_actuator`。当 DHD 已看到 `paper_recovery_state.next_safe_action` 指向可执行 recovery obligation，或已看到 ready dispatch / materialization preview，命令结束时必须对同一 study/current obligation 形成 exactly-one outcome：`provider_admission_pending`、`running_provider_attempt`、`owner_receipt_ref`、`typed_blocker_ref`、`human_gate_ref` 或 `route_back_evidence_ref`。若一个都没有，DHD 必须 fail closed，写 `artifacts/controller/domain_health_diagnostic_obligation_actuator/latest.json` typed control blocker 与 `history.jsonl`，并在 `managed_study_obligation_actuator_outcomes[]` / `managed_study_actions[].dhd_apply_postcondition` 投影该 blocker。materializer 只提供 callable carrier / provider-admission 输入，currentness/projection 只读 actuator 结果；它们都不能替代 command-side outcome。
