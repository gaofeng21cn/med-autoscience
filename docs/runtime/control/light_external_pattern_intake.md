...
本 runbook 用于 operator 把 Light 这类外部科研技能包的可复用模式吸收成 MAS-native 的 stage-quality、stage knowledge、reviewer briefing、stage skill companion content 和 advisory repair hint 规则。它服务当前 work unit 的 `next owner delta`：让下一 owner 更快拿到 verified refs、撞车风险、审稿拒稿预演、fresh evidence gate 和可消费内容模板，而不是把外部项目变成 MAS 的 runtime、router、knowledge authority 或 quality gate。

本 runbook 的 source-of-truth intake snapshot 是 Light upstream 默认分支 HEAD `d71033733bc4b357f3a2f0b6460ad7d8da070954` 加本地缓存 `/tmp/light0305-light-current` / `/tmp/light0305-light-intake` 中的 clean-room evidence；上一轮 intake head 是 `731c786e9434e8f6f9cd5284293003115c5b66c7`。核心证据包括：`README.md`、`WHATS_INCLUDED.md`、`CONVENTIONS.md`、`MODE_REGISTRY.md`、`ROUTER.md`、`skills/light-orchestrator/**`、`skills/light-idea-generation/SKILL.md`、`skills/light-idea-critique/SKILL.md`、`skills/light-self-review/SKILL.md`、`skills/light-citation/references/locator_audit.md`、`skills/light-literature-search/assets/litreview_template.md`、`skills/light-literature-search/scripts/prisma_flow.py`、`skills/light-research-plan/templates/experiment-matrix.md`、`skills/light-research-plan/templates/reproducibility-checklist.md`、`skills/light-paper-drafting/references/operational_modes.md`、`skills/light-review-rebuttal/templates/*.md`、`skills/light-figure-drawing/SKILL.md`、`skills/light-figure-drawing/scripts/figure_export.py`、`skills/light-figure-drawing/references/figure_integrity.md`、`skills/light-figure-drawing/scripts/figure_integrity_lint.py`、`skills/light-figure-planning/SKILL.md`、`skills/light-paper-polishing/references/argument_review.md`、`skills/light-paper-polishing/scripts/style_fingerprint.py`、`_verification_log/*.md`、`code_assets/` 和 `databases/`。这些材料只作为 external pattern evidence 读取；其中的技能名、知识库路径、评分模板、API 状态、库版本、项目记忆、期刊规格表、font defaults 和示例项目都不是 MAS current truth。

Latest upstream drift: `d71033733bc4b357f3a2f0b6460ad7d8da070954` 只吸收为 figure integrity / export QA advisory pattern：缩放到期刊栏宽后的有效字号校验、导出文件物理尺寸回读校验、多 panel 唯一科学问题 / 反冗余检查。MAS 落点是 `progress_first_external_learning_contract.compiled_visual_region_qa_contract` 的 advisory QA checks；它不引入 Light `figure_export.py`、不调用 Light runtime、不让图表 QA advisory 成为默认 admission 或 publication gate。
...
| `light-orchestrator` checkpoint gates | `adopt_template` | 借用“用户决策点 / 机器确认点”表达方式，映射到 MAS human gate、owner receipt readiness、AI reviewer request、route-back 或 typed blocker。 | 把每个 skill step 都升级成前置关卡，会把 Progress-first ordinary flow 卡在预检循环。 | 非 hard-gate 的 checkpoint 只进入 reviewer briefing 或 next owner hint；已有 nonbinding Codex route context 或 MAS owner-consumed current owner action 时继续，不为补 checklist 阻塞。 |
...
| source map / literature matrix content | `adopt_template` | 作为 `stage_knowledge_packet` / source-readiness content surface：记录 PICO、库名、原始检索式、命中、去重、筛选、纳入、排除标准、方法族、代表工作、资源 license 和 gap。 | 把 Light `databases/`、示例综述或检索表写成 MAS source truth，会把外部知识库变成第二 truth source。 | 缺 source map / literature matrix 默认只生成 source-readiness advisory gap；只有当前 route-required source/evidence ref 缺失且无法形成 owner output 时，才升级为 typed-blocker candidate。 |
| experiment matrix / reproducibility plan content | `adopt_template` | 作为 experiment / analysis-campaign content surface：实验 ID、假设、数据集、baseline、指标、随机种子、完成判定、派生数据规格、环境、锁版本、数据划分、命令和 lineage refs。 | 把通用 ML reproducibility checklist 套到医学研究而不绑定 cohort、endpoint、comparator、统计计划和安全数据边界，会产生医学语境错配。 | 缺实验矩阵不阻塞无关写作、reviewer 或 source owner action；只有当前 owner 要执行实验 / 复现 / artifact lineage 且 route-required ref 缺失时，才命名 blocker candidate。 |
...
| manuscript / rebuttal operational modes | `adopt_template` | 作为 writing / review entrypoint 的 bounded content surface：outline-only、section-redraft、self-review、simulated-review、revision-response 等只约束本次输出粒度、输入、诚信门和可交付 ref。 | 把 mode 当作 MAS route table 会制造 router pollution；把 self-review 当成独立 reviewer verdict 会绕过 AI-first quality gate。 | 缺 mode registry 或 mode 模板不阻塞 current owner action；只有当前 owner 缺 target surface / write boundary / reviewer-comment ref 时才升级。 |
...
| `light-figure-drawing` figure integrity / export QA | `adopt_template` | 作为 display / reviewer warning：提示 axis truncation、errorbar disclosure、dual-axis、3D/chart-junk、缩放后有效字号、导出物理尺寸和多 panel 冗余等图表风险。 | 静态 lint warning 或外部导出脚本被误用为 artifact mutation authority 或 publication gate，会误伤有效可解释图表，也会把 Light 脚本漂移带进 MAS runtime。 | warning 只进 display reviewer hint 或 layout work unit；只有当前 figure claim、caption disclosure、compiled artifact 或 publication hard gate 需要该 ref 时才升级。 |
| figure export / panel template content | `adopt_template`; upstream spec watch `watch_only` | 作为 display content surface：figure manifest、target journal / column、final size、editable-font export、vector / bitmap refs、DPI、panel map、caption binding、source-data/statistics/export QA。 | 期刊规格、font availability、Matplotlib export behavior 和示例 panel layout 都会 drift；直接复制 `JOURNAL_SPECS` 或 `.mplstyle` 会带来 license/API drift 与医学展示语境错配。 | export / panel 模板缺失默认只产生 display advisory gap；只有当前 owner 要提交图件、caption disclosure 或 publication hard gate 需要这些 refs 时才升级。 |
...
可选 skill-content refs 包括 `source_search_discipline_ref`、`data_access_sink_ref`、`citation_edge_retraction_ref`、`citation_locator_audit_ref`、`prisma_flow_reconciliation_ref`、`figure_manifest_check_ref`、`experiment_matrix_backlink_ref`、`statistical_analysis_triage_ref`、`overclaim_lint_warning_ref`、`argument_review_hint_ref`、`figure_integrity_warning_ref` 和 `style_fingerprint_hint_ref`。它们只在当前 owner payload 明确提供同类 section，或当前 delta 把同类 ref 声明为 route-required 时进入 bundle；缺失默认是 advisory gap。
...
- 当当前 nonbinding Codex route context 或 MAS owner-consumed current owner action 已有 owner、action、work unit、required output surface、source refs 和 forbidden write boundary，且 Light-derived advisory 缺失或陈旧时，operator 继续推进对应 next action；缺 advisory 不阻塞不相关 dispatch。旧 `current_executable_owner_action` 只能作为 superseded diagnostic carrier，不能单独恢复 admission。
...
- 只有当前 delta 的 route-required ref 缺失，且缺失会影响 source/data/evidence、owner-route identity、forbidden write boundary、irreversible mutation 或 reviewer/publication hard gate 时，才能生成 typed blocker。typed blocker 必须命名缺失 ref family，例如 `collision_check_ref_required`、`fresh_evidence_gate_ref_required`、`verified_asset_ref_required`、`refusal_rehearsal_ref_required`、`citation_locator_audit_ref_required`、`prisma_flow_reconciliation_ref_required`、`figure_manifest_check_ref_required`、`experiment_matrix_backlink_ref_required` 或 `figure_integrity_warning_ref_required`，并指明解除 owner。
