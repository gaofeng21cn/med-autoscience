...
- 哪些入口是正式依赖面
- `progress_projection` / `ensure_study_runtime` 如何推进
...
- [`src/med_autoscience/controllers/study_runtime_router.py`](../../../src/med_autoscience/controllers/study_runtime_router.py)
  - 作为 facade，保留正式入口 `progress_projection(...)` / `ensure_study_runtime(...)`
...
- `progress_projection(...)`
  - 只读，返回序列化后的 `ProgressProjectionStatus`
- `ensure_study_runtime(...)`
  - 读状态、跑 preflight、按决策执行 transport，并最终返回序列化后的 `ProgressProjectionStatus`
...
这意味着 `progress_projection(...)` 给出的 decision 还不是最终执行动作；
