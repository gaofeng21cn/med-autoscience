...
MAS direct/local legacy diagnostic 不再定义外部 scheduler 或 MAS-owned supervision tick script。需要在开发环境做一次性处理时，应调用 `medautosci domain route-reconcile --profile <profile> --mode developer_apply_safe --apply` one-shot，或按 `supervisor-scan -> supervisor-consume -> supervisor-execute-dispatch` 手动执行调试链。OPL-hosted provider 应通过 MAS sidecar export/dispatch 和 owner receipt 进入同一 domain surface。直接调用 `watch-runtime --max-ticks 1` 只覆盖外环检查，不覆盖同 tick 的 scan / consume / execute-dispatch 全链；上面的 scan 命令只用于调试单步扫描。
