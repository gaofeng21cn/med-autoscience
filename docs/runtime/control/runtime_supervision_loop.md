...
`medautosci runtime ensure-supervision` 默认注册或刷新 MAS-owned `local` adapter；macOS 会写入 LaunchAgent、tick script、install proof 和 scheduler receipt。显式 `--manager hermes` 会走 optional Hermes gateway cron adapter。已退役的 `systemd|cron|launchd|docker` manager 不再是公开 CLI 选项，也不再有 direct-call 兼容 payload；旧 workspace-local host service 文件或 loaded 状态只作为 Hermes optional adapter status 中的 `retired_cleanup_evidence` 读取和清理。清理后必须回到 MAS scheduler contract。
