...
Review currentness v2 逐 lane 记录 `fresh` 或 `reused_unchanged_scope`。fresh receipt 必须绑定当前 generation、manifest、candidate admissions，以及 OPL 物化的 immutable reviewer-input snapshot exact ref；snapshot binding 包含 Framework materialization owner、全 false domain-authority boundary，以及 `member_id`、role、hash、size 的完整 reviewed-member inventory，不含 locator/path/checkout/host/model/skill/runtime/version/timestamp。reviewer 必须读取 snapshot，不得在判断期间回读 live locator。reuse 只能在 receipt 自身 scope identity 与当前 MAS scope identity 完全一致时成立，并保留 origin generation、manifest、review request、review receipt 和 origin candidate receipt provenance。medical、statistical、reference、publication 四条专业 lane 还必须绑定 origin candidate semantic scope digest；candidate、evidence、claim scope 或 decision code 改变时，即使 manuscript bytes 不变也不得复用；新 generation 只要这些语义和 lane scope 均未改变，仍可复用。旧 origin receipt 没有 snapshot binding、或 binding 缺 materialization owner/false-authority metadata 时，均形成 lane-specific quality debt，不能产生 owner receipt，不再 grandfather。每个 lane 的 `current_rubric_ref` 都携带 rubric 内容哈希；fresh 或 reuse receipt 的 rubric 与当前 rubric 不一致时，只使该 lane 失效。scope、rubric、snapshot binding 或普通 review currentness 缺失/陈旧只让受影响 lane route back/形成 `completed_with_quality_debt`，无论是否处于 finalization 都允许普通 Stage transition，`typed_blocker=null`，同时继续阻断 quality/publication/export/submission-ready claims，绝不成为 hosted-action liveness rejection。MAS 在一次判定中聚合全部受影响 lane，并通过稳定排序的 `affected_review_lanes` 给出每个 lane 的 reason 和 resume condition，避免 display、publication、exact-byte-package 等依赖同一变化的 lane 被拆成多轮串行复审。`render_environment_and_font_manifest` 只保留在 root/exact-byte provenance，不进入 display 或 publication professional scope；真实视觉变化由 figure/PDF/page pixels 捕获。`exact_byte_package` 始终覆盖完整 root inventory，不能缩窄。

Display page cache 只复用证据候选。ScholarSkills 在固定 raster contract 下生成有序逐页 pixel SHA-256；cache key 只包含 ordered page pixel projection（page number、width、height、pixel format、pixel hash）、raster contract、`review_scope_sha256` 与 rubric 内容 SHA-256。OPL 持久化 cache 与 `opl_review_evidence_cache_receipt`；MAS 仍要求 fresh reviewer invocation、fresh receipt 和最终 judgment。cache hit 不得产生 verdict、owner receipt、typed blocker 或 readiness；缺 origin reviewer invocation/evidence 时不可复用。路径、checkout、host、timestamp、model/skill/runtime/version 只作 cache provenance；PDF metadata 仍可能因当前 display scope 绑定 PDF raw bytes 而改变 `review_scope_sha256`，本页像素证据本身稳定不代表整个 lane 可复用。renderer/tool 变化最多造成 pixel cache miss，不直接失效 scientific scope。
