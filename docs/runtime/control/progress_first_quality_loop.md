...
Review currentness v2 逐 lane 记录 `fresh` 或 `reused_unchanged_scope`。fresh receipt 必须绑定当前 generation、manifest、candidate admissions，以及 OPL 物化的 immutable reviewer-input snapshot exact ref；snapshot binding 只含 `member_id`、role、hash、size 的完整 reviewed-member inventory，不含 locator/path/checkout/host/model/skill/runtime/version/timestamp。reviewer 必须读取 snapshot，不得在判断期间回读 live locator。reuse 只能在 receipt 自身 scope identity 与当前 MAS scope identity 完全一致时成立，并保留 origin generation、manifest、review request、review receipt 和 origin candidate receipt provenance；旧 origin receipt 没有 snapshot binding 时仍可按此 grandfather，不要求五条专业 lane 全量重跑。每个 lane 的 `current_rubric_ref` 都携带 rubric 内容哈希；fresh 或 reuse receipt 的 rubric 与当前 rubric 不一致时，只使该 lane 失效。scope、rubric 或 fresh snapshot binding 缺失只让受影响 lane route back/形成 quality debt，不能拒绝 hosted action；在非最终 Stage，单纯 snapshot debt 允许 `completed_with_quality_debt` 继续推进，但继续阻断 quality/publication/export/submission-ready claims。MAS 在一次判定中聚合全部受影响 lane，并通过稳定排序的 `affected_review_lanes` 给出每个 lane 的 reason 和 resume condition，避免 display、publication、exact-byte-package 等依赖同一变化的 lane 被拆成多轮串行复审。`render_environment_and_font_manifest` 只保留在 root/exact-byte provenance，不进入 display 或 publication professional scope；真实视觉变化由 figure/PDF/page pixels 捕获。`exact_byte_package` 始终覆盖完整 root inventory，不能缩窄。
