...
| AcademicForge / Claude Science | `owner_surface_landed` for skill-first descriptor refs and MAS Scholar Skills external skill source；live compute remains OPL evidence | AcademicForge `54a2f333973147a1fd703caea6f12252e1f227d6` 的 Claude Science 32 个 skill 只按 Skill-first pattern 吸收：figure-style / figure-composer / paper-narrative / literature-review / pdf-explore 进入现有 MAS Scholar Skills 专业 skill；结构预测、蛋白设计、基因组、单细胞、indication dossier、PDF explorer 和 scientific compute runner 作为可同步外置专业 Skill；compute / SSH / Modal / endpoint lifecycle 归 OPL Runway / Connect substrate。 | 不把专业 Skill 硬转成 MAS 脚本，不引入 Claude Science runtime，不让 Skill 写 MAS truth、owner receipt、typed blocker、publication eval、controller decisions、current package、cloud credentials 或 provider attempts。缺 optional specialist skill、compute provider 或 endpoint 默认 fail-open；只有 current delta 命中 source/data/evidence/authority/human hard gate 时才 route-back。 |
