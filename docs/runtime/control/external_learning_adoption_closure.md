...
- 有 friction guard：缺失、失败、超时、低置信或预算耗尽时默认不阻断 canonical `NextActionEnvelope` / current owner action，除非命中命名 MAS hard gate；旧 `current_executable_owner_action` 只能作为 superseded diagnostic carrier 读取。
...
- 已存在完整 canonical `NextActionEnvelope` / current owner action 时，缺外部 advisory 不阻断 dispatch；旧 `current_executable_owner_action` 不能单独作为阻断或 dispatch authority。
