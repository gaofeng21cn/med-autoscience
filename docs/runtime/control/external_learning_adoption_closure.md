...
| Light | `sidecar_or_worker_landed` for MAS materializer refs only | `light-advisory-materialize` 只生成 verified / collision / refusal / fresh-evidence advisory refs，不引入 Light runtime。 | 继续保持 refs-only、budgeted、fail-open；缺 advisory 不阻断 owner action。 |
