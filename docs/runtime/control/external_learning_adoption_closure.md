...
- 有 friction guard：缺失、失败、超时、低置信或预算耗尽时默认不阻断 nonbinding Codex route context / MAS owner-consumed current owner action，除非命中命名 MAS hard gate；旧 `current_executable_owner_action` 只能作为 superseded diagnostic carrier 读取。
...
| AcademicForge / Claude Science | `owner_surface_landed` for skill-first descriptor refs and MAS Scholar Skills external skill source；live compute remains OPL evidence | AcademicForge `54a2f333973147a1fd703caea6f12252e1f227d6` 的 Claude Science 32 个 skill 只按 Skill-first pattern 吸收：figure-style 进入 dedicated `medical-figure-style`，figure-composer 进入 dedicated `medical-figure-composer`，二者都经 `medical-figure-design` orchestrator route 回到 active `mas-scholar-skills.display` 模块；paper-narrative / literature-review / pdf-explore 进入对应 MAS Scholar Skills 专业 skill；结构预测、蛋白设计、基因组、单细胞、indication dossier、PDF explorer 和 scientific compute runner 作为可同步外置专业 Skill；compute / SSH / Modal / endpoint lifecycle 归 OPL Runway / Connect substrate。 | `medical-figure-style` / `medical-figure-composer` 只是外部 ScholarSkills 子能力和触发面，输出 style/layout QA、candidate refs 与 route-back hints；不新增 MAS active module 或 authority。不把专业 Skill 硬转成 MAS 脚本，不引入 Claude Science runtime，不让 Skill 写 MAS truth、owner receipt、typed blocker、publication eval、controller decisions、current package、cloud credentials 或 provider attempts。缺 optional specialist skill、compute provider 或 endpoint 默认 fail-open；只有 current delta 命中 source/data/evidence/authority/human hard gate 时才 route-back。 |

### AcademicForge / Claude Science

本次吸收固定一条实现口径：`Skill` 是能力入口，`kernel/helper` 只是确定性小动作，`contract/registry` 只限边界。Claude Science 的价值主要在专业 Skill 保留 AI 判断、环境诊断、失败分层和现场修复能力；如果把它们机械改成 MAS 脚本，会降低上限，也会违背 MAS / OPL 的 `AI First, Contract Light` 原则。

MAS 侧只记录三类机器面：

| MAS surface | 学到的模式 | 边界 |
| --- | --- | --- |
| `academicforge_claude_science_skill_first_pack` | 图件、文献、PDF、论文叙事的 Skill-first playbook；现有八个 MAS Scholar Skills 专业 Skill 消费这些模式。 | descriptor-only、refs-only，不执行外部 runner，不授权 paper progress。 |
| `academicforge_life_science_specialist_skills` | AlphaFold / Boltz / Chai / ESMFold / DiffDock、ProteinMPNN、Borzoi / Evo2、scGPT / scVI、indication dossier 等专科能力。 | optional specialist skill pack；缺失不阻断 ordinary progress，产物进入 owner-gate request。 |
| `academicforge_scientific_compute_runner_skill` | env spec、SSH/SLURM、Modal、endpoint lifecycle 的诊断与操作 playbook。 | OPL Runway / Connect 管 provider、credential、submit/wait/harvest、endpoint 和 receipt；MAS 只消费 execution refs。 |
...
- 已存在完整 nonbinding Codex route context / MAS owner-consumed current owner action 时，缺外部 advisory 不阻断 dispatch；旧 `current_executable_owner_action` 不能单独作为阻断或 dispatch authority。
...
- 正式 typed blocker 必须由 MAS owner surface、Codex CLI semantic route selection、independent reviewer/auditor、human gate 或 typed blocker materializer 产出；sidecar completion 不能自己阻断 ordinary progress spine。
