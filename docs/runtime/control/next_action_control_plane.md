...
任何旧 work-unit allowlist、progress projection、OPL transport 队列 / StageAttempt 推断、materializer exact-id registry 都不能重新成为默认 next action owner。它们只能作为 history、provenance、diagnostic、migration input 或 no-resurrection guard。
...
| OPL transport 队列 / StageAttempt 推断 | provider transport observation、TransitionReceipt input、stale/running diagnostic。 | MAS owner action selection、paper progress、publication-ready、submission-ready。 |
...
## 2026-06-30 legacy producer noncanonical boundary

直接物理删除所有 legacy owner-successor producer 的候选已被拒绝吸收：active diagnostic / owner-successor projection caller 仍存在，过宽删除会破坏 current-control/current-work-unit regression。当前可维护边界改为先关闭 authority 复活路径：

- `terminal_next_forced_delta` 产出的 owner-successor projection 固定携带 `canonical_next_action_authority=false` 与 `projection_role=legacy_owner_successor_diagnostic`。
- `current_executable_owner_action_parts/*` 中仍声明 `_authority_boundary()` 的 legacy owner-successor producers 统一携带同一 noncanonical boundary。
- `tests/test_next_action_legacy_authority_retirement.py` 扫描所有这类 producer，防止新增 producer 忘记声明 noncanonical boundary。

该边界允许 legacy producer 继续服务 diagnostic、owner-successor provenance、no-resurrection guard 或 migration readback，但禁止它们成为默认 `NextActionEnvelope`、默认 owner selector、dispatch authority、submission-ready evidence 或 paper progress proof。后续若要物理删除，必须先证明对应 diagnostic consumer 已迁移到 `StageOutcome -> NextActionEnvelope`、OPL receipt readback 或 MAS owner-consumption surface，或者该 consumer 已删除；不能通过新增 compat shim、fallback alias 或第二套 projection 延长旧路径。

Physical deletion probe：`origin/main@af282e179` 上的窄删除试验删除 `gate_followthrough.py`、`publication_repair.py`、`stage_artifact_index.py`、`stage_kernel_readiness.py` 后，retirement guard 自身通过；同时 fresh main 证明 `tests/study_progress_cases/current_executable_owner_action.py` 已经是 73 failed / 55 passed。该结果把删除门从“让旧聚合入口重新变绿”改为 `retire_or_migrate_active_diagnostic_consumer`：这些 producer 不是 default authority，旧聚合入口也不是 current contract。下一步是把仍有价值的 behavior test 迁到 canonical envelope 或 MAS owner-consumption readback，然后删除 producer 与旧测试；没有保留价值的旧 selector 行为直接随 diagnostic consumer 退役。

## 2026-06-30 completion audit current wording

当前完成度审计按两本账读取：`source/control-plane` 已落地默认 NextAction authority、fail-closed 行为、synthetic route guard 和合同/文档口径；`live acceptance` 仍必须由对应 owner/live surface 单独证明。

| 项目 | 当前完成度 | 证据口径 | tail / 禁止替代 |
| --- | --- | --- | --- |
| 默认 NextAction authority | `done` for source/control-plane | 默认链路只承认 `StageOutcome -> NextActionEnvelope`，旧 exact-id、stage-native workspace next-action、legacy projection 和 attempt id fallback 不能再选择默认 next action。 | 不声明 DM002/DM003 live owner action、provider running、paper progress 或 submission-ready。 |
...
| synthetic route / false-claim guard | `done` for regression surface | synthetic fixture 和 no-resurrection guard 只证明旧面不能复活为默认 authority。 | synthetic route 不等于真实 OPL StageRun、真实 provider attempt、paper artifact delta 或 DM002/DM003 governed acceptance。 |
| contract docs / current wording | `done` for docs/current-plan | 本文、`docs/status.md`、`docs/active/mas-ideal-state-gap-plan.md` 和 `docs/decisions.md` 固定完成度分账和 forbidden claim boundary。 | docs、tests、`scripts/verify.sh` 或 `git diff --check` 只证明文档/源码约束，不得写成 publication-ready、submission-ready 或 current-package ready。 |
| historical producer physical deletion | `tail_open` | 仍有 active diagnostic / owner-successor projection consumer；删除门是 `retire_or_migrate_active_diagnostic_consumer`。 | 不为保留 producer 新增 compatibility shim、fallback alias 或第二套 projection。 |
| 大文件/source morphology 边界 | `tail_open` | `paper-mission` 大入口继续按 source-governance lane 收薄；这是可维护性边界。 | 大文件收薄不写 runtime state、owner receipt、typed blocker authority、paper body、publication eval、controller decision 或 current package。 |
| UI / workbench live 标签 | `tail_open` | Workbench / product shell 只能显示同一 envelope、OPL receipt readback 或 MAS owner consumption 的标签。 | UI 可见、trace 可见、zero worklist 或 operator text 不能成为 progress、submission-ready 或 runtime-ready evidence。 |
...
| 旧面退役 | tombstone / retired docs / caller audit 证明旧 work-unit allowlist、progress projection、OPL transport 推断、exact-id registry 不再作为 default next action selector；源码切片需证明默认 surface 不再返回 legacy projection，route gate 不再以 exact id 授权，仍保留的 legacy owner-successor producer 明确 `canonical_next_action_authority=false`。 |
...
当前状态是 `source_control_plane_landed_live_evidence_tail_open`：StageOutcome 后的 next-action 默认控制面已经收口到 `NextActionEnvelope` / `action_family` / OPL transition receipt 边界；旧 stage-native workspace next-action、legacy projection、exact-id route allowlist、loose current-work-unit fallback 和 OPL attempt-id fallback 不能再决定默认 next action；缺 envelope 时默认 fail closed；synthetic route / false-claim guard 和合同文档口径已落地。仍保留的 legacy owner-successor producers 已被标记为 noncanonical diagnostic/provenance producer。仍开放的是 historical producer physical deletion、大文件/source morphology 边界、UI/workbench live 标签和 live evidence tail：物理删除已由 73-failure probe 证明仍有 active diagnostic consumer，必须先迁移或删除这些 consumer；真实 DM002/DM003 runtime receipt、owner receipt、typed blocker、human gate、route-back、artifact delta、publication / submission readiness 必须 fresh 读取对应 owner surface，不能由源码退役、focused tests、`scripts/verify.sh` 或文档状态代替。
