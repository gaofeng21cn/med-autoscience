...
- `domain_action_request_materializer_parts.legacy_next_action_authority` 已物理退役；仍在 active source 中运行的是 `legacy_next_action_retirement_guard`，只负责把旧 authority/source carrier fail closed 到 canonical `NextActionEnvelope`，不是 legacy authority producer。
