...
| historical producer physical deletion | `partial` | `owner_action_diagnostics.repair_progress`、`owner_action_diagnostics.non_advancing_terminal_closeout` 和 `owner_action_diagnostics.action_types` 已物理删除；仍有 `owner_action_diagnostics.paper_recovery` active diagnostic / owner-successor projection consumer；删除门是 `retire_or_migrate_active_diagnostic_consumer`。 | 不为保留 producer 新增 compatibility shim、fallback alias 或第二套 projection。 |
