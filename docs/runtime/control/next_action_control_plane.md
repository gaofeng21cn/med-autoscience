...
- `study_progress` 在 canonical `NextActionEnvelope` 存在时，会从默认顶层 read model 清除旧 `current_work_unit`、`current_executable_owner_action`、provider-admission candidate / pending counter、transition-request candidate / pending counter、`progress_first_monitoring_summary` 和 `current_execution_envelope`。这些旧面不再能在 envelope 之后复活为第二 current owner/action；顶层只保留 `legacy_next_action_authority_retired` 诊断说明和 `stage_native_current_owner_action` 的 diagnostic-only 视图。
