...
当前完成度审计按两本账读取：`source/control-plane` 已落地默认 NextAction authority、fail-closed 行为、synthetic route guard、OPL handoff identity fixture 和合同/文档口径；`live acceptance` 仍必须由对应 owner/live surface 单独证明。2026-07-01 fresh verification 覆盖：NextAction / synthetic / authority-route / domain-materializer / legacy-retirement focused tests `59 passed`，current-work-unit / autonomy / execution-envelope / legacy-active-path retirement `32 passed`，PaperRecovery contract/state `37 passed`，provider-admission current-control `72 passed`，domain-action materializer `30 passed`，`make test-meta` `465 passed`，`scripts/verify.sh` passed。后续 source morphology follow-through 又把 `paper_recovery_state.py` 的 terminal closeout projection 拆入 `paper_recovery_state/terminal_closeout_projection.py`，入口降到 995 行；`display_surface_materialization/materialize.py` 拆到 816 行，`ai_reviewer_publication_eval.py` 拆到 593 行，`runtime_health_kernel.py` 拆到 858 行；`existing_projection_refresh.py` 拆到 881 行，`mission_summary.py` 拆到 843 行，`provider_admission_projection.py` 拆到 992 行；`domain_action_request_lifecycle.py` 拆出 AI reviewer record refs/currentness discovery helper 后降到 771 行；`projection_payload_assembly.py` 拆出 typed-blocker successor 和 current-work-unit typed-blocker status 后降到 709 行；`provider_admission_current_control.py` 拆出 transition-request candidate 和 projection helper 后降到 989 行；`opl_provider_attempts.py` 拆出 runtime helper 后降到 981 行；clean integration worktree 中 `make line-budget` 当前报告 3 个 advisory。该数字只说明 repo-wide source morphology tail，不声明 live paper/runtime acceptance。
...
| 大文件/source morphology 边界 | `partial` | `paper-mission` 大入口已继续拆出 one-shot migration、drive、materialized readback 和 candidate-package readback helper；`paper_mission_commands.py` 当前已降到 805 行，低于 1000 行 preferred advisory；`runtime_surface_retirement.py` 已把 runtime-health validator 拆到 `runtime_surface_retirement/runtime_health_kernel_validators.py`，主文件降到 951 行；`paper_recovery_state.py` 已把 terminal closeout projection 拆到 `paper_recovery_state/terminal_closeout_projection.py`，主文件降到 995 行；`display_surface_materialization/materialize.py` 已拆出 workspace / contract-figure helpers并降到 816 行；`ai_reviewer_publication_eval.py` 已拆出 planning / record materialization helpers 并降到 593 行；`runtime_health_kernel.py` 已拆出 event / diagnostic projection helpers 并降到 858 行；`existing_projection_refresh.py` 已拆出 current-control helpers 并降到 881 行；`mission_summary.py` 已拆出 paper-mission payload / receipt projection helpers 并降到 843 行；`provider_admission_projection.py` 已拆出 non-advancing / stage-ref helpers 并降到 992 行；`domain_action_request_lifecycle.py` 已拆出 AI reviewer record refs/currentness discovery helper 并降到 771 行；`projection_payload_assembly.py` 已拆出 typed-blocker successor 和 current-work-unit typed-blocker status 并降到 709 行；`provider_admission_current_control.py` 已拆出 transition-request candidate 和 projection helper 并降到 989 行；`opl_provider_attempts.py` 已拆出 OPL subprocess / provider readiness / candidate task-attempt filtering helper 并降到 981 行。clean integration worktree 中 provider-attempt focused direct tests `28 passed`，`make line-budget` 当前仍报告 3 个 advisory。 | 大文件收薄不写 runtime state、owner receipt、typed blocker authority、paper body、publication eval、controller decision 或 current package；line-budget advisory 也不能替代 live acceptance。 |
