...
| 大文件/source morphology 边界 | `partial` | 当前 retirement slice 已把 `runtime_surface_retirement` 收为 `__init__.py` + `authority_flags.py`，并删除旧 runtime-health/private-residue/work-order/rollup helper；`scripts/verify.sh` 当前只报告 `stage_knowledge_plane/__init__.py` 一项 preferred-boundary advisory。 | 该行只描述 source morphology；line-budget、focused tests 或 repo-source retirement 都不能替代 live acceptance。 |
