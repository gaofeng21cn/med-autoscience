...
- 精确 work-unit id / fingerprint 必须用于 MAS dispatch/currentness 与 owner receipt consumption，并作为 OPL `StageRun` / `TransitionReceipt` 的 retry、dedupe、repeat suppression、provider admission 和 terminal closeout identity；generic lifecycle 状态由 OPL 持有。
