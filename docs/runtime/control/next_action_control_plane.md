...
Physical deletion probe：`origin/main@af282e179` 上的窄删除试验删除 `gate_followthrough.py`、`publication_repair.py`、`stage_artifact_index.py`、`stage_kernel_readiness.py` 后，retirement guard 自身通过，但 `tests/study_progress_cases/current_executable_owner_action.py` 为 73 failed / 55 passed。该结果把当前删除门固定为 `active_diagnostic_consumer_present`：这些 producer 不是 default authority，但仍承担 progress-first / current-owner diagnostic successor 行为。下一步不是继续直接删文件，而是逐条迁移对应 behavior test 到 canonical envelope 或 MAS owner-consumption readback，再删除 producer 与测试。
