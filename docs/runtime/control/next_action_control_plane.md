...
| historical producer physical deletion | `partial` | `owner_action_diagnostics.repair_progress` 已物理删除；仍有 active diagnostic / owner-successor projection consumer；删除门是 `retire_or_migrate_active_diagnostic_consumer`。 | 不为保留 producer 新增 compatibility shim、fallback alias 或第二套 projection。 |
