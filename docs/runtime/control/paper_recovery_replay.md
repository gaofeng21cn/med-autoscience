...
DM002 / DM003 类卡点的 historical replay 仍可按旧 diagnostic chain 复盘，但该链已被 `Codex CLI selected declared stage` 取代；nonbinding route context 只作可选输入：

`legacy stage-route/current-work-unit diagnostic chain`

任何跳过 `MAS closeout consume_or_reject` 的边都只能是诊断；旧链里的 `current_work_unit` / `current_execution_envelope` 也只能作为 diagnostic drilldown，不能覆盖 nonbinding Codex route context。`queue_empty`、`active_run_id`、transport status、trace/span、read-model projection、stale selected dispatch、old route-back packet 和 provider completion 不能生成 recovery obligation、owner receipt、typed blocker、provider admission 或 paper progress。

`stage_packet_not_current_selected_dispatch` 只表示该旧 dispatch 的 currentness 失效，不阻止 Codex 选择其他 declared stage。安全出口是新的 Codex-selected stage attempt、显式 owner-surface authority action、human gate 或 route-back evidence；同一 stale work unit 不得自动 redrive，但其 artifact/diagnostic 必须保留为后续输入。
