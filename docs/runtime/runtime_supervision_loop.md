...
行为等价口径另见 [MDS Behavior Equivalence Gap Matrix](../references/mds_behavior_equivalence_gap_matrix.md)。当前 outer loop 能证明的是默认 MAS 独立监管和 scheduler-bound recovery；它不声明与旧 MDS resident daemon、WebSocket terminal streaming 或 connector background threads 完全等价。
