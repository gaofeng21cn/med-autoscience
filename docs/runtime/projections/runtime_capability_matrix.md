...
| `mas-scholar-skills.*` | `one-person-lab` for lifecycle/execution; `MedAutoScience` for owner gate | `codex_cli_or_hosted_capability_executor` | `one-person-lab` | `stage_affordance_contract_landed` | Module catalog、professional Skill 正文、gallery 和 optional specialist truth 来自外部 `mas-scholar-skills` repo。OPL package lifecycle 解析必需 dependency closure，Atlas/Pack/Stagecraft 在 Stage Tool Affordance Boundary 内发现 descriptor，Runway 执行并返回 receipt/artifact refs；MAS 只保留 refs-only execution receipt consumer、file-materialized package refs consumer、owner-gate request builder 与 no-authority guard。旧 `scientific_capability_registry` summary/inventory/index/resolve/invoke/CLI ABI 是 internal migration residue，不是 V2 public/default surface。 | 真实论文 truth 仍需要 MAS owner receipt、quality gate receipt、source readiness verdict、route-back evidence、stable typed blocker、human gate 或 canonical artifact delta；descriptor/readback/helper policy 不证明模块已运行、optional skill 已安装、helper 已执行或 owner gate 已接受。 |
