...
- live worker / live attempt 必须由 OPL `current_control_state` 证明；当 workspace-level `artifacts/supervision/opl_current_control_state/latest.json` 对当前 `study_id` 同时给出非空 `active_run_id`、`running_provider_attempt=true` 和 `runtime_health.runtime_liveness_status=live` 时，MAS `runtime_liveness_audit.source=opl_current_control_state_provider_attempt` 可以把该 OPL provider attempt 投影为 live liveness，并用该 `active_run_id` 覆盖 stale `.ds/runtime_state.json` 的空 active run。该 dominance 同样适用于 `.ds/runtime_state.json` 仍停留在 `waiting_for_user` 的 stale read：fresh OPL live provider attempt 表示执行权仍由 managed runtime 持有，`progress_projection` 必须投影 `execution_owner_guard.supervisor_only=true`，前台只能监督，不能直接写 study truth 或 paper package。若 OPL queue projection 未暴露当前 study 的 running task，但 OPL `family-runtime attempt list/inspect` 对同一 workspace/study/stage 给出 running provider-backed stage attempt，MAS 可以把该 attempt ledger projection 作为同等只读 liveness evidence，source 标记为 `opl_family_runtime_attempt_inspect`，并继续使用 `opl-stage-attempt://<stage_attempt_id>` 作为 active run ref。该投影只是 OPL-owned liveness evidence；MAS `runtime_liveness_audit`、`worker_running` 或 `active_run_id` 不能据此声明 owner receipt、publication readiness、artifact authority 或 package freshness。
...
- 同一 active run epoch 内的 launch / recover / relaunch attempt retry budget 由 OPL attempt ledger / retry-dead-letter substrate 计算；MAS 只能投影对应 OPL readback 与 diagnostic blocker，不能在本地创建第二套 retry counter。runtime-health consumer 需要 identity-bound OPL readback：`runtime_liveness_audit`、`opl_current_control_state`、`current_control_state`、`stage_run_readback` 或 `latest_terminal_stage_log` 必须携带与 snapshot `active_run_id` / worker liveness identity 相交的 `active_stage_attempt_id`、`stage_attempt_id`、`active_run_id`、`active_workflow_id`、`provider_attempt_ref` 或 StageRun ref；仅顶层 payload 字段或不匹配 identity 不得授权恢复决策。
