...
- 顶层 `current_work_unit` / `progress_first_monitoring_summary.current_work_unit` 是 Progress-first MVP 的 canonical current work-unit projection。它由 `current_work_unit` reducer 选择，并以 `status` 固定表达 `executable_owner_action`、`running_provider_attempt`、`typed_blocker` 或 `blocked_current_work_unit`。多个 candidate producer 可以继续存在，例如 `current_executable_owner_action`、`current_execution_envelope`、OPL action queue、`domain_transition` 和 `next_forced_delta`；Portal、workbench、MCP compact projection、materializer 和 `study-state-matrix` 必须优先消费 canonical projection 派生 `next_owner`、`controller_action`、`work_unit_id`、`next_forced_delta`、admission pending 与 running proof。旧散字段只能作为 provenance / ignored diagnostic，不得重新猜“下一步”。`admission_pending` 只表示 owner/provider admission 待启动；`running_provider_attempt` 只能来自 strict OPL/provider running proof，不能由 `opl-stage-attempt://...` handle、active_run provenance 或 stale handoff queue 推断。
- `current_execution_envelope` 是 canonical execution state；`current_execution_evidence` 是 refs-only evidence / drilldown surface。若 terminal default-executor typed closeout 已消费当前 action、`terminal_closeout_consumed=true`、或 `consumed_action_queue` 与当前 action identity 匹配，envelope 必须投影 typed blocker，`current_executable_owner_action` 必须清空，evidence action queue 也不得把同一旧 action 重新显示为下一步。只有未被 consumed handoff 覆盖、且来源为 `study_progress.next_forced_delta.owner_action` 的 action 才能作为 diagnostic evidence 保留。`domain_closeout_provided_incomplete_user_stage_log`、`medical_prose_review_request_rehydrate_required`、`stage_packet_not_current_selected_dispatch` 和 `typed_closeout_packet_required` 的 owner 固定为 `one-person-lab`；embedded domain `next_owner` 只说明被阻塞 work unit，不说明解除者。
