...
- 默认 outer supervision scheduler owner 是 OPL `opl_provider_runtime_manager` / `opl_family_runtime_provider`；MAS local scheduler surface 已物理退役为 tombstone/provenance refs，不再每 `300` 秒调用 MAS one-shot supervision tick，也不再暴露公开 status/remove/ensure command；Hermes gateway cron 只在显式选择时作为 optional adapter
...
- OPL provider/runtime manager 默认周期调用；显式 legacy local 或 Hermes adapter 只用于 diagnostic / optional path
