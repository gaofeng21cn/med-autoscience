...
`build_ai_first_operations_dashboard_summary()` 生成 `ai_first_operations_dashboard_summary`。它是 operations-facing read model，固定 `read_model=ai_first_operations_dashboard_read_model`，并声明 `product_frontdesk`、`workspace_cockpit` 与 `study_progress` 消费同一只读摘要。该 surface 只投影已有 progress/runtime/quality/artifact snapshot，不 refresh cache、不 route-back、不 materialize artifact、不修改 runtime。
