...
Currentness / running proof 的控制面口径固定为 `desired -> current -> status` 三层：desired 是 MAS owner action / owner-route intent，current 是 canonical `current_work_unit` / `current_execution_envelope`，status 是 OPL handoff、runtime health、stage progress log 和 action queue 的观测投影。status 只能在携带可比较的 `action_type`、`work_unit_id`、`work_unit_fingerprint` / `action_fingerprint` 或等价 currentness basis，并与 current work unit 匹配时，升级为 `running_provider_attempt`；裸 `active_run_id`、`active_stage_attempt_id`、Temporal workflow、transport liveness、stage progress log、action queue residue 或 read-model refresh 只能作为 observability。缺 comparable identity 时，Progress-first 必须 fail closed：保留 stale active attempt 诊断，继续显示 current owner action / typed blocker / parked state，而不能把旧 provider attempt 写成论文进展。

Stage artifact index 是 derived projection 和 artifact-first fallback。只有在没有 canonical `current_work_unit`、没有 current execution envelope、没有 OPL current-control action queue / typed blocker / running attempt 的情况下，它才可以派生 `current_executable_owner_action`。一旦 current owner surface 已经存在，stage artifact index 不得反向覆盖当前 owner、不得制造 provider admission、不得让 selected dispatch / packet residue 替代 currentness identity。read-model projection 可以暴露这些 refs 供 operator drilldown，但不能生成 domain authority。
