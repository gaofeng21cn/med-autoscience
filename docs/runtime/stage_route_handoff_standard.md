...
- OPL 读取 MAS `family_transition_spec`、`study_state_matrix`、route contract、stage control plane、`route_transition_contract` 和 `stage_graph_handoff` refs/hints。
- OPL 执行 transition / guard / matrix runner，并在 queue / stage attempt ledger 中记录 transport state、provider receipt、closeout、dead-letter、retry 和 human gate。
...
4. owner-callable handoff 必须声明 `allowed_write_surfaces`、`forbidden_surfaces`、`required_closeout_packet` 和 `completion_boundary.provider_completion_is_domain_ready=false`；runtime intake 只暴露既有 `opl_domain_progress_transition_request`，后续运行态只消费其 OPL readback。
...
Currentness / running proof 的控制面口径在 2026-06-29 之后由 [Next Action Control Plane](./control/next_action_control_plane.md) 收口：default desired action 只来自 `StageOutcome -> NextActionEnvelope`，current transport 只来自同 identity 的 OPL transition / StageRun / attempt receipt，status 是 OPL handoff、runtime health、stage progress log 和 action queue 的观测投影。旧 `current_work_unit` / `current_execution_envelope` 只能作为 diagnostic / provenance / drilldown 帮助解释历史 currentness，不再是 default next-action authority。status 只有在携带可比较的 `action_type`、`work_unit_id`、`work_unit_fingerprint` / `action_fingerprint` 或等价 currentness basis，并与 canonical `NextActionEnvelope` identity 匹配时，才能升级为 `running_provider_attempt`；裸 `active_run_id`、`active_stage_attempt_id`、Temporal workflow、transport liveness、stage progress log、action queue residue 或 read-model refresh 只能作为 observability。缺 comparable identity 时，Progress-first 必须 fail closed：保留 stale active attempt 诊断，继续显示 canonical next action、typed blocker 或 parked state，而不能把旧 provider attempt 写成论文进展。

Stage artifact index 是 derived projection 和 artifact-first diagnostic input。它不得派生默认 `current_executable_owner_action`，也不得在缺 `NextActionEnvelope` 时补造 owner action。旧 stage artifact / selected dispatch / packet residue 只能作为 operator drilldown、migration diagnostic 或 no-resurrection evidence；不得反向覆盖 canonical next action、不得制造 provider admission、不得替代 currentness identity。read-model projection 可以暴露这些 refs 供 operator drilldown，但不能生成 domain authority。
...
`CarryForwardRiskReceipt` 是同一 ordinary progress handoff 家族的非 fatal 风险回执。它必须绑定 current work-unit identity、target surface refs、source/currentness refs、originating reviewer/gate/repair refs、severity、why_nonfatal、deferred gap refs、next owner、next required delta、reentry condition、forbidden claims 和 MAS/OPL authority boundary。它不得携带 `fatal_blocker` severity；fatal 必须转为 stable `TypedBlocker`、`human_gate_ref` 或 route / owner redesign。OPL Agent Lab 的通用 loop-risk evaluator 只能提供 repeat / budget / stale / duplicate 的 generic signal；MAS 必须消费该 signal 并作医学 severity policy 决策，OPL 不能替 MAS 签 quality verdict、publication/submission readiness、artifact authority 或 carry-forward acceptance。
...
Budget-exhausted closeout 不能直接等同于 safe stop-loss。若 no-op / repeat suppression / stop-loss 候选来自 review / repair budget exhaustion，closeout 必须记录 severity decision：fatal -> `TypedBlocker` / `human_gate_ref` / route redesign；nonfatal -> `CarryForwardRiskReceipt` + next owner handoff。DM002 / DM003 类 `anti_loop_budget_exhausted` 是当前过渡样本，只说明同一 work-unit redrive 被压住；理想终态需要 successor work-unit、carry-forward receipt、owner receipt、human gate、route-back evidence 或 stable typed blocker 来表达下一步。

Mission-first non-advancing route-back 的 MAS domain policy 固定在 `contracts/mas-paper-study-stage-pack.json#/mission_first_non_advancing_route_back_policy`。`contracts/foundry_agent_series.json` 只声明 OPL Foundry canonical refs、policy fingerprint 和 MAS domain delta，不再复制该 domain policy。当 `route_back`、`domain_gate`、`owner_answer_shape=route_back_evidence_ref`、`paper_mission_stage_route_domain_gate_pending` 或 `non_advancing_apply` 反复返回时，operator 必须比较 `semantic_progress_signature`，而不是继续看 OPL queue / worker / stage attempt 是否活跃。签名至少绑定 study、PaperMissionRun、stage、current owner、action、work-unit id / fingerprint、route identity、attempt idempotency、required output surface 和 accepted answer shape；queue id、stage attempt id、active run id、worker readiness、provider liveness、retry count、read-model generated_at、测试结果、docs 或 contract commit 只属于 observability，不改变签名。
...
这条规则不授权 MAS 手写 OPL queue / StageRun / provider attempt，也不授权 OPL 代替 MAS 判断医学语义推进。它同样不把重复 route-back、domain gate、worker ready、queue blocked、provider liveness、focused tests、docs 更新或 contract landed 写成 DM002/DM003 ready、runtime-ready、submission-ready、publication-ready、provider running 或 paper progress。
...
```text
current_owner_delta
  -> opl_domain_progress_transition_request / OPL StageRun receipt
