...
- 展示优先级：用户消息、执行回合和最近回合必须优先于普通 runtime lifecycle event，避免用户真正关心的对话被低层事件淹没。
