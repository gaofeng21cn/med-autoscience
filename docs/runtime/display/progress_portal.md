...
- JSONL bounds：`user_messages.jsonl`、`turn_receipts.jsonl`、`mas_runtime_events.jsonl` 等长期日志只能作为 bounded read-model 输入；大文件必须 tail-bounded 读取并在 source refs 暴露截断元数据。Portal 需要展示最近可解释的对话/运行证据，不需要也不能为 UI 全量解析历史日志。
