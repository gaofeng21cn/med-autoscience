...
- Progress-first operator 列：每条 study 行应显示 `next_forced_delta` 的用户可见摘要，并用 delta 分账区分 `deliverable_progress_delta` / `paper_progress_delta`、`platform_repair_delta`、typed blocker / human gate / stop-loss。refs-only worklist 清零、receipt verify、stage replay accounting、typed-blocker payload record 只能显示在 operator diagnostic 或证据区，不能进入“最近论文进展”或“论文推进中”状态；`receipt_consumed` 只能作为已消费回执 / 等待下一 owner projection 的观测状态，不能映射成 `ready_owner_action` 或等待 pickup。
