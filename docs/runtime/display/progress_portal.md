...
Status: `read-model display builder; local materializer physically retired; OPL App workbench cutover pending`
...
Related operator runbook: `docs/runtime/control/progress_first_stage_outcome.md`

## 入口结论

`MAS Progress Portal` 是面向医生、PI 和研究团队的 workspace-local read-model / no-App evidence 展示入口。当前 MAS 仓只保留 payload / HTML 的读模型构建函数和 refs 字段；旧本地 materializer、study page 文件写入和 hosted package 生成模块已物理退役。workspace helper、本地 HTTP service、`--serve --enable-actions` 与 `ops/mas/bin/start-web` 已退役，不再作为 active/default caller。Portal 负责 progress / status / blocker / artifact pickup 的只读展示，不负责 terminal attach、日志流、执行器对话、runtime drilldown、pause/resume/stop 或长期 workbench shell；这些运行观察、物化、索引和控制能力归 OPL `current_control_state` / provider attempt projection / OPL App Runtime Workbench / domain-handler owner-route handoff。旧 MDS WebUI 的实时 console 和历史 MAS 私有 Console 材料只保留为 history/provenance。Progress Portal 的历史本地 evidence 位置是：
...
新 workspace 不再生成旧 `start-web`、`progress-portal` helper 或 MAS-owned hosted package。维护者需要本地 evidence 时，应由 OPL hosted workbench / StateIndex / runtime readback 消费 MAS refs 后物化；外部 MDS WebUI 只能通过显式 explicit archive import reference / backend audit 路径启动，医生/PI 默认不再在旧 WebUI 与 MAS Portal 之间判断进度 truth。
...
| --- | --- | --- |
| Workspace shell / per-study pages | `retired_local_materializer` | 旧 `ops/mas/progress/index.html` / per-study 文件写入和 hosted package 生成模块已从 MAS 源码删除；当前 MAS 只构建 read-model payload / HTML 字符串和 refs 字段，实际 shell/索引/物化归 OPL。 |
...
| Artifact-first mission summary | `landed_read_model_projection` | `study_progress.artifact_first_mission_summary` 是 Portal / study workbench 的默认论文叙事入口。顶层必须暴露 `mission_state`、`current_objective`、`latest_artifact_delta`、`next_owner_or_human_decision` 和 `platform_diagnostics`；内部 `paper_mission_run` 对齐 `contracts/paper_mission_run_contract.json` / `paper-mission-run.v1` 和 `med_autoscience.paper_mission_run.PaperMissionRun`。该 read model 只投影 mission candidate / owner handoff 边界，不写 owner receipt、typed blocker、human gate、publication eval、controller decision、current package 或 runtime queue/provider attempt。 |
| Progress-first operator visibility | `landed_read_model_projection` | Portal / OPL workbench 的 operator 面优先消费 `study_progress.progress_first_monitoring_summary.next_forced_delta`，把下一次必须产生的可见 delta 显示成 study-scoped work unit。该字段只解释下一步需要论文/交付物 delta、平台修复、typed blocker、human gate 还是 stop-loss；refs-only ledger closeout、typed-blocker accounting、stage replay receipt accounting 和 owner-route currentness 修复只能进入 operator diagnostic / platform repair 区，不能写成论文推进。若 Stage 08 stable medical-paper readiness blocker 已绑定 concrete `publication_eval` gaps，operator 面必须显示派生的 `run_quality_repair_batch` / `run_gate_clearing_batch` required delta、gap ids、expected output contract 和 source refs，而不是继续把 `complete_medical_paper_readiness_surface` 或 repeated not-ready proof 当作下一步；重复 readiness completion 只能作为 `superseded_by_readiness_blocker_derived_repair` diagnostic。若 matrix/progress summary 的 `execution_state_kind` 或 `monitoring_status` 为 `receipt_consumed`，Portal / OPL workbench 应显示为已消费 receipt、等待下一 owner projection 或 observability evidence；不得显示为 `ready_owner_action`、等待 owner pickup 或论文推进中。 |
| Progress-first stage outcome | `landed_operator_contract` | Portal / OPL workbench 的 study 行和单篇页只能把非终局 stage 显示为 `running`、`ready_for_owner_action`、`waiting_human`、`blocked_with_typed_owner`、`terminal_success` 或 `terminal_stop_loss`。`provider_attempt_start_requested` / `admission_requested` 只能显示为 handoff/admission requested；只有 `provider_attempt_running_proven=true` 且存在 active attempt/run/workflow ref 时才显示 running。hard gate 只显示 human gate、forbidden authority write、missing owner callable、missing source/data、irreversible mutation；telemetry/currentness/read-model/wording 缺口进入 diagnostics。 |
...
Progress-first operator visibility 必须把 `CarryForwardRiskReceipt` 投影成明确的 carry-forward 风险状态，而不是隐藏在 diagnostics。当前 read-model 字段为 `progress_first.carry_forward_risk`；HTML operator section 至少显示 `carry_forward_risk=advanced_with_carry_forward_risk` 或 `blocked_for_fatal_risk`、`carry_forward_severity=<severity>`，并保留 `ordinary_progress_may_advance`、`readiness_claim_allowed=false`、`next_route_policy`、`forbidden_claims` 和 authority boundary 供 OPL workbench 消费。该显示只证明普通进度已经带风险推进或 fatal risk 已阻断；不得写成 publication-ready、submission-ready、quality-clean、paper closure、artifact/package authority 或 live runtime currentness。
...
- `MAS` 只负责 domain-owned progress portal payload / HTML 的读模型构建和 refs 字段，不再负责本地写出 `artifacts/runtime/progress_portal/latest.json`、`ops/mas/progress/index.html` 或 hosted packaging manifest。
- hosted packaging / file materialization / workspace shell 归 OPL hosted workbench / StateIndex / runtime manager；MAS refs 不能被升级为 workbench shell authority、runtime readiness 或 publication readiness。
...
- 单篇 study 的默认 Progress Portal / workbench read surface 是 `artifact_first_mission_summary`；`current_owner_delta`、DHD、owner-route、dispatch、PaperRecovery、storage/currentness 和 provider-admission traces 只能作为 diagnostics / migration / provenance drilldown。
...
- `contracts/runtime/legacy-active-path-tombstones.json#/tombstoned_surfaces[dhd_owner_route_dispatch_paper_recovery_default_paper_mainline]` 固定旧 DHD / owner-route / dispatch / PaperRecovery 默认主路径退役边界：这些 surface 不能作为 product/default domain-handler paper mainline、paper progress、publication-ready、runtime-ready、provider-running、DM002 complete 或 DM003 complete claim。
...
默认单篇叙事必须先使用 mission summary：`mission_state` 说明 mission 状态，`current_objective` 说明当前论文目标，`latest_artifact_delta` 说明最近真实 artifact delta，`next_owner_or_human_decision` 说明下一 owner 或人类判断，`platform_diagnostics` 折叠 DHD/currentness/storage/dispatch/PaperRecovery 修复。若只有平台修复或 read-model repair，没有 mission artifact delta 或 owner outcome，页面必须把它显示为 diagnostics，不得显示为论文推进。
...
- Progress-first operator 列：每条 study 行应显示 `next_forced_delta` 的用户可见摘要，并用 delta 分账区分 `deliverable_progress_delta` / `paper_progress_delta`、`platform_repair_delta`、typed blocker / human gate / stop-loss。stable medical-paper readiness blocker 已有 concrete publication gaps 时，这一列应显示所需正文/证据/reviewer/gate 修复 work unit 和 required output，而不是重复证明论文 not ready。refs-only worklist 清零、receipt verify、stage replay accounting、typed-blocker payload record 只能显示在 operator diagnostic 或证据区，不能进入“最近论文进展”或“论文推进中”状态；`receipt_consumed` 只能作为已消费回执 / 等待下一 owner projection 的观测状态，不能映射成 `ready_owner_action` 或等待 pickup。
...
Portal 历史上曾生成 read-model payload 和展示文件，例如：
...
- live-readiness 尾项由 `contracts/runtime/mas-runtime-surface-retirement-inventory.json#/surfaces/progress_portal_study_workbench_overview_action_projection/opl_workbench_shell_readback_tail` 固定：只有 OPL Workbench Shell action transport、OPL current-control 和 OPL `DomainProgressTransitionRuntime` live readback，或 no-active workbench projection action caller proof，才能关闭该尾项；MAS Portal payload、`next_system_action` 摘要、operator intent refs、repo no-authority guard 和 focused tests 只能证明本仓 projection 边界，不证明 OPL Workbench Shell takeover、current-control ready、runtime readiness 或 production readiness。repo-source 删除另由 active caller migrated/deleted、replacement parity、no-forbidden-write、tombstone/provenance 和 no resurrection guard 验收。
...
- `src/med_autoscience/controllers/progress_portal_parts/read_model_materializer.py` 与 `hosted_package.py` 已物理退役且无 alias
- `artifacts/runtime/progress_portal/hosted_package.json` MAS-owned hosted packaging manifest 已退役；若需要 hosted package，由 OPL owner surface 生成/索引
- `ops/mas/progress/index.html` 与 per-study HTML 是历史静态 evidence 输出路径，不再由 MAS 本地 materializer 写出
