...
- 退役尾项由 `contracts/runtime/mas-runtime-surface-retirement-inventory.json#/surfaces/progress_portal_study_workbench_overview_action_projection/opl_workbench_shell_readback_tail` 固定：只有 OPL Workbench Shell action transport、OPL current-control 和 OPL `DomainProgressTransitionRuntime` live readback，或 no-active workbench projection action caller proof，才能关闭该尾项；MAS Portal payload、`next_system_action` 摘要、operator intent refs、repo no-authority guard 和 focused tests 只能证明本仓 projection 边界，不证明 OPL Workbench Shell takeover、current-control ready 或 physical delete allowed。
