...
私有/历史 runtime 面的当前边界：`runtime_lifecycle.sqlite`、worker lease、runtime session/recovery intent、MAS runtime transport、MAS 私有 console、conversation read model、terminal attach/read-model 和 Portal/Console combined soak 已按旧控制面退役；当前 MAS 侧只保留 `domain_authority_refs_index`、storage maintenance / artifact storage audit 这类 refs-only authority/provenance surface。它们可以暴露 owner receipt ref、study/domain status ref、storage/artifact locator ref、cleanup receipt ref 或 typed blocker；不能声明 generic runtime owner、generic persistence/lifecycle engine、generic terminal attach runtime、generic cleanup policy、restore-ready verdict 或 paper closure verdict。OPL / shared family layer 持有通用 lifecycle index、provider attempt ledger、runtime drilldown、storage lifecycle policy、queue/retry/dead-letter 和 workbench primitive；MAS 只保留 domain truth、quality/artifact authority、owner receipt 与 blocker 语义。
...
已完成 runtime implementation plans 和旧 outer-loop 设计说明归档到 [history/runtime](../history/runtime/)。这些材料只作 provenance，不是 active backlog，也不能用来重开旧 MDS daemon、WebUI、Hermes-first default path、workspace-local service path 或 MAS-owned generic runtime/platform work。
