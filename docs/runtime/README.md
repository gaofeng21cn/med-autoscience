...
| file | current handling |
| --- | --- |
| [supervision_scheduler_contract.md](supervision_scheduler_contract.md) | MAS-owned supervision scheduler contract、Hermes adapter 降级目标和 local adapter 落地计划；保持 active。 |
...
| [progress_portal.md](progress_portal.md) | MAS-native fixed progress entrance：静态快照 + 可选本地只读实时服务；作为 landed read-model / display artifact contract 保持 active。Live console observation 由 Runtime OS live-console contract 单独持有，Portal 只接 thin link/ref。 |
...
这些文件记录旧 P2 / cutover / migration 过程。当前 default runtime closeout 以 MAS Runtime OS + MAS-owned supervision scheduler contract + behavior-equivalence matrix 为准；`Hermes gateway cron` 只是当前 active scheduler adapter。这些 plan 不是默认真相入口，也不得作为重开外部 MDS runtime dependency、workspace-local service 或 Hermes-centered runtime dependency 的依据。后续做 inbound link 检查后可迁入 `docs/history/runtime/`。
