...
`docs/runtime/` 是 MAS 运行时合同、控制面、读模型和仍在执行的 runtime implementation plan 的工作层。默认阅读顺序是先读 active contracts，再读 control surfaces，随后读 read models / projection contracts；只有需要落地尚未完成的 runtime work 时，才读 still-active implementation plans。projection / generated-human docs 只作为人类展示或派生物合同；deprecated/history candidates 在确认 inbound links 后再迁入 `docs/history/runtime/`。
