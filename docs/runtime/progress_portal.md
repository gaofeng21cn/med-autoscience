...
2026-05-09 fresh assessment：Portal 的 repo capability、workspace helper、OPL handoff、Live Console link 和 real-workspace read-only soak 已经可用；当前主要缺口是默认 IA 仍以 workspace overview 为中心。下一轮优化不应再讨论“是否恢复 MDS WebUI”，而应直接做 MAS-native per-study/per-paper 工作台：study selector、deep link、单篇论文 Overview、Route/Decision Trail、Path/Stage、Runtime/Run、Conversation、Terminal/Logs、Artifacts 和 Source Refs。Route/Decision Trail 是 P0，不应被泛化成当前 stage 摘要；它必须解释研究路线、分支、失败或阻塞原因、转向理由和当前 active / winning path。
...
- P0：`portal-route-decision-trail`，把研究路线、分支、失败/阻塞原因、转向理由、superseded path、active/winning path 和对应 source refs 放到单篇论文视图。
