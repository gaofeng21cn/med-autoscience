...
- `Hermes gateway cron` 是当前已实现的默认 supervision scheduler adapter，每 300 秒调用 MAS-owned supervision tick script；desired script 依序运行 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`。
...
- MAS 外层行为：Hermes gateway cron 定时唤醒 MAS-owned supervision tick script，runtime state、event、owner route 和 progress read-model 由 MAS durable surface 持有。
...
- 需要长时唤醒时，当前正确入口是 Hermes gateway cron adapter 或未来受控 scheduler adapter，不是 workspace-local launchd/systemd/cron/docker service。
...
- MAS supervision scheduler contract；Hermes gateway cron 只是当前 active adapter。
