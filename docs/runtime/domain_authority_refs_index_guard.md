...
- Fresh `study progress` for DM002 and DM003 after the storage migration succeeded through the MAS profile and new runtime-state surface. DM002 remained `current_stage=queued`, `paper_stage=publishability_gate_blocked`, `active_run_id=null`, next action `produce_ai_reviewer_publication_eval_record_against_current_inputs`; DM003 remained `current_stage=queued`, `paper_stage=analysis-campaign`, `active_run_id=null`, next action `materialize_stage_artifact_delta`. These are paper owner/quality route states, not storage-location blockers.
