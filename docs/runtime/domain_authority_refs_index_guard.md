...
- 当前 active callers 只可发出 MAS domain authority refs source-adapter proof：owner receipt refs、typed blocker refs、artifact/source/status locator refs、restore/archive provenance refs；`db_path` 不再触发默认 SQLite 持久化；
- SQLite 仅可作为显式 opt-in replay / local inspection 的 MAS domain authority refs index，不得作为默认 active runtime/read-model 写面；
...
| 面 | 当前状态 | owner |
| --- | --- | --- |
| Domain authority refs index | `source_adapter_emitted_default` | body-free refs emitted for OPL StateIndexKernel; `artifacts/runtime/domain_authority_refs.sqlite` only with explicit persistence opt-in |
...
- `medautosci runtime-lifecycle-payload-retention --db <runtime_lifecycle.sqlite> --apply --cold-store-root <cold-store> --retire-cold-payloads --compact`
