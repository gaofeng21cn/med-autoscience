...
MAS 侧已删除私有 `paper_progress_transition_kernel` 方向，保留 `PaperProgressPolicyAdapter`。domain diagnostic provider-admission 候选现在只携带 MAS `paper_progress_policy_result` 和 `opl_domain_progress_transition_request`；MAS 明确 `mas_can_authorize_provider_admission=false`、`mas_can_run_fixed_point_reconciler=false`、`mas_can_own_event_log_or_outbox=false`、`mas_can_create_opl_outbox_record=false`，并把 transition runtime owner 指向 OPL。`opl_domain_progress_transition_request` 只能表达 MAS domain policy request，不携带 `projection_metadata`、StageRun identity、OPL event、OPL outbox、read-model generation metadata 或旧 current-control command；MAS projection metadata 只能留在 `paper_progress_policy_result`、candidate/read-model projection 和 study progress 派生面。OPL intake 负责把 clean request normalize 成 OPL-owned command / event / transactional outbox / StageRun readback。root `action_queue`、study `current_executable_owner_action` 和 `current_work_unit` 三类候选入口都必须消费同一 adapter / request shape，不能把 outbox record 作为 MAS 生成的新 authority。

domain diagnostic `obligation_actuator` 现在只作为 MAS policy / owner-answer readback 和 fail-closed typed blocker 投影：它可以识别 MAS owner callable receipt、MAS typed blocker、human/route-back diagnostic 和 OPL readback，但不能自签 provider admission。`provider_admission_pending` 在 MAS 侧必须有 OPL `opl_domain_progress_transition_runtime_live_readback` 完整 shape 支撑：`identity`、`causality`、`authority_boundary`、`exactly_one_outcome`、`projection_metadata` 与同一 transition 的 event/outbox/StageRun live readback 缺一不可；只有 MAS transition request、OPL command/event/outbox log、或只有旧式 `event_id` / `outbox_item_id` / `stage_run_id` projection 时，domain diagnostic 必须写 `NonAdvancingApply` typed blocker。MAS public consumer 不再暴露 `command_event_log.jsonl` readback extractor；command/event/outbox log 只能作为 OPL-owned provenance / replay input，由 OPL 发布 live readback surface 后才能进入 MAS admission。旧 `paper_autonomy_supervisor_apply` 不保留 alias；旧 `current_control_command`、ready dispatch、queue item 或 read-model carrier 不能作为入站 alias。provider admission arbiter、`paper_recovery_state`、`current_work_unit`、`current_executable_owner_action` 和 study_progress refresh 在迁移完成前只能是 projection / migration adapter；arbiter 的 authority boundary 已固定为 live-readback-only，event/outbox fragment 只能是 diagnostic，request without live readback 只能进入 `transition_request_pending` / `NonAdvancingApply`。当前 MAS refresh 只允许 single-pass projection normalization，fixed-point runtime owner 是 OPL。它们必须逐步消费 OPL transition event + MAS policy result，不能重新持有 event log、outbox、StageRun lifecycle、fixed-point 或 provider admission authority。

2026-06-18 follow-through：`paper_recovery_state` 的 provider-admission projection 也收紧到同一边界。仅有 `current_work_unit.state.provider_admission_pending=true`、MAS transition request 或 request-only provider candidate 时，不能压制 stale parked/operator projection，也不能进入 admission；只有完整 OPL live readback 的同 identity candidate 才可作为 admission evidence。该证据仍不关闭 live domain diagnostic apply、OPL outbox/StageRun readback或 DM002/DM003 live outcome gate。

2026-06-18 current-control closeout identity follow-through：OPL current-control handoff projection 进一步收紧 terminal closeout 消费边界。带完整 OPL live readback、StageRun / route / idempotency / stage packet identity 的 provider admission 或 transition request，只有在 closeout 命中同一强 identity 时才可被消费；旧 action_type + work_unit + fingerprint 弱匹配只保留给无 identity-bound 的 legacy / diagnostic candidate。这样 stale terminal closeout 不能清掉当前 live-readback-backed admission。该 repo proof 仍不关闭 live domain diagnostic apply、OPL outbox / StageRun readback、provider admission live consumption 或 DM002 / DM003 live outcome gate。

2026-06-18 live-readback consumer hardening follow-through：MAS `opl_transition_readback` consumer 现在把完整 shape 继续收紧到同一 transaction consistency。`stage_run_identity` 只有 route / idempotency key 时不可信，必须带 `stage_run_id` 或 `stage_run_identity_ref`；`identity.latest_event_id`、`identity.latest_outbox_item_id`、`identity.latest_transaction_id`、`causality.event_id` / `outbox_item_id` / `transaction_id`、`projection_metadata.derived_from_event_id` 和 `latest_transaction_readback` 中的 event / outbox / transaction ref 必须完全一致。任何错配都 fail closed，provider admission 回到 `transition_request_pending` / OPL runtime readback required。该 proof 关闭的是 MAS consumer 接受薄 StageRun 或 stale transaction ref 的复发口；它仍不运行 domain diagnostic apply，不产生 OPL outbox / StageRun live proof，也不声明 DM002 / DM003 paper-line 前进。

2026-06-19 supervisor decision readback follow-through：`study_progress` 现在从 OPL state 中只读消费 `family-runtime/paper-autonomy/supervisor/supervisor-decisions.jsonl`，把 `opl_paper_autonomy_supervisor_decision_readback` 按 study / obligation / work-unit fingerprint 绑定到 `paper_recovery_state.supervisor_decision` 和 `refs`。这关闭的是 MAS read model 在 `opl_supervisor_decision_readback_required` 状态下自行停留或重新合成 supervisor decision 的复发口；MAS 仍不能运行 SupervisorDecisionEngine、不能写 OPL ledger、不能签 MAS owner receipt / typed blocker，也不能把该 readback 解读为 provider admission、domain diagnostic apply success 或 live paper progress。

2026-06-19 stable blocker preview suppression follow-through：`domain-handler export` 与 domain diagnostic dry-run preview 不再把同 identity、已消费 OPL supervisor decision readback 的 `stop_with_stable_typed_blocker` 重新导出成 `stage_outcome/opl-handoff` owner-resolution task。`materialize_recovery_action` / `materialize_successor_owner_gate` 仍可生成 owner-resolution preview，但 stable typed blocker 是 exactly-one terminal outcome projection，不是下一轮自动 reconcile 请求。该修复关闭的是 DM003 `no_selected_dispatch_for_authorized_stage_packet` stable blocker 被 preview 反复重启的循环；它仍不声明 domain diagnostic apply success、provider running proof、paper/gate/artifact semantic delta 或 live paper progress。

2026-06-16 tail slice 进一步把 owner callable / worker handoff 执行面收回同一边界：`domain-handler export`、paper recovery successor dispatch、AI reviewer record handoff、quality writer handoff、`paper_repair_executor` 和 default executor dispatch 可以生成 `opl_domain_progress_transition_request` 及 refs-only handoff evidence，但只要缺 OPL `DomainProgressTransitionRuntime` 完整 readback shape、可信 OPL execution authorization、exact provider-hosted StageRun packet 或 active OPL provider attempt / lease，就不能被 MAS 计为 provider admission、provider attempt lease 或 `handoff_ready` execution。closeout receipt binding 只能作为 owner-answer / closeout evidence 被消费，不能打开 owner callable execution。`stage-outcome-authority` 现在在 action executor 返回后统一检查 transition request；缺带 `identity` / `causality` / `authority_boundary` / `exactly_one_outcome` / `projection_metadata` 的 OPL live readback 时保留 handoff refs 供 OPL 接管，同时把 execution fail-closed 为 `opl_execution_authorization_required` typed blocker，`executed_count=0`、`codex_dispatch_count=0`、`mas_creates_opl_outbox=false`。`paper_repair_executor` 同样只把 ready worker handoff 读作 OPL-authorized owner callable handoff；无 proof 的 `dispatch_status=ready` carrier 必须降级为 `opl_execution_authorization_required`，不得靠 MAS-local `ok=true` 或 handoff payload 自行升级。它不是兼容 alias，也不是 MAS 私有 attempt lifecycle；它只是 MAS policy adapter 到 OPL runtime 的 admission boundary。

当前 MAS replay fixture 已从 first slice 扩到 DM002 / DM003 代表性历史坏轨迹：`tests/test_paper_progress_transition_replay_fixtures.py` 现在覆盖 DM002 stable typed blocker、DM003 owner-action request 缺 OPL readback、owner receipt recorded 后同 identity 不推进、terminal owner-receipt closeout 后 stale transition-request projection 被消费、closeout / StageRun identity split、same-tick stale blocker / admission 冲突、`provider_admission_pending_count=0` 禁止解释、`current_work_unit` 与 `paper_recovery_state` 分歧、domain diagnostic/request 无 OPL readback 进入 `NonAdvancingApply`，以及 human gate / route-back accepted shape。`contracts/paper_progress_replay_live_evidence_status.json` 同步记录 replay coverage 与只读 live-evidence acceptance boundary：fresh live acceptance 只能是 strict current-identity running proof、owner receipt、stable typed blocker、human gate、route-back evidence 或 paper/gate/artifact semantic delta 中 exactly-one。它仍不是 live paper progress claim。2026-06-19 新增的 `replay_to_live_separation_gate` 明确把 provider-admission same-identity replay coverage 与 fresh DM002 / DM003 OPL live readback、provider running proof、MAS owner evidence 和 paper-line accepted outcome 分开；同 identity replay 消费 `transition_request_pending` 只能证明 repo-side projection 规则，不证明 live provider admission 或论文推进。完成门继续要求 OPL outbox / StageRun identity live readback、domain diagnostic apply exactly-one live outcome、provider admission arbiter 完全消费 OPL transition event，以及 DM002 / DM003 fresh live paper-line outcome。docs、contract、focused tests、projection clean、queue empty、`provider_admission_pending_count=0` 或 domain diagnostic dry-run 仍不能替代 running proof、owner receipt、stable typed blocker、human gate、route-back evidence 或 paper-gate-artifact semantic delta。

2026-06-17 MAS adapter 消费面补齐：`PaperProgressPolicyAdapter` 现在显式覆盖 owner receipt、typed blocker、publication gate / paper delta、forbidden write、human gate / route-back 和 `NonAdvancingApply` 结果形状，并把 clean `opl_domain_progress_transition_request` 保持为 MAS domain policy request；request 不携带 OPL runtime artifact 字段，authority boundary 继续声明 MAS 不能 authorize provider admission、event log、outbox、StageRun 或 fixed-point runtime。该状态由 `tests/test_paper_progress_policy_adapter.py` 与 replay fixture focused tests 证明；它仍不是 live domain diagnostic apply 前进、provider running proof 或 paper-line completion claim。

2026-06-19 completion audit foldback：`contracts/paper_progress_transition_runtime_completion_audit.json` 与 `contracts/runtime/mas-runtime-surface-retirement-inventory.json` 已把 repo-source private-surface retirement 固定为 `done` / `100%`：active callsites 已迁移或删除，旧 module / alias / wrapper / compat shim 已物理退役或收薄为合法 MAS minimal authority adapter / body-free projection，`open_repo_source_gaps=[]`。完整目标仍是 `partial`，但缺口现在只属于 live-runtime readiness / production acceptance：fresh domain diagnostic apply exactly-one outcome、OPL outbox / StageRun identity live readback、provider admission event consumption、DM002 / DM003 fresh live paper-line outcome，以及 Tool Arsenal、obligation actuator、owner dispatch、Workbench/Portal、runtime health、runtime lifecycle payload retention、runtime storage maintenance 等 live takeover / soak tail。该 audit 是两栏完成度护栏；repo-source 删除完成不能被写成 live runtime ready，live evidence 缺失也不能反向阻止旧 MAS 私有源码面删除。

2026-06-20 blueprint functional acceptance foldback：`contracts/paper_progress_transition_runtime_completion_audit.json#/blueprint_l0_l7_functional_acceptance` 进一步把 [MAS / OPL 进度运行时理想蓝图](./mas_opl_progress_runtime_ideal_blueprint.md) 拆成 L0-L7 功能面验收表。当前 repo/source/control-plane slice 为 `done` / `100%`：L1 OPL runtime contract/readback shape、L2 MAS policy adapter boundary、L3 replay fixtures、L4 consume-only projection、L5 transactional outbox side-effect boundary、L6 projection demotion / legacy retirement / rebuild surfaces 和 L7 live acceptance harness 都已有 repo/source/control-plane evidence refs。该 foldback 不执行 domain diagnostic apply、provider start、hydrate、tick、redrive 或 owner-route reconcile，不写 Yang runtime/study/paper artifacts；fresh OPL outbox/StageRun live readback、domain diagnostic apply exactly-one outcome、DM002/DM003 live paper-line accepted outcome、owner receipt、stable typed blocker、human gate、route-back evidence 或 paper/gate/artifact semantic delta 仍全部属于 live-only deferred。

2026-06-18 dispatcher closeout foldback：`stage_outcome_authority` 的 repo-level authorization proof 已覆盖所有 `SUPPORTED_ACTION_TYPES` 缺 OPL proof 时 fail closed、trusted OPL execution authorization、bound `DomainProgressTransitionRuntime` readback 和 exact provider-hosted StageRun packet；缺 proof 的 owner-callable path 只能产生 `opl_execution_authorization_required` typed blocker，并显式 `mas_dispatch_authority=false`、`mas_creates_opl_outbox/event/stage_run=false`、`provider_admission_pending=false`。该 proof 关闭的是 MAS-local owner callable execution authority 复发口；live every-active-caller soak 或 no-active-caller proof 是 dispatcher runtime-readiness / production-readiness 尾项，不再作为 repo-source 旧 carrier / alias / compat shim 删除前置。

2026-06-18 Workbench/Portal follow-through：Progress Portal 与 runtime workbench 的默认“下一步”读面已收成 `current_owner_delta` 只读摘要。顶层 `study.next_system_action`、study workbench overview、runtime workbench `user_next` / `next_action_summary`、workspace dashboard focus 和 materialized workspace-row fallback 不再直接消费 `user_visible`、cockpit 或 workspace row 的 legacy `next_system_action` / operator focus；旧值只进入 diagnostic legacy projection，不能生成 action、provider admission、worker attempt、paper progress 或 publication-ready claim。该 repo proof 仍不替代 OPL Workbench Shell 对 OPL transition event + MAS policy result 的 live event-driven rebuild proof。

2026-06-18 request-task alias / successor currentness foldback：`domain_action_request_materializer` 不再导出顶层 `request_tasks` alias；legacy request task refs 只保留在 `legacy_request_task_diagnostics.legacy_request_task_refs`，并显式为 body-free diagnostic refs，不再能被 domain diagnostic preview、stability evidence 或测试当作 active handoff body、outbox、command、event、StageRun、provider admission 或 next-action authority。`current_executable_owner_action` 同时要求 repair-progress consumption 与 paper-recovery successor 的 work-unit fingerprint / action fingerprint 匹配，防止 stale `repair_progress_projection` 在 fixed-point 中吞掉当前 `paper_recovery_state.next_safe_action.successor_owner_action`。两项 proof 关闭的是 repo-side projection / alias 复发口，不关闭 live domain diagnostic apply、OPL outbox / StageRun readback、provider admission running proof、DM002 / DM003 live outcome 或 legacy carrier physical-retirement gate。

## 目标判断

DM002 / DM003 最近一个月反复暴露的 currentness、owner-route、owner receipt、provider admission 和 supervision 修复，不是因为 MAS 缺少又一个私有控制模块，而是因为 OPL 尚未把“domain agent 的 current owner delta 如何事务化前进”做成一等通用基座。论文推进状态机因此被拆散在多个 projection / selector / arbiter / actuator 中：`current_work_unit`、`paper_recovery_state`、`current_executable_owner_action`、domain diagnostic provider admission、OPL current-control、Paper Autonomy Supervisor 和 read-model projection 都可能重新解释同一个 owner receipt、typed blocker、closeout 或 queue residue。
...
4. 测试过多锁定局部 reducer case，缺少跨 MAS / OPL / domain diagnostic / readback 的历史坏轨迹 replay fixture。局部绿不能证明 live apply 已产生 exactly-one outcome。
...
flowchart TD
  ...
  P --> M
  L --> R["Derived Read Models<br/>study_progress / domain diagnostic / current_work_unit / workbench"]
...
- `commands` 表示意图，`events` 表示已提交事实，`projections` 只是派生读面。domain diagnostic apply、owner-route reconcile 和 provider admission 只能提交 OPL command 或消费 OPL event，再由 MAS policy adapter 裁决 domain result；不能直接写出平行 truth。
...
每次 domain diagnostic apply、supervisor tick 或 owner-route reconcile 必须走同一 fixed-point loop：
...
- 把最近 DM002 / DM003 坏轨迹转成 event trace replay：owner receipt recorded 后不推进、same-tick admission 被 stale blocker 压掉、provider admission pending 为 0、current_work_unit 与 paper_recovery_state 分歧、domain diagnostic/request 无 OPL readback 后无 outcome、人类 gate / route-back accepted shape。
...
- Workbench 默认显示 kernel state；domain diagnostic / queue / stage index 进入 drilldown。
...
只读验收口径：fresh live acceptance 只能来自 fresh `study_progress`、domain diagnostic dry-run 或受委托 apply/readback、OPL current-control / StageRun readback 与 MAS owner evidence 的同 identity 交叉验证；`contracts/paper_progress_replay_live_evidence_status.json#/live_evidence_acceptance` 记录 allowed exactly-one family。
...
- L4 runtime currentness：fresh `study_progress`、domain diagnostic apply/readback、OPL current-control readback、owner receipt / typed blocker / human gate / route-back artifact refs。
