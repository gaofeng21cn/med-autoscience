...
Tombstone boundary：本文中的 `current_owner_delta`、`execute_current_owner_delta`、supervisor obligation、queue / attempt、provider admission、human-gate transport token、PaperRecovery 或 DomainProgressTransitionRuntime 语言，默认只按 historical target / diagnostic provenance 读取。当前默认完成判断和下一步 authority 仍是 `StageOutcome -> NextActionEnvelope -> OPL TransitionReceipt -> MAS owner consumption`；本文不得单独支撑 paper progress、submission-ready、publication-ready、runtime-ready、provider running、owner receipt closure 或 `current_package` authority。
...
- `current_owner_delta` 是历史 supervisor 设计中的 desired projection / drilldown。当前默认 desired root 是 `StageOutcome -> canonical NextActionEnvelope`；OPL queue、attempt ledger、domain diagnostic、Portal、stage index 和 read-model 都只能解释同 identity 的 canonical next action，不能反向发明下一步。
...
- domain diagnostic apply 的 obligation actuator 只能消费三类闭合 readback：OPL runtime readback、MAS owner-answer readback、MAS domain authority readback。MAS `opl_domain_progress_transition_request` 只是 request projection，不能算 success outcome；若 outcome 不在当前 `SupervisorDecision` 允许集合内，postcondition 必须 fail closed。
...
| Kind | Owner | 例子 | 输出 |
| --- | --- | --- | --- |
| `mas_control_plane_repair` | MAS repo | domain diagnostic currentness、owner gate projection、materializer selection | code/test/doc commit 或 stable typed blocker |
...
## Historical OPL substrate plan（superseded）

OPL 应把以下能力变成一等基座，而不是让 MAS/domain diagnostic/目标线程各自重算：
...
6. `Workbench Shell`
   ...
   - drilldown 才显示 domain diagnostic、queue、stage index、trace、lineage 和 telemetry。
...
## Historical MAS adapter plan（superseded）
...
## Historical runtime flow（superseded）
...
- MAS domain diagnostic / study_progress / domain-handler export 只消费 `SupervisorDecision`，不再各自从 queue / domain diagnostic / stage index 重算 currentness。
...
7. operator 默认读面不需要倒查 domain diagnostic/queue/stage index 就能判断下一步。
