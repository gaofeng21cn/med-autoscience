...
DM002 / DM003 暴露的顶层问题不是某个 reducer、DHD 白名单或 owner gate 缺口，而是 MAS / OPL 之间缺少一个一等的 `Paper Autonomy Supervisor` 协议。
...
4. `wait_for_owner_with_resume_token`：确实需要人类或 owner 决策，必须有 durable human gate、候选分支、超时策略和 resume token。
...
| AWS Step Functions callback task token | workflow 可携 task token 暂停，等待外部过程返回 token 和 payload 后继续。 | OPL `HumanGateTransport` 必须给每个 owner/human gate 一个 durable resume token；MAS 只消费 token 回来的 owner answer，不把聊天文字当恢复证据。 |
...
- `Autonomy Supervisor Obligation` 是 MAS / OPL 之间的新一等合同。它不是 read-model 文案，而是每个 current owner delta 的可恢复执行义务。
- `operator_decision_required` 必须降级为 obligation 的一种 pending state，并带 `owner`、`allowed_decisions`、`resume_token`、`timeout_policy`、`default_safe_branch`、`current_identity` 和 `evidence_required`。
...
OPL supervisor 只能输出五类 decision：
...
2. `HumanGateTransport`
   - 持久 callback token / resume token。
   - 每个 gate 有明确 choices、owner、timeout、default safe branch。
   - resume 后回到同一 current identity，不能靠聊天摘要或人工描述恢复。
...
4. `SupervisorDecisionEngine`
   ...
   - 输出五类 `SupervisorDecision`，不输出自由文本状态。
...
1. `PaperAutonomyAuthorityKernel`
...
- HumanGateTransport 支持 durable resume token、timeout、default safe branch。

完成门：OPL readback 能对同一 obligation 给出 execute / wait / stop / recovery / consume 五类 decision fixture。

### Lane 2：MAS authority kernel
...
- 默认视图从“状态解释”改成“当前五类 decision + next owner”。
- 对每篇论文显示：
  - `paper_delta`
...
2. 每次监督 tick 都输出五类 decision 之一。
3. 每个 wait state 都有 resume token、timeout 和 default safe branch。
