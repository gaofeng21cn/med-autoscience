...
- `live current_package handoff` 仍需要 fresh evidence：必须由 live `delivery-inspect` / `paper-mission inspect` / `study progress` 或真实 workspace package manifest 证明 mirror 从当前 source 刷新；docs、contract、focused tests 或 checkpoint 不能替代。
