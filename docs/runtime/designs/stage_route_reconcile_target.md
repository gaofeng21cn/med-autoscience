...
外部成熟工程经验映射到本设计时，采用 identity-first 而不是 status-first。Temporal 的 [Event History](https://docs.temporal.io/workflow-execution/event) 是 durable workflow replay truth，Temporal [Activity idempotency](https://docs.temporal.io/activity-definition) 建议用稳定 workflow/activity identity 支撑 retry，而非让 worker heartbeat 或 transport status 直接代表业务完成；AWS Builder's Library 的 [Making retries safe with idempotent APIs](https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/) 建议把 caller-provided client request identifier 纳入 API 合同并让服务端按该 intent 做幂等处理；Enterprise Integration Patterns 的 [Request-Response with Retry](https://www.enterpriseintegrationpatterns.com/patterns/conversation/RequestResponseRetry.html) / [idempotent receiver guidance](https://www.enterpriseintegrationpatterns.com/patterns/conversation/Introduction.html) 要求 receiver 能区分 retry duplicate 与新 intent。对应到 MAS/OPL：`route_identity_key` / `attempt_idempotency_key` / `owner_route_currentness_basis` 是 caller intent + receiver dedupe identity，OPL StageRun / Temporal status 是 durable execution evidence，MAS closeout consumption 才是 domain currentness transition；任何缺 identity 的 record-only ref、transport completion、queue empty 或 worker heartbeat 都只能进 audit。
