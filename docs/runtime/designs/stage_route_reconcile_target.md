...
MAS provider admission 必须同时带 selected stage packet binding：`dispatch_ref`、`stage_packet_ref`、`selected_dispatch_ref` 和 `stage_packet_refs`。这些字段的 currentness 由 OPL StageRun substrate 维护；MAS 的责任是完整输出和缺失 fail-closed。缺 stage packet binding 时，DHD / export 必须抑制 provider admission pending，并以 `stage_packet_not_current_selected_dispatch` 或等价 authorization blocker 解释，不得靠同 action label 或旧 dispatch 补齐。
