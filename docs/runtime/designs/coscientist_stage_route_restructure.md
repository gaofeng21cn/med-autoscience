...
| `strategy retrospective` | 只在 route 停滞、重复同一 work unit、reviewer 输出冲突、高风险 promotion 或 stop-loss 候选出现时触发，用来给 decision / route owner 生成 arbitration brief。 | 不作为每轮必经 gate，不阻断正常 admission，不替代 Stage Review、cross-Stage Meta Review、publication gate、human gate、owner receipt 或 stable typed blocker。 |
