...
`MAS Stage intent -> OPL Stage Tool Affordance Boundary -> capability descriptor -> OPL prepare / package lifecycle / run-context -> module invocation -> artifact refs / receipt -> MAS owner quality gate`
...
- 让 OPL 统一管理安装、依赖环境、版本锁、health/readback、run-context、artifact refs 和跨 agent 能力复用，避免每个 domain agent 维护第二套基座。
...
MAS / OPL 的能力审计先按能力模块切分，再看每个模块内部三层实现比例，最后才评估 Codex / workspace / quest / domain profile 的暴露方式。不要先按目录名、Skill 名称或 CLI 命令切分；这些只是能力模块在某一层的物理投影。

审计表的默认列是：

| 列 | 读法 |
| --- | --- |
| 能力模块 | 论文生产链上的 bounded capability，例如 literature、stats、display、tables、submit、data governance、research integrity、artifact lifecycle。 |
| `professional_skill` / `domain_skill_declaration` | AI-first 专业判断、rubric、策略、审稿 lens、route-back 和 candidate refs。 |
| `skill_local_deterministic_helper` | 随 Skill 分发的确定性小工具，例如 refs schema、normalization、lint、manifest / receipt shaping 或 env/log diagnostics。 |
| `programmatic_substrate / authority_surface` | OPL / MAS 程序化边界，例如 connector、credential、provider lifecycle、runtime queue、receipt、owner gate、artifact authority、publication readiness 和 App/operator projection。 |
| 暴露方式 | source-only、workspace / quest local、domain profile、developer profile 或显式 external-skills search/inspect/sync。 |
| 合理性 | 判断三层比例是否符合模块性质，以及是否存在 Skill 过度全局暴露、helper 越权、程序面硬编码专业判断或第二真相源。 |

经验比例只作为审计起点，不是硬性配额：OPL 基座模块通常应以程序化基座为主，专业 Skill 为辅；MAS 领域模块通常应以专业 Skill 和 stage prompt 为主，程序化 authority surface 只保留 owner gate、receipt、artifact/source/publication authority；helper 层应保持薄、确定、局部，不能升级成 runtime owner 或 reviewer。

## 能力升级归位规则

新增或吸收外部能力时，默认先走“更新现有 Skill / stage policy / support Skill”路径。只有当一个能力有稳定专业 workflow、独立依赖或 fixture / gallery / receipt、跨 agent 复用、独立发布节奏、明确 refs-only output 和按需同步策略时，才进入新增独立专业 Skill 评估。

| 候选能力 | 默认归位 | 不新增独立 Skill 的原因 |
| --- | --- | --- |
| `critique-as-repair-hint`、reviewer concern recall、revision coverage audit | `medical-manuscript-review`、`medical-manuscript-writing`、MAS `reviewer_revision` policy、OMA work-order route | 属于审阅/写作/自进化链路的质量地板；新增 skill 会割裂 reviewer -> repair -> owner gate。 |
| `strategy retrospective`、`next-delta tournament`、stop/continue arbitration | MAS stage / route policy、`medical-publication-routeback-reviewer` 的 methodology routeback / owner-gate handoff modes、OPL `stage-candidate-portfolio-reviewer` | 是 current-owner JIT arbitration，不是默认 admission gate、正式 Review 或独立 authority。 |
| `opportunistic knowledge prefetch`、source/citation support planning | `medical-research-lit` + OPL Connect scientific connector refs | provider access 和 receipt 归 Connect；文献判断归 Lit；缺失 prefetch 不能阻断 ordinary progress。 |
| `claimType` / `graphWarnings`、annotation-to-source regeneration、ledger pointer/hash | `medical-manuscript-review`、`medical-data-governance`、`medical-figure-*`、OPL Ledger / Workspace refs | 是候选 evidence warning / route-back hint，不是 artifact authority 或 source readiness verdict。 |
| rerun-to-reproduce receipt、compute/env diagnostics | `scientific-compute-runner` optional Skill + OPL Runway / Connect substrate | Skill 只保留诊断 playbook；credential、submit/wait/harvest、execution receipt 和 live readiness 归 OPL owner surface。 |
| omics / single-cell / structural biology / protein design | `medical-advanced-biomed-router` + optional named-specialty Skill | 只有明确 named specialty task 或 router 选中时同步一个具体 Skill；稳定 MAS omics workflow 出现前不进默认 core。 |
| journal response、statistical reporting、FAIR、figure evidence、reader grounding、presentation pack | 现有 `medical-*` core Skills 与 `references/professional-quality-ref-templates.md` | 这些是质量地板和 route hint，不是新的 physical Skill family。 |

这个矩阵约束的是物理实现，不压缩逻辑能力。一个逻辑能力可以同时有 stage policy、professional Skill、helper 和 programmatic substrate；但每个子职责只能有一个 owner，且 MAS docs / registry / readback 不能成为专业 Skill 正文的第二真相源。
...
当前外置形态收敛为八个 active `mas-scholar-skills.*` 模块，均由外部 `mas-scholar-skills` 仓维护并同步给 MAS 的真实专业 Skill 支撑：`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep` 和 `medical-data-governance`。Display module 仍是 active `mas-scholar-skills.display`；`medical-figure-style` 和 `medical-figure-composer` 是该 module 下的 dedicated 子 Skill / 触发面，分别输出 style QA / style reference candidate refs 与 panel composition / layout QA candidate refs，由 `medical-figure-design` orchestrator route 统一接回 figure stage 和 MAS owner gate。`omics` 没有稳定 MAS 组学专业 workflow 前不占 active 合同位；通用 source / external-learning intake、provider source discovery 和 capability discovery 归 OPL Framework，医学 source readiness 与 source truth consumption 归 MAS Stage/authority surface。能力通过 Stage Tool Affordance Boundary 进入六个公开 Stage action；MAS 只保留 refs-only owner-gate request builder 和医学论文 authority。机器 guard 由 `src/med_autoscience/scholarskills_capability_modules.py` 在各模块 descriptor 上暴露 classification / externalization readback，固定 `migration_target=mas-scholar-skills`、`mas_module_authority_owner=false`、`mas_second_truth_allowed=false` 和 `mas_may_write_scholarskills_authority=false`，防止后续把 MAS docs 或 registry readback 写成 ScholarSkills authority owner。
...
`MAS Scholar Skills` 是 MAS 医学研究产品的必需能力包；独立 repo 是开发、版本与发布边界，不是可选产品边界。MAS 侧消费的 active 机器模块 id 固定为 `mas-scholar-skills.<module>`，当前覆盖 `display`、`tables`、`stats`、`lit`、`write`、`review`、`submit` 和 `data` 八个模块。旧 `opl.scholarskills.<module>` id 只允许作为 payload 中的 `legacy_id_aliases` / `legacy_id_provenance` readback 保留，不再作为 active capability id 或 module id。OPL Atlas / Pack / Stagecraft 发现并解析这些 descriptor，OPL-hosted Stage action 通过 Tool Affordance Boundary 调用；MAS 不复制 ScholarSkills truth，也不持有模块执行器、依赖安装、artifact body transport 或 runtime queue。模块 classification 固定为：active module 均有外部真实专业 Skill 支撑；没有真实专业 Skill 的候选能力先留在 MAS / OPL 对应 owner surface，不在这里伪装成 active Skill。

当前状态是 `required_capability_package_consumer` + `active_module_descriptor_consumer` + `stage_tool_affordance_consumption` + `refs_only_execution_receipt_candidate_consumer` + `file_materialized_package_refs_consumer` + `candidate_artifact_owner_gate_request_readback`。MAS 通过 `contracts/opl_agent_package_manifest.json` 声明 provider version、ABI、11 个 required Skill 和 8 个 required module；OPL package lifecycle 内部读取 dependency readiness、digest/ABI、core exports 和 lifecycle receipt。个别已安装 capability 的 refs-only 调用仍可 fail-open，但整个 required package 缺失、digest/ABI 不兼容或核心 export 不全时必须由 OPL lifecycle fail closed，不能借用 capability-level fail-open 继续普通运行。

ScholarSkills 的执行可用性不来自 MAS 程序仓内的 repo-local mirror；该 mirror 已物理退役，并且必须保持缺席 / tombstone-only。用户只管理 MAS；OPL `packages` 在同一依赖闭包事务中解析并物化 `mas-scholar-skills`，记录 provider version、ABI、manifest/content digest、core exports 和 lifecycle receipt，并对依赖包实施卸载保护、闭包级更新和原子回滚。用户生命周期只使用：
...
MAS / OPL 能力实现统一按三层语义读取：

- `professional_skill`：专业 Skill，弹性保留在 `SKILL.md` 和 AI executor，承载医学 playbook、rubric、策略、审阅 lens、route-back 和 candidate refs。
- `skill_local_deterministic_helper`：随 Skill 同目录分发的确定性小工具，当前标准文件名是 `kernel.py`。MAS readback 只暴露 `skill_local_deterministic_helpers` policy、`helper_file_name=kernel.py` 和 expected helper skill ids，供 OPL Connect 同步完整 skill 目录时保留 helper。helper 只能做 refs-only schema、normalization、lint、manifest / receipt shaping 或 env/log diagnostics，不能写 MAS truth、owner receipt、typed blocker、human gate、publication eval、controller decision、current package、runtime queue 或 provider attempt。
- `programmatic_substrate / authority_surface`：MAS / OPL 程序权威面，承接 connector、credential、provider lifecycle、runtime queue、receipt、owner gate、artifact authority、publication readiness 和 App/operator projection。`scientific-compute-runner` 这类 compute helper 只准备 env/provider intent 和日志诊断；provider resource、credential、endpoint lifecycle、SLURM / Modal / cloud execution receipt 继续归 OPL Connect / Runway。

MAS Scholar Skills 的 source of truth 在外部 `mas-scholar-skills` repo：聚合入口是 `skills/mas-scholar-skills/SKILL.md`，真实专业 Skill 是 `skills/medical-research-lit/SKILL.md`、`skills/medical-manuscript-writing/SKILL.md`、`skills/medical-manuscript-review/SKILL.md`、`skills/medical-figure-design/SKILL.md`、`skills/medical-figure-style/SKILL.md`、`skills/medical-figure-composer/SKILL.md`、`skills/medical-statistical-review/SKILL.md`、`skills/medical-table-design/SKILL.md`、`skills/medical-submission-prep/SKILL.md` 和 `skills/medical-data-governance/SKILL.md`，optional advanced / medical-method specialist 正文也在外部 `skills/<skill_id>/SKILL.md`，模块 catalog snapshot 是 `contracts/scholar-skills-capability-modules.json`。MAS 的 `write`、`review`、`figure`、`data/cohort` 等 stage 主提示词留在 MAS overlay / stage owner surface，只消费外部专业 skill 和 refs-only catalog；不得在 MAS 侧手工维护第二套模块列表、optional specialist catalog、专业 skill 正文、gallery inventory、dependency truth 或 runtime readiness 结论。若外部 contract 更新，MAS 侧只更新消费边界、链接和不越权读法，不能把同步后的 descriptor/readback 写成 owner acceptance。

### 罕见专科外部技能库策略

八个 active module 和其同步的 `medical-*` 专业 Skill 是 MAS 的默认科学技能面。Codex discovery helper `external-scientific-skills` 只把罕见专科缺口路由到 OPL Connect external-skills；它是薄入口，不承载 K-Dense 全量能力或第二技能库，也不能让 K-Dense、外部 README 或同步目录成为 MAS source of truth。

允许触发 external-skills 的条件必须可追溯到当前 work unit：用户显式命名工具/数据库，核心 Skill route-back 指出缺口，stage policy 判断默认 active module / 专业 Skill 无法覆盖，或联网、云计算、敏感数据、外部凭据路径需要 policy / approval。典型缺口包括 `scanpy` 单细胞分析、`pydeseq2` 差异表达、pathway enrichment、Nextflow 工作流、RDKit 化学信息学、PyHealth 医疗 ML 等。
...
OpenScience master `2200ad2` (`2200ad2ec4e2ac7c7ff59c5dcdfaeb0b9a5fda66`) 对 MAS / OPL 的价值是 local-first research workspace pattern，不是 runtime 迁移目标；fresh evidence 是 `git ls-remote https://github.com/ai4s-research/open-science.git refs/heads/master` 与本地 checkout `git rev-parse HEAD` 同值。当前已落地 contract policy、runbook 和 `build_openscience_artifact_provenance_advisory` refs-only sidecar worker；该 worker 已能把候选 artifact 输入投影为 claim warning、annotation regeneration request、project ledger pointer/hash 和 native viewer watch-only refs。MAS 不接入 OpenScience Tauri / Electron / WebUI / OpenCode sidecar / MCP / runtime，不复制它的 skill catalog，不把 `science_artifact` MCP、native viewer、project-local ledger 或 artifact graph 当作 MAS truth、artifact authority、owner receipt、typed blocker、publication gate 或 source readiness verdict。可学习内容必须落在基座 / Stage / 专业 Skill / 工具的明确 owner 上：
...
| native viewer / workspace affordance | Console / Workspace | Display / Reviewer 可读取 watch-only preview refs | viewer 只是 operator display pattern；不接入 Tauri / Electron / WebUI / OpenCode sidecar / OpenScience runtime，不把 preview 当 publication readiness、source readiness 或 visual quality owner gate。 |
| environment / package / kernel capture | Workspace / Pack / Runway / Ledger | Stage / ScholarSkills 读取 dependency profile、package lock、kernel status 和 run-context refs | 只形成 execution receipt candidate 或 rerun precheck；不能证明真实研究质量、artifact authority、source readiness 或 owner acceptance。 |
| rerun-to-reproduce receipt | Runway / Ledger / Console | Quality / Reviewer 消费 rerun command、input refs、env fingerprint、stdout/stderr refs、artifact hash 和 failure class | rerun receipt 可支持 reviewer briefing 或 route-back candidate；不能自动关闭 quality gate、publication gate 或 current package freshness。 |
| interactive approval / permission cards | Console / Stagecraft / Runway | Stage / human-gate route 消费 action、resource、allow once / always / reject 和 free-text answer refs | approval card 是 operator interaction surface；只有 MAS human gate / owner route 消费后才能改变 truth、blocker 或 route。 |
...
| --- | --- | --- | --- |
| capability inventory boundary | `done_for_stage_affordance_contract` | OPL Atlas / Pack / Stagecraft 从 provider manifest 读取 capability ids、descriptor、required ref families、dependency/run-context refs 和 no-authority boundary，并通过 Stage Tool Affordance Boundary 暴露给 executor。 | 不证明外部 skill 已安装、OPL provider 已执行、package 已生成、owner gate 已接受或任何论文 truth 已变化。 |
| 八个 active module descriptor discovery | `done_for_opl_descriptor_consumption` | OPL resolver 可发现 `mas-scholar-skills.display`、`tables`、`stats`、`lit`、`write`、`review`、`submit`、`data`；MAS 不提供第二套 public registry action。 | 不证明 OPL Runway 已执行 provider、Console 已展示 live run、MAS 持有 ScholarSkills truth，或任何模块产物被 MAS owner 接受。 |
| required capability package readback | `done_for_mas_consumer_contract` | MAS manifest 声明 `scholarskills_required_package`；OPL lifecycle 内部把 missing/incompatible 映射为 closed failure 和 lifecycle receipt。 | MAS repo contract/tests 不证明 OPL 已完成依赖闭包安装；真实安装、digest lock、卸载保护、原子回滚与 current readback 由 OPL lifecycle receipt 证明。 |
...
| PaperMission submission milestone candidate | `internal_residue_not_public_action` | 旧 `paper-mission package-candidate` materializer 仍可产生候选 refs，但只能由 V2 Stage/authority path 迁移消费；`paper_mission_authority_evaluate` 是无用户 surface 的唯一 closed handler action。 | 不能作为用户入口，也不能声明 submission-ready、publication-ready、current package、owner receipt、typed blocker authority file、human gate 或 governed paper progress。 |
