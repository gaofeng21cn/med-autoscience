...
| Medical Data Governance | `opl.scholarskills.data` / `medical-data-governance` | `P1` | 数据资产 manifest、dataset manifest、dictionary/codebook、cleaning/normalization readiness、lineage、version impact、study binding、privacy/access、lifecycle guardrail 支撑 | source readiness verdict、study binding、不可逆 data mutation authorization |
...
`ScholarSkills` 是 OPL 面向学术产出的能力品牌命名；MAS 侧消费的 active 机器模块 id 固定为 `opl.scholarskills.<module>`，当前覆盖 `display`、`tables`、`stats`、`lit`、`write`、`review`、`submit` 和 `data` 八个模块。MAS 只把这些模块注册为 `scientific_capability_registry` 可发现、可 resolve 的 descriptor/readback bridge；不复制一套 ScholarSkills truth，也不把模块执行器、依赖安装、artifact body mutation 或 OPL runtime queue 搬进 MAS。模块 classification 固定为：active module 均有外部真实专业 Skill 支撑；没有真实专业 Skill 的候选能力先留在 MAS / OPL 对应 owner surface，不在这里伪装成 active Skill。
