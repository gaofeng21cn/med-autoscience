...
| `triggered meta-review`、`next-delta tournament`、stop/continue arbitration | MAS stage / route policy、`medical-publication-routeback-reviewer` 的 methodology routeback / owner-gate handoff modes、OPL `stage-candidate-portfolio-reviewer` | 是 current-owner JIT arbitration，不是默认 admission gate 或独立 authority。 |
...
ScholarSkills 的执行可用性不来自 MAS 程序仓内的 repo-local mirror；该 mirror 已物理退役，并且必须保持缺席 / tombstone-only。执行面必须由 OPL Connect 把外部 `mas-scholar-skills` source repo 同步为病种 workspace 或 runtime quest 的本地 Codex skill；链路固定为 `OPL workspace/profile/environment readback -> OPL Connect sync-skills -> workspace/quest .codex/skills -> Codex discovery -> specialist candidate refs -> MAS owner gate`：
...
默认同步的本地 Codex skill 是 `mas-scholar-skills`、`medical-research-lit`、`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-figure-style`、`medical-figure-composer`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep` 和 `medical-data-governance`，分别落到 `<workspace_root>/.codex/skills/<skill_id>` 或 `<quest_root>/.codex/skills/<skill_id>`。其中 `medical-figure-style` / `medical-figure-composer` 是 display module 的 dedicated 子 Skill，不是新的 active MAS module。外部 source 还声明 optional router/reviewer skills：`medical-advanced-biomed-router`、`medical-methodology-planner`、`medical-evidence-integrity-reviewer` 和 `medical-publication-routeback-reviewer`；optional advanced named-specialty skills：`medical-structural-biology`、`medical-protein-design`、`medical-genomics-foundation-models`、`medical-single-cell-modeling`、`medical-indication-dossier`、`research-pdf-evidence-explorer`、`scientific-compute-runner`；以及 optional medical-method named-specialty skills：`medical-protocol-and-sap-planner`、`medical-cohort-phenotyping`、`medical-evidence-synthesis-and-claim-map`、`medical-reference-integrity-auditor`、`medical-rebuttal-strategy`、`medical-display-qc`、`medical-causal-inference-plan`、`medical-survival-analysis-plan`、`medical-risk-model-transportability-reviewer`、`medical-registry-atlas-story-architect`、`medical-research-portfolio-memory-curator`、`medical-display-regression-debugger` 和 `medical-data-freeze-and-analysis-readiness-reviewer`。MAS 只把它们暴露为 `optional_skill_ids` 与 optional target path templates，供 OPL Connect 在 source 物化时按需同步真实 skill。`medical-evidence-gap-triage-reviewer`、`medical-methodology-routeback-reviewer`、`medical-owner-gate-handoff-reviewer` 和 `medical-publication-strategy-memory-curator` 只保留在 `retired_optional_skill_redirects`，分别由上述三个保留 skill 的 mode 覆盖，不再出现在 optional path templates 或 Codex discovery 目标中。当前可发现性从 OPL workspace/profile readback 与 generated `scientific_capability_registry` action 进入；MAS 不提供 `profile`、`doctor` 或 `workspace bootstrap` CLI，也不执行安装、复制 plugin 或写 authority/runtime surface。MAS 程序仓内的 ScholarSkills mirror 只能作为已退役 tombstone 读取，不是 dev/review source，也不是论文执行可用的默认 source。
...
| --- | --- | --- | --- |
| registry summary / inventory readback | `done_for_repo_operability_surface` | Generated `scientific_capability_registry` action 使用 `mode=summary|inventory`，暴露 `mas_scientific_capability_registry_summary`、`mas_scientific_capability_inventory`、capability ids、family/counts、refs-only / descriptor-only 标记和 no-authority boundary。 | 只证明 MAS repo capability inventory 可读；不证明外部 skill 已安装、OPL provider 已执行、package 已生成、owner gate 已接受或任何论文 truth 已变化。 |
| 八个 active module descriptor discovery | `done_for_repo_capability_surface` | Generated `scientific_capability_registry` action 使用 `mode=index|resolve` 可发现 `mas-scholar-skills.display`、`tables`、`stats`、`lit`、`write`、`review`、`submit`、`data`，并返回 module descriptor、required ref families、dependency/run-context refs 和 no-authority boundary。 | 不证明 OPL Pack 已安装依赖、Runway 已执行 provider、Console 已展示 live run、MAS 持有 ScholarSkills truth，或任何模块产物被 MAS owner 接受。 |
| workspace / quest local Codex skill install readback | `done_for_generated_registry_readback` | OPL workspace/profile readback 与 generated `scientific_capability_registry` action 暴露 `scholarskills_local_install` refs、目标 `.codex/skills/<skill_id>` path/template、外部 source repo ref 和 no-authority flags。 | 只证明 catalog/readback shape 已落地；不证明 OPL worker 已执行 sync、skill 已实际安装、provider 已运行、产物已生成或 MAS owner gate 已接受。 |
