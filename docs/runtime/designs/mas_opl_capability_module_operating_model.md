...
专业 Skill 的弹性保留在 `SKILL.md` 和 AI executor；确定性小工具随 Skill 以 `kernel.py` 形式同目录分发。MAS readback 只暴露 `skill_local_deterministic_helpers` policy、`helper_file_name=kernel.py` 和 expected helper skill ids，供 OPL Connect 同步完整 skill 目录时保留 helper。helper 只能做 refs-only schema、normalization、lint、manifest / receipt shaping 或 env/log diagnostics，不能写 MAS truth、owner receipt、typed blocker、human gate、publication eval、controller decision、current package、runtime queue 或 provider attempt。`scientific-compute-runner` 这类 compute helper 也只准备 env/provider intent 和日志诊断；provider resource、credential、endpoint lifecycle、SLURM / Modal / cloud execution receipt 继续归 OPL Connect / Runway。
