...
当前状态是 `required_capability_package_consumer` + `registry_summary_and_inventory_readback` + `active_module_descriptor_consumer` + `refs_only_execution_receipt_candidate_consumer` + `file_materialized_package_refs_consumer` + `candidate_artifact_owner_gate_request_readback`。MAS 通过 `contracts/opl_agent_package_manifest.json` 声明 provider version、ABI、11 个 required Skill 和 8 个 required module；通过公开 `opl connect agent-packages status --package-id mas --json` 消费 dependency readiness。个别已安装 capability 的 refs-only调用仍可 fail-open，但整个 required package 缺失、digest/ABI 不兼容或核心 export 不全时必须 fail-closed 到 doctor/repair，不能借用 capability-level fail-open 继续普通运行。
...
```bash
opl connect agent-packages status --package-id mas --json
opl connect agent-packages repair --package-id mas --agent-root <agent_root> --json
```
...
| required capability package readback | `done_for_mas_consumer_contract` | MAS manifest 与 generated `scientific_capability_registry` 暴露 `scholarskills_required_package`；doctor 只消费公开 `opl connect agent-packages status` 的 dependency readiness，并把 missing/incompatible 映射为 `operational_ready=false` 与统一 repair route。 | MAS repo contract/tests 不证明 OPL 已完成依赖闭包安装；真实安装、digest lock、卸载保护、原子回滚与 current readback 由 OPL lifecycle receipt 证明。 |
