...
- `systemd|cron|launchd|docker` workspace-local service managers 已从公开 CLI manager choices 和 controller direct-call payload 中移除；它们不安装旧 OS service / cron template，也不写旧 install proof。旧 service 文件或 loaded state 只作为 Hermes optional adapter 的 `retired_cleanup_evidence` 暴露。OS backend 只能通过默认 `local` adapter 进入；macOS LaunchAgent backend 已落地，Linux / container 持久 backend 仍 fail-closed。
