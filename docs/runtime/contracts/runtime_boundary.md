...
- `runtime-ensure-supervision --manager systemd|cron|launchd|docker` 当前返回 `retired_workspace_local_service_manager`，不安装旧 OS service / cron template，也不写旧 install proof。OS backend 只能通过默认 `local` adapter 进入；macOS LaunchAgent backend 已落地，Linux / container 持久 backend 仍 fail-closed。
