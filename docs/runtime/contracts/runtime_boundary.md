...
- `Hermes gateway cron`
  - 作为显式 legacy diagnostic cleanup adapter 读取旧 job/script/session/gateway 状态，或移除旧 job/script
...
截至 `2026-05-20` 的 repo 实现，默认 scheduler owner 已迁到 OPL replacement；`local` 是 tombstone/provenance-only，Hermes 只作为显式 legacy proof / diagnostic cleanup adapter 保留：

- `runtime-ensure-supervision --manager hermes` 已从公开 CLI manager choices 移除；controller direct-call 只返回 retired tombstone，不写 `~/.hermes/scripts/med-autoscience/<workspace-key>/watch_runtime_tick.py`，不通过 external Hermes CLI 调用 `cron create` / `cron edit` / `cron resume` / `cron run`。
...
- `runtime-remove-supervision --manager hermes` 只用于清理旧 Hermes cron job、旧 script 和 retired legacy service evidence；不创建 replacement job。
- MAS one-shot tick contract 仍是 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`，但调用 owner 是 OPL provider/runtime manager。较早注册的真实 workspace job 可能仍是单步 `watch-runtime` script，只能作为 legacy diagnostic drift 被读取或清理，不能通过 MAS Hermes ensure 刷新到新脚本形态。
...
一步到位的 scheduler contract 与迁移计划见 [Domain SLO Scheduler Projection Contract](../control/domain_slo_scheduler_projection_contract.md)。
