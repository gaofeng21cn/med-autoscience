...
这份文档定义 `MedAutoScience`、`MAS Runtime OS`、`MAS supervision scheduler contract`、默认 `local` adapter、optional `Hermes gateway cron` adapter 与 `MedDeepScientist` 在当前架构下的边界。

## 一句话版本

`MedAutoScience` 是唯一研究入口与 research gateway，外层由单一 MAS app skill 承接稳定 callable surface；`MAS Runtime OS` 是默认 runtime/backend owner；`MAS supervision scheduler contract` 是外层监管调度 owner；`local` 是默认 scheduler adapter，macOS 落到 MAS-owned LaunchAgent；`Hermes gateway cron` 只作为显式 optional adapter，不是研究运行中心；`MedDeepScientist`（仓库名 `med-deepscientist`）只作为 frozen source archive、historical fixture、explicit archive import reference / backend audit reference；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。
...
Executor adapter note：generic executor adapter 归 `OPL`；MAS 只声明 executor requirement、接收 OPL typed closeout / domain-task receipt。MAS 本地 `codex_cli_default` 只用于 standalone diagnostics；`Hermes-Agent` / `Claude` 只能作为 OPL 显式 opt-in provider/provenance/receipt 路径出现，不能扩展成 MAS-owned `executor_kind`、MAS-owned hosted executor 或 MAS runtime truth。
...
| --- | --- | --- |
| Runtime Core | 当前 run / worker / turn 到底在做什么、是否还活着、完成后如何续跑 | `MAS Runtime OS` / `mas_runtime_core` / MAS-owned watchdog 与 turn lifecycle kernel |
| Supervisor Scheduler | 多久触发一次外环检查、stale / crash 后谁叫醒 MAS | `MAS supervision scheduler contract`；默认 adapter 是 `local`，macOS backend 已实现为 MAS-owned LaunchAgent；Hermes 是显式 optional adapter |
...
- `local` scheduler adapter
  - 负责默认外层定时唤醒：注册、启停和执行 MAS 生成的 supervision tick script
  - macOS backend 使用 MAS-owned LaunchAgent；无持久 backend 的环境 fail-closed 并暴露 one-shot reconcile 语义
- `Hermes gateway cron`
  - 作为显式 optional adapter 注册、启停和执行 MAS 生成的 supervision tick script
...
- `MAS Runtime OS`
  - 负责默认 runtime/backend state、event、recovery 与 quest lifecycle
  - 负责 backend registry、backend selection、runtime binding 与 runtime durable metadata
...
截至 `2026-05-09` 的 repo 实现，`local` 是默认 MAS scheduler adapter，Hermes 在显式 optional adapter 路径里的事实如下：
...
- 当前 desired tick script 不是研究执行器；它只调用 MAS workspace entry，并把结果交还给 MAS durable surface。当前 desired command sequence 是 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`。较早注册的真实 workspace job 可能仍是单步 `watch-runtime` script，必须通过 `runtime-ensure-supervision --profile <profile>` 刷新到当前脚本形态。
- `med_autoscience.runtime_transport.hermes` 当前是 optional external Hermes-Agent diagnostics / provenance / binding contract。默认 `create_quest`、`resume_quest`、`pause_quest`、`stop_quest`、`get_quest_session`、`inspect_quest_live_runtime` 仍 fail-closed，并提示使用 `mas_runtime_core`；它没有成为默认 hosted executor、MAS-owned executor adapter 或 runtime truth。
- `systemd|cron|launchd|docker` workspace-local service managers 已从公开 CLI manager choices 中移除。内部 cleanup/audit surface 仍可把它们报告为 `retired_workspace_local_service_manager`，不安装旧 OS service / cron template，也不写旧 install proof。OS backend 只能通过默认 `local` adapter 进入；macOS LaunchAgent backend 已落地，Linux / container 持久 backend 仍 fail-closed。

因此，当前 Hermes 依赖的实际含义是“外层 scheduler adapter 依赖”，不是“MAS 依赖 Hermes 才能做研究执行、会话管理、论文判断或 runtime truth”。

## Scheduler 优化方向

最优方向不是新增一个 MDS 式 resident daemon，也不是继续把 `Hermes` 写成架构中心。当前 canonical 语义已经收敛为 `MAS supervision scheduler contract`：

- MAS 定义唯一 tick payload、运行顺序、幂等性、并发 / 去重、失败投影和 status schema。
- `local` 是该 contract 的默认 adapter；`Hermes gateway cron` 是 explicit optional adapter。
- Hermes 对本地运行的必要依赖已从默认路径移除；继续扩展 `local` adapter 时必须与 Hermes adapter 写同一类 status / SLO / latest-run projection，而不是复活旧 workspace-local host service。
- Hermes 后续只能作为 OPL 显式 opt-in provider/provenance/diagnostics 的候选来源，或在 parity 后按 `retire_after_parity` 降级/退役；不能作为 MAS 默认 executor adapter、研究运行 owner 或 hidden hosted runtime。
...
- [Outer-Loop Wakeup And Decision Loop](../control/outer_loop_wakeup_and_decision_loop.md)
...
旧 `ops/med-deepscientist/runtime/quests/` 只作为 compatibility reader、restore proof 和迁移输入保留；新 writer 不再把它作为默认写入目标。
