...
| Domain authority adapter | MAS domain refs、receipt、typed blocker、guarded apply 与 diagnostic explanation 怎么交还 owner chain。 | MAS domain authority functions + body-free `opl_state_index_source_adapter` |
| Product projection | 用户、PI、开发者怎么看进度、日志、阻塞和下一步。 | `study-progress` / cockpit / OPL App / hosted workbench，只读消费 OPL current-control-state 与 MAS domain authority refs。 |
...
## Active Runtime Vocabulary / Residue Readback Boundary

清理旧残留时，`runtime`、`lifecycle`、`workbench`、`scheduler`、`storage` 这些泛词本身不是违规信号。违规信号是它们重新变成 MAS 私有 runtime owner、默认执行入口、机器合同 owner claim、queue/attempt/readiness 真相，或绕过 OPL hosted surface 的 action surface。

- `lifecycle` 合法语境：OPL generic lifecycle、refs-only index family、restore / retention migration vocabulary。禁止语境：MAS-owned lifecycle engine、worker lease、queue/attempt owner、runtime truth owner。
- `workbench` 合法语境：OPL App / hosted workbench、product projection、read-only drilldown。禁止语境：MAS private Live Console、terminal command input、publication readiness UI verdict、second truth source。
- `scheduler` 合法语境：OPL provider/runtime manager、legacy diagnostic/provenance、future hosted scheduler gate。禁止语境：MAS-local LaunchAgent/systemd/cron/docker default active scheduler、MAS-owned generic scheduler、workspace-local scheduler owner。
- `storage` 合法语境：OPL-authorized maintenance adapter、refs-only state index、restore proof、cold-store retention、dataset boundary docs。禁止语境：unauthorized physical cleanup、runtime currentness proof、paper progress proof、artifact authority。
- `runtime` 合法语境：OPL generic runtime core、MAS authority adapter、product projection。禁止语境：MAS private generic runtime owner、queue/attempt ledger、retry/dead-letter、worker residency、transition runner。
- 旧 `DHD`、`owner-route-reconcile`、`domain-owner-action-dispatch/default-executor`、`progress-portal`、`runtime-storage-maintenance`、`sqlite-sidecars` 只能出现在 history/provenance/tombstone、explicit migration diagnostic、OPL-owned refs / maintenance 边界，不能恢复为 active path、默认 entrypoint、paper mainline 或 runtime-ready/progress-ready claim。
- denylist/readback 命中必须分类为 `allowed_active_vocabulary`、`legacy_residue_candidate` 或 `forbidden_resurrection`，并引用本文件、`contracts/runtime/legacy-active-path-tombstones.json` 或对应 OPL owner 文档；不能用 raw token hit 直接判定“残留未清理”。
- 本边界只说明 active surface 与 residue readback 的分类规则；它不声明 live runtime ready、paper progress、publication-ready、provider running、production-ready 或 OPL takeover 已经发生。
...
- MAS domain authority functions
  ...
  - 负责 MAS domain durable metadata，并通过 body-free `opl_state_index_source_adapter` 把 ref metadata 交给 OPL StateIndex owner。
...
`domain-handler export` 的默认论文入口必须使用当前 `PaperMissionRun` / `PaperMissionTransaction` readback 中的 `opl_runtime_carrier` identity，并只通过 `/paper_mission_default_tasks` 暴露 `paper_mission/start_or_resume` handoff。`ops/medautoscience/paper_mission_consumption_ledger/**/opl_route_handoff.json` 只能作为同一 transaction / stage terminal decision / route command / carrier aggregate identity 的 governed consume evidence 附加到该 handoff；旧 ledger、跨 identity ledger 或只证明 transport / owner-consumption 的 ledger 不得覆盖当前默认 carrier，不得把默认 handoff 改回 legacy `stage_outcome/opl-handoff`，也不得声明 OPL enqueue、StageRun created、provider running、runtime-ready、paper progress 或 MAS authority materialization。
...
- `runtime/artifacts/opl_state_index_source_adapter/authority_refs_source.json`：body-free source-adapter semantic ref；不要求或授权 MAS-local database persistence。
- `ops/medautoscience/`：唯一 MAS workspace ops 入口，承载受控 wrapper、profile、bootstrap、diagnostic 和 body-free authority-ref adapter。
- `ops/mas/**`：legacy bridge / static display residue；当前不再作为 active MAS workspace 入口。用户可见 workbench / display 归 OPL hosted surface，MAS 只输出 refs-only projection。

这些目录不是面向研究用户或 Agent 的正式研究入口。

`ops/mas/bin/start-web`、`ops/mas/bin/status`、`ops/mas/bin/doctor`、`ops/mas/bin/stop`、`ops/mas/config.env` 和 `ops/mas/behavior_equivalence_gate.yaml` 已退役；它们不得作为研究启动、runtime control、behavior readiness 或 fallback 决策面。所有默认 workspace runtime 路径的程序化派生应统一走 `med_autoscience.runtime_protocol.layout` 和 `ops/medautoscience/` 受控入口，而不是在 controller、workspace scaffold 或 wrapper 中重复硬编码。

旧 `ops/mas` bridge renderer、wrapper、alias、facade 只要已经没有 active caller，并具备 replacement parity、no-forbidden-write proof 与 tombstone/provenance ref，就可以按静态证明物理退役。`same_work_unit_live_evidence` 只约束当前 owner-answer compensation chain；它不应被泛化成阻止 retired no-active-caller 旧面删除的 live-run 前置条件。
...
- `runtime_lifecycle_payload_retention` 与 `runtime_storage_maintenance` 只能作为 OPL-authorized maintenance callable adapter：apply / physical mutation 必须消费绑定 operation、maintenance surface、scope、authorized outcome 和 authorization ref 的 OPL maintenance authorization proof；dry-run、restore-proof canary、refs-only index-only 或 planned retention 只能是 source-retained / body-free projection，不能声明 runtime currentness、provider admission、paper progress、publication readiness、OPL live takeover 或物理退役完成；
