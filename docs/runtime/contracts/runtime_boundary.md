...
这份文档定义 `MedAutoScience`、`MAS Runtime OS`、MAS direct/local diagnostic scheduler surface、optional `Hermes gateway cron` adapter 与 `MedDeepScientist` 在当前架构下的边界。

## 一句话版本

`MedAutoScience` 是唯一研究入口与 research gateway，外层由单一 MAS app skill 承接稳定 callable surface；`MAS Runtime OS` 持有当前 repo-side runtime/backend truth；MAS-owned local scheduler / LaunchAgent / 300 秒 tick 是 direct/local diagnostics 的当前活代码残留和迁移桥，后续应迁往 OPL provider/runtime manager 或退役；`Hermes gateway cron` 只作为显式 optional adapter，不是研究运行中心；`MedDeepScientist`（仓库名 `med-deepscientist`）只作为 frozen source archive、historical fixture、explicit archive import reference / backend audit reference；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。
...
| Supervisor Scheduler | direct/local diagnostics 中多久触发一次外环检查、stale / crash 后谁叫醒 MAS | 当前活代码仍是 `MAS supervision scheduler contract` + `local` adapter，macOS backend 已实现为 MAS-owned LaunchAgent；Hermes 是显式 optional adapter；OPL-hosted production wakeup / queue / attempt 归 OPL provider |
...
- `local` scheduler adapter
  - 负责 direct/local diagnostics 外层定时唤醒：注册、启停和执行 MAS 生成的 supervision tick script
...
截至 `2026-05-16` 的 repo 实现，`local` 仍是 MAS direct/local diagnostic scheduler adapter，Hermes 在显式 optional adapter 路径里的事实如下：
...
- `systemd|cron|launchd|docker` workspace-local service managers 已从公开 CLI manager choices 和 controller direct-call payload 中移除；它们不安装旧 OS service / cron template，也不写旧 install proof。旧 service 文件或 loaded state 只作为 Hermes optional adapter 的 `retired_cleanup_evidence` 暴露。OS backend 只能通过 direct/local `local` adapter 进入；macOS LaunchAgent backend 已落地，Linux / container 持久 backend 仍 fail-closed。

因此，当前 Hermes 依赖的实际含义是“外层 scheduler adapter 依赖”，不是“MAS 依赖 Hermes 才能做研究执行、会话管理、论文判断或 runtime truth”。

## Scheduler 优化方向

最优方向不是新增一个 MDS 式 resident daemon，也不是继续把 `Hermes` 写成架构中心。当前 direct/local diagnostic 语义已经收敛为 `MAS supervision scheduler contract`：

- MAS 定义唯一 tick payload、运行顺序、幂等性、并发 / 去重、失败投影和 status schema。
- `local` 是该 contract 的 direct/local adapter；`Hermes gateway cron` 是 explicit optional adapter。
