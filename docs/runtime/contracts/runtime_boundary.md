...
这份文档定义 `MedAutoScience`、OPL-owned stage/runtime control plane、MAS domain authority refs / owner receipt / typed blocker boundary、local scheduler tombstone/provenance refs、legacy `Hermes gateway cron` diagnostic adapter 与 `MedDeepScientist` 在当前架构下的边界。

## 一句话版本

`MedAutoScience` 是唯一研究入口与 research gateway，外层由单一 MAS app skill 承接稳定 callable surface；默认 generic runtime owner 是 OPL provider-backed stage runtime；MAS 只承担 domain authority refs、DomainIntent / owner route、owner receipt、typed blocker、artifact/source/quality refs、guarded apply receipt 与 diagnostic explanation；默认 outer supervision scheduler owner 已迁到 OPL `opl_provider_runtime_manager` / `opl_family_runtime_provider` replacement；MAS supervision contract 保留 paper-progress SLO 解释、owner receipt、typed blocker、safe action refs 和 legacy tombstone/provenance refs；`local` 已从公开 CLI manager choices 移除，macOS LaunchAgent 只作为历史 tombstone/provenance 对象；`Hermes gateway cron` 只作为显式 legacy diagnostic adapter，不是研究运行中心；`MedDeepScientist`（仓库名 `med-deepscientist`）只作为 frozen source archive、historical fixture、explicit archive import reference / backend audit reference；`DeepScientist` 只在指代其上游来源或兼容语义时单独出现。
...
Physical/source retirement note：`runtime_transport/mas_runtime_core*`、turn runner、worker lease、`lifecycle_refs_adapter.py`、runtime lifecycle CLI 和旧 runtime status/read-model owner 语义已按 no-alias physical retirement 处理，不再作为 domain receipt adapter、diagnostic fallback 或 compatibility surface 读取。Generic sidecar provider lifecycle CLI 已退役，剩余 owner-route handoff 只能表达 body-free MAS owner-route refs、owner receipt、typed blocker 和 OPL handoff input；不得重新承担 queue、attempt、retry/dead-letter、worker liveness、runtime lifecycle 或 operator current-state owner。

2026-05-23 physical retirement note：`runtime_transport_core_bridge`、`runtime_turn_runner_closeout_adapter`、`worker_lease_residency_projection` 与 `lifecycle_refs_sqlite_index` 不再是 retained active-path residue；当前 replacement 是 OPL current control state / provider attempt ledger，加 MAS `domain_authority_refs_index`、owner receipt 和 typed blocker。`legacy_supervisor_scheduler_tombstone` 继续只保留 history/tombstone/provenance refs，不得恢复 alias、facade 或 compat wrapper。
...
| Product Projection | 用户、PI、开发者怎么看进度、日志、阻塞和下一步 | `Progress Portal` / `study-progress` / cockpit / OPL App workbench，只读消费 OPL `current_control_state` 与 MAS domain authority refs；旧 Live Console 只作为 history/provenance 读取 |
...
- OPL current control plane
  - 负责默认 outer supervision cadence、scheduler lifecycle、provider SLO、job registry / latest-run projection、attempt、retry/dead-letter 与 runtime manager 投影
  - 通过 OPL `current_control_state` / provider attempt ledger 暴露；MAS 不再提供 `runtime-supervision-*` compatibility CLI，也不安装 MAS-owned OS scheduler
...
- `Hermes gateway cron`
  - 作为显式 legacy diagnostic / provenance refs 读取旧 job/script/session/gateway 状态
  - 不再注册、刷新、启停或执行 MAS 生成的 supervision tick script
  - 负责提供 Hermes adapter 的 job registry、schedule state、latest cron session record 与 gateway service liveness projection
...
截至 `2026-05-23` 的 repo 实现，默认 scheduler/runtime owner 已迁到 OPL `current_control_state`；`local` 是 tombstone/provenance-only，Hermes 只作为显式 legacy proof / provenance refs 保留：

- `runtime-supervision-status`、`runtime-ensure-supervision` 和 `runtime-remove-supervision` 已从 active CLI/MCP/product/workspace surface 退役；旧 direct-call/fixture 只能返回 retired tombstone，不写 `~/.hermes/scripts/med-autoscience/<workspace-key>/watch_runtime_tick.py`，不通过 external Hermes CLI 调用 `cron create` / `cron edit` / `cron resume` / `cron run`。
- 旧 Hermes cron jobs、sessions、gateway service 状态和 MAS 生成脚本状态只能作为 retired provenance refs 或 legacy drift evidence 读取，不形成 workspace supervision current state。
- MAS one-shot tick contract 已收敛为 `domain-health-diagnostic`、`owner-route-reconcile`、`domain-action-request-materialize` 与 `domain-owner-action-dispatch` 的 refs-only / developer-gated safe action surface；较早注册的真实 workspace job 可能仍是单步 `watch-runtime` script，只能作为 legacy diagnostic drift 被读取或清理，不能通过 MAS Hermes ensure 刷新到新脚本形态。
- `med_autoscience.runtime_transport.hermes` 当前是 explicit external Hermes-Agent diagnostics / provenance / binding contract。默认 `create_quest`、`resume_quest`、`pause_quest`、`stop_quest`、`get_quest_session`、`inspect_quest_live_runtime` 仍 fail-closed，并提示使用 OPL provider-backed stage runtime；它没有成为默认 hosted executor、MAS-owned executor adapter 或 runtime truth。
...
因此，当前 Hermes 依赖的实际含义是“旧 scheduler 生成物诊断/清理依赖”，不是“MAS 依赖 Hermes 才能做研究执行、会话管理、论文判断、周期调度或 runtime truth”。
...
- `local` 是 tombstone/provenance-only；`Hermes gateway cron` 是 explicit legacy diagnostic cleanup adapter。
- Hermes 和本机 LaunchAgent 对本地运行的必要依赖均已从默认路径移除；继续保留 `local` adapter 时必须服务 no-resurrection proof、cleanup proof 和同构诊断，而不是复活旧 workspace-local host service。
...
- 对仍残留 Hermes cron 的 workspace，MAS 只能投影 job / session / latest run 状态，或清理旧 job/script；文档和 UI 不得把 `Hermes-hosted` 泛化成 MAS 的研究运行 owner。

一步到位的 scheduler contract 与迁移计划见 [OPL Unique Control Plane Boundary Contract](../control/opl_unique_control_plane_boundary_contract.md)。
...
- OPL runtime 执行 stage/runtime 动作；MAS domain authority functions 只签收、阻断或解释被批准的 domain 动作
