...
- `ops/mas/progress/`：Progress Portal 静态只读投影。`ops/mas` 不再承载 launcher config、薄运维脚本、behavior gate 或 runtime control 面。
