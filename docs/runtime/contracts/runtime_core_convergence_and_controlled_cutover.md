...
- `MAS supervision scheduler contract` 仍是 direct/local diagnostics 的外层监管调度残留；`local` adapter 是 MAS-owned local scheduler，macOS backend 已落地为 LaunchAgent，每 300 秒调用 MAS-owned supervision tick script；desired script 依序运行 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`。`Hermes gateway cron` 只在显式选择时作为 optional adapter。该 surface 后续应迁往 OPL provider/runtime manager 或退役。
...
- MAS 外层 direct/local diagnostics 行为：Supervisor Scheduler adapter 定时唤醒 MAS-owned supervision tick script；local adapter 是 MAS-owned scheduler，macOS backend 是 LaunchAgent。runtime state、event、owner route 和 progress read-model 由 MAS durable surface 持有；OPL-hosted production wakeup / queue / attempt 归 OPL provider。
...
- 需要本机 direct/local diagnostic 长时唤醒时，当前正确入口是 MAS supervision scheduler contract 下的 adapter；`local` 是 direct/local adapter，Hermes gateway cron 是 explicit optional adapter，不是旧 workspace-local launchd/systemd/cron/docker service。OPL-hosted production 长时唤醒继续归 OPL provider。
...
- MAS supervision scheduler contract 的 direct/local diagnostic 残留；local adapter 是本机诊断路径，Hermes gateway cron 是 explicit optional hosted / provider adapter，后续按 replacement proof 迁移或退役。
