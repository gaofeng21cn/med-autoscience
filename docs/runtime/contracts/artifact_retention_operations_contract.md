...
OPL-owned lifecycle refs / StateIndex store 只作为 refs index/read model/receipt 查询层。MAS 通过 body-free source adapter 提供 retention plan、archive ref、checksum、restore proof 和 owner route receipt metadata，不保留 MAS-local SQLite index；论文真相、用户干预记忆、artifact mutation authorization 和可交付文件仍保留在 MAS authority surface 与文件形态中。
