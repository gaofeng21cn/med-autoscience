...
- Supervision scheduler tick：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL replacement，返回 `scheduler_owner=opl_provider_runtime_manager` 与 `adapter_id=opl_family_runtime_provider`，不注册或刷新 MAS-owned OS scheduler；显式 `--manager local` 才注册/刷新 legacy MAS-owned tick script，macOS backend 使用 LaunchAgent，desired script 依序运行 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`
- Scheduler 注册/刷新入口：`runtime-ensure-supervision --profile <profile>`；显式 `--manager local` 可做 legacy diagnostic / cleanup，显式 `--manager hermes` 可刷新 Hermes optional adapter，各 adapter 保持同构 status / receipt / SLO
...
同一条升级路径会把旧 workspace-local host-service 漂移识别为 retired cleanup evidence。旧 launchd/systemd/cron/docker service 不再作为 active runtime option 保留；当前 `runtime-ensure-supervision` 默认委托 OPL replacement，显式 `--manager local` 只用于 legacy diagnostic / cleanup 并可刷新 MAS-owned local adapter；显式 `--manager hermes` 只刷新 optional Hermes adapter。
...
- 默认 OPL scheduler replacement、显式 legacy `local` diagnostic adapter、显式 optional `hermes` adapter，或显式 `runtime-supervisor-reconcile --mode developer_apply_safe --apply` 负责持续刷新 outer supervisor evidence；没有新鲜 tick / reconcile evidence，`study-progress` 必须诚实降回 `managed_runtime_supervision_gap`
...
当前 closeout 已把 MAS Runtime OS / `mas_runtime_core` 切成默认 runtime/backend owner。默认 scheduler owner 已迁到 OPL replacement；显式 `local` scheduler 的意义是作为 legacy diagnostic / cleanup path 按固定间隔调用 MAS-owned supervision tick script；Hermes gateway cron 只在显式选择时承担同构 adapter 角色。它们都不把 external Hermes repo、external MDS repo 或旧 MDS daemon 重新变成默认 runtime truth。
