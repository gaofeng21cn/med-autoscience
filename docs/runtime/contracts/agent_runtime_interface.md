...
- MAS direct/local diagnostic scheduler surface = 当前仍由 `MAS supervision scheduler contract` + MAS-owned `local` adapter 承载，macOS backend 已落地为 LaunchAgent；`Hermes gateway cron` 是 explicit optional adapter；OPL-hosted production wakeup / queue / attempt 归 OPL provider
...
Current reading note：本文件里的 `Hermes` 指显式 `--manager hermes` 的 `Hermes gateway cron` scheduler adapter 或 optional hosted/runtime target；它不是 MAS study truth owner、runtime truth owner、session owner 或默认 executor owner。study truth 继续由 `MedAutoScience` 的 study-owned surfaces、controller decisions 与 publication gate 持有。
...
- 当前默认 runtime backend 固定为 MAS-owned `mas_runtime_core` / Runtime OS；外部 `med-deepscientist` repo、daemon、runtime root 或 WebUI 不参与默认 operation。
...
- 保持 MAS Runtime OS / `mas_runtime_core` 为默认 runtime/backend owner
...
- `runtime_escalation_record` 与 `runtime_watch` 继续是 quest-owned runtime artifact；旧 `ops/med-deepscientist/...` 只作为 historical fixture / explicit archive import reference
...
- sidecar provider 与 figure routes 指南：[`sidecar_figure_routes.md`](../../delivery/medical-display/contracts/sidecar_figure_routes.md)
...
如果任务已经进入需要正式纳管的自动科研推进，则应按契约先走 `doctor -> workspace bootstrap -> runtime overlay-status`，再进入对应 managed route。

## 唯一研究入口

在当前架构里，`MedAutoScience` 是唯一研究入口和独立 medical research domain agent；MAS direct/local diagnostic scheduler surface 当前仍由 `MAS supervision scheduler contract` + MAS-owned `local` adapter 承载，`MAS Runtime OS` 持有 repo-side runtime/backend truth；OPL-hosted production wakeup / queue / attempt 归 OPL provider；`MedDeepScientist`（仓库名 `med-deepscientist`）只在 frozen source archive、historical fixture、显式 archive import / provenance reference 和上游 intake 里出现。`DeepScientist` 只在上游比较、兼容审计和历史命名里单独出现。
...
- 所有正式研究推进都应经由 `doctor`、`workspace bootstrap`、`runtime overlay-status`、`study ensure-runtime` 和受管 route
...
- Supervision scheduler tick：`runtime-ensure-supervision --profile <profile>` 注册或刷新默认 `local` adapter 的 MAS-owned tick script；macOS backend 使用 LaunchAgent，desired script 依序运行 `watch-runtime --max-ticks 1`、`supervisor-scan`、`supervisor-consume`、`supervisor-execute-dispatch`
- Scheduler 注册/刷新入口：`runtime-ensure-supervision --profile <profile>`；显式 `--manager hermes` 可刷新 Hermes optional adapter，二者保持同构 status / receipt / SLO
...
如果 workspace 来自较早的骨架版本，应先重跑一次 `init-workspace`。当前 controller 会在不加 `--force` 的前提下，自动升级 `_shared.sh`、`watch-runtime` 和 supervisor entry scripts，并删除已退役的 workspace-local service wrapper；scheduler 安装、状态和移除统一走 `runtime-ensure-supervision`、`runtime-supervision-status` 和 `runtime-remove-supervision`。新 workspace 默认 no root Git / no quest Git；current workspaces 的 root Git 已完成 restore-proof full retirement。未来接入外部或旧 workspace 时若发现 root `.git`，只走显式 inventory / archive / remove / verify diagnostic，不属于普通 bootstrap 成功条件，也不得重新成为 Agent 状态面。
...
同一条升级路径会把旧 workspace-local host-service 漂移识别为 retired cleanup evidence。旧 launchd/systemd/cron/docker service 不再作为 active runtime option 保留；当前 `runtime-ensure-supervision` 负责清理旧 service evidence，并默认注册/刷新 MAS-owned local adapter；显式 `--manager hermes` 只刷新 optional Hermes adapter，二者复用同一 scheduler contract。
...
- `workspace-cockpit` 应直接投影 repo 主线快照、workspace 级 readiness、latest task intake、MAS scheduler status、stale / missing progress signal 聚合，以及按优先级排好的 workspace attention queue
...
- Scheduler adapter（默认为 `local`，可显式选择 `hermes`）或显式 `runtime-supervisor-reconcile --mode developer_apply_safe --apply` 负责持续刷新 outer supervisor evidence；没有新鲜 tick / reconcile evidence，`study-progress` 必须诚实降回 `managed_runtime_supervision_gap`
...
历史上真实 workspace 曾使用 workspace-local launchd service 验证监管刷新；该路径现在只作为历史/debug 事实保留。当前 direct/local diagnostic contract 以 MAS supervision scheduler contract 为准，local adapter 承载 one-shot tick，旧 launchd/systemd/cron/docker service 出现时应被清理，不再作为“产品态常驻路径”或替代 scheduler 推荐给 Agent。
...
- MAS direct/local diagnostic scheduler surface 负责周期唤醒 MAS one-shot tick；当前 local adapter 由 `MAS supervision scheduler contract` 承载，Hermes gateway cron 是 explicit optional adapter；OPL-hosted production wakeup / queue / attempt 归 OPL provider
- `MAS Runtime OS` 负责默认 runtime state、event、recovery 与 quest lifecycle
...
当前 closeout 已把 MAS Runtime OS / `mas_runtime_core` 切成默认 runtime/backend owner。默认 `local` scheduler 的意义是按固定间隔调用 MAS-owned supervision tick script；Hermes gateway cron 只在显式选择时承担同构 adapter 角色。它们都不把 external Hermes repo、external MDS repo 或旧 MDS daemon 重新变成默认 runtime truth。

### 当前 `Hermes` 名义与宿主安装的关系

`med_autoscience.runtime_transport.hermes` 当前只能按 explicit hosted/runtime target 或 compatibility reference 理解。默认 MAS operation 走 `mas_runtime_core`，也就是说：
