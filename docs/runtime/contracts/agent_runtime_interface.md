...
- 默认 OPL scheduler replacement、显式 legacy `local` diagnostic adapter、显式 `hermes` diagnostic adapter，或显式 `domain-route-reconcile --mode developer_apply_safe --apply` 负责持续刷新 outer supervisor evidence；没有新鲜 tick / reconcile evidence，`study-progress` 必须诚实降回 `managed_runtime_supervision_gap`
