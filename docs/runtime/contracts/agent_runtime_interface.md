...
- Scheduler owner 入口：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL；`ensure --manager hermes` 已从公开入口移除，controller direct-call 只返回 retired tombstone；显式 Hermes 只保留 status/remove cleanup；`local` 不再作为 active status/remove/ensure command 暴露
...
同一条升级路径会把旧 workspace-local host-service 漂移识别为 retired tombstone/provenance evidence。旧 launchd/systemd/cron/docker service 不再作为 active runtime option 保留；当前 `runtime-ensure-supervision` 默认委托 OPL replacement，`local` 不再作为 active CLI manager 暴露；显式 Hermes 只读取或清理旧 proof/diagnostic adapter，不刷新 cron job 或 tick script。
