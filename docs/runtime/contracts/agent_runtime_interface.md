...
- Scheduler owner 入口：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL；显式 `--manager hermes` 可刷新 Hermes optional adapter；`local` 不再作为 active status/remove/ensure command 暴露
...
同一条升级路径会把旧 workspace-local host-service 漂移识别为 retired tombstone/provenance evidence。旧 launchd/systemd/cron/docker service 不再作为 active runtime option 保留；当前 `runtime-ensure-supervision` 默认委托 OPL replacement，`local` 不再作为 active CLI manager 暴露；显式 `--manager hermes` 只刷新 optional Hermes adapter。
