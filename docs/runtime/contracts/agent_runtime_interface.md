...
- `MAS Runtime OS` = domain runtime adapter / owner receipt surface，只保留 quest lifecycle refs、runtime event refs、guarded apply、typed blocker、paper-progress SLO 解释和 standalone diagnostic
...
- 当前默认 generic runtime owner 固定为 OPL provider-backed stage runtime；`mas_runtime_core` 只作为 MAS domain owner receipt adapter / diagnostic surface，外部 `med-deepscientist` repo、daemon、runtime root 或 WebUI 不参与默认 operation。
...
- 保持 MAS Runtime OS / `mas_runtime_core` 只作为 domain adapter、owner receipt、typed blocker、event refs 与 diagnostic surface
...
在当前架构里，`MedAutoScience` 是唯一研究入口和独立 medical research domain agent，默认 outer supervision scheduler owner 是 OPL `opl_provider_runtime_manager`，默认 adapter 是 `opl_family_runtime_provider`；`MAS supervision scheduler contract` 保留 paper-progress SLO/read-model、domain tick payload、owner receipt、typed blocker、safe action refs 和 legacy diagnostic projection；默认 generic runtime owner 是 OPL provider-backed stage runtime，`MAS Runtime OS` / `mas_runtime_core` 只承担 MAS domain adapter、owner receipt、typed blocker、event refs 与 diagnostic surface；`MedDeepScientist`（仓库名 `med-deepscientist`）只在 frozen source archive、historical fixture、显式 archive import / provenance reference 和上游 intake 里出现。`DeepScientist` 只在上游比较、兼容审计和历史命名里单独出现。
...
- Supervision scheduler tick：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL replacement，返回 `scheduler_owner=opl_provider_runtime_manager` 与 `adapter_id=opl_family_runtime_provider`，不注册或刷新 MAS-owned OS scheduler；`local` 已从 active CLI manager choices 移除，只保留 tombstone/provenance refs
- Scheduler owner 入口：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL；显式 `--manager hermes` 只可刷新旧 Hermes proof/diagnostic adapter；`local` 不再作为 active status/remove/ensure command 暴露
...
同一条升级路径会把旧 workspace-local host-service 漂移识别为 retired tombstone/provenance evidence。旧 launchd/systemd/cron/docker service 不再作为 active runtime option 保留；当前 `runtime-ensure-supervision` 默认委托 OPL replacement，`local` 不再作为 active CLI manager 暴露；显式 `--manager hermes` 只刷新旧 Hermes proof/diagnostic adapter。
...
- 默认 OPL scheduler replacement、显式 legacy `local` diagnostic adapter、显式 `hermes` diagnostic adapter，或显式 `runtime-supervisor-reconcile --mode developer_apply_safe --apply` 负责持续刷新 outer supervisor evidence；没有新鲜 tick / reconcile evidence，`study-progress` 必须诚实降回 `managed_runtime_supervision_gap`
...
当前 closeout 已把默认 generic runtime owner 切到 OPL provider-backed stage runtime。MAS Runtime OS / `mas_runtime_core` 只作为 MAS domain adapter、owner receipt、typed blocker、event refs 与 diagnostic surface；默认 scheduler owner 已迁到 OPL replacement；显式 `local` scheduler 的意义只剩 legacy tombstone/provenance 或 cleanup diagnostic，不再按固定间隔调用 MAS-owned supervision tick script；Hermes gateway cron 只在显式选择时承担同构 diagnostic adapter 角色。它们都不把 external Hermes repo、external MDS repo 或旧 MDS daemon 重新变成默认 runtime truth。
