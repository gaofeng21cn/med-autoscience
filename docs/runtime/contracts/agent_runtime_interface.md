...
- Supervision scheduler tick：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL replacement，返回 `scheduler_owner=opl_provider_runtime_manager` 与 `adapter_id=opl_family_runtime_provider`，不注册或刷新 MAS-owned OS scheduler；显式 `--manager local` 也不注册/刷新 legacy MAS-owned tick script，只用于 legacy status/remove cleanup
- Scheduler owner 入口：`runtime-ensure-supervision --profile <profile>` 默认委托 OPL；显式 `--manager local` 返回 cleanup-only blocker，显式 `--manager hermes` 可刷新 Hermes optional adapter，各 adapter 保持同构 status / receipt / SLO
