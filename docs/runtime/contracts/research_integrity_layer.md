...
Research Integrity Layer 的目标态是 host-mediated receipt consumption。`review_and_quality_gate` / `finalize_and_publication_handoff` 的 mandatory Stage hook 声明 `opl_connect_reference_verification` 解析需求，由 OPL host 调用 OPL Connect 并注入只读 receipt/evidence；MAS pure builders 只消费该输入并组装 Research Integrity gate input。MAS 不直接调用 Connect，也没有独立 public action。
...
- 缺少 provider evidence 时，该层返回 `provider_receipt_required`、缺失 reference IDs 与 request-only `provider_resolution_action`；它不自行调用 OPL Connect。
...
当前非 live gate-input 达标条件：每条 reference 至少有一个 host 注入的 provider crosscheck 得到 `verified`，或被明确标成 `unresolved`、`needs_review`、`contradicted`、`retracted` 之一；不能因 receipt/evidence 缺失而静默通过。所有 provider evidence 都只是 gate input，不是权威 live citation verification；任何 live truth claim 仍需要对应 external provider invocation/readback 与 MAS owner surface consumption。
...
2. OPL host 根据 stage hook 的 request-only action requirement 调用 OPL Connect；Connect 查询已配置 provider 并返回 canonical read-only receipt，MAS 不实现也不触发 provider transport。
...
pure builders 不依赖旧 domain entry，不创建独立专业 skill、provider cache、runtime queue、provider attempt 或 authority receipt，也不通过临时文件或子进程调用 OPL。
