...
Research Integrity Layer 的目标态不是 MAS 只消费人工已有的 `provider_evidence`。流程是：review / publication gate 的 stage hook 触发 MAS 的 `research-integrity-reference-verification` domain action；它优先归一化已经提供的 evidence，再由 MAS domain provider lookup 查询 Crossref、PubMed、OpenAlex、Semantic Scholar、Crossmark 或 publisher evidence，并把结果交给 MAS Research Integrity Gate 生成 gate input。

这不是独立的专业 skill，也不把 PubMed 实现留在 OPL Connect。MAS 持有医学文献 lookup 的调用、证据归一化和 gate-input 组装；外部 provider source system 才是其 metadata/status 的事实来源。OPL 可在显式配置时提供通用 Crossref/OpenAlex metadata 或 coverage connector input，但不拥有 PubMed lookup、医学 citation acceptance、provider truth、publication authority 或 domain receipt。MAS 只把 lookup 结果作为 evidence input，不写 provider attempt、cache 或 receipt，也不能把网络响应包装成 live provider truth 或 publication-ready 结论。
...
- `MedAutoScienceDomainEntry.dispatch({"command": "research-integrity-reference-verification", ...})` 接受顶层 `provider_evidence` 和已提供的 `provider_receipts`，并把 receipt root 的 `provider_evidence`、`references[].provider_evidence`、`opl_connect_reference_verification.provider_evidence` 归一化为同一 gate input evidence；缺少可用 evidence 时，它调用 MAS domain provider lookup 查询 Crossref、PubMed、OpenAlex、Semantic Scholar first-slice evidence。Crossmark 使用 Crossref metadata signal；publisher 查询缺少可验证输入时返回 `publisher_connector_required` evidence，不伪造 publisher truth，也不声明 live provider authority。
- `MedAutoScienceDomainEntry.dispatch({"command": "research-integrity-review-publication-gate-stage-hook", ...})` 是 Review / Publication Gate 的 mandatory hook 输入面；它触发 `research-integrity-reference-verification` 并返回包含 `stage_obligation`、`stage_launch_required_input`、`target_stage_ids`、`triggered_domain_provider_lookup_contract` 和 `manuscript_consistency_meta_review` 的 `research_integrity_gate_input_bundle`。这些字段声明 mandatory gate input 和 MAS domain-owned provider lookup contract，不声明 live owner consumption。
...
- 当显式配置时的 Crossref/OpenAlex 通用 metadata、coverage 或 citation-graph connector input；其结果仍只作为 MAS domain action 的输入。

MAS 持有 PubMed 及医学 research-integrity provider lookup 的 domain implementation、证据归一化和 gate-input 组装。它不写 provider attempt、runtime queue、cache 或 receipt，不形成第二套 provider truth。
...
- MAS domain provider lookup 负责必要的医学文献 evidence 查询和输入归一化；外部 provider source system 保持 metadata/status 的事实来源。
- MAS pure builders 消费 evidence payload、比较 metadata、解释医学风险并输出 gate input。
