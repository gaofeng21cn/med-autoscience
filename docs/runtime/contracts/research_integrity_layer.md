...
这不是独立的专业 skill。provider lookup 的 provider truth 归 OPL Connect provider receipt / connector substrate；MAS 默认消费 OPL receipt/evidence。MAS thin provider client 只能作为 `transition_only_receipt_first` 的 dev/test 或 first-slice fallback，把联网 evidence 拉成 builder 可消费 payload；它不拥有 skill 路由、provider runtime、provider receipt、live provider truth 或 publication authority，也不能被写成 Claude Science 等价联网核验。
...
过渡态允许 MAS thin provider client 作为 first slice fetcher；它仍按 connector substrate 边界读取，不写 provider attempt、runtime queue、cache 或 receipt，不形成第二套 provider truth。
...
- OPL/provider lookup 负责联网拉取 provider evidence payload 并持有 provider receipt truth；MAS thin provider client 只可作为 transition-only / receipt-first 同步 fetcher。
