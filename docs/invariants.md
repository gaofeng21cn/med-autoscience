...
- 环境依赖在 `contracts/runtime_environment_requirements.json` 声明；prepare/run 归 OPL。临时任务以缓存复用和低启动开销为先，使用 `opl env run` 自动准备/复用 uv、renv 环境；运行记录引用已生成的环境版本清单，不要求每次完整冻结科研输入。MAS 可保留 `mas_provisioning_allowed=false` 的只读环境检查/投影，但不在 import、workspace 或 installer 中安装、修复环境，也不授权 ready。
