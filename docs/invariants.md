...
- `study_macro_state/latest.json` 是用户宏观状态的 materialized read model；状态矩阵、lifecycle report 和外部 operator 面应优先读取该 surface，没有 materialized surface 时才按当前 status/progress 派生 shadow projection。
