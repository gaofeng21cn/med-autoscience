...
- `MDS` 只能作为显式可选的 runtime/native/review event source、backend audit、legacy diagnostic、upstream intake 或行为等价 oracle；MAS 的 `StudyTruthKernel` 持有用户可见 `canonical_next_action`、publication gate 解释、package authority 解释和 delivery state。任何 truth/gate/status 事故必须同时落 reducer rule、fixture test 与 runbook entry。
