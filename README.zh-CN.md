...
- MAS package 的用户生命周期统一使用 `opl packages install mas`、`opl packages update mas` 和 `opl packages uninstall mas`；OPL 可默认随包提供并物化 `mas-scholar-skills` 作为可选专业增强，但其缺失或不兼容只形成 diagnostic，不阻断 MAS 安装、启动或 Stage route。
