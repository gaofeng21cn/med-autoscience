...
- 在 OPL family 中，MAS 是 domain agent package：MAS 保留医学领域 authority，OPL 持有通用 runtime、package carrier、generated wrapper 和 hosted surface。
...
- 旧 MAS-local scheduler、runner、session store、status shell 和 workbench wrapper 只作为迁移输入、诊断投影或 provenance 保留。新增 MAS-local 程序面必须说明医学 authority 理由，并只返回 owner receipt、typed blocker、domain ref 或 safe action ref。
...
- 当前 next action authority 是 MAS stage terminal outcome 物化出的 `NextActionEnvelope`。`OPL TransitionReceipt` 只是 transport receipt-only evidence；OPL queue / attempt / provider readiness 不能选择 MAS owner、证明论文进展，或替代 MAS owner receipt / typed blocker / human gate。
...
- MAS 的 canonical plugin source 在 `plugins/med-autoscience/`；旧 `plugins/mas` 只保留迁移兼容 alias。repo-local `.agents/plugins/marketplace.json` 是已退役本地状态，Codex marketplace 注册由 OPL wrapper / startup maintenance surface 持有。
...
- OPL Full online runtime 集成使用 `medautosci domain-handler export --profile <profile> --format json` 和 `medautosci domain-handler dispatch --task <task.json> --format json`。本地 CLI/status/manifest 只用于诊断 provider readiness，不是 provider admission、default next action 或 stage completion authority；Temporal 暂不可用时，状态面应明确报告 OPL production required dependency blocker。
