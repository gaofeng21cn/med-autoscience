...
- OPL Full online runtime 集成使用 `medautosci domain-handler export --profile <profile> --format json` 和 `medautosci domain-handler dispatch --task <task.json> --format json`。本地 CLI/status/manifest 只用于诊断 provider readiness；Temporal 暂不可用时，状态面应明确报告 OPL production required dependency blocker。
