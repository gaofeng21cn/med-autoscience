...
- MAS package 的用户生命周期统一使用 `opl packages install mas`、`opl packages update mas` 和 `opl packages uninstall mas`；OPL 在同一生命周期中解析并物化必需的 `mas-scholar-skills` 依赖闭包，依赖缺失或不兼容时 MAS readiness fail-closed 并进入托管 doctor/repair。
