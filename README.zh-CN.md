...
- `product-start`：先读取统一的启动 companion，查看推荐启动模式、可选模式、resume surface 与 human gate 摘要，再决定从哪条入口进入
...
- 如果你想单独看某一个阶段当前怎么用、有哪些入口、退出条件是什么，再看：`uv run python -m med_autoscience.cli mainline-phase --phase <current|next|phase_id>`
...
- 如果你想先拿到统一的 machine-readable 启动面，再决定走哪条入口：`uv run python -m med_autoscience.cli product-start --profile <profile> --format <markdown|json>`
