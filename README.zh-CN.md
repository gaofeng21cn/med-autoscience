...
在 OPL 语境里，这条研究线由 route 保持下一步 owner、human gate、receipt、typed blocker 和 route-back 证据；真正的医学专家工作发生在 MAS stage attempt 内部。执行器可以围绕研究问题生成候选假设、读取 source refs、比较分析路线、修订稿件主线、调用统计或文献工具，并把结果交给独立 AI reviewer、publication gate 或研究者决策。工具在这里是带权限和凭据边界的 affordance，不是写死的医学 workflow。
...
每个研究 stage 都保留开放式执行空间：可以产生多个研究主张候选，回到证据包核对，比较替代分析，吸收审阅意见，再形成下一版 manuscript delta 或 typed blocker。MAS 声明 stage 目标、skill、knowledge、tool affordance 和质量门；OPL 托管 attempt 和投影，不替 MAS 做医学结论。
...
- MAS 的工具声明遵循 Tool Affordance Boundary：声明能力、权限、凭据边界、可写范围、side effect、forbidden authority 和证据入口；不把 executor 的文献阅读、统计检查、候选生成、路线比较或追问顺序固化成 prompt 外的流程脚本。
