...
- The current operator entry surfaces are `CLI`, `MCP`, `product-entry`, and `controller`. Current execution maps live under `docs/active/`, while product-entry and runtime contracts live under `docs/product/` and `docs/runtime/`, so an agent can start there instead of reverse-engineering the codebase. The repo-root OPL standard pack is the generated-interface source for OPL; the local CLI, MCP tools, product-entry surfaces, controller-authorized workspace commands/scripts, durable surfaces, and repo-tracked contracts remain the MAS-owned action targets and authority surfaces behind those generated descriptors.
