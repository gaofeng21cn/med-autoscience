...
- MAS tracks its canonical plugin source under `plugins/med-autoscience/`; legacy `plugins/mas` remains only as a compatibility alias, repo-local `.agents/plugins/marketplace.json` is retired local state, and Codex marketplace registration is owned by the OPL wrapper/startup maintenance surface.
