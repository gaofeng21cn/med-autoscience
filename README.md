...
- MAS now keeps its canonical OPL Agent semantic pack under repo-root `agent/` and declares those required files through repo-root `contracts/*.json`. OPL compiles that pack into the generated CLI / MCP / Skill / product-entry / tool descriptors. The existing MAS CLI, MCP, product-entry, sidecar, and controller surfaces are strict migration inputs or domain handler targets until OPL generated/default callers take over; they are not part of the final standard-agent source shape. Long-term retained MAS code is limited to medical authority functions, owner receipt / typed blocker production, domain handler targets, and necessary medical helper implementation.
...
- The current operator entry surfaces are still discoverable as `CLI`, `MCP`, `product-entry`, and `controller`, but under strict OPL Agent purity they are migration inputs, not permanent repo-owned wrappers. Current execution maps live under `docs/active/`, while product-entry and runtime contracts live under `docs/product/` and `docs/runtime`, so an agent can find the cutover target before touching code. The repo-root `agent/` pack is the generated-interface semantic source for OPL; local CLI/MCP/product-entry/sidecar/controller commands must shrink to MAS domain handler targets, medical authority functions, owner receipt / typed blocker producers, or be removed after OPL generated/default caller parity is proven.
