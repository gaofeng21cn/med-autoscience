...
- `product-start` for exposing the current recommended start surface, available modes, resume surface, and human-gate summary before you choose how to enter
...
- Read the current start companion when you want one machine-readable start surface first: `uv run python -m med_autoscience.cli product-start --profile <profile> --format <markdown|json>`
