...
- Current next action authority is `StageOutcome -> NextActionEnvelope -> OPL TransitionReceipt`. OPL queue / attempt / provider readiness is transport evidence only; it cannot select MAS owner, prove paper progress, or replace MAS owner receipt / typed blocker / human gate.
...
- MAS tracks only its plugin source under `plugins/mas/`; repo-local `.agents/plugins/marketplace.json` is retired local state, and Codex marketplace registration is owned by the OPL wrapper/startup maintenance surface.
