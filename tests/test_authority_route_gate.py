...
def test_route_gate_fails_closed_without_snapshot() -> None:
    ...
    gate = module.authorize_authority_route("paper_write", {})

    assert gate["authorized"] is False
    assert gate["blocking_reasons"] == ["authority_snapshot_missing"]
...
def test_route_gate_blocks_submission_notice_materialize_when_bundle_build_false() -> None:
    ...
    gate = module.authorize_authority_route(
        "submission_notice_materialize",
        {"authority_snapshot": _snapshot(bundle_build_allowed=False)},
    )
...
def test_route_gate_rejects_cleanup_apply_as_unsupported() -> None:
    module = importlib.import_module("med_autoscience.controllers.authority_route_gate")

    with pytest.raises(ValueError, match="unsupported authority route action: cleanup_apply"):
        module.authorize_authority_route(
            "cleanup_apply",
            {"authority_snapshot": _snapshot()},
        )
...
def test_publication_gate_replay_route_authorizes_submission_materialize_under_downstream_bundle_gate() -> None:
    module = importlib.import_module("med_autoscience.controllers.authority_route_gate")
    ...
    assert gate["authorized"] is True
    assert gate["controller_route_gate"]["authorized"] is True
    assert gate["controller_repair_authorization_ref"]["work_unit_id"] == (
        "dm003_publication_gate_replay_after_current_ai_reviewer_record"
    )
    assert gate["blocking_reasons"] == []
...
def test_submission_authority_sync_closure_route_authorizes_submission_and_delivery_actions() -> None:
    ...
    delivery_gate = module.authorize_authority_route("delivery_sync", context)
    bundle_gate = module.authorize_authority_route("bundle_build", context)
...
def test_dm002_after_quality_medical_prose_repair_routes_to_paper_write_not_submission_materialize() -> None:
    module = importlib.import_module("med_autoscience.controllers.authority_route_gate")
    route_context = {
        "authority_snapshot": _snapshot(
            gate_state="blocked",
    ...
    assert paper_write_gate["controller_route_gate"]["authorized"] is True
...
def test_controller_owned_route_does_not_authorize_unsupported_cleanup_action() -> None:
    module = importlib.import_module("med_autoscience.controllers.authority_route_gate")
...
def test_route_gate_assertion_raises_for_blocked_managed_context() -> None:
    ...
    with pytest.raises(PermissionError):
        module.assert_authority_route_authorized(
            "paper_write",
            {"authority_snapshot": _snapshot(paper_write_allowed=False)},
        )
...
from tests.test_authority_route_gate_cases.synthetic_action_family_routes import (
    ...
    test_synthetic_next_action_envelope_is_nonbinding_context_for_open_authority_snapshot,
    test_synthetic_submission_next_action_cannot_override_authority_snapshot,
)
