...
def test_stage_artifact_index_detects_partial_and_artifact_delta_present(tmp_path: Path) -> None:
    study_root = tmp_path / "studies" / "001-risk"
    _write_json(
    ...
    assert by_stage["scout"]["artifact_status"] == "partial"
    assert by_stage["scout"]["stage_progress_status"] == "artifact_partial"
    assert by_stage["scout"]["observed_artifact_refs"]
    assert by_stage["baseline"]["artifact_status"] == "artifact_delta_present"
    assert by_stage["baseline"]["stage_progress_status"] == "artifact_delta_present"
    assert by_stage["baseline"]["next_missing_surface"] is None
...
def test_stage_artifact_index_marks_stale_platform_repair_without_counting_as_progress(tmp_path: Path) -> None:
    ...
    assert write_stage["artifact_status"] == "partial"
    assert write_stage["observed_artifact_refs"][0]["ref"].endswith("manuscript_draft.json")
