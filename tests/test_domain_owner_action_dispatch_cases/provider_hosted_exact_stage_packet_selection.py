...
def test_provider_hosted_projection_without_carrier_refs_binds_canonical_stage_packet(
    tmp_path: Path,
    monkeypatch,
) -> None:
    profile = make_profile(tmp_path)
    ...
    projection_without_refs = _transition_request_projection(dispatch)
    projection_without_refs.pop("refs", None)
    projection_without_refs.pop("stage_packet_ref", None)

    execution = domain_owner_action_dispatch._execute_dispatch(
        profile=profile,
        study_id=study_id,
        dispatch_payload=projection_without_refs,
        developer_mode_payload={"mode": "developer_apply_safe", "safe_actions_enabled": True},
        apply=False,
        scan_payload={"studies": [{"study_id": study_id}]},
        fresh_progress={
