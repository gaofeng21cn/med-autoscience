...
def test_refresh_controller_decision_routes_finalize_gate_replay_to_gate_clearing(
    monkeypatch,
    tmp_path: Path,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.current_controller_decision_refresh")
    ...
    materialize_kwargs = calls["materialize_kwargs"]
    assert refresh["refresh_status"] == "materialized"
    assert materialize_kwargs["decision_type"] == "route_back_same_line"
    assert materialize_kwargs["route_target"] == "finalize"
    assert materialize_kwargs["work_unit_fingerprint"] == (
        "domain-transition::route_back_same_line::owner_authorized_publication_gate_replay"
    )
    assert materialize_kwargs["next_work_unit"]["unit_id"] == "owner_authorized_publication_gate_replay"
    assert materialize_kwargs["controller_actions"][0]["action_type"] == "run_gate_clearing_batch"


def test_refresh_controller_decision_rejects_gate_replay_with_stale_write_action(
    monkeypatch,
    tmp_path: Path,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.current_controller_decision_refresh")
    ...
    study_root = write_study(profile.workspace_root, study_id, quest_id=study_id)
    publication_eval_ref = {
        "eval_id": "publication-eval::dm003::stale-gate-replay-action",
        "artifact_path": str(study_root / "artifacts" / "publication_eval" / "latest.json"),
    }
    _write_json(
        study_root / "artifacts" / "publication_eval" / "latest.json",
    ...
    status_payload = {
        ...
        "domain_transition": {
            ...
            "route_target": "finalize",
            "owner": "publication_gate",
            "next_work_unit": dict(gate_replay_work_unit),
        },
    }
    stale_tick_request = {
        "study_root": study_root,
        ...
        "route_rationale": "Old same-line route with the right work-unit id but the wrong owner action.",
        "source_route_key_question": None,
        "requires_human_confirmation": False,
        "controller_actions": [{"action_type": "run_quality_repair_batch", "payload_ref": str(controller_decision_path)}],
        "reason": "Old same-line route with the right work-unit id but the wrong owner action.",
        "work_unit_fingerprint": "domain-transition::route_back_same_line::owner_authorized_publication_gate_replay",
        "next_work_unit": dict(gate_replay_work_unit),
        "blocking_work_units": [dict(gate_replay_work_unit)],
    }
    calls: dict[str, object] = {}
