...
def test_build_hydration_payload_returns_protocol_surface() -> None:
    payload = runtime.build_hydration_payload(
        create_payload={
            "startup_contract": {
...
def test_build_hydration_payload_includes_workspace_literature_and_study_reference_context(
    ...
    reference_context = {"selected_record_ids": ["pmid:12345"], "records": records}
    workspace_status = {"record_count": 1}

    monkeypatch.setattr(
        runtime,
        "_build_study_reference_context",
        lambda **_: reference_context,
    )
    monkeypatch.setattr(
        runtime,
        "workspace_literature_status",
        lambda **_: workspace_status,
    )
    ...
    assert payload["workspace_literature"] == workspace_status
    assert payload["study_reference_context"] == reference_context
...
def test_runtime_escalation_record_path_resolves_to_stable_quest_local_artifact(tmp_path: Path) -> None:
    ...
    assert runtime._runtime_escalation_record_path(quest_root) == (
...
def test_write_runtime_escalation_record_persists_full_artifact_and_read_runtime_escalation_record_ref_returns_summary_echo(
    ...
    record = runtime.RuntimeEscalationRecord(
        schema_version=1,
        ...
        trigger=runtime.RuntimeEscalationTrigger(
    ...
    written = runtime.write_runtime_escalation_record(quest_root=quest_root, record=record)
    ...
    assert runtime.read_runtime_escalation_record_ref(quest_root=quest_root) == runtime.RuntimeEscalationRecordRef(
...
def test_write_study_decision_record_persists_study_local_artifact_and_latest_pointer(tmp_path: Path) -> None:
    ...
    written = runtime.write_study_decision_record(
        study_root=study_root,
        record=decision_record.StudyDecisionRecord(
            schema_version=1,
            ...
            charter_ref=decision_record.StudyDecisionCharterRef(
                charter_id="charter::001-risk::v1",
                artifact_path=str(study_root / "artifacts" / "controller" / "study_charter.json"),
            ),
            runtime_escalation_ref=runtime.RuntimeEscalationRecordRef(
            ...
            publication_eval_ref=decision_record.StudyDecisionPublicationEvalRef(
                eval_id="publication-eval::001-risk::quest-001::2026-04-05T06:03:00+00:00",
                artifact_path=str(study_root / "artifacts" / "publication_eval" / "latest.json"),
            ),
            requires_human_confirmation=False,
            controller_actions=(
                decision_record.StudyDecisionControllerAction(
...
def test_write_startup_hydration_report_persists_typed_protocol_surface(tmp_path: Path) -> None:
    quest_root = tmp_path / "workspace" / "ops" / "med-deepscientist" / "runtime" / "quests" / "001-risk"
    report = runtime.StartupHydrationReport(
        status=runtime.StartupHydrationStatus.HYDRATED,
    ...
    written = runtime.write_startup_hydration_report(quest_root=quest_root, report=report)
...
def test_write_startup_hydration_validation_report_persists_typed_protocol_surface(tmp_path: Path) -> None:
    quest_root = tmp_path / "workspace" / "ops" / "med-deepscientist" / "runtime" / "quests" / "001-risk"
    report = runtime.StartupHydrationValidationReport(
        status=runtime.StartupHydrationValidationStatus.CLEAR,
    ...
    written = runtime.write_startup_hydration_validation_report(quest_root=quest_root, report=report)
...
def test_startup_hydration_report_rejects_missing_required_fields() -> None:
    with pytest.raises(ValueError, match="startup hydration payload missing recorded_at"):
        runtime.StartupHydrationReport.from_payload({"status": "hydrated"})
...
def test_startup_hydration_validation_report_rejects_missing_required_fields() -> None:
    with pytest.raises(ValueError, match="startup hydration validation payload missing checked_paths"):
        runtime.StartupHydrationValidationReport.from_payload(
            {
                "status": "clear",
                "recorded_at": "2026-04-03T08:05:00+00:00",
...
def test_build_hydration_payload_rejects_invalid_payload(create_payload: dict[str, object], message: str) -> None:
    with pytest.raises(ValueError, match=message):
        runtime.build_hydration_payload(create_payload=create_payload)


def test_validate_startup_contract_resolution_returns_clear_for_resolved_contracts() -> None:
    result = runtime.validate_startup_contract_resolution(
        startup_contract={
    ...
    assert result == runtime.StartupContractValidation(
        status=runtime.StartupContractValidationStatus.CLEAR,
        blockers=(),
        medical_analysis_contract_status="resolved",
        medical_reporting_contract_status="resolved",
...
def test_startup_contract_validation_from_payload_reconstructs_protocol_surface() -> None:
    result = runtime.StartupContractValidation.from_payload(
        {
            "status": "blocked",
    ...
    assert result == runtime.StartupContractValidation(
        status=runtime.StartupContractValidationStatus.BLOCKED,
        blockers=("missing_medical_reporting_contract",),
        medical_analysis_contract_status="resolved",
...
def test_startup_contract_validation_from_payload_rejects_missing_protocol_fields() -> None:
    with pytest.raises(ValueError, match="startup contract validation payload missing contract_statuses"):
        runtime.StartupContractValidation.from_payload(
            {
                "status": "clear",
                "blockers": [],
                "reason_codes": {
...
def test_startup_contract_validation_from_payload_round_trips_protocol_surface() -> None:
    ...
    result = runtime.StartupContractValidation.from_payload(payload)
...
def test_startup_contract_validation_rejects_unknown_status() -> None:
    with pytest.raises(ValueError, match="unknown startup contract validation status"):
        runtime.StartupContractValidation(
            status="unexpected",
            blockers=(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"decision": "blocked", "quest_exists": True, "quest_status": "running", "reason": "startup_boundary_not_ready_for_resume"}, False),
        ({"decision": "blocked", "quest_exists": True, "quest_status": "paused", "reason": "other_reason"}, False),
    ],
)
def test_should_refresh_startup_hydration_for_runtime_hold_classifies_status(
    status: dict[str, object],
    expected: bool,
) -> None:
    assert runtime.should_refresh_startup_hydration_for_runtime_hold(status) is expected
