...
def test_study_runtime_status_keeps_live_reviewer_intake_running_with_open_blockers(
    ...
    task_intake_module.write_task_intake(
        ...
        constraints=("Do not keep the previous submission-ready/finalize parking state as current truth.",),
        first_cycle_outputs=("revision checklist mapping each reviewer concern to manuscript deltas",),
    )
        ...
        json.dumps(
            {
                ...
                "active_interaction_id": "progress-live-002",
                "continuation_policy": "auto",
                "continuation_anchor": "decision",
                "continuation_reason": "decision:decision-live-002",
                "pending_user_message_count": 0,
            }
        )
        + "\n",
    ...
    monkeypatch.setattr(
        ...
        lambda *, runtime_root, quest_id: {
            ...
            "bash_session_audit": {
                ...
                "session_count": 1,
                "live_session_count": 1,
                "live_session_ids": ["sess-live-002"],
            },
        },
    )
    monkeypatch.setattr(
        _managed_runtime_transport(module),
        "pause_quest",
        lambda **kwargs: pytest.fail("live reviewer repair with open blockers must keep runtime ownership"),
    )
    monkeypatch.setattr(
        module,
        "_resume_quest",
    ...
    assert result["publication_supervisor_state"]["current_required_action"] == "continue_bundle_stage"
    assert ensured["decision"] == "resume"
    assert ensured["controller_owned_runtime_reply"]["content"]
    binding = yaml.safe_load(Path(ensured["runtime_binding_path"]).read_text(encoding="utf-8"))
    launch_report = json.loads(
        Path(ensured["autonomous_runtime_notice"]["launch_report_path"]).read_text(encoding="utf-8")
    )
    assert binding["last_action"] == "noop"
...
def test_study_runtime_status_resumes_running_delivered_package_without_worker_when_reviewer_blockers_open(
    ...
    study_quality_truth["reviewer_first"] = {
        **dict(study_quality_truth.get("reviewer_first") or {}),
        ...
        "summary": "review ledger still has an unresolved reviewer concern.",
    }
    evaluation_summary["study_quality_truth"] = study_quality_truth
    write_text(evaluation_summary_path, json.dumps(evaluation_summary, ensure_ascii=False, indent=2) + "\n")
    write_text(quest_root / "quest.yaml", "quest_id: 001-risk\n")
