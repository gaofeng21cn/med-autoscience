...
def test_codex_exec_runner_prefers_blocked_closeout_owner_handoff_over_stale_controller_decision(
    ...
    assert "recover_transport_model_provenance" in prompt
    assert "source_provenance_owner" in prompt
    assert "--action-types recover_transport_model_provenance" in prompt
    assert "unit_harmonized_external_validation_rerun" not in prompt
    assert "unit_harmonized_validation_uncertainty_and_grouped_calibration" not in prompt
    assert authorization["authorization_basis"] == "blocked_turn_closeout_owner_handoff"
    assert authorization["active_run_id"] == "run-source-provenance"
    assert authorization["controller_actions"] == ["recover_transport_model_provenance"]
    assert authorization["work_unit_id"] == "recover_transport_model_provenance"
