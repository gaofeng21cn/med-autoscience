...
def test_runtime_health_recovery_decision_rejects_cross_identity_opl_readback(
        ...
        make_status_payload(
            ...
            opl_lifecycle_readback={
                "opl_observability_readback": {
                    "surface_kind": "opl_runtime_health_observability_readback",
                    "status": "live",
                    "active_stage_attempt_id": "opl-stage-attempt://runtime-timeout-stale",
                },
                "opl_route_reconciler_readback": {
                    "surface_kind": "opl_runtime_health_route_reconciler_readback",
                    "status": "live",
                    "active_stage_attempt_id": "opl-stage-attempt://runtime-timeout-stale",
                },
            },
    ...
    assert gate["matched_runtime_identity"] == []
    assert gate["readback_sources"] == [
        "opl_lifecycle_readback.opl_observability_readback",
        "opl_lifecycle_readback.opl_route_reconciler_readback",
    ]
    assert gate["unbound_opl_ref_can_authorize_decision"] is False
    assert "runtime_recovery_lifecycle" not in status.extras
...
def test_runtime_health_recovery_decision_requires_opl_current_control_readback(
    ...
    gate = status.extras["runtime_health_decision_gate"]
    assert gate["decision_authorized"] is False
    assert gate["decision_source"] is None
    assert gate["suppressed_reason"] == "opl_runtime_readback_required_for_runtime_health_decision"
    assert gate["identity_bound_opl_readback_required"] is True
    assert gate["required_runtime_identity"] == [
    ...
    assert gate["matched_runtime_identity"] == []
    assert gate["readback_sources"] == []
    assert gate["required_runtime_health_tail_families"] == [
        "opl_observability_live_readback",
        "opl_route_reconciler_live_readback",
    ]
    assert gate["matched_runtime_health_tail_families"] == []
    assert gate["missing_runtime_health_tail_families"] == [
        "opl_observability_live_readback",
        "opl_route_reconciler_live_readback",
    ]
    assert gate["unbound_opl_ref_can_authorize_decision"] is False
...
def test_runtime_health_recovery_decision_requires_both_observability_and_route_reconciler_readback(
    tmp_path: Path,
) -> None:
    module = importlib.import_module(
        "med_autoscience.controllers.study_runtime_decision.runtime_health_dominance"
    ...
    assert gate["matched_runtime_identity_by_tail_family"] == {
        "opl_observability_live_readback": [
            "opl-stage-attempt://runtime-timeout-001",
            "runtime-timeout-001",
        ],
