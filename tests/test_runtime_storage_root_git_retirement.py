...
def test_audit_workspace_storage_git_only_archives_stashes_during_workspace_root_git_retirement(
    tmp_path: Path,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.runtime_storage_maintenance")
    profile = make_profile(tmp_path)
    ...
    module.git_garbage._run_git_command(["commit", "-m", "baseline"], workspace_root=workspace_root, check=True)
    (workspace_root / "README.md").write_text("tracked\nstashed\n", encoding="utf-8")
    module.git_garbage._run_git_command(["stash", "push", "-m", "root-git-retirement-test"], workspace_root=workspace_root, check=True)
    ...
    assert retirement["status"] == "retired"
    manifest = json.loads(Path(retirement["manifest_path"]).read_text(encoding="utf-8"))
    assert manifest["before"]["health"]["stash_count"] == 1
    assert "has_stashes_archived" in manifest["before"]["health"]["workspace_root_git_retirement_eligibility"]["reasons"]
    assert "root-git-retirement-test" in manifest["before"]["git_stash_list"]["stdout"]
    assert not (workspace_root / ".git").exists()
