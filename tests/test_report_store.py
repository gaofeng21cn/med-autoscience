from __future__ import annotations

import importlib
import json
from pathlib import Path


def test_write_timestamped_report_writes_json_markdown_and_latest_aliases(tmp_path: Path) -> None:
    module = importlib.import_module("med_autoscience.adapters.report_store")
    quest_root = tmp_path / "runtime" / "quests" / "q001"
    report = {"schema_version": 1, "status": "blocked"}

    json_path, md_path = module.write_timestamped_report(
        quest_root=quest_root,
        report_group="domain_diagnostic_report",
        timestamp="2026-03-29T03:50:50+00:00",
        report=report,
        markdown="# Report\n",
    )

    assert json_path.name == "2026-03-29T035050Z.json"
    assert md_path.name == "2026-03-29T035050Z.md"
    assert json.loads(json_path.read_text(encoding="utf-8")) == report
    assert md_path.read_text(encoding="utf-8") == "# Report\n"
    latest_json_path = quest_root / "artifacts" / "reports" / "domain_diagnostic_report" / "latest.json"
    latest_md_path = quest_root / "artifacts" / "reports" / "domain_diagnostic_report" / "latest.md"
    assert json.loads(latest_json_path.read_text(encoding="utf-8")) == report
    assert latest_md_path.read_text(encoding="utf-8") == "# Report\n"
