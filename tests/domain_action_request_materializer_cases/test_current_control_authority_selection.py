...
@pytest.mark.parametrize(
    ("current_next_owner", "current_reason", "current_allowed_actions"),
    (
        ("gate_clearing_batch", "current_gate_replay", ["run_gate_clearing_batch"]),
        ("ai_reviewer", "current_ai_reviewer_recheck", ["return_to_ai_reviewer_workflow"]),
    ),
)
def test_materializer_rejects_top_level_action_queue_when_current_route_disagrees(
    monkeypatch,
    tmp_path: Path,
    current_next_owner: str,
    current_reason: str,
    ...
    current_route = _owner_route(
        study_id=study_id,
        quest_id=quest_id,
        next_owner=current_next_owner,
        owner_reason=current_reason,
        allowed_actions=current_allowed_actions,
    )
