...
def test_phase3_clearance_lane_uses_shared_builder(monkeypatch) -> None:
    ...
    monkeypatch.setattr(program_surfaces, "_build_shared_product_entry_program_step", lambda **kwargs: kwargs)
    monkeypatch.setattr(program_surfaces, "_build_shared_product_entry_program_surface", lambda **kwargs: kwargs)

    def _fake_build_clearance_lane(**kwargs: object) -> dict[str, object]:
        captured.update(kwargs)
        return {"surface_kind": "phase3_host_clearance_lane"}

    monkeypatch.setattr(program_surfaces, "_build_shared_clearance_lane", _fake_build_clearance_lane)
    ...
    assert captured["clearance_loop"][2]["surface_kind"] == "paper_mission_readback_refresh"
    assert captured["clearance_loop"][2]["command"] == (
        "uv run python -m med_autoscience.cli paper-mission inspect "
        "--profile <profile> --study-id <study_id> --format json"
    )
    assert captured["proof_surfaces"][2] == {
        "surface_kind": "paper_mission_readback",
        "command": (
            "uv run python -m med_autoscience.cli paper-mission inspect "
            "--profile <profile> --study-id <study_id> --format json"
...
def test_phase4_backend_deconstruction_uses_shared_builder(monkeypatch) -> None:
    ...
    monkeypatch.setattr(program_surfaces, "build_backend_deconstruction_lane", _fake_build_backend_lane)
...
def test_platform_target_uses_shared_builder(monkeypatch) -> None:
    ...
    monkeypatch.setattr(program_surfaces, "_build_shared_platform_target", _fake_build_platform_target)
