...
def test_quality_repair_batch_uses_paper_route_for_upstream_repair_under_downstream_bundle_gate(
    ...
    assert result["control_plane_route_gate"]["blocking_reasons"] == []
...
def test_mutating_delivery_write_surfaces_are_registered_behind_control_plane_write_route() -> None:
    for path, function_name, route_action in WRITE_ROUTE_SURFACES:
        ...
        assert "control_plane_route_context" in _function_parameters(node)
        assert "route_context" in _function_parameters(node)
        assert "resolve_control_plane_write_route_context" in called_names
        assert "blocked_control_plane_write_payload" in called_names
        assert route_action in _literal_keyword_values(
            node,
            function_name="resolve_control_plane_write_route_context",
            keyword_name="action",
        )
...
def test_submission_minimal_post_materialization_replays_preserve_route_context() -> None:
    for path, function_name in ROUTE_CONTEXT_REPLAY_SURFACES:
        ...
        route_context_keywords = [
            ...
            for keyword in child.keywords
            if keyword.arg == "control_plane_route_context" and isinstance(keyword.value, ast.Name)
        ]
