...
def test_product_entry_manifest_uses_bounded_mainline_projection(monkeypatch, tmp_path: Path) -> None:
    ...
    def _unexpected_full_mainline_status() -> dict[str, object]:
        raise AssertionError("product-entry manifest must not build the full mainline status projection")

    def _fake_product_entry_mainline_projection() -> dict[str, object]:
    ...
    monkeypatch.setattr(module.mainline_status, "read_mainline_status", _unexpected_full_mainline_status)
    monkeypatch.setattr(
        module.mainline_status,
        "read_product_entry_mainline_projection",
        _fake_product_entry_mainline_projection,
    )
