...
def _snapshot(payload: dict[str, object]) -> dict[str, object]:
    module = importlib.import_module("med_autoscience.controllers.study_control_plane_kernel")
    return module.build_control_plane_snapshot(payload)


def test_control_plane_snapshot_blocks_dispatch_without_truth_and_health_epochs() -> None:
    snapshot = _snapshot(
        {
            "study_id": "003-dpcc",
            "quest_id": "003-dpcc",
    ...
    assert snapshot["surface"] == "control_plane_snapshot"
...
def test_control_plane_snapshot_fail_closes_non_ai_reviewer_publication_ready() -> None:
    snapshot = _snapshot(
        {
            "study_id": "003-dpcc",
            "quest_id": "003-dpcc",
...
def test_control_plane_snapshot_blocks_recovery_when_retry_budget_exhausted() -> None:
    snapshot = _snapshot(
        {
            "study_id": "004-invasive",
            "quest_id": "004-invasive",
...
def test_control_plane_snapshot_is_embedded_in_status_projection(tmp_path) -> None:
    ...
    kernel = importlib.import_module("med_autoscience.controllers.study_control_plane_kernel")
    status.extras["control_plane_snapshot"] = kernel.build_control_plane_snapshot(status.to_dict())

    payload = status.to_dict()

    assert payload["control_plane_snapshot"]["surface"] == "control_plane_snapshot"
    assert payload["control_plane_snapshot"]["dispatch_gate"]["state"] == "open"
    assert payload["control_plane_snapshot"]["authority_refs"]["study_truth"]["epoch"] == "truth-event-4"
