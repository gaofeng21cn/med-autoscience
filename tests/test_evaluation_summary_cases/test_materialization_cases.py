...
from tests.test_evaluation_summary_cases.shared import (
    MODULE_NAME,
    _ready_medical_prose_quality,
    _stable_inputs,
    _write_json,
    _write_reporting_contract_task_intake,
    importlib,
    json,
    pytest,
)
...
def test_materialize_evaluation_summary_artifacts_treats_charter_question_and_eval_statement_as_same_objective(
    tmp_path: Path,
) -> None:
    module = importlib.import_module(MODULE_NAME)
    inputs = _stable_inputs(tmp_path)
    ...
    charter_path = inputs["charter_path"]
    charter_payload = json.loads(charter_path.read_text(encoding="utf-8"))
    charter_payload["publication_objective"] = (
        "In the registry cohort, can we characterize metabolic phenotypes?"
    )
    _write_json(charter_path, charter_payload)
    publication_eval_payload = dict(inputs["publication_eval_payload"])
    publication_eval_payload["charter_context_ref"] = {
        "ref": str(inputs["charter_path"]),
        "charter_id": "charter::001-risk::v1",
        "publication_objective": "In the registry cohort, characterize metabolic phenotypes.",
    }
    _write_json(publication_eval_path, publication_eval_payload)
...
def test_materialize_evaluation_summary_artifacts_aligns_bundle_only_agenda_with_latest_task_intake_scope(
    ...
    assert summary["quality_review_agenda"] == {
        ...
        "next_review_focus": "复核 medical_reporting_audit、domain_diagnostic_report 与 publication gate 状态是否已清掉 stale reporting blockers。",
        "agenda_summary": (
            ...
            "下一轮复评重点：复核 medical_reporting_audit、domain_diagnostic_report 与 publication gate 状态是否已清掉 stale reporting blockers。"
        ),
    }
    assert summary["quality_revision_plan"] == {
        "policy_id": "medical_publication_critique_v1",
        ...
        "items": [
            {
                ...
                "done_criteria": "下一轮复评能够明确确认：复核 medical_reporting_audit、domain_diagnostic_report 与 publication gate 状态是否已清掉 stale reporting blockers。",
                "route_target": "finalize",
            }
        ],
        "next_review_focus": ["复核 medical_reporting_audit、domain_diagnostic_report 与 publication gate 状态是否已清掉 stale reporting blockers。"],
