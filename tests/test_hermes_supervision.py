...
def test_ensure_supervision_creates_job_and_triggers_run(monkeypatch, tmp_path: Path) -> None:
    ...
    assert any(command[1:3] == ["cron", "create"] for command in commands)
    assert any(command[1:3] == ["cron", "run"] for command in commands)
