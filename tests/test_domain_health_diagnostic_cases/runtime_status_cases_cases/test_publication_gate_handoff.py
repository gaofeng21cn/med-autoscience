...
def test_domain_health_diagnostic_applies_publication_gate_when_clear_status_still_needs_draft_handoff_sync(
    tmp_path: Path,
) -> None:
...
def test_domain_health_diagnostic_does_not_reapply_after_draft_handoff_sync_stabilizes(tmp_path: Path) -> None:
    module = importlib.import_module("med_autoscience.controllers.domain_health_diagnostic")
    quest_root = make_quest(tmp_path, "q001", status="running")
    calls: list[bool] = []
...
def test_domain_health_diagnostic_reapplies_publication_gate_when_ai_reviewer_eval_masks_return_to_gate(
    monkeypatch,
    tmp_path: Path,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.domain_health_diagnostic")
    ...
    monkeypatch.setattr(
        ...
        lambda quest_root: module.domain_health_diagnostic_protocol.DomainHealthDiagnosticState(
            schema_version=1,
            updated_at="2026-04-30T12:00:00+00:00",
            controllers={
                "publication_gate": module.domain_health_diagnostic_protocol.DomainHealthDiagnosticControllerState(
...
def test_domain_health_diagnostic_registers_medical_runtime_audits() -> None:
    module = importlib.import_module("med_autoscience.controllers.domain_health_diagnostic")
    runners = module.build_default_controller_runners()
...
def test_domain_health_diagnostic_orders_publication_surface_before_gate(tmp_path: Path) -> None:
    module = importlib.import_module("med_autoscience.controllers.domain_health_diagnostic")
    quest_root = make_quest(tmp_path, "q001", status="running")
