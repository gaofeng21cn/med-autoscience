...
def test_provider_admission_merge_dedupes_same_opl_runtime_readback_identity() -> None:
    report = importlib.import_module(
        "med_autoscience.controllers.domain_health_diagnostic_parts.provider_admission_report"
    )
    study_id = "003-dpcc-primary-care-phenotype-treatment-gap"
    ...
    readback = _opl_transition_live_readback(
        study_id=study_id,
        work_unit_id=work_unit_id,
    ...
    progress_candidate = {
        ...
        "work_unit_fingerprint": fingerprint,
        "action_fingerprint": fingerprint,
        "route_identity_key": f"paper-recovery::{study_id}::{fingerprint}",
        "attempt_idempotency_key": f"paper-recovery::{study_id}::{fingerprint}",
    ...
    merged = report._merge_provider_admission_candidates(
        [progress_candidate],
        [same_tick_candidate],
    )

    assert len(merged) == 1
    assert merged[0]["source"] == "same_tick_materialized_dispatch"
    assert merged[0]["route_identity_key"] == idempotency_key
    assert merged[0]["attempt_idempotency_key"] == idempotency_key
    assert merged[0]["opl_domain_progress_transition_runtime_live_readback"] == readback
