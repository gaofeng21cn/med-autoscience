...
def test_watch_runtime_stale_opl_handoff_allows_current_ai_reviewer_inputs_record_dispatch(
    tmp_path: Path,
    monkeypatch,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.domain_health_diagnostic")
    ...
    work_unit = {
        ...
        "lane": "review",
        "summary": "Produce a current AI reviewer publication-eval record from the current paper inputs.",
    }
    fingerprint = (
        "domain-transition::ai_reviewer_re_eval::"
    ...
    tick_request = {
        ...
        "route_key_question": (
            "produce_ai_reviewer_publication_eval_record_against_current_inputs: "
            "Rebuild current-input AI reviewer authority."
        ),
        "route_rationale": "The repair closeout requested a fresh AI reviewer record against current inputs.",
        "requires_human_confirmation": False,
        "controller_actions": [
            {
                "action_type": "return_to_ai_reviewer_workflow",
        ...
        "reason": "Return current paper inputs to the AI reviewer workflow.",
        "work_unit_fingerprint": fingerprint,
        "next_work_unit": work_unit,
    }
    status_payload = {
