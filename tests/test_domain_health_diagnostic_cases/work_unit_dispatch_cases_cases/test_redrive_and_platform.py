...
def test_watch_runtime_redrives_repeated_work_unit_until_attempt_closes(
    ...
    assert calls == ["domain_health_diagnostic_outer_loop_wakeup", "domain_health_diagnostic_outer_loop_wakeup"]
...
def test_watch_runtime_records_blocked_quality_repair_without_crashing(
    tmp_path: Path,
    monkeypatch,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.domain_health_diagnostic")
    ...
    tick_request = {
        ...
        "publication_eval_ref": _write_publication_eval(
            ...
            work_unit_fingerprint="publication-blockers::blocked",
        ),
        "decision_type": "route_back_same_line",
        "route_target": "publication_eval",
        "route_key_question": "analysis_claim_evidence_repair: Repair claim-evidence blockers.",
        "route_rationale": "Run one controller-owned quality repair batch before returning to publishability gate.",
        "requires_human_confirmation": False,
        "controller_actions": [
            {
    ...
    assert result["managed_study_outer_loop_dispatches"] == []
    assert result["managed_study_no_op_suppressions"][0]["outcome"] == "controller_work_unit_blocked"
    assert wakeup_latest["outcome"] == "controller_work_unit_blocked"
    assert wakeup_latest["controller_work_unit_block"]["blocked_reason"] == "canonical_paper_inputs_rehydrate_required"
    assert wakeup_latest["controller_work_unit_block"]["blocking_reasons"] == [
        "canonical_paper_inputs_rehydrate_required",
        "blocked_no_paper_root",
    ]
    assert [event["event_type"] for event in ledger_events] == ["controller_work_unit_blocked"]
