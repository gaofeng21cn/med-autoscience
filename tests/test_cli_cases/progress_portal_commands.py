...
def test_workspace_portal_console_soak_grouped_command_materializes_read_only_report(
    ...
    class FakePortalConsoleSoak:
        @staticmethod
        def run_portal_console_soak(**kwargs):
            calls.update(kwargs)
            return {
                "status": "passed",
                "surface_kind": "mas_portal_console_soak",
                "report_path": str(
                    tmp_path / "workspace" / "artifacts" / "runtime" / "portal_console_soak" / "latest.json"
                ),
                "evidence": {
                    "portal_refresh": {"status": "passed"},
                    "live_console_study_run_disambiguation": {"status": "passed"},
                },
            }

    monkeypatch.setattr(cli, "portal_console_soak", FakePortalConsoleSoak)
    ...
    assert payload["surface_kind"] == "mas_portal_console_soak"
    assert calls["profile"].name == "nfpitnet"
    assert calls["profile_ref"] == profile_path
    assert calls["study_id"] == "002-dm-china-us-mortality-attribution"
    assert calls["study_root"] is None
