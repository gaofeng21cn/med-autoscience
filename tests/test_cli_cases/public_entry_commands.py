...
def test_shell_argv_grouped_subcommand_dispatches(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    markdown_projection = importlib.import_module(
        "med_autoscience.controllers.study_progress.markdown_projection_rendering"
    )
    monkeypatch.setattr(progress_projection, "read_study_progress", fake_read_study_progress)
    monkeypatch.setattr(
        markdown_projection,
