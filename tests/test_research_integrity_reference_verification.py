...
def test_reference_verification_consumes_existing_provider_evidence_without_network() -> None:
    ...
    consumption = payload["surfaces"]["provider_receipt_consumption_bundle"]
    assert consumption["provider_receipt_required"] is False
...
def test_reference_verification_consumes_provider_receipts_without_network() -> None:
    payload = build_reference_verification_payload(
        payload={
...
def test_reference_verification_requests_host_resolution_when_receipt_has_no_evidence() -> None:
    payload = build_reference_verification_payload(
        payload={
    ...
    consumption = payload["surfaces"]["provider_receipt_consumption_bundle"]
    assert consumption["provider_receipt_required"] is True
    assert consumption["missing_provider_evidence_reference_ids"] == ["doe2026"]
...
def test_reference_verification_never_runs_provider_transport_when_evidence_is_absent() -> None:
    payload = build_reference_verification_payload(
        payload={
            "reference": {"id": "doe2026", "doi": "10.2000/missing"},
    ...
    consumption = payload["surfaces"]["provider_receipt_consumption_bundle"]
    assert consumption["surface_kind"] == "reference_provider_receipt_consumption_bundle"
    assert consumption["provider_receipt_required"] is True
