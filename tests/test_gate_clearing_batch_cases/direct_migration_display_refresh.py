...
def test_run_gate_clearing_batch_allows_current_f3_payload_after_transport_sync(
    monkeypatch,
    tmp_path: Path,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.gate_clearing_batch")
    ...
    study_root = write_study(
        ...
        endpoint_type="time_to_event",
        manuscript_family="prediction_model",
        paper_framing_summary="China-US transportability paper with current risk-group summary payload.",
    )
    quest_root = profile.managed_runtime_home / "quests" / "quest-002"
    paper_root = quest_root / ".ds" / "worktrees" / "paper-run-002" / "paper"
    paper_root.mkdir(parents=True, exist_ok=True)
            ...
            "displays": [
                {
                    "display_id": "km_risk_stratification",
                    "template_id": "fenggaolab.org.medical-display-core::time_to_event_risk_group_summary",
                    "title": "Transported risk-group summary",
                    "caption": "Risk groups are already in the current risk-group summary shape.",
    ...
    def fake_transportability_sync(**_: object) -> dict[str, object]:
        ...
        return {"status": "updated", "written_files": []}

    def fake_materialize(*, paper_root: Path) -> dict[str, object]:
        call_order.append("materialize")
        payload = json.loads(payload_path.read_text(encoding="utf-8"))
        assert "groups" not in payload["displays"][0]
        assert payload["displays"][0]["risk_group_summaries"]
    ...
    assert call_order == ["transportability_sync", "materialize"]
    result_by_unit = {item["unit_id"]: item for item in result["unit_results"]}
    assert result_by_unit["stale_time_to_event_grouped_payload_blocker"]["status"] == "current"
