...
def test_run_gate_clearing_batch_executes_delivery_refresh_fast_lane_for_stale_projection_only(
    ...
    profile, study_root, quest_root, paper_root = _make_gate_clearing_study(tmp_path)
...
def test_run_gate_clearing_batch_skips_submission_refresh_only_when_inputs_match_and_previous_run_succeeded(
    ...
    _write_followup_bundle_stage_publication_eval(study_root, quest_id="quest-004")
...
def test_run_gate_clearing_batch_reruns_delivery_sync_when_previous_matching_run_was_route_blocked(
    monkeypatch,
    tmp_path: Path,
) -> None:
    module = importlib.import_module("med_autoscience.controllers.gate_clearing_batch")
    ...
    gate_report = {
        ...
        "study_delivery_manifest_path": str(study_root / "manuscript" / "delivery_manifest.json"),
        "study_delivery_current_package_root": str(study_root / "manuscript" / "current_package"),
    ...
    _write_json(
        latest_record_path,
        {
            ...
            "unit_fingerprints": {
                "sync_submission_minimal_delivery": matching_fingerprint,
            },
        },
    )
    _write_followup_bundle_stage_publication_eval(
        study_root,
        quest_id="quest-004",
        next_work_unit={
