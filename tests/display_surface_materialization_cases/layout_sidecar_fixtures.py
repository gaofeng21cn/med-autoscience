...
def _cohort_flow_sidecar(template_id: str) -> dict[str, object]:
    return _sidecar(
        template_id,
        layout_boxes=[
            _box("participant_step_screened", "main_step", 0.12, 0.62, 0.74, 0.72),
            _box("participant_step_included", "main_step", 0.12, 0.30, 0.74, 0.40),
            _box("participant_exclusion_repeat", "exclusion_box", 0.78, 0.46, 0.96, 0.56),
        ],
        panel_boxes=[
            _box("participant_flow_main", "subfigure_panel", 0.06, 0.26, 0.98, 0.76),
        ],
        guide_boxes=[
            _box("flow_spine_screened_to_included", "flow_connector", 0.50, 0.40, 0.50, 0.62),
            _box("flow_branch_repeat", "flow_branch_connector", 0.74, 0.50, 0.78, 0.52),
        ],
        metrics={
            ...
            "flow_nodes": [
                {
                    ...
                    "line_count": 2,
                    "max_line_chars": 24,
                    "rendered_height_pt": 92.0 if box_type == "main_step" else 56.0,
                    "rendered_width_pt": 410.0 if box_type == "main_step" else 180.0,
                    "padding_pt": 9.0,
                    "detail_truncated": False,
                }
                for box_id, box_type in (
                    ("participant_step_screened", "main_step"),
                    ("participant_step_included", "main_step"),
                    ("participant_exclusion_repeat", "exclusion_box"),
                )
            ],
        },
    )
