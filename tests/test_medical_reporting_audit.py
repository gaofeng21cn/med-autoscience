...
def test_default_structured_reporting_contract_for_time_to_event_prediction_sets_first_draft_gates() -> None:
    ...
    assert "unit_standardized_model_application_or_sensitivity" in contract["variable_harmonization"]
    ...
    assert "calibration_curve_or_grouped_calibration_interval" in contract["validation_uncertainty_reporting"]
    assert "calibration_curve_with_uncertainty" in contract["prediction_display_reporting"]
    assert "main_text_table_rendering_in_submission_package" in contract["prediction_display_reporting"]
    assert "decision_curve_not_main_display_without_verified_net_benefit" in contract["prediction_display_reporting"]
