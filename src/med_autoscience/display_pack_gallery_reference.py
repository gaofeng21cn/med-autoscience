...
PDF 画册分为三部分：前部展示 6 类页面级图页方案，中部展示非数据设计/流程图起点，后部展示 28 个 R/ggplot2 数据证据图起点。cohort/participant reporting-flow 起点的 dependency intent 指向 `ggconsort`-capable R profile；当前 checked-in 可视实现仍是 Python generated fallback，不能宣称已执行 `ggconsort`。未证明优于 R/ggplot2 的 Python 数据证据模板不进入默认画册。
...
这些图件用于 cohort flow、graphical abstract、研究流程或机制性说明。cohort flow 作为数据驱动 reporting-flow shell 需要 OPL prepared dependency receipt 覆盖 `ggconsort`-capable profile；当前 Python participant-flow 输出只是 generated fallback，不等于 ggconsort render。其他设计图可以使用 SVG、Python composition 或 imagegen-assisted art direction；涉及真实结果数字时必须保留来源引用，不能替代程序化数据证据图。
...
- 当前数据证据图默认渲染器：`r_ggplot2={renderer_inventory.get("r_ggplot2", 0)}`，`python={renderer_inventory.get("python", 0)}`，`n/a={renderer_inventory.get("n/a", 0)}`
- 当前 R/ggplot2 数据证据图起点：`{r_evidence_count}`
- 当前 Python 数据证据模板：`0`
- 当前非数据设计/流程图起点：`{illustration_shell_count}`
- 已渲染 Gallery 可视示例：`{rendered_count}`
- 当前视觉证据图族：`{canonical_gallery_family_count}`
- 页面级图页方案：`{composition_recipe_count}`
...
- Nature-Skills 学习版本：`{nature_skills_head}`
