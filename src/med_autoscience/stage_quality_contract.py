...
def _build_pack(pack_id: str) -> dict[str, Any]:
    ...
    if pack_id in _LITERATURE_SEARCH_SOURCE_PACKS:
        pack["literature_search_source_pack"] = _literature_search_source_pack()
    if pack_id in _JOURNAL_POLICY_CURRENTNESS_PACKS:
        pack["journal_policy_currentness_pack"] = _journal_policy_currentness_pack()
    if pack_id == "citation_integrity_pack":
        pack["citation_verification_pack"] = _citation_verification_pack()
    return pack
...
def _literature_search_source_pack() -> dict[str, object]:
    return {
        "source_tiers": [
            {
                "tier": "T1",
...
def _journal_policy_currentness_pack() -> dict[str, object]:
    return {
...
def _citation_verification_pack() -> dict[str, object]:
    return {
...
_LITERATURE_SEARCH_SOURCE_PACKS: tuple[str, ...] = (
    "citation_integrity_pack",
    "paper_reader_grounding_pack",
)

_JOURNAL_POLICY_CURRENTNESS_PACKS: tuple[str, ...] = (
    "journal_response_pack",
    "data_availability_fair_pack",
    "citation_integrity_pack",
)
