...
from med_autoscience.display_pack_gallery_parts.render_cache import (
    cache_hit,
    render_cache_key,
    write_render_cache,
)
...
from med_autoscience.display_pack_gallery_parts.dependency_run_context import (
...
def _python_renderer_source_paths(record: TemplateRecord) -> list[Path]:
    ...
    if record.template_id == "submission_graphical_abstract":
        source_paths.append(paths.REPO_ROOT / "src" / "med_autoscience" / "display_pack_gallery_parts" / "design_svg_renderer.py")
    return source_paths
