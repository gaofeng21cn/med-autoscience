...
Package-only 打包复用状态：`{asset_reuse.get("status", "")}`；复制资产数：`{asset_reuse.get("copied_file_count", 0)}`；更新资产数：`{asset_reuse.get("updated_file_count", 0)}`。代码、payload、style profile 或 dependency receipt 未变化时，Gallery 可从既有 assets 重打包 HTML/PDF/docs；真正的渲染 freshness 仍由 render cache key、layout sidecar、dependency run-context 和真实论文 artifact 审计证明。
