...
def build_gallery_status_markdown(manifest: dict[str, Any]) -> str:
    ...
    asset_reuse = (
        manifest.get("asset_reuse")
        if isinstance(manifest.get("asset_reuse"), dict)
        else {}
    )
    profile_coverage = (
...
Package-only 打包复用状态：`{asset_reuse.get("status", "")}`；复制资产数：`{asset_reuse.get("copied_file_count", 0)}`。代码、payload、style profile 或 dependency receipt 未变化时，Gallery 可从既有 assets 重打包 HTML/PDF/docs；真正的渲染 freshness 仍由 render cache key、layout sidecar、dependency run-context 和真实论文 artifact 审计证明。
...
- reporting flow gallery default surface: `{manifest.get("template_surface_policy", {}).get("reporting_flow_gallery_default_surface", "")}`
