...
def _embedding_feature_matrix() -> list[dict[str, Any]]:
    latent_points = (
        ("Subtype A", -1.72, 0.72, 0.36),
        ("Subtype A", -1.58, 0.48, -0.18),
        ("Subtype A", -1.38, 0.92, 0.14),
        ("Subtype A", -1.18, 0.66, -0.42),
        ("Subtype A", -1.02, 1.04, 0.52),
        ("Subtype A", -0.86, 0.38, -0.34),
        ("Subtype A", -1.36, 0.22, 0.08),
        ("Transition", -0.52, -0.24, -0.12),
        ("Transition", -0.34, -0.62, 0.26),
        ("Transition", -0.10, -0.40, -0.42),
        ("Transition", 0.14, -0.72, 0.18),
        ("Transition", 0.38, -0.30, -0.20),
        ("Transition", 0.56, -0.58, 0.44),
        ("Subtype B", 0.88, 0.54, -0.46),
        ("Subtype B", 1.08, 0.92, 0.22),
        ("Subtype B", 1.24, 0.42, 0.54),
        ("Subtype B", 1.42, 0.78, -0.18),
        ("Subtype B", 1.62, 0.28, 0.34),
        ("Subtype B", 1.80, 0.68, -0.36),
        ("Subtype A", -1.74, 1.16, -0.08),
        ("Subtype A", -0.96, 1.28, 0.02),
        ("Subtype B", 0.98, 1.20, -0.02),
        ("Subtype B", 1.78, 1.04, 0.10),
        ("Transition", 0.02, -0.96, -0.02),
    )
    feature_names = (
        "immune_signal",
        "stromal_signal",
    ...
    for index, (group, latent_x, latent_y, latent_z) in enumerate(latent_points, start=1):
        values = (
            latent_x + 0.22 * latent_z,
            latent_y + 0.16 * latent_x,
            latent_z + 0.28 * latent_y,
            latent_x * latent_y - 0.24 * latent_z,
            (latent_x**2) * 0.42 - 0.36 * latent_y,
            (latent_y**2) * 0.55 + 0.24 * latent_x,
            (latent_z**2) * 0.70 - 0.30 * latent_x * latent_y,
            latent_x - latent_y + 0.62 * latent_z,
        )
        rows.append(
            {
