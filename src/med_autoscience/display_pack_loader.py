...
def iter_enabled_local_pack_roots(repo_root: Path) -> list[tuple[str, Path]]:
...
def load_enabled_local_display_packs(repo_root: Path) -> list[DisplayPackManifest]:
    ...
    for pack_id, pack_root in iter_enabled_local_pack_roots(repo_root):
...
def load_enabled_local_display_pack_templates(repo_root: Path) -> list[DisplayTemplateManifest]:
    return [item.template_manifest for item in load_enabled_local_display_template_records(repo_root)]


def load_enabled_local_display_template_records(repo_root: Path) -> list[LoadedDisplayTemplate]:
...
def load_enabled_local_display_pack_template_records(repo_root: Path) -> list[LoadedDisplayTemplate]:
    return load_enabled_local_display_template_records(repo_root)
