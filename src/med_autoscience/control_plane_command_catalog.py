...
CONTROL_PLANE_OPERATIONS_COMMANDS: tuple[ControlPlaneOperationsCommand, ...] = (
    ...
    ControlPlaneOperationsCommand(
        command="control-plane-governance-report",
        cli_command="control-plane-governance-report",
        mcp_mode="governance_report",
    ...
    ControlPlaneOperationsCommand(
        ...
        required_fields=("workspace_roots",),
        optional_fields=("apply", "control_plane_snapshot", "retention_report"),
        contract_gated=True,
        read_only=False,
    ),
    ControlPlaneOperationsCommand(
        command="control-plane-safe-cache-cleanup-apply",
        cli_command="control-plane-safe-cache-cleanup-apply",
        mcp_mode="safe_cache_cleanup_apply",
        surface="control_plane_safe_cache_cleanup_apply",
        description="Contract-gated safe-cache cleanup apply surface limited to allowlisted delete-safe-cache actions.",
        required_fields=("workspace_roots",),
        optional_fields=("apply", "control_plane_snapshot", "retention_report"),
        contract_gated=True,
        read_only=False,
