...
from med_autoscience.paper_mission_domain.submission_gate_readback import (
...
from med_autoscience.controllers.study_progress.progress_first_payload import (
    progress_first_status_payload as _study_progress_status_payload,
)
...
from med_autoscience.paper_mission_domain.receipt_owner_consumption import (
...
def _serialize_study_runtime_result(result: dict[str, Any] | Any) -> dict[str, Any]:
    payload = result if isinstance(result, dict) else result.to_dict()
    safe_payload = json_safe(payload)
    if not isinstance(safe_payload, dict):
        raise TypeError("study runtime controller result must serialize to a mapping")
    return safe_payload
