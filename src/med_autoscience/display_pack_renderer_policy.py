    ...
    if kind == EVIDENCE_FIGURE_KIND:
        if renderer == R_GGPLOT2_RENDERER:
            return RendererPolicyDecision(
                ...
                python_renderer_role="legacy_comparison_or_migration_input_only",
            )
        return RendererPolicyDecision(
            surface_class="programmatic_data_evidence",
            default_surface_allowed=False,
            default_surface_reason="python_evidence_requires_explicit_advantage_or_r_parity_before_default",
            statistical_evidence_authority=True,
            programmatic_precision_required=True,
            first_class_renderer_family=R_GGPLOT2_RENDERER,
            python_renderer_role="non_default_migration_input",
        )
    if kind == ILLUSTRATION_SHELL_KIND:
        return RendererPolicyDecision(
...
def renderer_policy_completion(records: list[RendererPolicyRecord]) -> dict[str, object]:
    ...
    non_default_python_evidence = [
        record
        for record in records
        if record not in visible
        and record.kind == EVIDENCE_FIGURE_KIND
        and record.renderer_family == PYTHON_RENDERER
    ]
    r_evidence_family_ids = {
        record.canonical_family_id
        for record in r_evidence
    }
    python_only_family_ids = sorted(
        {
            record.canonical_family_id
            for record in non_default_python_evidence
    ...
    return {
        ...
        "non_default_python_evidence_template_count": len(non_default_python_evidence),
