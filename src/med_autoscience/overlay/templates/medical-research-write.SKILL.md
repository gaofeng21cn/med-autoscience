...
- variable harmonization details for transported or external-validation models: unit system per predictor, cross-cohort unit conversion, range plausibility by cohort, and sensitivity or claim downgrade for any harmonization anomaly such as HDL-C unit mismatch
...
- for prediction-model external validation papers, do not write the first full draft until Table 1, Table 2/performance metrics, model reproducibility details, variable-unit harmonization, calibration/risk-distribution displays, uncertainty intervals, and survey-weighting or unweighted framing are represented in durable evidence or explicitly routed back as typed blockers
