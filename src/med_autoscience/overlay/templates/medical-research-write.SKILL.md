...
- If the latest user/reviewer feedback arrived after a stopped, submission-ready, or finalize milestone, treat it as same-line revision reactivation. Read the durable `reviewer_revision` intake first, work on the canonical paper authority, and regenerate `manuscript/current_package/`; do not treat foreground edits to that projection as completed OPL attempt or MAS owner-route work.
- For reviewer-driven revisions, keep a revision-delta audit: compare the revised manuscript against the previous reviewable manuscript/package and prior blocker list, then record which blockers were materially closed, which were routed to analysis/human gate/TODO, and which remain unresolved. Cleaner prose or refreshed packaging alone is not a substantive revision.
...
## Medical publication surface contract

For this writing stage, default to a general-medical-journal publication surface unless the user or submission contract explicitly overrides it.

At write-stage entry, durably set and keep:

- `writing_profile`: `general_medical_journal`
- `citation_style`: `AMA`
- `submission_minimal_required`: `true`

Record this contract in durable state, not only chat:

- the decision artifact that routed into `write`
- `paper/section_contracts.md` or an equivalent writing-contract file

Language redlines under this profile:
...
- model reproducibility details: model type, penalty or regularization form, tuning parameter selection, baseline survival or absolute-risk extraction, predictor coding/reference levels, continuous predictor transformations, and software/source-code environment
- variable harmonization details for transported or external-validation models: unit system per predictor, cross-cohort unit conversion, range plausibility by cohort, unit-standardized model application or sensitivity analysis, and claim downgrade for any harmonization anomaly such as HDL-C unit mismatch
- external-validation details: external cohort definition, whether coefficients were transported without refit or recalibration, case mix and covariate support, risk-group occupancy, observed event rates with denominators, and any calibration update policy
...
- do not write authoring-strategy or workflow-status sentences such as `calendar enrollment period is not promoted`, `this restriction is intentional`, `R ggplot2 renderer using the current MAS display-pack style profile`, `TRIPOD is cited only as a boundary reference`, `submission metadata remain incomplete`, or `the manuscript has a defensible clinical story` in manuscript body text; replace them with verified methods, omit nonessential tool details, or route missing facts to the submission checklist / human gate
...
- for descriptive registry or phenotype-atlas manuscripts, the first complete draft should write completed study methods rather than authoring status: enrollment/source-specific data windows, analytic data lock or export date, inclusion/exclusion flow, ethics/consent or waiver, deidentification, funding/COI/data availability, BMI calculation and adult/child applicability, age distribution or adult-only sensitivity plan, diagnostic variable ascertainment, and missingness handling must be present when evidence exists; unresolved facts belong in checklist, handoff, typed blocker, or human gate surfaces, not in article body prose
- for descriptive registry figures, avoid `burden` or `prevalence` wording unless the design supports population inference; use recorded/populated diagnostic fields, show n/N or denominators, and keep captions restricted to variables visible in the figure
...
- the current line still passes a publishability gate rather than merely having enough material to form a draft
- `writing_profile` and `submission_minimal_required` are durably recorded for this writing line

If major claims lack evidence, surface the gap first.
If the line looks unlikely to support a strong paper and further writing would mainly package weak evidence, route back to `decision` instead of drafting harder.
...
Maintain `paper/methods_implementation_manifest.json`.

This file is the durable bridge between code, analysis artifacts, and manuscript Methods prose.
Do not leave critical implementation detail only in transient chat or only in the codebase.

At minimum, the manifest must make the following explicit:
...
For each main manuscript model, record enough detail that an informed reviewer can answer:
...
If the manuscript uses biological, mechanistic, or causal interpretation, keep the explanation boundaries explicit.
Do not blur predictive association into causal effect language unless the actual design supports that leap.

If the manuscript uses labels such as `knowledge-guided`, `causal`, `mechanistic`, or `calibration-first`, define them operationally in the manifest instead of treating them as rhetorical decoration.
Each such label should map to:

- the label text
- an operational definition
- an implementation anchor that can be checked against code, analysis artifacts, or decision logs

### 1.1 Manuscript-safe reproducibility supplement

Maintain `paper/manuscript_safe_reproducibility_supplement.json`.

This supplement is the manuscript-safe reproducibility appendix surface for medical readers and reviewers.
At minimum, it should export:
...
If baseline verification, intake audit, or endpoint-contract files still flag an endpoint provenance caveat, maintain `paper/endpoint_provenance_note.md`.

That note should record:

- endpoint name
- provenance caveat
- manuscript-required statement

If the note exists, ensure the manuscript-facing surface actually contains the manuscript-required statement.
Do not let endpoint caveats disappear between verification artifacts and the final paper text.

### 2. Results narration contract

Maintain `paper/results_narrative_map.json`.

Each main Results subsection must be organized around:

- `research_question`
- `direct_answer`
- key quantitative findings
- supporting display items
- clinical meaning
- claim boundary

The draft should follow that map.
Do not write Results as a sequence of figure-by-figure or table-by-table commentary.
Avoid body-text patterns such as:
...
Before step 1, durably establish the writing-surface contract:
...
- for prediction-model manuscripts, close the structured TRIPOD/TRIPOD+AI fields before prose expansion: data-source years, inclusion/exclusion, endpoint ascertainment, follow-up start, censoring, missingness, variable coding, model tuning, center effects, model type, penalty/tuning, baseline survival or absolute-risk extraction, predictor transformations, unit harmonization, external-validation cohort definition, case mix/covariate support, risk-group occupancy, validation event counts, calibration, uncertainty intervals, Table 1, Table 2/performance metrics, DCA threshold actions, NHANES weighting or unweighted framing when applicable, and time-to-event competing-event screen when applicable
...
- for prediction-model external validation papers, do not write the first full draft until Table 1, Table 2/performance metrics, model reproducibility details, variable-unit harmonization, unit-standardized application or sensitivity, calibration/risk-distribution displays, uncertainty intervals, and survey-weighting or unweighted framing are represented in durable evidence or explicitly routed back as typed blockers
- unavailable evidence should be handled as typed blocker or limitation metadata, not as article-body phrases like `not available from verified outputs` or `requires author confirmation before external submission`
...
- writing-profile contract integrity (`writing_profile` and `submission_minimal_required` are durable and consistent with the active line)
- terminology redline compliance for manuscript body text, figure titles, and captions
- `submission_minimal` exists and is passed when `submission_minimal_required = true`
...
### `methods_implementation_manifest.json` minimum shape
...
```json
{
  "study_design": {
    ...
    "ethics": "Approved by the institutional review board.",
    "inclusion_criteria": "Adults with NF-PitNET undergoing first surgery.",
    "exclusion_criteria": "Repeat surgery or missing endpoint data.",
  ...
  "statistical_analysis": {
    ...
    "subgroup_strategy": "Prespecified subgroup comparisons."
  },
  "causal_boundary": {
    "claim_level": "associational",
    "allowed_language": "prediction and association",
    "not_allowed": "causal effect claims"
  },
  "method_labels": [
    {
      "label": "calibration-first",
      "operational_definition": "Model selection prioritized calibration and decision-curve utility before discrimination gains.",
      "implementation_anchor": "Nested validation and utility analysis artifacts."
    }
  ]
}
```

### `results_narrative_map.json` minimum shape
  ...
  "sections": [
    {
      ...
      "research_question": "Does the extended model improve early residual-risk assessment?",
      "direct_answer": "Yes. It improves discrimination, calibration, and decision-curve utility.",
      "supporting_display_items": ["F1", "T1"],
      "key_quantitative_findings": [
        "Performance improved over the baseline clinical model."
      ],
      "clinical_meaning": "Supports preoperative risk stratification.",
      "boundary": "Supports prediction, not causal inference."
    }
  ]
}
```
...
```json
{
  ...
  "random_seed_policy": "Fixed seeds recorded in the run package.",
  "key_hyperparameters": [
    {
      "model_id": "M1",
      "parameters": {
...
### `endpoint_provenance_note.md` minimum shape

```markdown
# Endpoint Provenance Note

- endpoint_name: removal_rate
- provenance_caveat: `removal_rate` is treated as a working proxy for early residual status and retains a 3-month MRI provenance caveat.
...
```json
{
  ...
  "checks": [
    ...
    },
    {
      ...
      "status": "pass",
      "notes": "Submission-minimal contract is complete."
    },
    {
      "key": "methods_completeness",
      "status": "pass",
      "notes": "Methods implementation manifest matches the manuscript-facing Methods section."
    },
    {
      "key": "results_narrative_structure",
      "status": "pass",
      "notes": "Results sections are question-led and clinically integrated rather than figure-led."
    },
    {
      "key": "reproducibility_supplement",
      "status": "pass",
      "notes": "Manuscript-safe reproducibility supplement records package versions, seeds, hyperparameters, missing-data strategy, and metric definitions."
    },
    {
      "key": "endpoint_provenance",
      "status": "pass",
      "notes": "Endpoint provenance caveats remain explicit on the manuscript-facing surface."
    }
  ],
  "blocking_items": [],
  "handoff_ready": true
...
### `submission_minimal.json` minimum shape
...
```json
{
  ...
  "writing_profile": "general_medical_journal",
  "required": true,
  "terminology_redline_passed": true,
...
- figure_table_led_results_narration
...
For any submission-ready claim, also require:

- `submission_minimal_required = true` is durably recorded
- `submission_minimal` has status `passed`
