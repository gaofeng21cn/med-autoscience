...
- Prediction-model or time-to-event drafts expose TRIPOD-style reporting: candidate predictor count, event count, validation N, censoring / competing-risk interpretation, calibration metric, Brier score or prediction error, and decision-curve threshold rationale
- Clinical-use claims are bounded to internal validation and risk stratification unless an external validation or deployment-grade evidence package is actually present
