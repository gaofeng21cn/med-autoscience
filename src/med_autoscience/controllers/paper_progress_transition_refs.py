...
def decision_trace_projection(*payloads: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for payload in payloads:
        if not isinstance(payload, Mapping):
            continue
        decision_summary = decision_summary or _trace_summary(payload)
        decision_refs.extend(_trace_refs_for_keys(payload, DECISION_TRACE_REF_KEYS))
        failed_refs.extend(_trace_refs_for_keys(payload, FAILED_PATH_REF_KEYS))
        consumed_refs.extend(_trace_refs_for_keys(payload, CONSUMED_FAILED_PATH_REF_KEYS))

        source_refs = _trace_mapping(payload.get("source_refs"))
        if source_refs:
            decision_refs.extend(_trace_refs_for_keys(source_refs, DECISION_TRACE_REF_KEYS))
            failed_refs.extend(_trace_refs_for_keys(source_refs, FAILED_PATH_REF_KEYS))
            consumed_refs.extend(_trace_refs_for_keys(source_refs, CONSUMED_FAILED_PATH_REF_KEYS))

        for key in DECISION_TRACE_KEYS:
            surface = _trace_mapping(payload.get(key))
            if not surface:
                continue
            decision_summary = decision_summary or _trace_summary(surface)
            refs = _trace_refs_from_field(surface.get("refs") or surface.get("source_refs") or surface.get("receipt_refs"))
            for ref, role in refs:
                if role == "failed_path" or _looks_like_failed_path_ref(ref):
                    failed_refs.append(ref)
                else:
                    decision_refs.append(ref)
            decision_refs.extend(_trace_refs_for_keys(surface, DECISION_TRACE_REF_KEYS))
            failed_refs.extend(_trace_refs_for_keys(surface, FAILED_PATH_REF_KEYS))
            consumed_refs.extend(_trace_refs_for_keys(surface, CONSUMED_FAILED_PATH_REF_KEYS))
            suppressed = suppressed or bool(surface.get("repeated_failed_path_suppressed"))

        for key in FAILED_PATH_KEYS:
            surface = _trace_mapping(payload.get(key))
            if not surface:
                continue
            failed_summary = failed_summary or _trace_summary(surface)
            failed_refs.extend(ref for ref, _role in _trace_refs_from_field(surface.get("refs") or surface.get("source_refs")))
            failed_refs.extend(_trace_refs_for_keys(surface, FAILED_PATH_REF_KEYS))
            consumed_refs.extend(_trace_refs_for_keys(surface, CONSUMED_FAILED_PATH_REF_KEYS))
            suppressed = suppressed or bool(surface.get("repeated_failed_path_suppressed"))

    decision_refs = _unique_trace_texts(decision_refs)
    failed_refs = _unique_trace_texts(failed_refs)
    consumed_refs = _unique_trace_texts(consumed_refs)
...
def repeated_failed_path_suppressed(
    *,
    ...
    recorded_refs = set(_trace_text_items(trace_projection.get("consumed_failed_path_refs")))
...
def _refs_consumed_by_action(action: Mapping[str, Any]) -> set[str]:
    refs: set[str] = set()
    for key in ("consumes_failed_path_refs", "consumed_failed_path_refs", "failed_path_refs"):
        refs.update(_trace_text_items(action.get(key)))
    return refs


def _trace_refs_for_keys(payload: Mapping[str, Any], keys: Iterable[str]) -> list[str]:
    refs: list[str] = []
    for key in keys:
        refs.extend(ref for ref, _role in _trace_refs_from_field(payload.get(key)))
    return refs


def _trace_refs_from_field(value: object) -> list[tuple[str, str | None]]:
    refs: list[tuple[str, str | None]] = []
    if isinstance(value, str):
        text = _trace_text(value)
        return [(text, None)] if text else []
    if isinstance(value, Mapping):
        ref = _trace_text(value.get("ref")) or _trace_text(value.get("path")) or _trace_text(value.get("artifact_path"))
        role = _trace_text(value.get("role")) or _trace_text(value.get("ref_role"))
        if ref:
        ...
        for key, item in value.items():
            ...
            if str(key).endswith("refs") or str(key).endswith("ref") or str(key).endswith("path"):
                refs.extend(_trace_refs_from_field(item))
        return refs
...
def _trace_summary(payload: Mapping[str, Any]) -> str | None:
    for key in ("summary", "decision_summary", "route_summary", "failed_path_summary", "reason"):
        if text := _trace_text(payload.get(key)):
            return text
    return None
...
def _trace_mapping(value: object) -> dict[str, Any]:
    return dict(value) if isinstance(value, Mapping) else {}


def _trace_text_items(value: object) -> list[str]:
    return [ref for ref, _role in _trace_refs_from_field(value)]


def _unique_trace_texts(values: Iterable[object]) -> list[str]:
    seen: set[str] = set()
    result: list[str] = []
    for value in values:
        text = _trace_text(value)
...
def _trace_text(value: object) -> str | None:
