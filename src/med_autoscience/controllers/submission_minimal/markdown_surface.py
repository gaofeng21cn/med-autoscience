...
from .markdown_surface_blocks import (
    collect_named_second_level_blocks,
    collect_named_top_level_blocks,
    extract_top_level_markdown_block,
    INTERNAL_MANUSCRIPT_INSTRUCTION_PATTERNS,
    MANUSCRIPT_MAJOR_HEADING_PATTERN,
...
from .markdown_surface_tables import (
...
def _ensure_submission_image_width(line: str) -> str:
    normalized = str(line or "").strip()
    if not normalized:
        return ""
    if re.search(r"\{\s*[^}]*\bwidth\s*=", normalized):
        return normalized
    return f"{normalized}{{width=100%}}"
