...
def _current_control_transition_request_tasks(
    *,
    ...
    ) and _currentness_consumes_current_control_transition_request(
    ...
    if mas_owner_action_source.startswith(
        "opl_current_control_state."
    ) and _current_owner_action_supersedes_transition_request(
...
def _currentness_consumes_current_control_transition_request(
...
def _transition_consuming_terminal_surfaces(
    *,
    ...
    for surface in (
        ...
        status = text(surface.get("status")) or text(surface.get("state_kind")) or text(
            surface.get("execution_state_kind")
        ) or text(
...
def _current_owner_action_supersedes_transition_request(
