...
BEHAVIOR_EQUIVALENCE_SURFACES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "behavior_difference": "MAS has a fixed Progress Portal and study rows, but the current default view can mix multiple papers in one workspace overview instead of making one paper/study the primary navigation unit.",
        "default_user_impact": "Users can inspect MAS progress without running MDS WebUI, but multi-paper workspaces still need a clearer per-study/per-paper drilldown to match the old user workflow.",
        "mas_contract": "Progress Portal consumes MAS payload refs, freshness, source refs and artifact locators without reinterpreting study truth; future UX parity requires a study-scoped Portal view and stable deep links.",
