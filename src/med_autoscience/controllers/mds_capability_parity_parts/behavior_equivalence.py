...
BEHAVIOR_EQUIVALENCE_SURFACES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "mas_behavior": {
            ...
            "primary_scope": "workspace_overview_with_study_rows",
            "paper_scoped_navigation": "not_primary_in_current_landed_scope",
            "route_decision_trail": "not_landed",
        },
        "behavior_difference": "MAS has a fixed Progress Portal and study rows, but the current default view can mix multiple papers in one workspace overview instead of making one paper/study the primary navigation unit; it also lacks a study-scoped route/decision trail for failed paths, route changes, and active/winning path.",
        "default_user_impact": "Users can inspect MAS progress without running MDS WebUI, but multi-paper workspaces still need a clearer per-study/per-paper drilldown and route/decision trail to match the old user workflow.",
        "mas_contract": "Progress Portal consumes MAS payload refs, freshness, source refs and artifact locators without reinterpreting study truth; future UX parity requires a study-scoped Portal view, stable deep links, and a read-only route/decision trail from controller decisions, evidence/review ledgers, and runtime lifecycle lineage/canvas.",
        "future_parity_candidate": "portal_route_decision_trail",
    ...
    },
    {
        ...
        "behavior_difference": "MAS preserves the observation purpose through Progress Portal plus Live Console; the old resident WebSocket terminal attach and UI-issued daemon/control actions are not implemented in the landed read-only scope and remain future parity candidates rather than retired requirements.",
        "default_user_impact": "Users now have MAS-owned progress and read-only live observation surfaces; low-latency interactive terminal attach and authorized UI control still require a separate safety/owner/audit-gated lane.",
        "mas_contract": "live-console-parity owns workspace/study/run, terminal.tail, log.tail, runtime.health, runtime.supervision and artifact.delta read models without writing authority surfaces.",
