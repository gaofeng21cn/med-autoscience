...
def authorize_authority_route(
    ...
    if not snapshot and not bool(controller_route_gate.get("authorized")):
        blocking_reasons.append("authority_snapshot_missing")
    ...
    if not _has_authority_epoch(snapshot):
        blocking_reasons.append("authority_epoch_missing")
...
def assert_authority_route_authorized(
    action: str,
    context: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    gate = authorize_authority_route(action, context)
    if not gate["authorized"]:
        raise PermissionError(
            "authority route blocked "
            f"{gate['action']}: {', '.join(gate['blocking_reasons'])}"
        )
    return gate


def attach_authority_route_gate(
    payload: Mapping[str, Any],
    gate: Mapping[str, Any],
) -> dict[str, Any]:
    result = dict(payload)
    result["authority_route_gate"] = dict(gate)
    return result
...
def _snapshot_ref(snapshot: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "surface": _text(snapshot.get("surface")) or "authority_snapshot",
...
__all__ = [
    "ROUTE_ACTIONS",
    "assert_authority_route_authorized",
