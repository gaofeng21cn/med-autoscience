...
def author_operator_payload(
    *,
    ...
    if existing:
        return existing
    provider_backed = _payload_from_provider_adapters(
        study_root=root,
...
def _payload_from_provider_adapters(
    *,
    study_root: Path,
    generated_at: str,
    surface_key: str | None,
    ...
    if anchor is None:
        return _blocked_payload("missing_pubmed_anchor_record", surface_key=surface_key)
    if guideline is None:
        return _blocked_payload("missing_guideline_reference_for_provider_runtime", surface_key=surface_key)
    if systematic is None:
        return _blocked_payload("missing_systematic_review_reference_for_provider_runtime", surface_key=surface_key)
    ...
    if not pubmed_provider:
        return _blocked_payload("provider_adapter_payload_failed_pubmed", surface_key=surface_key)
    ...
    if not crossref_provider:
        return _blocked_payload("provider_adapter_fetch_failed_crossref", surface_key=surface_key)
    ...
    if not semantic_provider:
        return _blocked_payload("provider_adapter_fetch_failed_semantic_scholar", surface_key=surface_key)
...
def _records_from_literature_materialization(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in _mapping_list(payload.get("records")):
        pubmed = _mapping(item.get("pubmed"))
        records.append(
            {
                "record_id": _text(item.get("record_id")),
                "title": _text(item.get("title")),
                "doi": _text(item.get("doi")),
                "pmid": _text(pubmed.get("pmid")) or _text(item.get("pmid")),
