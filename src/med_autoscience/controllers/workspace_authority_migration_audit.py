...
def run_migration_audit(*, workspace_roots: Iterable[str | Path], dry_run: bool = True) -> dict[str, Any]:
    if not dry_run:
        raise ValueError("workspace authority migration audit is dry-run only; delivery backfill requires a separate MAS authority apply contract")
    ...
    return {
        "surface": "workspace_authority_migration_audit",
        "schema_version": 1,
        "report_id": f"workspace-authority-migration-audit::{report_digest[:_FINGERPRINT_LENGTH]}",
