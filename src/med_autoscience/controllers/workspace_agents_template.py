...
def render_workspace_agents(*, workspace_name: str) -> str:
    return (
        ...
        "`ops/medautoscience/bin/enter-study`、`ops/medautoscience/bin/paper-mission`、"
        "`ops/medautoscience/bin/study-progress`、`ops/medautoscience/bin/study-state-matrix` 调用 MAS paper mission / domain refs 工作流。\n"
        ...
        "- `runtime/` 是运行态目录，`runtime/artifacts/` 是 refs index 与维护 ledger，`ops/medautoscience/` 是唯一 MAS workspace ops 入口；用户可见 workbench / display 归 OPL hosted surface 承载，MAS 只输出 refs-only projection。\n\n"
        "## 用户论文修改请求处理\n\n"
