...
def render_workspace_agents(*, workspace_name: str) -> str:
    return (
        ...
        "`ops/medautoscience/bin/enter-study`、`ops/medautoscience/bin/progress-projection` 与 "
        "`ops/medautoscience/bin/study-state-matrix`、`ops/medautoscience/bin/domain-health-diagnostic` 调用 MAS domain refs 工作流。\n"
        "- Progress-first 监控入口：单 study 使用 `ops/medautoscience/bin/progress-projection <study_id> --format json`，"
        ...
        "- 动手前必须查询对应 study 的 `progress-projection`，确认 OPL current-control-state refs、`execution_owner_guard`、`supervisor_only` 与 publication gate 状态；OPL runtime 接管中时只做监督，除非已经显式暂停或接管。\n"
