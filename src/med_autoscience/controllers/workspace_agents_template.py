...
def render_workspace_agents(*, workspace_name: str) -> str:
    return (
        ...
        "`ops/medautoscience/bin/enter-study`、`ops/medautoscience/bin/study-progress`、"
        "`ops/medautoscience/bin/progress-projection` 与 "
        "`ops/medautoscience/bin/study-state-matrix`、`ops/medautoscience/bin/domain-health-diagnostic` 调用 MAS domain refs 工作流。\n"
        "- Progress-first 监控入口：单 study 使用 `ops/medautoscience/bin/study-progress <study_id>`；"
        "JSON 兼容读面使用 `ops/medautoscience/bin/progress-projection <study_id> --format json`；"
        "workspace 矩阵使用 `ops/medautoscience/bin/study-state-matrix --format json`；旧 `progress-projection <study_id> --json` "
        "仅作为 JSON alias 兼容。\n"
        ...
        "- 动手前必须查询对应 study 的 `study-progress`，确认 OPL current-control-state refs、`execution_owner_guard`、`supervisor_only` 与 publication gate 状态；OPL runtime 接管中时只做监督，除非已经显式暂停或接管。\n"
        ...
        "- 对这种 revision reactivation，必须先写入 durable `reviewer_revision` task intake，再由 OPL provider attempt hydration/resume 消费 MAS owner refs 后接管 canonical paper surface。\n"
        ...
        "- MDS 恢复同一论文线前，必须优先读取 latest revision handoff/evidence surface，再决定继续走 MAS controller 还是 MDS paper surface。\n"
