...
def _workspace_directories(workspace_root: Path) -> list[Path]:
    return [
        ...
        workspace_root / "ops" / "deepscientist" / "bin",
        workspace_root / "ops" / "deepscientist" / "runtime",
        workspace_root / "ops" / "deepscientist" / "startup_briefs",
        workspace_root / "ops" / "deepscientist" / "startup_payloads",
    ]
...
def _render_workspace_readme(*, workspace_name: str, profile_relpath: Path) -> str:
    return (
        ...
        f"2. 编辑 `{profile_relpath.as_posix()}`，补全 publication profile 与 DeepScientist repo 信息。\n"
...
def _render_workspace_profile(
    *,
    ...
    return (
        ...
        'deepscientist_repo_root = "/ABS/PATH/TO/DeepScientist"\n'
...
def _render_deepscientist_config() -> str:
    return (
        "# Set the absolute path to the local DeepScientist launcher binary.\n"
        'DEEPSCIENTIST_LAUNCHER="/ABS/PATH/TO/ds"\n'
    )
...
def _render_watch_runtime_script() -> str:
    return (
        ...
        '  --runtime-root "${WORKSPACE_ROOT}/ops/deepscientist/runtime/quests" \\\n'
        '  "$@"\n'
    )


def _render_deepscientist_shared() -> str:
    return (
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n\n"
        'SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"\n'
        'DEEPSCIENTIST_OPS_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"\n'
        'WORKSPACE_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"\n'
        'DEEPSCIENTIST_CONFIG_ENV_PATH="${DEEPSCIENTIST_OPS_ROOT}/config.env"\n'
        ...
        'if [[ -f "${DEEPSCIENTIST_CONFIG_ENV_PATH}" ]]; then\n'
        "  # shellcheck disable=SC1090\n"
        '  source "${DEEPSCIENTIST_CONFIG_ENV_PATH}"\n'
        "fi\n\n"
        'if [[ -z "${DEEPSCIENTIST_LAUNCHER:-}" ]]; then\n'
        '  echo "DEEPSCIENTIST_LAUNCHER is not configured. Set it in ${DEEPSCIENTIST_CONFIG_ENV_PATH}." >&2\n'
        "  exit 1\n"
        "fi\n\n"
        'if [[ "${DEEPSCIENTIST_LAUNCHER}" != /* ]]; then\n'
        '  echo "DEEPSCIENTIST_LAUNCHER must be an absolute path: ${DEEPSCIENTIST_LAUNCHER}" >&2\n'
        "  exit 1\n"
        "fi\n\n"
        'if [[ ! -x "${DEEPSCIENTIST_LAUNCHER}" ]]; then\n'
        '  echo "DEEPSCIENTIST_LAUNCHER is not executable: ${DEEPSCIENTIST_LAUNCHER}" >&2\n'
        "  exit 1\n"
        "fi\n\n"
        "load_deepscientist_contract() {\n"
        "  local payload_json\n"
        '  payload_json="$(\n'
        '    PYTHONPATH="${MED_AUTOSCIENCE_REPO_RESOLVED}/src${PYTHONPATH:+:${PYTHONPATH}}" \\\n'
        ...
        "PY\n"
        '  )"\n\n'
        '  export MEDAUTOSCI_DEEPSCIENTIST_CONTRACT_JSON="${payload_json}"\n\n'
        ...
        '    "deepscientist_runtime_root": profile["deepscientist_runtime_root"],\n'
        '    "deepscientist_repo_root": profile.get("deepscientist_repo_root") or "",\n'
        '    "runtime_root_matches_deepscientist_runtime": str(\n'
        '        bool(runtime_contract.get("checks", {}).get("runtime_root_matches_deepscientist_runtime"))\n'
        '    ).lower(),\n'
        ...
        '    case "${key}" in\n'
        '      workspace_root) DEEPSCIENTIST_WORKSPACE_ROOT="${value}" ;;\n'
        '      runtime_root) DEEPSCIENTIST_RUNTIME_ROOT="${value}" ;;\n'
        '      deepscientist_runtime_root) DEEPSCIENTIST_HOME="${value}" ;;\n'
        '      deepscientist_repo_root) DEEPSCIENTIST_REPO_ROOT_AUDIT="${value}" ;;\n'
        '      runtime_root_matches_deepscientist_runtime) RUNTIME_ROOT_MATCHES_DEEPSCIENTIST_RUNTIME="${value}" ;;\n'
        '      runtime_contract_ready) DEEPSCIENTIST_RUNTIME_CONTRACT_READY="${value}" ;;\n'
        '      launcher_contract_ready) DEEPSCIENTIST_LAUNCHER_CONTRACT_READY="${value}" ;;\n'
        '      phase_25_ready) DEEPSCIENTIST_PHASE_25_READY="${value}" ;;\n'
        "    esac\n"
        '  done <<< "${contract_lines}"\n\n'
        '  if [[ "${RUNTIME_ROOT_MATCHES_DEEPSCIENTIST_RUNTIME:-false}" != "true" ]]; then\n'
        '    echo "runtime_root does not match deepscientist_runtime_root/quests for profile ${PROFILE_PATH}" >&2\n'
        "    exit 1\n"
        "  fi\n\n"
        '  if [[ -z "${DEEPSCIENTIST_HOME:-}" ]]; then\n'
        '    echo "Failed to resolve deepscientist_runtime_root from profile ${PROFILE_PATH}" >&2\n'
        "    exit 1\n"
        "  fi\n"
        "}\n\n"
        "render_deepscientist_config_json() {\n"
        '  CONTRACT_JSON="${MEDAUTOSCI_DEEPSCIENTIST_CONTRACT_JSON}" \\\n'
        '  LAUNCHER_PATH="${DEEPSCIENTIST_LAUNCHER}" \\\n'
        '  python3 - <<'"'"'PY'"'"'\n'
        "import json\n"
        "import os\n\n"
        'payload = json.loads(os.environ["CONTRACT_JSON"])\n'
        ...
        '            "runtime_root": profile["runtime_root"],\n'
        '            "deepscientist_runtime_root": profile["deepscientist_runtime_root"],\n'
        '            "deepscientist_repo_root": profile.get("deepscientist_repo_root"),\n'
        ...
        "PY\n"
        "}\n\n"
        "run_deepscientist_launcher() {\n"
        '  exec "${DEEPSCIENTIST_LAUNCHER}" --home "${DEEPSCIENTIST_HOME}" "$@"\n'
        "}\n"
    )


def _render_deepscientist_forward(script_command: str) -> str:
    return (
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n\n"
        'SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"\n'
        ...
        "load_deepscientist_contract\n"
        f"run_deepscientist_launcher {script_command} \"$@\"\n"
    )


def _render_deepscientist_show_config() -> str:
    return (
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n\n"
        'SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"\n'
...
def _rendered_files(
    *,
    ...
    return [
        ...
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "config.env",
            content=_render_deepscientist_config(),
        ),
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "config.env.example",
            content=_render_deepscientist_config(),
        ),
        RenderedFile(
        ...
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "README.md",
            content=_render_deepscientist_readme(),
        ),
        RenderedFile(
        ...
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "bin" / "_shared.sh",
            content=_render_deepscientist_shared(),
            executable=True,
        ),
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "bin" / "doctor",
            content=_render_deepscientist_forward("doctor"),
            executable=True,
        ),
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "bin" / "show-config",
            content=_render_deepscientist_show_config(),
            executable=True,
        ),
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "bin" / "start-web",
            content=_render_deepscientist_forward("--port 20999"),
            executable=True,
        ),
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "bin" / "status",
            content=_render_deepscientist_forward("--status"),
            executable=True,
        ),
        RenderedFile(
            path=workspace_root / "ops" / "deepscientist" / "bin" / "stop",
            content=_render_deepscientist_forward("--stop"),
            executable=True,
        ),
    ]
    ...
    *,
    ...
    return {
        ...
        "next_steps": [
            ...
            f"edit {workspace_root / 'ops' / 'deepscientist' / 'config.env'}",
