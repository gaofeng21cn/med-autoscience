...
def resolve_control_plane_write_route_context(
    *,
    action: str,
    ...
    gate = authorize_control_plane_route(action, route_context)
    ...
    if not route_context.get("control_plane_snapshot") and not explicit_controller_route:
        _append_unique(blocking_reasons, "control_plane_snapshot_missing")
...
def blocked_control_plane_write_payload(
    *,
    ...
    **fields: Any,
    ...
    return {
        ...
        **fields,
        "control_plane_route_gate": dict(gate),
    }


def attach_write_route_gate(payload: Mapping[str, Any], gate: Mapping[str, Any]) -> dict[str, Any]:
    return attach_control_plane_route_gate(payload, gate)
