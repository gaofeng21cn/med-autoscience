...
def build_audit_compaction_governance_report(
    ...
    *,
    ...
    compaction_contract = _build_compaction_pre_contract()
...
def _compaction_implementation_allowed(contract: Mapping[str, Any]) -> bool:
    gates = [gate for gate in _list(contract.get("gates")) if isinstance(gate, Mapping)]
    required = {_text(gate.get("gate_id")) for gate in gates if _text(gate.get("status")) == "passed"}
    return set(REQUIRED_COMPACTION_GATES) <= required
