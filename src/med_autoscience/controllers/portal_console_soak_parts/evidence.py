...
def build_soak_evidence(
    *,
    ...
    return {
        ...
        "terminal_log_refs": artifact_surfaces.terminal_log_refs(console_snapshot),
...
def _portal_refresh_evidence(
    *,
    ...
    portal_view = mapping(portal_payload.get("portal_view"))
    status = "passed" if portal_result.get("status") == "materialized" and portal_html_path.is_file() else "blocked"
    return {
        "status": status,
        "payload_generated_at": text(portal_payload.get("generated_at")),
...
def _live_console_disambiguation(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    study_ids = [text(item.get("study_id")) for item in studies]
    run_ids = [
        text(run.get("run_id"))
        for item in studies
    ...
    if not run_ids:
        run_ids = [text(item.get("active_run_id")) for item in studies]
    ...
    return {
        ...
        "selected_study_id": text(snapshot.get("selected_study_id")),
    }
...
def _per_study_workbench(
    *,
    ...
    workspace = mapping(portal_payload.get("workspace"))
    ...
    tabs = mapping_list(workbench.get("tabs"))
    tab_ids = {text(item.get("id")) for item in tabs if text(item.get("id"))}
    expected_tabs = {
        "overview",
        "route_map",
    ...
    materialized_pages = artifact_surfaces.materialized_study_page_refs(
        portal_result=portal_result,
        portal_html_path=portal_html_path,
        studies=studies,
    )
    has_rows = bool({text(item.get("study_id")) for item in studies if text(item.get("study_id"))})
    ...
    return {
        ...
        "workbench_surface_kind": text(workbench.get("surface_kind")),
...
def _route_decision_trail(
    *,
    ...
    trails = artifact_surfaces.portal_route_trails(portal_result=portal_result, portal_payload=portal_payload)
    ...
    all_conditions = [
        ...
        for condition in string_refs(mapping(trail.get("conditions")).get("missing"))
    ]
    has_nodes = any(mapping_list(trail.get("nodes")) for trail in available)
    has_active_path = any(text(trail.get("active_path")) for trail in available)
    has_winning_path = any(text(trail.get("winning_path")) for trail in available)
    has_source_refs = any(string_refs(trail.get("source_refs")) for trail in available)
    return {
        ...
        "active_paths": dedupe_text(trail.get("active_path") for trail in available),
        "winning_paths": dedupe_text(trail.get("winning_path") for trail in available),
        "source_ref_count": sum(len(string_refs(trail.get("source_refs"))) for trail in available),
        "blockers": blockers(
        ...
        + [f"route_decision_trail:{condition}" for condition in dedupe_text(all_conditions)],
    }
...
def _route_map_visualization(
    *,
    ...
    maps = artifact_surfaces.portal_route_maps(portal_result=portal_result, portal_payload=portal_payload)
    available = [
        item
        for item in maps
    ...
    node_kinds = sorted(
        {
            ...
            for node in mapping_list(item.get("nodes"))
            if (kind := text(node.get("kind")))
        }
    )
    edge_kinds = sorted(
        {
            kind
            for item in available
            for edge in mapping_list(item.get("edges"))
            if (kind := text(edge.get("kind")))
        }
    )
    source_refs = dedupe_text(
        ref
        for item in available
        for ref in string_refs(item.get("source_refs"))
    )
    html_refs = artifact_surfaces.route_map_html_refs(portal_result=portal_result, portal_html_path=portal_html_path)
...
def _per_study_deep_link(
    *,
    ...
    handoff = mapping(portal_payload.get("opl_handoff"))
    live_console = mapping(portal_payload.get("live_console"))
    hosted_package = mapping(portal_result.get("hosted_package"))
    package_study_pages = mapping(mapping(hosted_package.get("package_refs")).get("study_pages"))
    refs = [
        text(handoff.get("deep_link")),
        text(live_console.get("html_ref")),
        *string_refs(portal_payload.get("per_study_deep_links")),
    ]
    ...
    for item in workspace.get("studies") or []:
        ...
        refs.extend(
            [
                text(item.get("href")),
                text(item.get("portal_href")),
                text(item.get("deep_link")),
                text(item.get("portal_deep_link")),
                text(item.get("live_console_href")),
                text(item.get("live_console_deep_link")),
            ]
        )
...
def _conversation_read_model(*, conversation_payload: Mapping[str, Any]) -> dict[str, Any]:
    timeline = mapping_list(conversation_payload.get("timeline"))
    studies = mapping_list(conversation_payload.get("studies"))
    source_refs = artifact_surfaces.conversation_source_refs(conversation_payload)
    selected_study_id = text(conversation_payload.get("selected_study_id"))
    scoped = bool(selected_study_id or any(text(item.get("study_id")) for item in studies + timeline))
    has_conversation_items = bool(
        timeline
        or studies
        or conversation_payload.get("surface_kind") == "mas_runtime_conversation_read_model"
    )
    timeline_kinds = sorted({kind for item in timeline if (kind := text(item.get("item_kind")))})
    return {
        "status": "passed" if has_conversation_items and scoped and source_refs else "blocked",
        "surface_kind": text(conversation_payload.get("surface_kind")),
...
def _conversation_portal_panel(
    *,
    ...
    panels = artifact_surfaces.portal_conversation_panels(portal_result=portal_result, portal_payload=portal_payload)
    visible_html_refs = artifact_surfaces.conversation_panel_html_refs(
        portal_result=portal_result,
        portal_html_path=portal_html_path,
    )
    available = [
    ...
    timeline_kinds = sorted(
        {
            ...
            for item in mapping_list(panel.get("timeline_items"))
            if (kind := text(item.get("item_kind")))
        }
    )
    source_refs = [
        ref
        for panel in panels
        for ref in string_refs(panel.get("source_refs"))
    ]
    return {
...
def _study_scoped_console(
    *,
    ...
    selected = text(console_snapshot.get("selected_study_id")) or text(console_ui_payload.get("selected_study_id"))
    studies = [dict(item) for item in console_snapshot.get("studies") or [] if isinstance(item, Mapping)]
    selected_rows = [item for item in studies if item.get("selected") is True or text(item.get("study_id")) == selected]
    stream_sources = [
        dict(item)
    ...
    scoped_sources = [
        ...
        if selected and text(item.get("study_id")) == selected
    ]
    return {
...
def _action_receipts(
    *,
    ...
    intents = mapping_list(console_snapshot.get("controller_action_intents"))
    intents.extend(mapping_list(console_ui_payload.get("controller_action_intents")))
    receipts: list[dict[str, Any]] = []
    for item in intents:
        if text(item.get("receipt_ref")) or text(item.get("audit_ref")) or text(item.get("command")):
    ...
    return {
        ...
        "direct_execution_intents": [text(item.get("intent")) for item in direct_exec],
        "blockers": blockers(
...
def _authorized_action_apply_receipts(
    *,
    ...
    receipts = [read_json_object(path) for path in sorted(receipt_root.glob("*.json"))] if receipt_root.is_dir() else []
    for item in mapping_list(mapping(portal_payload.get("progress_portal_actions")).get("receipts")):
        receipts.append(item)
    ...
    forbidden_writes_ok = all(
        "controller_decisions/latest.json" in string_refs(item.get("forbidden_writes"))
        and "runtime_sqlite_authority" in string_refs(item.get("forbidden_writes"))
        for item in apply_receipts
    )
    actions = sorted({text(item.get("action")) for item in applied if text(item.get("action"))})
    return {
...
def _terminal_attach_gate(*, console_ui_payload: Mapping[str, Any]) -> dict[str, Any]:
    gate = mapping(console_ui_payload.get("terminal_attach_gate"))
    contract = mapping(gate.get("required_owner_contract"))
    required = {"token", "lease", "idempotency", "audit", "input", "resize", "detach"}
    observed = {key for key in required if text(contract.get(key))}
    ...
    return {
        ...
        "surface_kind": text(gate.get("surface_kind")),
        "gate_status": text(gate.get("status")),
        "forbidden_owner": text(gate.get("forbidden_owner")),
        "read_only_default": gate.get("read_only_default"),
        "attach_started": bool(gate.get("attach_started")),
        "study_id": text(gate.get("study_id")),
...
def _latency_slo_source_refs(
    *,
    ...
    study = mapping(portal_payload.get("study"))
    freshness = mapping(portal_payload.get("freshness"))
    slo = mapping(study.get("outer_supervision_slo")) or mapping(portal_payload.get("outer_supervision_slo"))
    events = mapping_list(console_snapshot.get("events"))
    sources = [
        *string_refs(portal_payload.get("source_refs")),
        *source_ref_objects(console_snapshot.get("source_refs")),
        *string_refs(console_ui_payload.get("source_refs")),
    ]
    has_latency = bool(
        text(freshness.get("status"))
        or text(freshness.get("latest_event_at"))
        or text(slo.get("state"))
        or any(text(item.get("observed_at")) for item in events)
    )
    has_slo = bool(text(slo.get("state")) or text(slo.get("surface_kind")) == "outer_supervision_slo")
    has_source_refs = bool(sources)
    return {
        "status": "passed" if has_latency and has_slo and has_source_refs else "blocked",
        "freshness_status": text(freshness.get("status")),
        "outer_supervision_slo_state": text(slo.get("state")),
...
def _source_ref_cleanliness(
    *,
    ...
    refs = [
        *string_refs(portal_payload.get("source_refs")),
        *source_ref_objects(console_payload.get("source_refs")),
        *string_refs(console_ui_payload.get("source_refs")),
    ]
    forbidden = [
        ref
