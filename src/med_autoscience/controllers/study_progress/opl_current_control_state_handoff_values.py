...
def _current_control_currentness_fields(matching: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if "provider_attempt_pending_count" in matching:
        projection["provider_attempt_pending_count"] = int(
            matching.get("provider_attempt_pending_count") or 0
        )
    if "provider_attempt_candidates" in matching:
        projection["provider_attempt_candidates"] = [
            dict(item)
            for item in matching.get("provider_attempt_candidates") or []
