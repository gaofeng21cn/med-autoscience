    ...
    *,
    ...
    action_cost = (
        runtime_dispatch_cost.reconcile_dry_run_contract(
            reason="domain_route_reconcile_dry_run",
            action_fingerprint=f"domain_route_reconcile::{','.join(resolved_study_ids) or 'workspace'}",
        )
        if not apply
        else runtime_dispatch_cost.controller_apply_contract(
            reason="domain_route_reconcile_controller_apply",
            action_fingerprint=f"domain_route_reconcile::{','.join(resolved_study_ids) or 'workspace'}",
        )
    )
