...
def _write_latest_domain_health_diagnostic_alias(*, report_dir: Path, report: Mapping[str, Any], markdown: str) -> tuple[Path, Path]:
    ...
    *,
    ...
    return [
        family_orchestration.build_family_human_gate(
            ...
            request_surface_kind="domain_health_diagnostic",
            request_surface_id="domain_health_diagnostic",
            evidence_refs=[
                {
                    "ref_kind": "repo_path",
                    "ref": evidence_ref,
...
def _stable_gate_id_seed(report: Mapping[str, Any]) -> str:
    ...
    return family_orchestration.resolve_active_run_id(quest_root, scanned_at) or "domain-health-diagnostic"
...
def _attach_family_companion_to_quest_report(report: dict[str, Any], *, quest_root: Path) -> None:
    ...
    companion = family_orchestration.build_family_orchestration_companion(
        ...
        surface_id="domain_health_diagnostic/latest.json",
        event_name="domain_health_diagnostic.quest_scanned",
        source_surface="domain_health_diagnostic",
        session_id=f"domain-health-diagnostic:{quest_root.name}",
        program_id=None,
        ...
        checkpoint_id=f"domain-health-diagnostic-quest:{quest_root.name}:{report.get('quest_status')}",
        checkpoint_label="domain health diagnostic quest scan",
        ...
        node_id="domain_health_diagnostic_quest_scan",
        ...
        resume_handle=f"domain_health_diagnostic:{quest_root.name}",
...
def _attach_family_companion_to_runtime_report(report: dict[str, Any], *, runtime_root: Path) -> None:
    ...
    companion = family_orchestration.build_family_orchestration_companion(
        surface_kind="domain_health_diagnostic",
        surface_id="domain_health_diagnostic/runtime_tick",
        event_name="domain_health_diagnostic.runtime_scanned",
        source_surface="domain_health_diagnostic",
        session_id=f"domain-health-diagnostic:{runtime_root}",
        program_id=None,
        study_id=None,
        quest_id=None,
        active_run_id=None,
        ...
        event_time=scanned_at,
        checkpoint_id=f"domain-health-diagnostic-runtime:{runtime_root.name}:{len(report.get('scanned_quests') or [])}",
        checkpoint_label="domain health diagnostic runtime scan",
        audit_refs=[
            {
                "ref_kind": "repo_path",
                "ref": str(item.get("latest_report_json") or item.get("report_json") or "").strip(),
                "label": "domain_health_diagnostic_quest_report",
            }
        ...
        action_graph_id="mas_runtime_orchestration",
        node_id="domain_health_diagnostic_runtime_scan",
        gate_id=(human_gates[0].get("gate_id") if human_gates else None),
        resume_mode="reenter_human_gate" if human_gates else "resume_from_checkpoint",
        resume_handle=f"domain_health_diagnostic:{runtime_root}",
        human_gate_required=bool(human_gates),
        human_gates=human_gates,
    )
    report["family_event_envelope"] = companion["family_event_envelope"]
...
def _render_domain_health_diagnostic_header(report: Mapping[str, Any]) -> list[str]:
    return [
...
def render_domain_health_diagnostic_markdown(report: dict[str, Any]) -> str:
    lines = []
    lines.extend(_render_domain_health_diagnostic_header(report))
...
def write_domain_health_diagnostic_report(quest_root: Path, report: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    markdown = render_domain_health_diagnostic_markdown(report)
    json_path, md_path = domain_health_diagnostic_protocol.write_domain_health_diagnostic_report(
        quest_root=quest_root,
        report=report,
        markdown=markdown,
    )
    latest_json, latest_markdown = _write_latest_domain_health_diagnostic_alias(
