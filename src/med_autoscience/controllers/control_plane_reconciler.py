...
def reconcile_next_action(
    *,
    ...
    if current_state == "blocked_platform" or failure_action in {
        ...
        action = "sync_package" if _has_package_sync_work(payload, next_work_unit) else "run_work_unit"
    elif bool(auto_runtime_parked.get("parked")):
        action = "wait_human"
    elif current_state == "live":
        action = "complete" if _text(payload.get("status")) == "complete" else "run_work_unit"
    else:
        action = "run_work_unit"

    return {
        "surface": "control_plane_reconciler",
        "schema_version": 1,
        "canonical_next_action": action,
        "owner": state_spec.get("owner"),
        "human_gate_required": bool(state_spec.get("human_gate_required")),
        "quality_gate_relaxation_allowed": False,
        "runtime_recovery_required": action == "recover_runtime",
