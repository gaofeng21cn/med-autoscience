...
SCHEMA_VERSION = 1
CONTRACT_NAME = "delivery_authority_backfill_apply.json"
...
def run_backfill_apply(
    *,
    ...
    route_gate = _route_gate(apply=apply, authority_snapshot=authority_snapshot)
    ...
    return {
        ...
        "authority_route_action": "delivery_sync",
        "authority_route_gate": route_gate,
        "mutation_policy": {
...
def _route_gate(
    *,
    apply: bool,
    authority_snapshot: Mapping[str, Any] | None,
) -> dict[str, Any]:
    if apply:
        return authorize_authority_route(
            "delivery_sync",
            {"authority_snapshot": dict(authority_snapshot or {})},
        )
    gate = authorize_authority_route("delivery_sync", {"projection_only": True})
    return {
        **gate,
        "authorized": True,
        "allowed": True,
...
def _plan_backfill_action(
    *,
    ...
    return {
        ...
        "blockers": blockers,
        "authority_route_gate": dict(route_gate),
...
def _lifecycle_hook(manifest_path: Path) -> dict[str, Any]:
    ...
    return {
        "authority_sync": {
            "source": "delivery_authority_backfill_apply",
