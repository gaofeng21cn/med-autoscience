...
from med_autoscience.display_pack_e2e_runtime.subprocess_renderers import (
...
from .contract_figures import _materialize_contract_backed_figure
from .pre_materialized_tables import _dpcc_medication_capture_markdown_path, _existing_table_catalog_entry, _materialize_pre_materialized_markdown_table, _pre_materialized_markdown_table_path
...
def materialize_display_surface(*, paper_root: Path) -> dict[str, Any]:
    ...
    for item in _iter_display_surface_entries(
        ...
        if requirement_short_id == "table1_baseline_characteristics":
            ...
            pre_materialized_md_path = _pre_materialized_markdown_table_path(
                paper_root=resolved_paper_root,
                requirement_short_id=requirement_short_id,
                table_id=table_id,
            )
            if pre_materialized_md_path is not None:
                entry = _materialize_pre_materialized_markdown_table(
                    paper_root=resolved_paper_root,
                    source_md_path=pre_materialized_md_path,
                    output_md_path=output_md_path,
                    output_csv_path=output_csv_path,
