...
from .markdown_user_visible import (
    _current_stage_context,
    _next_step_summary,
...
def _build_markdown_context(payload: dict[str, Any]) -> dict[str, Any]:
    normalized_payload, user_visible_projection = _normalized_payload_with_user_visible(payload)
    ...
    status_human_view = _status_narration_human_view(normalized_payload)
    current_stage, current_judgment = _current_stage_context(
        normalized_payload,
        status_human_view,
        user_visible_projection,
    )
    next_step_summary = _next_step_summary(normalized_payload, status_human_view, user_visible_projection)
    ...
    runtime_health, supervisor_tick_status = _progress_supervision_context(normalized_payload)
    return {
        "latest_events": [dict(item) for item in (normalized_payload.get("latest_events") or []) if isinstance(item, dict)],
        "blockers": _progress_blocker_labels(normalized_payload),
...
from .markdown_medical_readiness import _append_medical_paper_readiness
