...
def _progress_blocker_labels(payload: Mapping[str, Any]) -> list[str]:
    ...
    blocker_source = (
        ...
        if user_visible
        else payload.get("current_blockers")
    )
    blockers: list[str] = []
...
def _next_step_summary(
    ...
    if user_visible_projection:
        return _non_empty_text(user_visible_projection.get("next_system_action")) or _non_empty_text(
            user_visible_projection.get("next_step")
        ) or ""
