...
def _escalation_ref(
    *,
    ...
    if quest_root is None or quest_id is None:
        return None
    escalation_reason = runtime_reason or "runtime_supervision_escalated"
...
def _status_payload_continuation_field(status_payload: Mapping[str, Any], field_name: str) -> str | None:
...
def _runtime_health_worker_state(runtime_health_snapshot: Mapping[str, Any] | None) -> str | None:
    if not isinstance(runtime_health_snapshot, Mapping):
        return None
    worker_state = runtime_health_snapshot.get("worker_liveness_state")
    if not isinstance(worker_state, Mapping):
        return None
    return _non_empty_text(worker_state.get("state"))
...
def _status_payload_supervisor_tick_status(status_payload: Mapping[str, Any]) -> str | None:
    ...
    if not isinstance(supervisor_tick_audit, Mapping):
        return None
    return _non_empty_text(supervisor_tick_audit.get("status"))


def _status_payload_interaction_action(status_payload: Mapping[str, Any]) -> str | None:
    interaction_arbitration = status_payload.get("interaction_arbitration")
    if not isinstance(interaction_arbitration, Mapping):
        return None
    return _non_empty_text(interaction_arbitration.get("action"))


def _status_payload_interaction_requires_user_input(status_payload: Mapping[str, Any]) -> bool:
    interaction_arbitration = status_payload.get("interaction_arbitration")
    if not isinstance(interaction_arbitration, Mapping):
        return False
