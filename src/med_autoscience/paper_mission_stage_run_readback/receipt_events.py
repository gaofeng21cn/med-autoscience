...
def event_opl_stage_attempt_receipt(
    *,
    events: object,
    ...
    for event in reversed(events):
        payload = mapping(mapping(event).get("payload"))
        receipt = mapping(payload.get("opl_stage_attempt_receipt"))
...
def first_opl_stage_attempt_receipt(
    carrier: Mapping[str, Any],
    *sources: Mapping[str, Any],
) -> dict[str, Any] | None:
    for source in sources:
        receipt = mapping(source.get("opl_stage_attempt_receipt"))
        if matches_opl_stage_attempt_receipt(receipt=receipt, carrier=carrier):
            return dict(receipt)
    return None
...
def matches_opl_stage_attempt_receipt(
    *,
...
def carrier_command_kind(carrier: Mapping[str, Any]) -> str | None:
    route = mapping(carrier.get("ai_route_context"))
