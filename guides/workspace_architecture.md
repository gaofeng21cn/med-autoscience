...
这份文档定义 `MedAutoScience` 体系下医学研究 workspace 的标准形态，以及从历史遗留的 inline-DeepScientist workspace 迁移到标准形态的约束。

它面向两类对象：

- 未来要新建疾病项目 workspace 的 Agent / 技术同事
- 当前已经运行中的 legacy workspace，例如最早直接内嵌过 `DeepScientist` 程序与重依赖的项目
...
- `DeepScientist` quest 仓库、日志、记忆、配置等运行状态
...
- `DeepScientist` 程序源码仓库
- `Codex`、`~/.codex`
- Python、`uv`、Node、npm、pandoc、TinyTeX 等系统级或共享级依赖
- `DeepScientist` 的受管 Python 环境、bundle、uv cache、runtime tools
...
对于 `DeepScientist` 来说，这一层最终必须通过显式路径契约或正式兼容层来接入，而不是靠人工挪目录、临时 symlink 或手改 `site-packages`。

## DeepScientist 的推荐形态

`DeepScientist` 当前天然区分两个概念：
...
- repo root 是程序源码仓库
- runtime home 默认是 `~/DeepScientist/`
- `runtime/` 下包含 `python-env`、`uv-cache`、`bundle`、`tools`

因此在 `MedAutoScience` 体系下，推荐进一步把 `DeepScientist` 的使用方式收敛为：

- 外部共享 repo：`deepscientist_repo_root`
- workspace 内项目状态 home：`deepscientist_runtime_root`
- 外部共享重依赖：由 `DeepScientist` launcher / runtime 管理，不和疾病 workspace 绑定

这里要特别注意命名：

- 在当前实现里，`deepscientist_runtime_root` 仍然沿用 `DeepScientist home` 的名字
- 在目标架构里，它表示“project-local DeepScientist state root”

也就是说，这个路径在迁移完成前，可能仍暂时包含 upstream 默认放在 home 下的 `runtime/python-env`、`runtime/uv-cache`、`runtime/bundle`、`runtime/tools`

迁移的目的不是立刻改字段名，而是逐步把这部分重依赖从同一路径中剥离出去，并保留 project-local state 不变。

### 保留在 workspace 内的 DeepScientist 内容
...
- `workspace_root`
  - 当前疾病 workspace 根目录
- `runtime_root`
  - 当前 workspace 中 `DeepScientist` quest 根目录，通常是 `ops/deepscientist/runtime/quests`
- `deepscientist_runtime_root`
  - 当前 workspace 中 `DeepScientist` 项目状态根目录
- `deepscientist_repo_root`
  - 外部共享 `DeepScientist` 源码仓库，或受控的 `med-deepscientist` sibling fork
...
- 只调用外部共享 `MedAutoScience` / `DeepScientist`
...
- `ops/deepscientist/bin/*`
  - 面向 daemon 启动、doctor、runtime 运维等 `DeepScientist` 原生入口

wrapper 不应继续做：

- 自动搜索某个“看起来像 repo 的目录”
- 默认假设 `DeepScientist` 程序本体就在当前 workspace 里
...
- 不再在 wrapper 中硬编码 `ops/deepscientist/runtime/quests`
...
如果 Phase 1 已经切到受控的 `med-deepscientist` fork，那么这里的“外部共享 `deepscientist_repo_root`”可以具体落到该 fork；但命名仍暂时保留 `deepscientist_repo_root`，直到 runtime protocol convergence 完成。
...
- workspace 内没有 `DeepScientist` 程序源码副本
- workspace 内只保留项目状态，不保留程序级重依赖
- `MedAutoScience` 和 `DeepScientist` 都通过显式配置指向外部共享 repo
...
3. 再把 `NF-PitNET` 的 `DeepScientist` 调用方式逐步迁成“外部程序 + 本地运行状态”
