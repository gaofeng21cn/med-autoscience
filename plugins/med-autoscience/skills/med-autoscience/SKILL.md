...
6. For analysis/display work with environment dependencies, use `opl env run --domain mas --profile <profile> --artifact-root <path> -- <command...>`. OPL reuses prepared dependencies or prepares them on demand using uv/Python and renv/R; do not install packages ad hoc or repeat environment preparation for every command. Bind the environment manifest and execution record to the result. Environment records support traceability; formal scientific freezing remains MAS-owned, and execution success never proves medical or publication quality.
