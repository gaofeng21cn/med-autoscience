...
OPL 持有通用 lifecycle substrate：stage attempt、queue、retry/dead-letter、resume、attempt ledger、locator/index、restore/retention shell、generic memory/artifact transport、observability 和 App/workbench shell。Domain agent 持有领域真相与不可声明化 authority；对 MAS 这类医学研究 agent，study truth、quality verdict、publication route、artifact mutation、memory accept/reject、source readiness、owner receipt 和 typed blocker 必须留在 MAS。通用 substrate 可以运输、索引和展示 refs，不能把 transport / lifecycle / ledger evidence 升格成医学 authority。

## Target-First / Substrate-As-Path

对 MAS 论文、研究产物、submission milestone 或 study delivery 任务，真实进度按 manuscript/package delta、research artifact、owner decision、human gate、可接力 route-back 或用户可审阅结果计算；runtime、queue、stage、read model、测试、docs、provider liveness 和平台修复只是支持证据。MAS/OPL 执行基座是首选路径，不是论文推进的前置条件：基座顺畅时走基座，基座卡住时，前端执行者继续推进合法的论文或研究交付增量，并把暴露出的基座问题作为 side repair lane 记录或修复。只有继续动作会越权写 publication/study authority surface、缺少必要 source/data、或同一写集 ownership 冲突且无法隔离时，才暂停具体目标动作。repair lane 不能吞掉论文主线，除非用户目标本身就是平台修复。
